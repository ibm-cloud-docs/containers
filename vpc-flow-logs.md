...
When you use {{site.data.keyword.fl_full}} with a {{site.data.keyword.containershort}} VPC cluster, you can enable flow logs at the VPC level, at the VPC subnet level, or at the VPC load balancer level. You cannot specify which worker nodes to gather flow logs for. However, you can review the flow log output to identify information that is specific to the worker nodes you want to investigate.
