...
Don't use the `ibmcloud ks worker rm` [command](/docs/containers?topic=containers-kubernetes-service-cli#worker-rm-cli) to remove individual worker nodes from your worker pool, which can unbalance the worker pool.
