...
<col width="40%">
 ...
 <tr>
   <td>Overview</td>
   <td>Generally available now, your clusters on IBM Cloud infrastructure (SoftLayer) Classic include all of the {{site.data.keyword.containerlong_notm}} mature and robust features for compute, networking, and storage. To get started, [create a Kubernetes cluster](/docs/containers?topic=containers-cs_cluster_tutorial#cs_cluster_tutorial).</td>
 ...
 <tr>
   ...
   <td>[Virtual](/docs/containers?topic=containers-planning_worker_nodes#vm), [bare metal](/docs/containers?topic=containers-planning_worker_nodes#bm), and [software-defined storage](/docs/containers?topic=containers-planning_worker_nodes#sds) machines are available for your worker nodes. Your worker node instances reside in your IBM Cloud infrastructure (SoftLayer) account, but you can manage them through {{site.data.keyword.containerlong_notm}}. You own the worker node instances.</td>
 ...
 <tr>
   ...
   <td>VPC on Classic clusters cannot be reloaded or updated. Instead, use the [`worker-replace` operation **TODO**](TODO) to replace worker nodes that are outdated or in a troubled state. When you delete a cluster, the attached load balancers are not deleted.<!--this LB not deleted is a temporary option, not sure when will be removed--></td>
 </tr>
 <tr>
   <td>Cluster networking</td>
   <td>Your worker nodes are provisioned on private VLANs that provide private IP addresses to communicate on the private IBM Cloud infrastructure (SoftLayer) network. For communication on the public network, you can also provision the worker nodes on a public VLAN. Communication to the cluster master can be on the public or private service endpoint. For more information, see [Understanding cluster network basics](/docs/containers?topic=containers-plan_clusters#plan_basics).</td>
 ...
 <tr>
  ...
  <td>You can choose to create [native Kubernetes or OpenShift clusters](/docs/containers?topic=containers-faqs#container_platforms) to manage your containerized apps. Your app build processes do not differ because of the infrastructure provider, but how you expose the app does, as described in the next _App networking_ entry.</td>
  <td>You can create only native Kubernetes clusters, not OpenShift clusters.</td>
   <!--Future statement<td colspan="2">For both classic and VPC on Classic clusters, you can choose to create [native Kubernetes or OpenShift clusters](/docs/containers?topic=containers-faqs#container_platforms) to manage your containerized apps. Your app build processes do not differ because of the infrastructure provider, but how you expose the app does as described in the next _App networking_ entry.</td>-->
   <td></td>
 </tr>
 <tr>
 ...
 <tr>
   ...
   <td>VPC on Classic clusters are available in the **Dallas** multizone metro.</td>
 </tr>
 <tr>
...
<hidden><!--old text when it was a simpler table-->
<td>Classic</td>
<td>Generally available now, your clusters on IBM Cloud infrastructure (SoftLayer) Classic include all of the {{site.data.keyword.containerlong_notm}} mature and robust features for compute, networking, and storage. Classic clusters are fully supported in the {{site.data.keyword.containershort_notm}} [v1 API ![External link icon](../icons/launch-glyph.svg "External link icon")](https://containers.cloud.ibm.com/global/swagger-global-api/), [CLI](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli), and [console ![External link icon](../icons/launch-glyph.svg "External link icon")](https://cloud.ibm.com/kubernetes/clusters).<br><br>
To get started, [create a Kubernetes cluster](/docs/containers?topic=containers-cs_cluster_tutorial#cs_cluster_tutorial).</td>
</tr>
<tr>
<td>VPC on Classic</td>
<td>With the VPC on Classic, you can create your cluster in the next generation of the {{site.data.keyword.cloud_notm}} platform, in your [Virtual Private Cloud](/docs/infrastructure/vpc?topic=vpc-about). VPC gives you the security of a private cloud environment with the dynamic scalability of a public cloud.<!-- You can create worker nodes that run on three subproviders within VPC: IBM Cloud infrastructure (SoftLayer), IBM Cloud infrastructure (SoftLayer) Classic, and Hyper Protect Containers on IBM Z Systems (see next entry). -->Because of the added security and cluster networking simplicity, VPC is a good default infrastructure provider for your clusters unless you have a specific use case due to the following limitations.<br><br>
The VPC on Classic uses the next version of {{site.data.keyword.containerlong_notm}}, with a select group of v2 API, CLI, and console functionality. Infrastructure differences from the Classic infrastructure provider include:<ul>
<li>**Compute**: Only select flavors for worker nodes are supported. VPC is available in only the [six worldwide multizone metro regions](/docs/containers?topic=containers-regions-and-zones#zones).</li>
<li>**Networking**: Your cluster is provisioned in your VPC, which means that your worker nodes use the VPC subnet for connectivity to the master. You do not need to configure VLANs. To expose your apps on the public network, you can create a VPC load balancer.</li>
<li>**Storage**: Only block storage is supported; file, object, and local software-defined storage are not available. Only 5 volumes can be attached per worker node, with volume limits of 2TB and 20,000 IOPS.</li></ul><br><br>
To get started, try out the [VPC on Classic cluster tutorial](/docs/containers?topic=containers-vpc_ks_tutorial#vpc_ks_tutorial).</td>
</tr>
<!--<tr>
<td>[VPC, Hyper Protect Containers on IBM Z Systems](/docs/containers?topic=containers-z_tutorial#z_tutorial)</td>
<td>TODO description
<br><br>
The VPC Hyper Protect Containers on IBM Z Systems beta uses the next version of {{site.data.keyword.containerlong_notm}}, with a select group of v2 API, CLI, and console functionality. Infrastructure differences from the Classic infrastructure provider include:<ul>
<li>**Compute**: Only one flavor for your IBM Z worker node is supported.</li>
<li>**Networking**: Your cluster is provisioned in your VPC, which means that your IBM Z worker nodes use the VPC subnet for connectivity to the Kubernetes master. You do not need to configure VLANs.</li>
<li>**Storage**: Only local storage on your IBM Z worker node is supported.</li></ul></td>
</tr>-->
</hidden>
