...
You cannot use [VPC security groups](/docs/vpc?topic=vpc-using-security-groups) to control traffic for your worker nodes. VPC security groups are applied to the network interface of a single virtual server to filter traffic at the hypervisor level. However, the worker nodes of your VPC cluster exist in a service account and are not listed in the VPC infrastructure dashboard. You cannot attach a security group to your worker nodes instances, but you can use security groups at the level of the VPC. For more information, see [Opening required ports in VPC security groups](#security_groups).
{: note}
