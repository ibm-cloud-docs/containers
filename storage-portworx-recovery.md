...
- [**Metro DR**](https://docs.portworx.com/portworx-enterprise/operations/disaster-recovery/px-metro){: external}: Use this option when your Kubernetes clusters are in the same region, such as one or more zones in `us-south`. All clusters use the same Portworx cluster and share the same Portworx key-value store. Because the Portworx storage layer is shared, data is automatically replicated between the clusters. The recovery point objective (RPO) and recovery time objective (RTO) for this configuration are typically less than 60 seconds.
- [**Asynchronous DR**](https://docs.portworx.com/portworx-enterprise/operations/disaster-recovery/async-dr){: external}: Use this option when your Kubernetes clusters are in different regions, such as `us-south` and `us-east`. Each cluster has its own Portworx installation and its own Portworx key-value store. To replicate data between clusters, you must configure scheduled replication. Because of the higher latency and scheduled replication times, the RPO for this scenario can be up to 15 minutes.

To include your cluster in a Portworx disaster recovery configuration:

1. [Choose the disaster recovery configuration that works for your cluster setup](https://docs.portworx.com/portworx-enterprise/operations/disaster-recovery){: external}.
2. Review the prerequisites for the [**Metro DR**](https://docs.portworx.com/portworx-enterprise/operations/disaster-recovery/px-metro/prerequisites){: external} and [**Asynchronous DR**](https://docs.portworx.com/portworx-enterprise/operations/disaster-recovery/async-dr/prerequisites){: external} configuration.
3. Configure disaster recovery for your cluster. 
    **Metro DR**:
    ...
    3. Set up a supported external etcd-based store for your Portworx key-value store. Because both Kubernetes clusters must share the key-value store, you can't use the internal Portworx KVDB.
    ...
    5. Follow the instructions to [install Portworx](/docs/containers?topic=containers-storage_portworx_deploy) with the disaster recovery plan in both of your clusters. If you installed Portworx without the disaster recovery plan in one of your clusters already, you must reinstall Portworx in that cluster with the disaster recovery plan. Make sure that you select the external etcd-based metadata store option and that you enter the same API endpoint and Kubernetes secret name in both clusters.
    6. Continue following the [Portworx documentation](https://docs.portworx.com/portworx-enterprise/operations/disaster-recovery/px-metro/pair-clusters){: external} to pair your clusters, sync data between them, and try out a failover of an application.
    ...
    **Asynchronous DR**:
    ...
    2. Make sure that all your clusters have sufficient [raw and unformatted block storage](/docs/containers?topic=containers-utilities#manual_block) so that you can build your Portworx storage layer.
    3. Review your [options to configure a Portworx key-value store](/docs/containers?topic=containers-storage_portworx_kv_store). Because both clusters are in different regions, each cluster must use its own key-value store. You can use the internal Portworx KVDB or another supported external etcd-based store.
