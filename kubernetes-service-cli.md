...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. To create a VPC Gen 1 compute cluster, use the [`ibmcloud ks cluster create vpc-classic` command](#cli_cluster-create-vpc-classic) instead.</ff-roks311-vpc>
...
```
ibmcloud ks cluster create vpc-classic --name NAME --zone ZONE --vpc-id VPC_ID --subnet-id VPC_SUBNET_ID --flavor WORKER_FLAVOR [--kube-version MAJOR.MINOR.PATCH --workers NUMBER_WORKERS_PER_ZONE] [--disable-public-service-endpoint] [--pod-subnet SUBNET] [--service-subnet SUBNET] [--skip-advance-permissions-check] [-s]
```
{: pre}
...
```
ibmcloud ks cluster ls <ff-roks311-vpc>[--provider classic|vpc-classic] </ff-roks311-vpc>[--location LOCATION] [--json] [-s]
```
{: pre}<ff-roks311-vpc>
...
This command is deprecated. Create a worker pool by running [`ibmcloud ks worker-pool create classic`](#cs_worker_pool_create)<ff-roks311-vpc> or [`ibmcloud ks worker-pool create vpc-classic](#cli_worker_pool_create_vpc_classic)</ff-roks311-vpc>, or add workers to an existing worker pool by running [`ibmcloud ks worker-pool resize`](#cs_worker_pool_resize).
{: deprecated}
...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. To reload a worker node in a VPC Gen 1 compute cluster, use the [`ibmcloud ks worker replace` command](#cli_worker_replace) instead.</ff-roks311-vpc>
...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. To update a worker node in a VPC Gen 1 compute cluster, use the [`ibmcloud ks worker replace` command](#cli_worker_replace) instead.</ff-roks311-vpc>
...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. To add a zone to worker pools in a VPC Gen 1 compute cluster, use the [`ibmcloud ks zone add vpc-classic` command](#cli_zone-add-vpc-classic) instead.</ff-roks311-vpc>
...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. For VPC Gen 1 compute clusters, use the [`ibmcloud ks alb configure vpc-classic` command](#cli_alb_configure_vpc_classic) instead.</ff-roks311-vpc>
...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. For VPC clusters, see [`ibmcloud ks nlb-dns create vpc-classic`](#cs_nlb-dns-create-vpc).</ff-roks311-vpc>
...
**Supported infrastructure provider**: <img src="images/icon-classic.png" alt="Classic infrastructure provider icon" width="15" style="width:15px; border-style: none"/> Classic. For VPC clusters, see [`ibmcloud ks nlb-dns rm vpc-classic`](#cs_nlb-dns-rm-vpc).</ff-roks311-vpc>
...
```
ibmcloud ks subnets <ff-roks311-vpc>[--provider (CLASSIC|VPC-CLASSIC)] [--vpc-id <VPC_ID> --zone <VPC_ZONE>] </ff-roks311-vpc>[--location LOCATION] [--json] [-s]
```
{: pre}<ff-roks311-vpc>
...
<dd>The zone to list VPC subnets for. This flag is required when you specify the `vpc-classic` provider type. To list available VPC zones, run `ibmcloud ks zone ls --provider vpc-classic`.</dd></ff-roks311-vpc>
...
<dl>
<dt><code>--zone <em>ZONE</em></code></dt>
<dd>Enter the zone where you want to list available flavors. This value is required. To see available zones for classic clusters, run `ibmcloud ks zone ls`.<ff-roks311-vpc> To see available zones for VPC Gen 1 compute clusters, run `ibmcloud ks zone ls --provider vpc-classic`.</ff-roks311-vpc></dd>
