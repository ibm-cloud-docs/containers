...
Use these commands to create and manage **both community Kubernetes or {{site.data.keyword.redhat_openshift_notm}} clusters** in {{site.data.keyword.containerlong}}.
{: shortdesc}
...
#### `cluster addon disable alb-oauth-proxy` command options
{: #cluster-addon-disable-options}
...
#### `cluster addon disable headlamp` command options
...
#### `cluster addon disable istio` command options
...
Disable the managed Istio extras add-on, which is unsupported. Removes Grafana, Jaeger, and Kiali from the cluster.
{: shortdesc}
...
#### `cluster addon disable istio-extras` command options
...
#### `cluster addon disable istio-sample-bookinfo` command options
...
#### `cluster addon disable kube-terminal` command options
...
#### `cluster addon disable vpc-block-csi-driver` command options
...
Enable the add-on for the ALB OAuth Proxy in a cluster. When your ALBs run the Kubernetes Ingress image, you can use the ALB OAuth proxy to enforce authentication for your apps by configuring Ingress with {{site.data.keyword.appid_full_notm}}.
{: shortdesc}
...
#### `cluster addon enable alb-oauth-proxy` command options
{: #cluster-addon-enable-options}
...
#### `cluster addon enable headlamp` command options
...
#### `cluster addon enable istio` command options
...
#### `cluster addon enable static-route` command options
...
#### `cluster addon enable vpc-block-csi-driver` command options
...
#### `cluster addon get` command options
...
```sh
ibmcloud ks cluster addon get --addon istio --cluster my_cluster
```
{: pre}
...
#### `cluster addon ls` command options
...
```sh
ibmcloud ks cluster addon ls --cluster my_cluster
```
{: pre}
...
#### `cluster addon options` command options
...
```sh
ibmcloud ks cluster addon options --addon istio
```
{: pre}
...
#### `cluster addon update` command options
...
#### `cluster addon versions` command options
...
#### `cluster ca create` command options
...
#### `cluster ca get` command options
...
Before you run this command, follow the steps in [Rotating CA certificates in your cluster](/docs/containers?topic=containers-cert-rotate) to ensure that any tooling that uses the old CA certificates is updated to use the new certificates and to update your worker nodes.
{: important}
...
#### `cluster ca rotate` command options
...
#### `cluster ca status` command options
...
* **Platform**: If you have only a platform access role, you can perform this command, but you need a [service access role](/docs/containers?topic=containers-iam-platform-access-roles) to perform Kubernetes actions in the cluster.
...
#### `cluster config` command options
...
`--admin`
:    Optional: The **Manager** Service role is required to use this option. Download the TLS certificates and permission files for the Super User role. The files are downloaded to `<user_home_directory>/.bluemix/plugins/kubernetes-service/clusters/<cluster_name>-<cluster-id>-admin`. Administrator certificates are unique to the IAM ID for the active session. You can verify by viewing the subject name in the downloaded certificates, which has a `CName` with the format of `CN=KubeCert#<IAM_ID>`. The certificates can be revoked or rotated. For more information, see [Rotating CA certificates in your cluster](/docs/containers?topic=containers-cert-rotate).
...
#### `cluster create classic` command options
...
VPC cluster flavors with instance storage are available for allowlisted accounts. To get added to the allowlist, [open a case](https://cloud.ibm.com/unifiedsupport/cases/form){: external} with support.
{: note}
...
:   [**Administrator** platform access role](/docs/containers?topic=containers-iam-platform-access-roles) for {{site.data.keyword.containerlong_notm}} at the account level.
:   [**Writer** or **Manager** service access role](/docs/containers?topic=containers-iam-platform-access-roles) for {{site.data.keyword.containerlong_notm}}.
:   [**Administrator** platform access role](/docs/containers?topic=containers-iam-platform-access-roles) for {{site.data.keyword.registrylong_notm}} at the account level.

#### `cluster create vpc-gen2` command options
...
#### `cluster image-security disable` command options
...
#### `cluster image-security enable` command options
...
#### `cluster master audit-webhook get` command options
{: #cluster-master-audit-webhook-options}
...
#### `cluster master audit-webhook set` command options
...
#### `cluster master audit-webhook unset` command options
...
#### `cluster master console-oauth-access get` command options
...
```sh
ibmcloud ks cluster master console-oauth-access get --cluster my_cluster
```
{: pre}
...
#### `cluster master console-oauth-access set` command options
...
```sh
ibmcloud ks cluster master console-oauth-access set --cluster my_cluster --type legacy
```
{: pre}
...
#### `cluster master pod-security get` command options
...
#### `cluster master pod-security policy disable` command options
...
#### `cluster master pod-security policy enable` command options
...
#### `cluster master pod-security policy get` command options
...
#### `cluster master pod-security set` command options
...
#### `cluster master pod-security unset` command options
...
Private service endpoint allowlists are no longer supported. Migrate from allowlists to context based restrictions as soon as possible. For more information, see [Migrating from a private service endpoint allowlist to context based restrictions (CBR)](/docs/containers?topic=containers-pse-to-cbr-migration).
{: unsupported}
...
#### `cluster master private-service-endpoint allowlist add` command options
...
#### `cluster master private-service-endpoint disable` command options
...
#### `cluster master private-service-endpoint enable` command options
...
Before you disable the public endpoint, you first must complete the following steps to enable the private cloud service endpoint:
...
#### `cluster master public-service-endpoint disable` command options
...
#### `cluster master public-service-endpoint enable` command options
...
#### `cluster master refresh` command options
...
```sh
ibmcloud ks cluster master refresh --cluster my_cluster
```
{: pre}
...
#### `cluster master update` command options
...
#### `cluster pull-secret apply` command options
...
#### `cluster service bind` command options
...
#### `cluster service ls` command options
...
#### `cluster service unbind` command options
...
#### `cluster subnet add` command options
...
#### `cluster subnet create` command options
...
#### `cluster subnet detach` command options
...
#### Example ``ibmcloud ks dedicated flavors`` command
...
#### Example ``ibmcloud ks dedicated host create`` command
...
#### Example ``ibmcloud ks dedicated host get`` command
...
#### Example ``ibmcloud ks dedicated host ls`` command
...
#### Example ``ibmcloud ks dedicated host placement disable`` command
...
#### Example ``ibmcloud ks dedicated host placement enable`` command
...
#### Example ``ibmcloud ks dedicated host rm`` command
...
#### Example ``ibmcloud ks dedicated pool create`` command
...
#### Example ``ibmcloud ks dedicated pool get`` command
...
#### Example ``ibmcloud ks dedicated pool ls`` command
...
#### Example ``ibmcloud ks dedicated pool rm`` command
...
#### `worker replace` command options
...
#### `worker-pool create classic` command options
...
#### `worker-pool create vpc-gen2` command options
...
#### `worker-pool get` command options
...
#### `worker-pool label rm` command options
...
#### `worker-pool label set` command options
...
#### `worker-pool ls` command options
...
#### `worker-pool operating-system set` command options
...
```sh
ibmcloud ks worker-pool operating-system set --cluster my_cluster --operating-system UBUNTU_24_64 --worker-pool my_pool
```
{: pre}
...
#### `worker-pool rebalance` command options
...
#### `worker-pool resize` command options
...
#### `worker-pool rm` command options
...
#### `worker-pool taint set` command options
...
#### `worker-pool taint rm` command options
...
#### `worker-pool zones` command options
...
#### `zone add classic` command options
...
#### `zone add vpc-gen2` command options
...
#### Example ``ibmcloud ks zone ls`` command
...
#### `zone network-set` command options
...
#### `ingress alb autoscale get` command options
...
#### `ingress alb autoscale set` command options
...
#### `ingress alb autoscale unset` command options
...
#### `ingress alb autoupdate disable` command options
...
#### `ingress alb autoupdate enable` command options
...
```sh
ibmcloud ks ingress alb autoupdate enable --cluster my_cluster
```
{: pre}
...
#### `ingress alb autoupdate get` command options
...
```sh
ibmcloud ks ingress alb autoupdate get --cluster my_cluster
```
{: pre}
...
#### `ingress alb create classic` command options
...
#### `ingress alb create vpc-gen2` command options
...
#### `ingress alb disable` command options
...
#### `ingress alb enable classic` command options
...
#### `ingress alb enable vpc-gen2` command options
...
#### `ingress alb get` command options
...
#### `ingress alb health-checker disable` command options
...
#### `ingress alb health-checker enable` command options
...
#### `ingress alb health-checker get` command options
...
#### `ingress alb ls` command options
...
#### `ingress alb update` command options
...
#### `ingress alb versions` command options
...
#### `ingress domain create` command options
...
#### `ingress domain default replace` command options
...
#### `ingress domain get` command options
...
#### `ingress domain ls` command options
...
#### `ingress domain rm` command options
...
#### `ingress domain secret regenerate` command options
...
#### `ingress domain secret rm` command options
...
#### `ingress domain update` command options
...
#### `ingress instance default set` command options
...
#### Example ``ibmcloud ks ingress instance default set`` command
...
#### `ingress instance default unset` command options
...
#### Example ``ibmcloud ks ingress instance default unset`` command
...
#### `ingress instance get` command options
...
#### Example ``ibmcloud ks ingress instance get`` command
...
#### `ingress instance ls` command options
...
#### Example ``ibmcloud ks ingress instance ls`` command
...
#### `ingress instance register` command options
...
#### Example ``ibmcloud ks ingress instance register`` command
...
#### `ingress instance unregister` command options
...
#### Example ``ibmcloud ks ingress instance unregister`` command
...
#### `ingress lb get` command options
...
#### `ingress lb backend set` command options
...
#### `ingress lb proxy-protocol disable` command options
...
#### `ingress lb proxy-protocol enable` command options
...
#### `ingress secret create` command options
...
#### `ingress secret field add` command options
...
#### `ingress secret field ls` command options
...
#### `ingress secret field rm` command options
...
#### `ingress secret get` command options
...
#### `ingress secret ls` command options
...
#### `ingress secret rm` command options
...
#### `ingress secret update` command options
...
```sh
ibmcloud ks ingress status --cluster CLUSTER [--output OUTPUT] [-q]
```
{: pre}
...
#### `ingress status` command options
...
#### `ingress status-report disable` command options
...
#### `ingress status-report enable` command options
...
#### `ingress status-report get` command options
...
#### `ingress status-report ignored-errors add` command options
...
#### `ingress status-report ignored-errors ls` command options
...
#### `ingress status-report ignored-errors rm` command options
...
#### `logging autoupdate disable` command options
...
#### Example `logging autoupdate disable` command
{: #logging-autoupdate-disable-example}

```sh
ibmcloud ks logging autoupdate disable --cluster my_cluster
```
{: pre}
...
#### `logging autoupdate enable` command options
...
#### Example `logging autoupdate enable` command
{: #logging-autoupdate-enable-example}

```sh
ibmcloud ks logging autoupdate enable --cluster my_cluster
```
{: pre}
...
#### `logging autoupdate get` command options
...
#### Example `logging autoupdate get` command
{: #logging-autoupdate-get-example}

```sh
ibmcloud ks logging autoupdate get --cluster my_cluster
```
{: pre}
...
#### `logging config create` command options
...
#### `logging config get` command options
...
#### `logging config rm` command options
...
#### `logging config update` command options
...
#### `logging filter create` command options
...
#### `logging filter get` command options
...
#### `logging filter rm` command options
...
#### `logging filter update` command options
...
#### `logging refresh` command options
...
#### `nlb-dns create classic` command options
...
#### `nlb-dns create vpc-gen2` command options
...
#### `nlb-dns monitor configure` command options
...
#### `nlb-dns monitor disable` command options
...
#### `nlb-dns monitor enable` command options
...
#### `nlb-dns monitor get` command options
...
#### `nlb-dns monitor ls` command options
...
#### `nlb-dns replace` command options
...
#### `nlb-dns rm classic` command options
...
#### `vpc secure-by-default enable` command options
...
```sh
ibmcloud ks vpc secure-by-default enable --cluster my_cluster
```
{: pre}
...
View the name and email address for the owner of the {{site.data.keyword.cloud_notm}} Identity and Access Management (IAM) API key that {{site.data.keyword.containerlong_notm}} uses to authenticate certain requests like infrastructure in the region and resource group.
{: shortdesc}
...
#### `credential get` command options
...
Before you use this command, make sure that the user whose credentials are used has the required [{{site.data.keyword.containerlong_notm}} and IBM Cloud infrastructure permissions](/docs/containers?topic=containers-iam-platform-access-roles).
{: important}
...
#### `credential set classic` command options
...
#### `credential unset` command options
...
For most use cases, [set up the API key](/docs/containers?topic=containers-iam-platform-access-roles) for the region and resource group with the appropriate infrastructure permissions. If you need to use another infrastructure account that differs from your current account, [set up manual credentials](/docs/containers?topic=containers-classic-credentials).
...
#### `infra-permissions get` command options
...
Enable a [key management service (KMS) provider](/docs/containers?topic=containers-encryption-setup) in your cluster to encrypt the etcd component and Kubernetes secrets with a root key that you control.
{: shortdesc}
...
Encrypt your Kubernetes secrets by [enabling a key management service (KMS) provider](/docs/containers?topic=containers-encryption) in your cluster. To rotate a key in a cluster with existing key encryption, rerun this command with a new root key ID.
{: shortdesc}
...
List available [key management service (KMS) instances](/docs/containers?topic=containers-encryption-setup) in your {{site.data.keyword.cloud_notm}} account that you can choose to enable in your cluster.
{: shortdesc}
...
#### `kms instance ls` command options
...
#### Example ``ibmcloud ks kms instance ls`` command
...
#### Example ``ibmcloud ks quota ls`` command
...
#### `vlan spanning get` command options
...
#### `vpc outbound-traffic-protection disable` command options
...
```sh
ibmcloud ks vpc outbound-traffic-protection disable --cluster my_cluster
```
{: pre}
...
#### `vpc outbound-traffic-protection enable` command options
...
```sh
ibmcloud ks vpc outbound-traffic-protection enable --cluster my_cluster
```
{: pre}
...
#### `vni attach baremetal` command options
...
```sh
ibmcloud ks vni attach baremetal --vlan 100 --vni my_vni --cluster-id my_cluster_id
```
{: pre}
...
```sh
ibmcloud ks vni detach --vni my_vni --cluster-id my_cluster_id
```
{: pre}
...
```sh
ibmcloud ks vni ls --cluster-id my_cluster_id
```
{: pre}
...
`--show-storage`
:    Optional: Show additional raw disks that are available for SDS worker node flavors. For more information, see [Software-defined storage (SDS) machines](/docs/containers?topic=containers-classic-flavors). In the tables for each metro area section, SDS flavors are in the **Bare Metal** tabs and end with `.ssd`.
...
List the locations that are supported by {{site.data.keyword.containerlong_notm}}. For more information about the locations that are returned, see [{{site.data.keyword.containerlong_notm}} locations](/docs/containers?topic=containers-regions-and-zones).
{: shortdesc}
...
Reset or sync a security group to the [default traffic rules](/docs/containers?topic=containers-vpc-security-group-reference).
{: shortdesc}
...
#### `security-group ls` command options
...
```sh
ibmcloud ks security-group ls --cluster my_cluster
```
{: pre}
...
[Deletes all existing security group rules and reapplies the default rules](/docs/containers?topic=containers-vpc-security-group-reference).
{: shortdesc}
...
#### `security-group reset` command options
...
#### Example ``ibmcloud ks security-group reset`` command
...
Reapplies the [default security group rules](/docs/containers?topic=containers-vpc-security-group-reference) to add any missing rules. Does not delete preexisting rules.
{: shortdesc}
...
#### `security-group sync` command options
...
#### Example ``ibmcloud ks security-group sync`` command
...
#### `storage attachment create` command options
...
#### Example ``ibmcloud ks storage attachment create`` command
...
#### Example ``ibmcloud ks storage attachment get`` command
...
#### Example ``ibmcloud ks storage attachment ls`` command
...
#### `storage attachment rm` command options
...
#### Example ``ibmcloud ks storage attachment rm`` command
...
#### `storage volume get` command options
...
#### Example ``ibmcloud ks storage volume get`` command
...
#### `storage volume ls` command options
...
#### Example ``ibmcloud ks storage volume ls`` command
...
#### `experimental trusted-profile default get` command options
...
```sh
ibmcloud ks experimental trusted-profile default get --region us-south --resource-group my_group
```
{: pre}
...
#### `experimental trusted-profile default set` command options
...
```sh
ibmcloud ks experimental trusted-profile default set --region us-south --resource-group my_group --trusted-profile my_profile_id
```
{: pre}
...
#### `experimental trusted-profile get` command options
...
```sh
ibmcloud ks experimental trusted-profile get --cluster my_cluster
```
{: pre}
...
#### `experimental trusted-profile set` command options
...
```sh
ibmcloud ks experimental trusted-profile set --cluster my_cluster --trusted-profile my_profile_id
```
{: pre}
