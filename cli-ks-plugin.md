...
```sh
ibmcloud ks cluster create vpc-gen2 --name mycluster --version 1.34 --zone us-south-1 --vpc-id a0123456-78b9-0c1d-23d4-567890123ef4 --subnet-id 1ab23c45-6789-0123-456d-789ef01gh234 --flavor bx2.4x16 --workers 3
```
{: pre}
...
`--cluster-id ID`, `-c ID`
:    The ID of the cluster. To list cluster IDs, run `ibmcloud ks clusters`. Specify either `--cluster-id` or `--worker`, but not both. When you specify `--cluster-id`, the VNI is configured as a floating attachment that can follow workloads between workers in the same zone.

`--worker WORKER`, `-w WORKER`
:    The ID of the worker node. To list worker IDs, run `ibmcloud ks workers --cluster CLUSTER`. Specify either `--worker` or `--cluster-id`, but not both. When you specify `--worker`, the VNI is attached to that specific worker node only.
...
```sh
ibmcloud ks vni attach baremetal --vni 0716-aac49630-f3b4-4ef6-9a3a-5145481697ba --cluster-id c9pqfcmw0tq431jdnrrg --vlan 251 --auto-delete
```
{: pre}
...
```sh
ibmcloud ks vni detach --vni VNI [--output OUTPUT] [-q] (--cluster-id ID | --worker WORKER)
```
{: pre}
...
`--cluster-id ID`, `-c ID`
:    The ID of the cluster. To list cluster IDs, run `ibmcloud ks clusters`. Specify either `--cluster-id` or `--worker`, but not both.

`--worker WORKER`, `-w WORKER`
:    The ID of the worker node. To list worker IDs, run `ibmcloud ks workers --cluster CLUSTER`. Specify either `--worker` or `--cluster-id`, but not both. For floating VNIs, you must first list the VNIs to find the current worker ID.
...
```sh
ibmcloud ks vni ls [--output OUTPUT] [-q] (--cluster-id ID | --worker WORKER)
```
{: pre}
...
`--worker WORKER`, `-w WORKER`
:    The ID of the worker node. To list worker IDs, run `ibmcloud ks workers --cluster CLUSTER`. Specify either `--worker` or `--cluster-id`, but not both.
...
```sh
ibmcloud ks vni ls --worker kube-c9pqfcmw0tq431jdnrrg-mycluster-default-00000123
```
{: pre}
