...
3.  Assign a platform role to a user to set access for {{site.data.keyword.containershort_notm}}.
      ...
      3. From the **Service instance** drop-down list, select the cluster to invite the user to. To find the ID of a specific cluster, run `ibmcloud cs clusters`.
...
<dl>
  ...
  <dt>IBM Cloud infrastructure (SoftLayer) credentials via <code>ibmcloud cs credentials-set</code></dt>
    ...
    <p>To remove IBM Cloud infrastructure (SoftLayer) credentials that were manually set, you can use the [<code>ibmcloud cs credentials-unset</code>](cs_cli_reference.html#cs_credentials_unset) command. After the credentials are removed, the IAM API key is used to order infrastructure.</p></dd>
</dl>
