...
| --- | --- | --- | --- |
| `2.0.3_404` | `v2.0.7` | 25 November 2020 | 1.15 - 1.19 | Updates in this patch:<ul><li>`v.2.0.7` contains a fix for vulnerability scan issues.</li><li>Updates the base image from `alpine` to `UBI`.</li><li>Pods and containers now run as `non-root` except for the `node-server` pod's containers.</li></ul> |
