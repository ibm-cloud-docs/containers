...
1. [Enable your {{site.data.keyword.cloud_notm}} account to use service endpoints](/docs/iam?topic=iam-vrf-service-endpoint#service-endpoint).
...
4. If your account [restricts service ID creation](/docs/iam?topic=iam-restrict-service-id-create), add the **Service ID creator** role to **Identity and Access Management** in the console (`iam-identity` in the API or CLI).
 
5. If your account [restricts API key creation](/docs/iam?topic=iam-allow-api-create), add the **User API key creator** role to **Identity and Access Management** in the console (`iam-identity` in the API or CLI).
...
6. Complete your setup by using one of the following options.
    ...
    * To restrict the image pull secret access to particular registry resources such as namespaces or regions:
        ...
        1. [Edit the {{site.data.keyword.cloud_notm}} IAM policies](/docs/iam?topic=iam-serviceids&interface=ui#update_serviceid) for the service ID, or [create another image pull secret](#other_registry_accounts).
