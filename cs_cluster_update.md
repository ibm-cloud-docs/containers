...
![Classic infrastructure provider icon.](images/icon-classic.png) Applies to only classic clusters. Have a VPC cluster? See [Updating VPC worker nodes](#vpc_worker_node) instead.
{: note}
...
![VPC infrastructure provider icon.](images/icon-vpc.png)  Applies to only VPC clusters. Have a classic cluster? See [Updating classic worker nodes](#worker_node) instead.
{: note}
...
3. Create a worker node with the new machine type.
    - **For worker nodes in a worker pool**:
        ...
        * ![VPC infrastructure provider icon.](images/icon-vpc.png) VPC Generation 2 clusters:
            ```sh
