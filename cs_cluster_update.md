...
For example, if your current Kubernetes API server version is 1.16 (`n`) and you want to update to 1.18, you must first update to 1.17 (`n+1`) and then to 1.18 (`n+2`). Next, you can update the worker nodes up to two version ahead, such as 1.16 to 1.18 (`n+2`).
...
4.  Replace the worker node to update either the patch version or the `major.minor` version that matches the master version.
    *  To update the worker node to the same `major.minor` version as the master, such as from 1.17.11 to 1.18.8, include the `--update` flag.
       ```
       ibmcloud ks worker replace --cluster <cluster_name_or_ID> --worker <worker_node_ID> --update
       ```
       {: pre}
    *  To update the worker node to the latest patch version at the same `major.minor` version, such as from 1.17.8_1530 to 1.17.9_1533, do not include the `--update` flag.
       ```
...
Control when the Ingress application load balancer (ALB) component is updated. For information about keeping ALBs up-to-date, see [Managing the Ingress ALB lifecycle](/docs/containers?topic=containers-ingress-manage).
{: shortdesc}
