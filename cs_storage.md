...
**Note**: Block storage is a `ReadWriteOnce` access mode device. You can be mount it to only one worker node in the cluster at a time.
...
**Note**: Block storage is a `ReadWriteOnce` access mode device. You can mount it to only one worker node in the cluster at a time. NFS file storage is a `ReadWriteMany` access mode, so you can mount it to multiple workers within the cluster.
