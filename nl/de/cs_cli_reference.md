...
Festlegen von Berechtigungsnachweisen für das Konto der IBM Cloud-Infrastruktur (SoftLayer) für eine {{site.data.keyword.containerlong_notm}}-Region.
...
### ibmcloud ks logging-filter-create --cluster CLUSTER --type PROTOKOLLTYP [--logging-configs KONFIGURATIONEN][--namespace KUBERNETES_NAMESPACE] [--container CONTAINERNAME][--level LOGGING_LEVEL] [--regex-message NACHRICHT][--force-update]  [--json][-s]
...
### ibmcloud ks logging-filter-update --cluster CLUSTER --id FILTER-ID --type PROTOKOLLTYP [--logging-configs KONFIGURATIONEN][--namespace KUBERNETES_NAMESPACE] [--container CONTAINERNAME][--level LOGGING_LEVEL] [--message NACHRICHT][--force-update] [--json][-s]
...
<strong>Befehlsoptionen</strong>:
   ...
   <dt><code>--worker <em>WORKERKNOTE-ID</em></code></dt>
...
3. Überprüfen Sie, ob die Pod-Planung für Ihren Workerknoten inaktiviert ist
 ...
 5. Führen Sie einen Warmstart für den Workerknoten durch. Verwenden Sie die Worker-ID, die vom Befehl `ibmcloud ks workers <cluster_name_or_ID>` zurückgegeben wird.
    ```
    ibmcloud ks worker-reboot --cluster <clustername_oder_-id> --worker <workername_oder_-id>
    ```
    {: pre}
...
### ibmcloud ks worker-pool-create --name POOLNAME --cluster CLUSTER --machine-type MASCHINENTYP --size-per-zone WORKER_PRO_ZONE [--hardware ISOLATION][--labels LABELS] [--disable-disk-encrypt][-s] [--json]
