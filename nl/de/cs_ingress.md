...
Sie können mehrere Apps in Ihrem Cluster öffentlich zugänglich machen, indem Sie Ingress-Ressourcen erstellen, die von der durch IBM bereitgestellten Lastausgleichsfunktion für Anwendungen verwaltet werden. Eine Lastausgleichsfunktion für Anwendungen ist eine externe HTTP- oder HTTPS-Lastausgleichsfunktion, die einen gesicherten und eindeutigen öffentlichen oder privaten Eingangspunkt zum Weiterleiten von eingehenden Anforderungen an Ihre Apps innerhalb und außerhalb Ihres Clusters verwendet. Mit Ingress können Sie individuelle Weiterleitungsregeln für jede App definieren, die Sie für öffentliche oder private Netze zugänglich machen. Allgemeine Informationen zu Ingress-Services finden Sie in [Externen Netzbetrieb mit Ingress planen](cs_network_planning.html#ingress).
...
Wenn Sie einen Standardcluster erstellen, wird automatisch eine von IBM bereitgestellte Lastausgleichsfunktion für Anwendungen aktiviert, der eine portierbare öffentliche IP-Adresse und eine öffentliche Route zugewiesen ist. Jeder App, die öffentlich über Ingress zugänglich gemacht wird, ist ein eindeutiger Pfad zugewiesen, der an die öffentliche Route angehängt wird, sodass Sie eine eindeutige URL verwenden können, um öffentlich auf eine App in Ihrem Cluster zuzugreifen. Um Ihre App öffentlich zugänglich zu machen, können Sie Ingress für die folgenden Szenarios konfigurieren.
...
Sie können die Lastausgleichsfunktion für Anwendungen als Lastausgleichsfunktion für eingehenden HTTP-Netzdatenverkehr für die Apps in Ihrem Cluster konfigurieren und die von IBM bereitgestellte Domäne für den Zugriff auf Ihre Apps über das Internet verwenden.
...
Sie können die Ingress-Lastausgleichsfunktion für Anwendungen so konfigurieren, dass eingehende TLS-Verbindungen für Ihre Apps verwaltet, der Netzverkehr mithilfe des von IBM bereitgestellten TLS-Zertifikats entschlüsselt und die nicht verschlüsselte Anforderung an die Apps weitergeleitet wird, die in Ihrem Cluster zugänglich sind.
...
Wenn Sie einen Standardcluster erstellen, wird automatisch eine von IBM bereitgestellte Lastausgleichsfunktion für Anwendungen erstellt, der eine portierbare öffentliche IP-Adresse und eine private Route zugewiesen ist. Die standardmäßige private Lastausgleichsfunktion für Anwendungen wird jedoch nicht automatisch aktiviert. Um Ihre App für private Netze zugänglich zu machen, [aktivieren Sie zunächst die private Lastausgleichsfunktion für Anwendungen](#private_ingress). Anschließend können Sie Ingress für die folgenden Szenarios konfigurieren.
...
Sie können private Lastausgleichsfunktionen für Anwendungen verwenden, um den eingehenden Netzverkehr an die Apps in Ihrem Cluster weiterzuleiten, und Sie können Ihr eigenes TLS-Zertifikat zum Verwalten der TLS-Terminierung nutzen, wobei Sie Ihre angepasste Domäne verwenden.
{:shortdesc}
...
Gehen Sie wie folgt vor, um eine App unter Verwendung einer angepassten Domäne mit TLS nicht öffentlich zugänglich zu machen:
...
## Optional: Lastausgleichsfunktion für Anwendungen konfigurieren
{: #configure_alb}
