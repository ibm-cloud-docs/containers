...
<caption>Sicherheit für Kubernetes-API-Server und 'etcd'</caption>
  ...
  <tbody>
    ...
    <tr>
      ...
      <li>[`PodSecurityPolicy`](/docs/containers?topic=containers-psp#ibm_psp) (Kubernetes 1.8.13, 1.9.8 oder 1.10.3 und höher)</li>
...
Wenn Sie eingehenden Netzverkehr aus dem Internet zulassen möchten, müssen Sie Ihre Apps mit einem [NodePort-Service, einem LoadBalancer-Service oder einer Ingress-Lastausgleichsfunktion für Anwendungen](/docs/containers?topic=containers-cs_network_planning#external) zugänglich machen.  
...
Das Beibehalten der IP des Clients ist nützlich, z. B. wenn App-Server Sicherheits- und Zugriffssteuerungsrichtlinien genügen müssen. Um die ursprüngliche Quellen-IP-Adresse der Clientanforderung beizubehalten, können Sie die Quellen-IP-Beibehaltung für [Lastausgleichsfunktionen](/docs/containers?topic=containers-loadbalancer#node_affinity_tolerations) oder [Ingress-ALBs](/docs/containers?topic=containers-ingress#preserve_source_ip) aktivieren.
...
Sie können auch einen {{site.data.keyword.Bluemix_notm}}Datenbankservice verwenden, wie zum Beispiel den [{{site.data.keyword.cloudant}} NoSQL-Datenbankservice](/docs/services/Cloudant?topic=cloudant-getting-started-with-cloudant#getting-started-with-cloudant), um Daten in einer verwalteten Datenbank außerhalb des Clusters als persistent zu erhalten. Auf Daten, die mit einem Datenbankservice gespeichert wurden, kann cluster-, zonen- und regionsübergreifend zugegriffen werden. Sicherheitsrelevante Informationen zur IBM Cloudant NoSQL-Datenbank finden Sie in der [Servicedokumentation](/docs/services/Cloudant/offerings?topic=cloudant-security#security).
...
Sie können auch integrierte Tools verwenden, wie z. B. die {{site.data.keyword.containerlong_notm}}-Detailseite oder das Kubernetes-Dashboard oder Sie können [Integrationen von Drittanbietern einrichten](/docs/containers?topic=containers-integrations#health_services), wie z. B. Prometheus, Sysdig, LogDNA, Weave Scope und andere.
...
1. **Erstellungsprozess (Build) automatisieren und Berechtigungen  eingrenzen**: </br>
...
3. **Regelmäßiges Prüfen von aktiven Containern:** </br>
