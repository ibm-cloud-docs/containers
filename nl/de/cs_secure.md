...
Setzen Sie den [Befehl](cs_cli_reference.html#cs_worker_update) `ibmcloud ks worker-update` in regelmäßigen Abständen (z. B. monatlich) ab, um Aktualisierungen und Sicherheitspatches für das Betriebssystem bereitzustellen und um die Kubernetes-Version zu aktualisieren. Sind Aktualisierungen verfügbar, werden Sie benachrichtigt, wenn Sie Informationen zu den Master- und Workerknoten in der {{site.data.keyword.Bluemix_notm}}-Konsole oder der -CLI anzeigen, beispielsweise mithilfe des Befehls `ibmcloud ks clusters` oder `ibmcloud ks workers <cluster_name>`.
{: important}
