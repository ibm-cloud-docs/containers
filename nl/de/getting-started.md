...
Rufen Sie vor Beginn den für Sie geeigneten [{{site.data.keyword.cloud_notm}}-Kontotyp](https://cloud.ibm.com/registration) ab:
* **Belastbar (nutzungsabhängig oder Abonnement)**: Sie können einen kostenlosen Cluster erstellen. Sie können auch Ressourcen der IBM Cloud-Infrastruktur für die Erstellung und Verwendung in Standardclustern bereitstellen.
...
*   **Typ**: Der kostenlose Cluster verfügt über einen in einem Worker-Pool gruppierten virtuellen Workerknoten mit zwei CPUs, 4 GB Speicher und einer einzelnen 100-GB-SAN-Platte, die für die Apps zur Verfügung steht. Wenn Sie einen Standardcluster erstellen, können Sie zwischen physischen (Bare-Metal-Maschinen) oder virtuellen Maschinen sowie verschiedenen Maschinengrößen wählen.
...
*   **Infrastrukturressourcen**: Die für die Ausführung des Clusters erforderlichen Ressourcen, z. B. VLANs und IP-Adressen, werden in einem Konto von {{site.data.keyword.IBM_notm}} für die IBM Cloud-Infrastruktur verwaltet. Wenn Sie einen Standardcluster erstellen, verwalten Sie diese Ressourcen in Ihrem eigenen Konto der IBM Cloud-Infrastruktur. Weitere Informationen zu diesen Ressourcen und den [erforderlichen Berechtigungen](/docs/containers?topic=containers-users#infra_access) erhalten Sie, wenn Sie einen Standardcluster erstellen.
...
* Arbeiten Sie das [{{site.data.keyword.containerlong_notm}}-Lernprogramm](/docs/containers?topic=containers-cs_cluster_tutorial#cs_cluster_tutorial) durch, um die CLI zu installieren oder das Kubernetes-Terminal zu verwenden, eine private Registry zu erstellen, Ihre Clusterumgebung einzurichten, Ihrem Cluster einen Service hinzuzufügen und eine {{site.data.keyword.watson}}-App bereitzustellen.
...
* Testen Sie einen [{{site.data.keyword.openshifshort}}-Cluster](/docs/openshift?topic=openshift-openshift_tutorial).
