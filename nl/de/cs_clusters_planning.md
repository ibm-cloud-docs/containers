...
1.  Lesen Sie die Informationen im Abschnitt [Ausschließlich private Clusternetze planen](cs_network_planning.html#private_vlan).
...
4.  Wenn Sie eine App über einen privaten NodePort-, LoadBalancer- oder Ingress-Service in einem privaten Netz zugänglich machen möchten, lesen Sie die Informationen im Abschnitt [Nur private externe Netze für private VLAN-Konfiguration planen](cs_network_planning.html#private_vlan). Auf den Service kann nur über die private IP-Adresse zugegriffen werden und Sie müssen die Ports in Ihrer Firewall so konfigurieren, dass sie die private IP-Adresse verwenden.
...
SDS-Typen (SDS = Software-defined Storage) sind physische Maschinen, die mit einer unformatierten Platte für den physischen lokalen Speicher bereitgestellt werden. Da sich die Daten auf derselben Maschine wie der Rechenknoten befinden, eignen sich SDS-Maschinen für hohe Workloads.
{: shortdesc}
