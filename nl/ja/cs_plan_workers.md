...
{{site.data.keyword.cloud_notm}} で標準クラスターを作成する場合は、ワーカー・プールを構成するワーカー・ノードとして、物理マシン (ベアメタル) を使用するか、または物理ハードウェア上で実行される仮想マシンを使用するかを選択できます。また、ワーカー・ノードのフレーバー、つまり、メモリー、CPU、ディスク・ストレージなどのその他のマシン仕様の組み合わせを選択します。
{:shortdesc}
