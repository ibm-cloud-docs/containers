...
ストレージのプロビジョニングを開始する前に、Kubernetes の永続ボリュームと永続ボリューム請求という概念と、それらのクラスター内での機能を理解することが重要です。{: shortdesc}
...
開発者にストレージを自由にプロビジョンさせる場合は、動的プロビジョニングを使用します。{: shortdesc}
...
IBM Cloud インフラストラクチャー (SoftLayer) アカウントに既存の永続ストレージ・デバイスがある場合は、静的プロビジョニングを使用してクラスターでストレージ・インスタンスを使用可能にすることができます。{: shortdesc}
...
永続ストレージを動的にプロビジョンするには、ストレージのタイプと構成を定義する必要があります。{: shortdesc}

Kubernetes ストレージ・クラスを使用して、{{site.data.keyword.Bluemix_notm}} でサポートされる基礎のストレージ・プラットフォームを抽象化しておけば、サポートされるサイズ、IOPS、保存ポリシーについて詳しく知らなくても、永続ストレージをクラスターにプロビジョンできます。{{site.data.keyword.containershort_notm}} には、サポートされるストレージ・タイプごとに、事前定義のストレージ・クラスが用意されています。各ストレージ・クラスは、サポートされているストレージ層を抽象化した設計になっていますが、サイズや IOPS や保存ポリシーを選択することも可能です。 
...
求めているストレージ・クラスが見つからない場合は、独自のカスタマイズ・ストレージ・クラスを作成して、必要なタイプのストレージをプロビジョンできます。{: tip}
...
ストレージ・クラスを使用して永続ストレージを動的にプロビジョンする場合は、特定の構成 (フレーバー) で永続ストレージをプロビジョンすることになります。ストレージのプロビジョン後にこのフレーバーを変更することはできません。 

ストレージ・デバイスのサイズや IOPS やタイプや保存ポリシーを変更する場合は、新しいストレージ・デバイスをプロビジョンして、古いストレージから新しいストレージにデータをコピーする必要があります。 

1. プロビジョンする永続ストレージのフレーバーを決定します。
   - [ファイル・ストレージ](cs_storage_file.html#predefined_storageclass)
   - [ブロック・ストレージ](cs_storage_block.html#predefined_storageclass)
   
2. 永続ストレージをプロビジョンするための PVC を作成します。 
   - [ファイル・ストレージ](cs_storage_file.html#add_file)
   - [ブロック・ストレージ](cs_storage_block.html#add_block)
   
3. 既存のストレージ・デバイスから新しいストレージ・デバイスにデータをコピーします。データのバックアップ/リストアのオプションは、ストレージのタイプによって異なります。 
   - [ファイル・ストレージ](cs_storage_file.html#backup_restore)
   - [ブロック・ストレージ](cs_storage_block.html#backup_restore)
   
4. 新しいストレージ・デバイスを参照するようにアプリ・デプロイメントを更新します。 
   1. 作成した PVC の名前を取得します。
      ```
      kubectl get pvc
      ```
      {: pre}
      
   2. クラスター内のデプロイメントを表示します。
      ```
      kubectl get deployments
      ```
      {: pre}
      
   3. 古い永続ストレージ・デバイスを使用するデプロイメントのデプロイメント yaml を取得します。
      ```
      kubectl get deployment <deployment_name> -o yaml
      ```
      {: pre}
      
   4. デプロイメントの `spec.volumes` セクションにある PVC の名前を変更します。
   5. 変更内容をデプロイメントに適用します。 変更を適用すると、新しいポッドが作成され、PV をバインドする PVC がポッドにマウントされます。古い永続ストレージ・インスタンスをマウントしたポッドが削除されます。
      ```
...
単一ゾーン・クラスターを複数ゾーン・クラスターに更新した場合に、既存の永続ボリューム (PV) があれば、Kubernetes のゾーンと地域のラベルをその PV に追加します。ラベルを付けることによって、このストレージをマウントするポッドが、永続ストレージが存在するゾーンに確実にデプロイされるようになります。{:shortdesc}

**注:** この手順が必要になるのは、複数ゾーン機能を有効にする前に作成した既存の PV がある場合に限られます。複数ゾーンを有効にした後に作成した PV には、Kubernetes の領域とゾーンのラベルが既に含まれています。 
...
- IBM Cloud インフラストラクチャー (SoftLayer) アカウントの [VLAN スパンニング](/docs/infrastructure/vlans/vlan-spanning.html#vlan-spanning)を有効にして、ワーカー・ノードがプライベート・ネットワーク上で相互に通信できるようにします。この操作を実行するには、**「ネットワーク」>「ネットワーク VLAN スパンニングの管理」**で設定する[インフラストラクチャー権限](cs_users.html#infra_access)が必要です。ない場合は、アカウント所有者に対応を依頼してください。VLAN スパンニングの代わりに仮想ルーター機能 (VRF) を使用することもできます。ただし、IBM Cloud インフラストラクチャー (SoftLayer) アカウントでその機能を有効にする必要があります。
