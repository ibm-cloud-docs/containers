...
**最小限必要な許可**: {{site.data.keyword.registrylong_notm}} に対する**リーダー**のサービス・アクセス役割。 **注**: {{site.data.keyword.registryshort_notm}} のポリシーをリソース・グループ・レベルで割り当てないでください。
...
ALB シークレットに関する情報を取得する場合の例:
```
ibmcloud ks alb-cert-get --cluster my_cluster --secret-name my_alb_secret
```
{: pre}

指定された証明書 CRN と一致する、すべての ALB シークレットに関する情報を取得する場合の例:
```
...
<dl>
<dt><code>--region <em>REGION</em></code></dt>
<dd>地域を指定します。 使用可能な地域をリストするには、<code>ibmcloud ks regions</code> を実行します。 この値は必須です。</dd>
...
<dl>
<dt><code>--cluster <em>CLUSTER</em></code></dt>
<dd>スナップショットを作成するクラスターの名前または ID。</dd>
...
<dd>{{site.data.keyword.cos_short}} インスタンスの HMAC 鍵。</dd>

<dt><code>--type <em>LOG_TYPE</em></code></dt>
<dd>オプション: スナップショットを作成する対象のログのタイプ。 現在、`master` が唯一のオプションであり、デフォルトでもあります。</dd>
...
**最小限必要な許可**: `kube-audit` 以外のログ・ソースの場合はクラスターに対する**エディター**のプラットフォーム役割、`kube-audit` ログ・ソースの場合はクラスターに対する**管理者**のプラットフォーム役割
...
<dd>ヘルス・チェック・モニターの構成対象のホスト名。 ホスト名をリストするには、<code>ibmcloud ks nlb-dnss --cluster CLUSTER</code> を実行します。</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>1 つのホスト名の状況のみを表示するには、このフラグを含めます。 ホスト名をリストするには、<code>ibmcloud ks nlb-dnss --cluster CLUSTER</code> を実行します。</dd>
...
選択可能な地域のリストを参照するには、[ロケーション](/docs/containers?topic=containers-regions-and-zones)を確認するか、`ibmcloud ks regions` [コマンド](#cs_regions)を使用します。</dd></dl>
...
<td><code><em>&lt;private_VLAN&gt;</em></code> を、ワーカー・ノードに使用するプライベート VLAN の ID に置き換えます。 使用可能な VLAN をリストするには、<code>ibmcloud ks vlans --zone <em>&lt;zone&gt;</em></code> を実行して、<code>bcr</code> で始まる VLAN ルーター (バックエンド・ルーター) を探します。</td>
</tr>
<tr>
...
<dd>ワーカー・ノードの名前。 クラスター内のワーカー・ノードの ID を表示するには、<code>ibmcloud ks workers --cluster <em>CLUSTER</em></code> を実行します。 この値は必須です。</dd>
...
1. クラスター内のすべてのワーカー・ノードをリストして、削除するワーカー・ノードの**名前**をメモします。
   ...
   ```
   {: pre}
   このコマンドで返される**名前**は、ワーカー・ノードに割り当てられたプライベート IP アドレスです。 `ibmcloud ks workers --cluster <cluster_name_or_ID>` コマンドを実行して同じ**プライベート IP** アドレスのワーカー・ノードを探せば、より詳しいワーカー・ノードの情報が得られます。
...
4. ポッドをワーカー・ノードから強制的に削除し、クラスター内の残りのワーカー・ノードにスケジュールを変更します。
  ...
  {: pre}
  この処理には数分かかる場合があります。
5. ワーカー・ノードをリブートします。 `ibmcloud ks workers --cluster <cluster_name_or_ID>` コマンドから返されるワーカー ID を使用します。
  ```
...
3. ワーカー・ノードでポッドのスケジューリングが無効になっていることを確認します。
 ...
 4. ポッドをワーカー・ノードから強制的に削除し、クラスター内の残りのワーカー・ノードにスケジュールを変更します。
    ...
    {: pre}
    この処理には数分かかる場合があります。
 5. ワーカー・ノードを再ロードします。 `ibmcloud ks workers --cluster <cluster_name_or_ID>` コマンドから返されるワーカー ID を使用します。
    ```
...
4. ポッドをワーカー・ノードから強制的に削除し、クラスター内の残りのワーカー・ノードにスケジュールを変更します。
   ...
   {: pre}
   この処理には数分かかる場合があります。
5. ワーカー・ノードを削除します。 `ibmcloud ks workers --cluster <cluster_name_or_ID>` コマンドから返されるワーカー ID を使用します。
   ```
...
<dt><code>--private-only </code></dt>
<dd>オプション: パブリック VLAN を設定解除して、このゾーン内のワーカーがプライベート VLAN のみに接続されるようにします。</dd>
...
<li>使用するパブリック VLAN ID とプライベート VLAN ID に互換性があることを確認します。 対応している VLAN は、<strong>Router</strong> のポッド ID が同じです。 必ず、プライベート VLAN ルーターの先頭は <code>bcr</code> (バックエンド・ルーター)、パブリック VLAN ルーターの先頭は <code>fcr</code> (フロントエンド・ルーター) になります。 クラスターを作成し、パブリック VLAN とプライベート VLAN を指定するときには、それらの接頭部の後の番号と文字の組み合わせが一致する必要があります。<pre class="pre"><code>ibmcloud ks vlans --zone &lt;zone&gt;</code></pre><p>出力例:</p>
