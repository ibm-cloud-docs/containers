...
IBM Bluemix Infrastructure (SoftLayer) アカウントでサブネットを作成し、{{site.data.keyword.containershort_notm}} において指定されたクラスターでそのサブネットを使用できるようにします。
...
<dd>ログ転送を有効にする対象のログ・ソース。指定可能な値は <code>application</code>、<code>worker</code>、<code>kubernetes</code>、<code>ingress</code> です。この値は、Docker コンテナー名前空間以外のログ・ソースの場合に必須です。</dd>
...
<strong>コマンド・オプション</strong>:
   ...
   <dd>クラスターの名前または ID。この値は必須です。</dd>
   <dt><code>--logsource <em>LOG_SOURCE</em></code></dt>
   <dd>フィルター操作で取得するログ・ソースの種類。クラスター内のこのログ・ソースのロギング構成のみが返されます。指定可能な値は <code>namespace</code>、<code>application</code>、<code>worker</code>、<code>kubernetes</code>、<code>ingress</code> です。この値はオプションです。</dd>
   </dl>
...
<strong>コマンド・オプション</strong>:
   ...
   <dd>クラスターの名前または ID。この値は必須です。</dd>
   <dt><code>--namespace <em>KUBERNETES_NAMESPACE</em></code></dt>
   <dd>syslog へのログの転送を停止する対象の Docker コンテナー名前空間。この値は Docker コンテナー名前空間の場合に必須です。</dd>
   <dt><code>--id <em>LOG_SOURCE_LOGGING_ID</em></code></dt>
   <dd>ログ・ソースから削除するロギング構成 ID。この値は、Docker コンテナー名前空間以外のログ・ソースの場合に必須です。</dd>
   </dl>
...
<strong>コマンド・オプション</strong>:
   ...
   <dt><code>--logsource <em>LOG_SOURCE</em></code></dt>
   <dd>ログ転送を更新する対象のログ・ソース。指定可能な値は <code>application</code>、<code>worker</code>、<code>kubernetes</code>、<code>ingress</code> です。この値は、Docker コンテナー名前空間以外のログ・ソースの場合に必須です。</dd>
   <dt><code>--id <em>LOG_SOURCE_LOGGING_ID</em></code></dt>
