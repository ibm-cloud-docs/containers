...
この例の特権的な `initContainer` を実行するには、すべての名前空間に対して[**管理者**の {{site.data.keyword.cloud_notm}} IAM サービス役割](/docs/containers?topic=containers-users#platform)を持っている必要があります。 デプロイメントのコンテナーが初期化された後、特権は除去されます。
{: note}
...
開始する前に、この例の特権的な `initContainer` を実行するために、すべての名前空間に対して[**管理者**の {{site.data.keyword.cloud_notm}} IAM サービス役割](/docs/containers?topic=containers-users#platform)を持っていることを確認してください。 デプロイメントのコンテナーが初期化された後、特権は除去されます。
