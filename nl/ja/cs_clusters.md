...
1.  [{{site.data.keyword.Bluemix_notm}} 従量課金またはサブスクリプション・アカウントを作成するか、ご使用のアカウントをこの種のアカウントにアップグレードします](https://console.bluemix.net/registration/)。
...
3.  ご使用のアカウントで複数のリソース・グループが使用されている場合は、[リソース・グループを管理](cs_users.html#resource_groups)するためのアカウントの戦略を考えます。 
    ...
    *  クラスターのリソース・グループを変更することはできません。クラスターは、同じリソース・グループ内の他の {{site.data.keyword.Bluemix_notm}} サービスとしか統合できません。
...
3.  [クラスターのセットアップを計画](cs_clusters_planning.html#plan_clusters)します。
    *  [単一ゾーン](cs_clusters_planning.html#single_zone)・クラスターと[複数ゾーン](cs_clusters_planning.html#multizone)・クラスターのどちらを作成するかを決定します。複数ゾーン・クラスターは、特定の場所のみで使用できることに注意してください。
...
4.  標準クラスターの場合、[料金カリキュレーターを使用してコストを見積もることができます ![外部リンク・アイコン](../icons/launch-glyph.svg "外部リンク・アイコン")](https://console.bluemix.net/pricing/configure/iaas/containers-kubernetes)。**注**: 見積もりツールに含まれない可能性がある料金について詳しくは、[価格設定および請求](cs_why.html#pricing)を参照してください。
<br>
<br>

**次の作業**
* [GUI でのクラスターの作成](#clusters_ui)
* [CLI でのクラスターの作成](#clusters_cli)
...
始めに、[クラスター作成の準備](#cluster_prepare)に必要な手順を確認します。
...
開始前に、以下のことを行います。
* [クラスター作成の準備](#cluster_prepare)に必要な手順を確認します。
...
1. ワーカー・プール内のワーカー・ノードをデプロイする、クラスター内の**ワーカー・ゾーン**を選択します。既存のワーカー・プールにあるワーカー・ノードを複数のゾーン間に分散させることを計画している場合は、ゾーンを選択するか、[複数ゾーン対応](cs_regions.html#zones)クラスターに[新しいゾーンを追加](#add_zone)します。`ibmcloud ks zones` を実行して、使用可能なゾーンをリストできます。
   ```
