...
グローバルな可用性を達成するために、開発、テスト、実動用のシステムが世界中のいくつかのデータ・センターにデプロイされます。HA に関しては、複数の地理的地域に配置するクラスターと、複数ゾーン・クラスターの組み合わせを使用します。ヨーロッパの現地の法規制に準拠するために、フランクフォートのクラスターに研究用アプリを簡単にデプロイすることができます。また、アプリを米国内のクラスターにもデプロイして、その地域での可用性を確保してリカバリーできるようにします。さらに、ヨーロッパのアプリを使用できるようにするためと、ワークロードのバランスを効率的に取るようにするために、研究用のワークロードをフランクフォートにある複数ゾーン・クラスター全体に分散します。研究者は研究共有アプリを使用して機密データをアップロードするので、このアプリのクラスターはより厳しい規制が適用される地域でホストされます。
