...
## {{site.data.keyword.Bluemix_notm}} 中的地區
{: #bluemix_regions}
...
若要檢查您目前所在的 {{site.data.keyword.Bluemix_notm}} 地區，請執行 `ibmcloud info`，並檢閱**地區**欄位。
...
* **多區域都會城市**：叢集裡在多區域都會城市中建立的工作者節點可以分散到各區域。
* **單一區域城市**：叢集裡在單一區域城市中建立的工作者節點都會保留在某個區域中。您無法將工作者節點分散到多個區域。
