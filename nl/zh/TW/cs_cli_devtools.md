...
<dl>
   ...
   <dd>（標準叢集的選用項目。精簡叢集無法使用。）建立標準叢集之 YAML 檔案的路徑。您可以使用 YAML 檔案，而不是使用此指令中所提供的選項來定義叢集的特徵。
    ...
    <dd>（精簡叢集無法使用。）
        ...
        <li>如果您之前已在此位置中建立叢集，或之前已在 {{site.data.keyword.BluSoftlayer_notm}} 中建立專用 VLAN，則必須指定該專用 VLAN。
    ...
    <dd>（精簡叢集無法使用。）
        ...
        <li>如果您之前已在此位置中建立叢集，或之前已在 {{site.data.keyword.BluSoftlayer_notm}} 中建立公用 VLAN，則必須指定該公用 VLAN。
    ...
    <dd>（標準叢集的選用項目。精簡叢集無法使用。）您要在叢集中部署的工作者節點數目。如果您未指定此選項，則會建立具有 1 個工作者節點的叢集。
...
<strong>指令選項</strong>：
   ...
   <dd>（必要）叢集的名稱或 ID。</dd>
    ...
    <dd>（必要）如果您有可用於位置的專用 VLAN，則必須指定該 VLAN。如果這是您在這個位置所建立的第一個叢集，請不要使用此旗標。會自動建立一個專用 VLAN。

        <p><strong>附註：</strong>您使用 create 指令所指定的公用及專用 VLAN 必須相符。專用 VLAN 路由器的開頭一律為 <code>bcr</code>（後端路由器），而公用 VLAN 路由器的開頭一律為 <code>fcr</code>（前端路由器）。這些字首後面的數字與字母組合必須相符，才能在建立叢集時使用這些 VLAN。請不要使用不相符的公用及專用 VLAN 來建立叢集。</p></dd>

    <dt><code>--public-vlan <em>PUBLIC_VLAN</em></dt>
    <dd>（必要）如果您有可用於位置的公用 VLAN，則必須指定該 VLAN。如果這是您在這個位置所建立的第一個叢集，請不要使用此旗標。會自動建立一個公用 VLAN。
