...
2.  檢查負載平衡器服務配置檔的正確性。
    ...
    2.  確定您在 LoadBalancer 服務 `spec.selector` 區段使用的 `<selector_key>` 及 `<selector_value>`，與您在部署 yaml `spec.template.metadata.labels` 區段使用的金鑰/值配對相同。如果標籤不符，則 LoadBalancer 服務中的 **Endpoints** 區段會顯示 **<none>**，且無法從網際網路存取您的應用程式。
    3.  確認您已使用應用程式所接聽的**埠**。
...
2.  擷取 Ingress 應用程式負載平衡器子網域及公用 IP 位址，然後對其進行連線測試。
    ...
    如果 CLI 傳回 Ingress 應用程式負載平衡器公用 IP 位址或子網域的逾時，並且已設定保護工作者節點的自訂防火牆，您可能需要開啟[防火牆](cs_troubleshoot_clusters.html#cs_firewall)中的其他埠及網路群組。
...
<table>
 ...
 <td>請洽詢帳戶管理者，指派您 {{site.data.keyword.cloudcerts_full_notm}} 實例的**操作員**和**編輯者**角色。如需詳細資料，請參閱 {{site.data.keyword.cloudcerts_short}} 的<a href="/docs/services/certificate-manager/access-management.html#managing-service-access-roles">管理服務存取</a>。</td>
 </tr>
 <tr>
...
2.  按一下您用來建立叢集之 VLAN 的 **VLAN 號碼**。檢閱**子網路**區段，以查看是否有 40 個以上的子網路。

{: tsResolve}
如果您需要新的 VLAN，請[與 {{site.data.keyword.Bluemix_notm}} 支援中心聯絡](/docs/get-support/howtogetsupport.html#getting-customer-support)來訂購。然後，[建立一個使用這個新 VLAN 的叢集](cs_cli_reference.html#cs_cluster_create)。

如果您有另一個可用的 VLAN，可以在現有叢集中[設定 VLAN 跨距](/docs/infrastructure/vlans/vlan-spanning.html#enable-or-disable-vlan-spanning)。之後，您便可以將新的工作者節點新增至使用具有可用子網路之另一個 VLAN 的叢集。
...
3. 請檢查下列設定，並視需要進行變更，以反映已刪除或新增的工作者節點。
    ...
    <table>
     ...
     <tr>
     <td><code>localSubnetNAT</code></td>
     <td>如果您使用子網路 NAT 來重新對映特定的專用本端 IP 位址，請從這個設定移除舊工作者節點中的任何 IP 位址。如果您使用子網路 NAT 來重新對映整個子網路，且子網路上沒有剩餘任何工作者節點，請從此設定中移除該子網路 CIDR。</td>
     </tr>
     <tr>
     <td><code>nodeSelector</code></td>
...
5. 檢查圖表部署狀態。圖表就緒時，輸出頂端附近的 **STATUS** 欄位，會具有 `DEPLOYED` 值。
...
- Windows：<ol>
    ...
    <li> 檢視 Calico 控制器 Pod 的詳細資料。</br> <pre class="codeblock"><code>kubectl describe pod -n kube-system calico-policy-controller-&lt;calico_pod_ID&gt;</code></pre>
