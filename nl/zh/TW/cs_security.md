...
* 當已針對工作者節點設定防火牆，或是在 IBM Cloud 基礎架構 (SoftLayer) 帳戶中自訂了防火牆設定時，打算容許 Kubernetes 主節點與工作者節點之間的通訊
