...
若要防範整個地區失敗，請建立[多個叢集，並將它們分散在各 {{site.data.keyword.containerlong_notm}} 地區](/docs/containers?topic=containers-plan_clusters#multiple_clusters)之中。設定叢集的網路負載平衡器 (NLB)，即可達到叢集的跨地區負載平衡及跨地區網路。
