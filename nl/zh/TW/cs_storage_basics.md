...
使用儲存空間類別來動態佈建持續性儲存空間時，您可以使用特定配置（也稱為特性）來佈建持續性儲存空間。在佈建您的儲存空間之後，無法變更此特性。 

如果想要變更儲存裝置的大小、IOPS、類型或保留原則，您必須佈建新的儲存裝置，並將資料從舊的儲存空間複製到新的儲存空間。 

1. 決定您想要佈建的持續性儲存空間特性。
   - [檔案儲存空間](cs_storage_file.html#predefined_storageclass)
   - [區塊儲存空間](cs_storage_block.html#predefined_storageclass)
   
2. 建立 PVC 來佈建您的持續性儲存空間。 
   - [檔案儲存空間](cs_storage_file.html#add_file)
   - [區塊儲存空間](cs_storage_block.html#add_block)
   
3. 將資料從現有儲存裝置複製到新的儲存裝置。每種類型的儲存空間都有不同的選項，用來備份及還原資料。 
   - [檔案儲存空間](cs_storage_file.html#backup_restore)
   - [區塊儲存空間](cs_storage_block.html#backup_restore)
   
4. 更新應用程式部署，以參照新的儲存裝置。 
   1. 取得您先前所建 PVC 的名稱。
      ```
      kubectl get pvc
      ```
   ...
   3. 針對使用舊持續性儲存裝置的部署，取得其部署 yaml。
      ```
      kubectl get deployment <deployment_name> -o yaml
      ```
      {: pre}
      
   4. 在部署的 `spec.volumes` 區段中變更 PVC 的名稱。
   5. 將變更套用至您的部署。套用變更時，會建立新的 Pod，而且連結 PV 的 PVC 會裝載至您的 Pod。會移除已裝載舊持續性儲存空間的 Pod。
      ```
...
**附註：**只有在您已有在多區域功能可用之前建立的現有 PV 時，才需要進行這些步驟。在多區域可用之後建立的 PV 已經包含 Kubernetes 地區及區域標籤。 
