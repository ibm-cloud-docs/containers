...
建立 IBM 提供的應用程式負載平衡器所管理的 Ingress 資源，以公開叢集中的多個應用程式。應用程式負載平衡器是一個外部 HTTP 或 HTTPS 負載平衡器，使用安全且唯一的公用或專用進入點，將送入要求遞送至叢集內外部的應用程式。您可以使用 Ingress，為每個公開給公用或專用網路使用的應用程式定義個別的遞送規則。如需 Ingress 服務的一般資訊，請參閱[利用 Ingress 規劃外部網路](cs_network_planning.html#ingress)。
...
當建立標準叢集時，會自動啟用 IBM 提供的應用程式負載平衡器，並為其指派可攜式公用 IP 位址及公用路徑。每個透過 Ingress 公開給公用網路使用的應用程式都會獲指派附加至公用路徑的唯一路徑，因此，您可以使用唯一 URL 來公開存取叢集中的應用程式。若要將應用程式公開給公用網路使用，您可以針對下列情境配置 Ingress。
...
您可以配置應用程式負載平衡器，對送入應用程式的 HTTP 網路資料流量進行負載平衡，以及使用 IBM 提供的網域從網際網路存取應用程式。
...
2.  為要公開的應用程式，建立 Kubernetes 服務。只有在您的應用程式是透過叢集內的 Kubernetes 服務公開時，應用程式負載平衡器才能將應用程式併入 Ingress 負載平衡。
    ...
    2.  為您要公開給公用網路使用的應用程式定義應用程式負載平衡器服務。
    ...
    4.  在叢集中建立服務。
        ...
        ```
        {: pre}
    5.  針對每個您要公開給公用網路使用的應用程式，重複這些步驟。
3.  取得叢集的詳細資料，以檢視 IBM 提供的網域。將 _&lt;mycluster&gt;_ 取代為要公開給公用網路使用的應用程式部署到其中的叢集名稱。
...
您可以配置 Ingress 控制來管理應用程式的送入 TLS 連線、使用 IBM 提供的 TLS 憑證來解密網路資料流量，以及將未加密的要求轉遞至叢集中公開的應用程式。
...
您可以配置應用程式負載平衡器，將應用程式併入位於叢集外的叢集負載平衡中。IBM 提供的網域或自訂網域上的送入要求，會自動轉遞給外部應用程式。
...
當建立標準叢集時，會自動建立 IBM 提供的應用程式負載平衡器，並為其指派可攜式專用 IP 位址及專用路徑。不過，不會自動啟用預設專用應用程式負載平衡器。若要將應用程式公開給專用網路使用，請先[啟用預設專用應用程式負載平衡器](#private_ingress)。然後，您可以針對下列情境配置 Ingress。
...
在可以使用預設專用應用程式負載平衡器之前，您必須使用由 IBM 提供的可攜式專用 IP 位址，或您自己的可攜式專用 IP 位址來啟用該負載平衡器。**附註**：如果您在建立叢集時使用 `--no-subnet` 旗標，則必須先新增可攜式專用子網路或使用者管理的子網路，才能啟用專用應用程式負載平衡器。如需相關資訊，請參閱[要求叢集的其他子網路](cs_subnets.html#request)。
...
7.  建立 Ingress 資源。Ingress 資源定義您為應用程式所建立之 Kubernetes 服務的遞送規則，並且供應用程式負載平衡器用來將送入網路資料流量遞送至服務。只要每個應用程式都是透過叢集內的 Kubernetes 服務公開時，您就可以使用一個 Ingress 資源來為多個應用程式定義遞送規則。
    ...
    2.  在配置檔中定義 Ingress 資源，以使用自訂網域將送入的網路資料流量遞送至您的服務。針對每個服務，您可以定義附加至自訂網域的個別路徑，以建立應用程式的唯一路徑，例如 `https://mydomain/myapp`。當您將此路徑輸入 Web 瀏覽器時，會將網路資料流量遞送至應用程式負載平衡器。應用程式負載平衡器會查閱相關聯的服務，然後將網路資料流量傳送至服務，並進一步傳送至執行應用程式的 Pod。
        ...
        **附註：**應用程式必須接聽 Ingress 資源中所定義的路徑。否則，無法將網路資料流量轉遞至應用程式。大部分的應用程式不會接聽特定路徑，但使用根路徑及特定埠。在此情況下，請將根路徑定義為 `/`，並且不要為應用程式指定個別路徑。
        ...
        <td>將 <em>&lt;private_ALB_ID&gt;</em> 取代為專用應用程式負載平衡器的 ID。執行 <code>bx cs albs --cluster <my_cluster></code> 以尋找應用程式負載平衡器 ID。如需此 Ingress 註釋的相關資訊，請參閱[專用應用程式負載平衡器遞送](cs_annotations.html#alb-id)。</td>
        </tr>
        <td><code>host</code></td>
...
您可以使用專用應用程式負載平衡器，將送入的網路資料流量遞送至叢集中的應用程式，以及使用自己的 TLS 憑證來管理 TLS 終止，同時使用自訂網域。
{:shortdesc}
