...
提供了重新设计的 {{site.data.keyword.containerlong_notm}} 插件的以下 Beta 版本。
...
**最低必需许可权**：对 {{site.data.keyword.registrylong_notm}} 的**读取者**服务访问角色。**注**：不要在资源组级别为 {{site.data.keyword.registryshort_notm}} 分配策略。
...
<dd>要在可移植子网中创建的 IP 地址数。接受的值为 8、16、32 或 64。<p class="note"> 添加子网的可移植 IP 地址时，会使用 3 个 IP 地址来建立集群内部联网。所以不能将这 3 个 IP 地址用于 Ingress 应用程序负载均衡器 (ALB) 或用于创建网络负载均衡器 (NLB) 服务。例如，如果请求 8 个可移植公共 IP 地址，那么可以使用其中 5 个地址向公众公开应用程序。</p> </dd>

<dt><code>--vlan <em>VLAN_ID</em></code></dt>
<dd>要在其中创建子网的公用或专用 VLAN 的标识。必须选择现有工作程序节点连接到的公用或专用 VLAN。要查看工作程序节点连接到的公用或专用 VLAN，请运行 <code>ibmcloud ks cluster-get --cluster &lt;cluster&gt; --showResources</code>，然后在输出中查找 <strong>Subnet VLANs</strong> 部分。子网会在 VLAN 所在的区域中进行供应。</dd>
...
<dd>子网无类域间路由 (CIDR)。此值是必需的，且不得与 IBM Cloud Infrastructure (SoftLayer) 使用的任何子网相冲突。支持的前缀范围从 `/30`（1 个 IP 地址）到 `/24`（253 个 IP 地址）。如果将 CIDR 设置为一个前缀长度，而稍后需要对其进行更改，请先添加新的 CIDR，然后[除去旧 CIDR](#cs_cluster_user_subnet_rm)。</dd>
...
```
{: pre}
**最低必需许可权**：对 {{site.data.keyword.containerlong_notm}}
...
列出 IBM Cloud Infrastructure (SoftLayer) 帐户中的可用可移植子网。
{: shortdesc}

这将返回所有位置中的子网。要按特定位置过滤子网，请包含 `--locations` 标志。
...
获取与指定证书 CRN 匹配的所有 ALB 私钥相关信息的示例：
```
...
可以使用此命令来执行以下操作：
* 启用缺省专用 ALB。创建集群时，会在具有工作程序和可用专用子网的每个专区中创建缺省专用 ALB，但未启用缺省专用 ALB。但是，请注意，所有缺省公共 ALB 都会自动启用，并且缺省情况下，使用 `ibmcloud ks alb-create` 命令创建的任何公共或专用 ALB 也都会启用。
* 启用先前禁用的 ALB。
* 在新 VLAN 上创建 ALB 后，禁用旧 VLAN 上的 ALB。有关更多信息，请参阅[在 VLAN 之间移动 ALB](/docs/containers?topic=containers-ingress#migrate-alb-vlan)。
* 禁用 IBM 提供的 ALB 部署，以便您可以部署自己的 Ingress 控制器，并利用 IBM 提供的 Ingress 子域的 DNS 注册或利用用于公开 Ingress 控制器的 LoadBalancer 服务。
...
<dd>包含此标志可在集群中禁用 ALB。<p class="note">如果禁用了 ALB，那么 ALB 使用的 IP 地址会返回给可用可移植 IP 池，以便其他服务可以使用该 IP。如果日后尝试重新启用 ALB，但其先前使用的 IP 地址现在正由其他服务使用，那么 ALB 可能会报告错误。您可以停止运行其他服务，或者指定在重新启用 ALB 时要使用的其他 IP 地址。</p></dd>
...
<dd>可选：如果使用 <code>--enable</code> 标志启用 ALB，那么可以指定在其中创建 ALB 的专区中 VLAN 上的 IP 地址。ALB 已启用并使用此公共或专用 IP 地址。请注意，此 IP 地址不能由集群中的其他负载均衡器或 ALB 使用。如果未提供任何 IP 地址，那么该 ALB 将使用创建集群时自动供应的可移植公用或专用子网中的公共或专用 IP 地址进行部署，或者使用先前分配给 ALB 的公共或专用 IP 地址进行部署。</dd>
...
在专区中创建公共或专用 ALB。缺省情况下，已启用创建的 ALB。
{: shortdesc}
...
<dd>ALB 的类型：<code>public</code> 或 <code>private</code>。</dd>

<dt><code>--zone <em>ZONE</em></code></dt>
<dd>在其中创建 ALB 的专区。</dd>

<dt><code>--vlan <em>VLAN_ID</em></code></dt>
<dd>要在其中创建 ALB 的 VLAN 的标识。此 VLAN 必须与 ALB <code>type</code> 相匹配，并且必须位于要创建的 ALB 所在的 <code>zone</code> 中。</dd>

<dt><code>--user-ip <em>IP</em></code></dt>
<dd>可选：要分配给 ALB 的 IP 地址。此 IP 必须位于指定的 <code>vlan</code> 上，并且必须位于要创建的 ALB 所在的 <code>zone</code> 中。请注意，此 IP 地址不能由集群中的其他负载均衡器或 ALB 使用。</dd>
...
如果最近更新了 ALB pod，但 ALB 的定制配置受到最新构建的影响，那么您可以将更新回滚到 ALB pod 先前运行的构建。集群中的所有 ALB pod 都会还原为其先前的运行状态。
{: sortdesc}

回滚更新后，会禁用 ALB pod 的自动更新。要重新启用自动更新，请使用 [`alb-autoupdate-enable` 命令](#cs_alb_autoupdate_enable)。
...
强制将集群中的 Ingress ALB pod 更新为最新版本。
{: shortdesc}
...
如果未返回任何 ALB 标识，说明集群没有可移植子网。您可以为集群[创建](#cs_cluster_subnet_create)或[添加](#cs_cluster_subnet_add)子网。
...
如果将 {{site.data.keyword.Bluemix_notm}} 帐户设置为使用其他凭证来访问 IBM Cloud Infrastructure (SoftLayer) 产品服务组合，请为当前的目标区域和资源组获取基础架构用户名。
{: shortdesc}
...
为资源组和区域设置凭证，这些凭证允许您通过 {{site.data.keyword.Bluemix_notm}} 帐户访问 IBM Cloud Infrastructure (SoftLayer) 产品服务组合。
{: shortdesc}
...
<dd>IBM Cloud Infrastructure (SoftLayer) 帐户 API 用户名。此值是必需的。请注意，基础架构 API 用户名与 IBM 标识不同。要查看基础架构 API 用户名，请参阅[管理经典基础架构 API 密钥](/docs/iam?topic=iam-classic_keys)。</dd>

<dt><code>--infrastructure-api-key <em>API_KEY</em></code></dt>
<dd>IBM Cloud Infrastructure (SoftLayer) 帐户 API 密钥。此值是必需的。要查看或生成基础架构 API 密钥，请参阅[管理经典基础架构 API 密钥](/docs/iam?topic=iam-classic_keys)。</dd>
...
除去资源组和区域的凭证，这些凭证允许您通过 {{site.data.keyword.Bluemix_notm}} 帐户访问 IBM Cloud Infrastructure (SoftLayer) 产品服务组合。
{: shortdesc}
...
检查允许[访问 IBM Cloud Infrastructure (SoftLayer) 产品服务组合](/docs/containers?topic=containers-users#api_key)（对于目标资源组和区域）的凭证是否缺少建议或必需的基础架构许可权。
{: shortdesc}

**`必需`和`建议`基础架构许可权是什么意思？**<br>
如果区域和资源组的基础架构凭证缺少任何许可权，那么此命令的输出会返回`必需`和`建议`许可权的列表。
*   **必需**：成功订购和管理基础架构资源（例如，工作程序节点）需要这些许可权。如果基础架构凭证缺少其中某个许可权，那么对区域和资源组中的所有集群执行的 `worker-reload` 等常见操作会失败。
*   **建议**：将这些许可权包含在基础架构许可权中会非常有用，并且在某些用例中这些许可权可能是必需的。例如，`使用公用网络端口添加计算`基础架构许可权是建议许可权，因为如果需要公用网络，那么会需要此许可权。但是，如果用例是位于仅专用 VLAN 上的集群，那么无需此许可权，因此不会将其视为`必需`。

有关按许可权列出的常见用例的列表，请参阅[基础架构角色](/docs/containers?topic=containers-access_reference#infra)。

**如果看到某个基础架构许可权，但在控制台或[基础架构角色](/docs/containers?topic=containers-access_reference#infra)表中找不到该许可权，该怎么做？**<br>
`支持案例`许可权和基础架构许可权是在控制台的不同部分中进行管理的。请参阅[定制基础架构许可权](/docs/containers?topic=containers-users#infra_access)中的步骤 8。

**我能分配哪些基础架构许可权？**<br>
如果您公司的许可权策略非常严格，那么可能需要限制集群用例的`建议`许可权。否则，请确保区域和资源组的基础架构凭证包含所有`必需`和`建议`许可权。

对于大多数用例，请使用相应的基础架构许可权为区域和资源组[设置 API 密钥](/docs/containers?topic=containers-users#api_key)。如果需要使用不同于当前帐户的其他基础架构帐户，请[设置手动凭证](/docs/containers?topic=containers-users#credentials)。

**如何控制用户可以执行的操作？**<br>
设置基础架构凭证后，可以通过为用户分配 [{{site.data.keyword.Bluemix_notm}} IAM 平台角色](/docs/containers?topic=containers-access_reference#iam_platform)来控制用户可以执行的操作。
...
查看可用于工作程序节点的工作程序机器类型或类型模板的列表。机器类型因专区而变化。
{:shortdesc}

每种机器类型都包含集群中每个工作程序节点的虚拟 CPU 量、内存量和磁盘空间量。缺省情况下，存储所有容器数据的辅助存储器磁盘目录将使用 LUKS 加密进行加密。如果在创建集群期间包含了 `disable-disk-encrypt` 选项，那么不会加密主机的容器运行时数据。[了解有关加密的更多信息](/docs/containers?topic=containers-security#encrypted_disk)。

可以将工作程序节点作为虚拟机在共享或专用硬件上进行供应，也可以作为物理机器在裸机上进行供应。[了解有关机器类型选项的更多信息](/docs/containers?topic=containers-planning_worker_nodes#planning_worker_nodes)。
...
<dl>
<dt><code>--cluster <em>CLUSTER</em></code></dt>
<dd>要为其创建快照的集群的名称或标识。</dd>

<dt><code>--cos-bucket <em>BUCKET_NAME</em></code></dt>
<dd>要用于存储日志的 {{site.data.keyword.cos_short}} 存储区的名称。</dd>

<dt><code>--cos-endpoint <em>ENDPOINT</em></code></dt>
<dd>要存储日志的存储区的区域端点、跨区域端点或单数据中心 {{site.data.keyword.cos_short}} 端点。有关可用端点，请参阅 {{site.data.keyword.cos_short}} 文档中的[端点和存储位置](/docs/services/cloud-object-storage/basics?topic=cloud-object-storage-endpoints) 。</dd>

<dt><code>--hmac-key-id <em>HMAC_KEY_ID</em></code></dt>
<dd>{{site.data.keyword.cos_short}} 实例的 HMAC 凭证的唯一标识。</dd>

<dt><code>--hmac-key <em>HMAC_KEY</em></code></dt>
<dd>{{site.data.keyword.cos_short}} 实例的 HMAC 密钥。</dd>

<dt><code>--type <em>LOG_TYPE</em></code></dt>
<dd>可选：要创建其快照的日志的类型。目前，`master` 是唯一的选项，也是缺省选项。</dd>
...
**最低必需许可权**：对集群的**编辑者**平台角色（对于除 `kube-audit` 以外的其他所有日志源）和对集群的**管理员**平台角色（对于 `kube-audit` 日志源）
...
<dt><code>--type <em>LOG_TYPE</em></code></dt>
<dd>要转发日志的位置。选项为 <code>ibm</code>（将日志转发到 {{site.data.keyword.loganalysisshort_notm}}）和 <code>syslog</code>（将日志转发到外部服务器）。<p class="deprecated">不推荐使用 {{site.data.keyword.loganalysisshort_notm}}。对此命令选项的支持持续到 2019 年 9 月 30 日为止。</p></dd>
...
<dd>过滤掉处于指定级别及更低级别的日志。规范顺序的可接受值为 <code>fatal</code>、<code>error</code>、<code>warn/warning</code>、<code>info</code>、<code>debug</code> 和 <code>trace</code>。此值是可选的。例如，如果过滤掉 <code>info</code> 级别的日志，那么还会过滤掉 <code>debug</code> 和 <code>trace</code>。**注**：仅当日志消息为 JSON 格式且包含 level 字段时，才能使用此标志。输出示例：<code>{"log": "hello", "level": "info"}</code></dd>
...
<dd>要为其配置运行状况检查监视器的主机名。要列出主机名，请运行 <code>ibmcloud ks nlb-dnss --cluster CLUSTER</code>。</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>监视器对其执行运行状况检查的主机名。要列出主机名，请运行 <code>ibmcloud ks nlb-dnss --cluster CLUSTER</code>。</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>包含此标志以仅查看一个主机名的状态。要列出主机名，请运行 <code>ibmcloud ks nlb-dnss --cluster CLUSTER</code>。</dd>
...
### 不推荐：ibmcloud ks region-get
{: #cs_region}

查找当前设定为目标的 {{site.data.keyword.containerlong_notm}} 区域。
{: shortdesc}

您可以使用任何位置中您有权访问的资源，即使通过运行 `ibmcloud ks region-set` 设置了区域，但要使用的资源位于其他区域中，也是如此。如果在不同区域中具有同名的集群，那么可以在运行命令时使用集群标识，也可以在运行命令时使用 `ibmcloud ks region-set` 命令设置区域并使用集群名称。

<p class="deprecated">旧版行为：<ul><li>如果使用 {{site.data.keyword.containerlong_notm}} V<code>0.3</code> 或更高版本插件，并且只需要列出和使用一个区域中的资源，那么可以使用 <code>ibmcloud ks init</code> [命令](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_init)将区域端点而不是全局端点设定为目标。</li><li>如果[在 {{site.data.keyword.containerlong_notm}} 插件中将 <code>IKS_BETA_VERSION</code> 环境变量设置为 <code>0.2</code>](/docs/containers-cli-plugin?topic=containers-cli-plugin-kubernetes-service-cli#cs_beta)，那么将创建和管理特定于该区域的集群。使用 <code>ibmcloud ks region-set</code> 命令可更改区域。
</li></ul></p>
...
### 不推荐：ibmcloud ks region-set
{: #cs_region-set}

设置 {{site.data.keyword.containerlong_notm}} 的区域。
{: shortdesc}
...
如果使用的是 {{site.data.keyword.containerlong_notm}} 插件的 `0.2` Beta 版本（旧版本），那么将创建和管理特定于该区域的集群。例如，您可以登录到美国南部区域的 {{site.data.keyword.Bluemix_notm}} 并创建集群。接下来，您可以使用 `ibmcloud ks region-set eu-central` 将欧洲中部区域作为目标，并创建另一个集群。最后，您可以使用 `ibmcloud ks region-set us-south` 返回到美国南部，以管理该区域中的集群。
{: deprecated}
...
要获取可用区域的列表，请查看[位置](/docs/containers?topic=containers-regions-and-zones)或使用 `ibmcloud ks regions` [命令](#cs_regions)。</dd></dl>
...
列出 {{site.data.keyword.containerlong_notm}} 支持的位置。有关返回的位置的更多信息，请参阅 [{{site.data.keyword.containerlong_notm}} 位置](/docs/containers?topic=containers-regions-and-zones#locations)。
{: shortdesc}
...
这将返回所有位置中的专区。要按特定位置过滤专区，请包含 `--locations` 标志。

如果使用的是 {{site.data.keyword.containerlong_notm}} 插件的 `0.2` Beta 版本（旧版本），那么可用专区随登录到的区域而变化。要切换区域，请运行 `ibmcloud ks region-set`。
{: deprecated}
...
向集群添加独立工作程序节点。不推荐使用此命令。通过运行 [`ibmcloud ks worker-pool-create`](#cs_worker_pool_create) 来创建工作程序池，或者通过运行 [`ibmcloud ks worker-pool-resize`](#cs_worker_pool_resize) 将工作程序添加到现有工作程序池。
{: deprecated}
...
<td>将 <code><em>&lt;machine_type&gt;</em></code> 替换为要将工作程序节点部署到的机器类型。可以将工作程序节点作为虚拟机部署在共享或专用硬件上，也可以作为物理机器部署在裸机上。可用的物理和虚拟机类型随集群的部署专区而变化。有关更多信息，请参阅 `ibmcloud ks machine-types` [命令](#cs_machine_types)。</td>
</tr>
<tr>
<td><code><em>private-vlan</em></code></td>
<td>将 <code><em>&lt;private_VLAN&gt;</em></code> 替换为要用于工作程序节点的专用 VLAN 的标识。要列出可用的 VLAN，请运行 <code>ibmcloud ks vlans --zone <em>&lt;zone&gt;</em></code> 并查找以 <code>bcr</code>（后端路由器）开头的 VLAN 路由器。</td>
</tr>
<tr>
<td><code>public-vlan</code></td>
<td>将 <code>&lt;public_VLAN&gt;</code> 替换为要用于工作程序节点的公用 VLAN 的标识。要列出可用的 VLAN，请运行 <code>ibmcloud ks vlans --zone &lt;zone&gt;</code> 并查找以 <code>fcr</code>（前端路由器）开头的 VLAN 路由器。<br><strong>注</strong>：如果工作程序节点设置为仅使用专用 VLAN，那么必须通过[启用专用服务端点](/docs/containers?topic=containers-cs_network_cluster#set-up-private-se)或[配置网关设备](/docs/containers?topic=containers-plan_clusters#workeruser-master)，允许工作程序节点和集群主节点进行通信。</td>
</tr>
<tr>
...
<dt><code>--public-vlan <em>PUBLIC_VLAN</em></code></dt>
<dd>创建集群时指定的公用 VLAN。此值是可选的。如果希望工作程序节点仅存在于专用 VLAN 上，请不要提供公用 VLAN 标识。专用 VLAN 路由器始终以 <code>bcr</code>（后端路由器）开头，而公用 VLAN 路由器始终以 <code>fcr</code>（前端路由器）开头。创建集群并指定公用和专用 VLAN 时，在这些前缀之后的数字和字母组合必须匹配。<p class="note">如果工作程序节点设置为仅使用专用 VLAN，那么必须通过[启用专用服务端点](/docs/containers?topic=containers-cs_network_cluster#set-up-private-se)或[配置网关设备](/docs/containers?topic=containers-plan_clusters#workeruser-master)，允许工作程序节点和集群主节点进行通信。</p></dd>
...
<dd>工作程序节点的名称。运行 <code>ibmcloud ks workers --cluster <em>CLUSTER</em></code> 可查看集群中工作程序节点的标识。此值是必需的。</dd>
...
在执行重新引导操作后，工作程序节点 IP 地址保持不变。
{: shortdesc}

重新引导工作程序节点可能导致工作程序节点上发生数据损坏。请谨慎使用此命令，仅在确信重新引导可以帮助恢复工作程序节点时使用。在其他所有情况下，请改为[重新装入工作程序节点](#cs_worker_reload)。
{: important}
...
```
   {: pre}
   此命令中返回的 **name** 是分配给工作程序节点的专用 IP 地址。通过运行 `ibmcloud ks workers --cluster <cluster_name_or_ID>` 命令并查找具有相同 **Private IP** 地址的工作程序节点，可以了解有关工作程序节点的更多信息。
...
4. 强制从工作程序节点中除去 pod，并将其重新安排到集群中的剩余工作程序节点上。
  ...
  {: pre}
   此过程可能需要几分钟时间。
5. 重新引导工作程序节点。使用从 `ibmcloud ks workers --cluster <cluster_name_or_ID>` 命令返回的工作程序标识。
  ```
...
重新装入工作程序节点的配置。如果工作程序节点遇到问题（例如，性能降低），或者如果工作程序节点卡在非正常运行状态，那么重新装入会非常有用。请注意，在重新装入期间，工作程序节点机器将使用最新映像进行更新，并且如果数据未[存储在工作程序节点外部](/docs/containers?topic=containers-storage_planning#persistent_storage_overview)，那么将删除数据。在执行重新装入操作后，工作程序节点公共和专用 IP 地址保持不变。
{: shortdesc}
...
如果阶段状态显示为 **SchedulingDisabled**，说明已禁止工作程序节点用于 pod 安排。
 4. 强制从工作程序节点中除去 pod，并将其重新安排到集群中的剩余工作程序节点上。
    ...
    {: pre}
    此过程可能需要几分钟时间。
 5. 重新装入工作程序节点。使用从 `ibmcloud ks workers --cluster <cluster_name_or_ID>` 命令返回的工作程序标识。
    ```
...
4. 强制从工作程序节点中除去 pod，并将其重新安排到集群中的剩余工作程序节点上。
   ...
   {: pre}
   此过程可能需要几分钟时间。
5. 除去工作程序节点。使用从 `ibmcloud ks workers --cluster <cluster_name_or_ID>` 命令返回的工作程序标识。
   ```
...
更新工作程序节点以将最新的安全性更新和补丁应用于操作系统，并更新 Kubernetes 版本以与 Kubernetes 主节点的版本相匹配。可以使用 `ibmcloud ks cluster-update` [命令](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_cluster_update)来更新 Kubernetes 主节点的版本。
...
创建集群或工作程序池后，可以添加专区。添加专区后，会向新专区添加工作程序节点，以匹配为工作程序池指定的每个专区的工作程序数。
仅当集群位于多专区大城市中时，才能添加多个专区。
{: shortdesc}
...
<dt><code>--private-only </code></dt>
<dd>使用此选项可阻止创建公用 VLAN。仅当指定 `--private-vlan` 标志并且不包含 `--public-vlan` 标志时，此项才是必需的。<p class="note">如果工作程序节点设置为仅使用专用 VLAN，那么必须启用专用服务端点或配置网关设备。有关更多信息，请参阅[规划专用集群和工作程序节点设置](/docs/containers?topic=containers-plan_clusters#private_clusters)。</p></dd>
...
<dt><code>--private-only </code></dt>
<dd>可选：取消设置公用 VLAN，以便此专区中的工作程序仅连接到专用 VLAN。</dd>
...
<li>检查要使用的公用和专用 VLAN 标识是否兼容。要使两者兼容，<strong>Router</strong> 必须具有相同的 pod 标识。专用 VLAN 路由器始终以 <code>bcr</code>（后端路由器）开头，而公用 VLAN 路由器始终以 <code>fcr</code>（前端路由器）开头。创建集群并指定公用和专用 VLAN 时，在这些前缀之后的数字和字母组合必须匹配。<pre class="pre"><code>ibmcloud ks vlans --zone &lt;zone&gt;</code></pre><p>输出示例：</p>
