  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
       ...
       <td><ul><li>如果您有防火墙，请[配置防火墙设置以允许出局流量流至相应的端口和 IP 地址](cs_firewall.html#firewall_outbound)。</li><li>通过运行 `bx cs workers <mycluster>`. 如果未列出任何公共 IP，说明集群仅具有专用 VLAN。<ul><li>如果希望集群仅具有专用 VLAN，请确保已设置 [VLAN 连接](cs_clusters.html#worker_vlan_connection)和[防火墙](cs_firewall.html#firewall_outbound)。</li><li>如果希望集群具有公共 IP，请[添加新的工作程序节点](cs_cli_reference.html#cs_worker_add)（具有公用和专用 VLAN）。</li></ul></li></ul></td>
     </tr>
      <tr>
