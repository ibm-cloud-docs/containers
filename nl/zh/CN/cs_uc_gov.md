...
* [通过 {{site.data.keyword.cos_full_notm}} 提供对公共数据的访问](https://console.bluemix.net/docs/services/cloud-object-storage/about-cos.html#about-ibm-cloud-object-storage)
...
* 将应用程序构建成一组在 {{site.data.keyword.containerlong_notm}} 中运行的协作微服务，这基于应用程序的功能性区域及其依赖关系。
...
为什么选择 {{site.data.keyword.Bluemix_notm}}：{{site.data.keyword.containerlong_notm}} 支持政府和公共合作伙伴关系使用外部云服务及适合协作的开放式源代码工具。容器提供了一个可共享平台，港口和运输公司对于共享信息在该安全平台上进行托管都很放心。此外，这些公司从小型开发/测试系统发展到生产规模的系统时，该平台也会相应进行扩展。开放式工具链通过自动执行构建、测试和部署，进一步加速了开发工作。 
