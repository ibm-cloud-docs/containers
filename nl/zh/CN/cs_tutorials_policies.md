...
- 了解如何通过创建高位 DNAT 前策略，阻止流至所有 NodePort 的所有入局流量。
- 了解如何通过创建低位 DNAT 前策略，允许列入白名单的源 IP 地址访问 LoadBalancer 公共 IP 和端口。低位策略会覆盖高位策略。
- 了解如何通过创建低位 DNAT 前策略，阻止列入黑名单的源 IP 地址访问 LoadBalancer 公共 IP 和端口。
...
## 第 1 课：使用 LoadBalancer 部署应用程序并将其公开
{: #lesson1}

第一课说明如何从多个 IP 地址和端口公开应用程序，以及公共流量进入集群的位置。
{: shortdesc}

首先，部署要在整个教程中使用的样本 Web 服务器应用程序。`echoserver` Web 服务器显示有关从客户机与集群建立的连接的数据，并允许您测试对公关公司集群的访问。然后，通过创建 LoadBalancer 服务公开应用程序。LoadBalancer 服务通过 LoadBalancer 服务 IP 地址和工作程序节点的 NodePort 使应用程序可用。

下图显示了在第 1 课结束时，Web 服务器应用程序如何通过公共 NodePort 和公共 LoadBalancer 公开到因特网：
...
5. 验证是否可以从您的计算机公开访问由 LoadBalancer 公开的应用程序。
    ...
    2. 创建备忘单文本文件，并将 LoadBalancer IP 复制到该文本文件中。备忘单将帮助您更快地在后面课程中使用值。

    3. 验证是否可以公开访问 LoadBalancer 的外部 IP。
        ```
        curl --connect-timeout 10 <loadbalancer_IP>:80
        ```
        {: pre}

        以下示例输出确认 LoadBalancer 在 `169.1.1.1` 公共 LoadBalancer IP 地址上公开了应用程序。`webserver-855556f688-76rkp` 应用程序 pod 收到了 curl 请求：
        ```
...
6. 验证是否可以从您的计算机公开访问由 NodePort 公开的应用程序。LoadBalancer 服务通过 LoadBalancer 服务 IP 地址和工作程序节点的 NodePort 使应用程序可用。
    ...
    3. 将工作程序节点的公共 IP 以及 NodePort 复制到文本备忘单中，以便在后面的课程中使用。

    4. 验证是否可以通过 NodePort 访问工作程序节点的公共 IP 地址。
        ```
        curl  --connect-timeout 10 <worker_IP>:<NodePort>
        ```
        {: pre}

        以下示例输出确认对应用程序的请求通过工作程序节点的专用 IP 地址 `10.1.1.1` 和 `31024` NodePort 传入。`webserver-855556f688-xd849` 应用程序 pod 收到了 curl 请求：
        ```
...
此时，应用程序已从多个 IP 地址和端口公开。这些 IP 中大部分都是集群内部 IP，只能通过专用网络进行访问。只有公共 NodePort 和公共 LoadBalancer 端口会公开到公用因特网。

接下来，您可以开始创建并应用 Calico 策略来阻止公共流量。

## 第 2 课：阻止流至所有 NodePort 的所有入局流量
{: #lesson2}

要保护公关公司的集群，必须阻止对 LoadBalancer 服务以及公开应用程序的 NodePort 的公共访问。首先是阻止对 NodePort 的访问。下图显示在第 2 课结束时，如何允许流量流至 LoadBalancer，但不允许流至 NodePort：

<img src="images/cs_tutorial_policies_Lesson2.png" width="450" alt="在第 2 课结束时，Web 服务器应用程序仅通过公共 LoadBalancer 公开到因特网。" style="width:450px; border-style: none"/>

1. 在文本编辑器中，创建名为 `deny-nodeports.yaml` 的高位 DNAT 前策略，以拒绝来自任何源 IP 的入局 TCP 和 UDP 流量流至所有 NodePort。
    ```
...
3. 使用备忘单中的值，验证是否无法公开访问工作程序节点公共 IP 地址和 NodePort。
    ```
    curl  --connect-timeout 10 <worker_IP>:<NodePort>
    ```
    {: pre}

    由于创建的 Calico 策略阻止流至 NodePort 的流量，因此连接会超时。
    ```
...
5. 使用备忘单中的值，验证是否仍可公开访问 LoadBalancer 外部 IP 地址。
    ```
    curl --connect-timeout 10 <loadbalancer_IP>:80
    ```
    {: pre}
    ...
    ```
    {: screen}
    在输出的 `Request Information` 部分中，请注意源 IP 地址的内容，例如 `client_address=1.1.1.1`。源 IP 地址是用于运行 curl 的系统的公共 IP。反之，如果通过代理或 VPN 连接到因特网，代理或 VPN 可能会隐藏系统的实际 IP 地址。在任一情况下，LoadBalancer 都会将系统源 IP 地址视为客户机 IP 地址。

6. 将系统的源 IP 地址（上一步输出中的 `client_address=1.1.1.1`）复制到备忘单中以在后面的课程中使用。

太棒了！此时，应用程序仅从公共 LoadBalancer 端口公开到公用因特网。将阻止流至公共 NodePort 的流量。您已经部分锁定集群以阻止不需要的流量。
...
<img src="images/cs_tutorial_policies_L3.png" width="600" alt="Web 服务器应用程序通过公共 LoadBalancer 仅公开到系统 IP。" style="width:600px; border-style: none"/>
1. 在文本编辑器中，创建名为 `deny-lb-port-80.yaml` 的高位 DNAT 前策略，以拒绝来自任何源 IP 的所有入局 TCP 和 UDP 流量流至 LoadBalancer IP 地址和端口。
    将 `<loadbalancer_IP>` 替换为备忘单中的 LoadBalancer 公共 IP 地址。
    ```
    apiVersion: projectcalico.org/v3
    kind: GlobalNetworkPolicy
    metadata:
...
3. 使用备忘单中的值，验证是否现在无法访问公共 LoadBalancer IP 地址。由于创建的 Calico 策略阻止流至 LoadBalancer 的流量，因此连接会超时。
    ```
    curl --connect-timeout 10 <loadbalancer_IP>:80
    ```
    {: pre}

4. 在文本编辑器中，创建名为 `whitelist.yaml` 的低位 DNAT 前策略，以允许来自系统 IP 的流量流至 LoadBalancer IP 地址和端口。使用备忘单中的值，将 `<loadbalancer_IP>` 替换为 LoadBalancer 的公共 IP 地址，将 `<client_address>` 替换为系统源 IP 的公共 IP 地址。
    ```
    apiVersion: projectcalico.org/v3
    kind: GlobalNetworkPolicy
    metadata:
...
6. 使用备忘单中的值，验证是否现在可以访问公共 LoadBalancer IP 地址。
    ```
    curl --connect-timeout 10 <loadbalancer_IP>:80
    ```
    {: pre}

7. 如果您有权访问具有不同 IP 地址的另一个系统，请尝试从该系统访问 LoadBalancer。
    ```
    curl --connect-timeout 10 <loadbalancer_IP>:80
    ```
    {: pre}
...
此时，将阻止流至公共 NodePort 和 LoadBalancer 的所有流量。只允许来自列入白名单的系统 IP 的流量。

## 第 4 课：拒绝来自列入黑名单的 IP 的入局流量流至 LoadBalancer
{: #lesson4}
...
在本课程中，您将通过阻止来自自己系统的源 IP 地址的流量来测试黑名单功能。在第 4 课结束时，将阻止流至公共 NodePort 的所有流量，但允许流至公共 LoadBalancer 的所有流量。只阻止来自列入黑名单的系统 IP 的流量流至 LoadBalancer：
...
4. 使用备忘单中的值，从系统验证您是否无法访问 LoadBalancer IP，因为系统的 IP 已列入黑名单。
    ```
    curl --connect-timeout 10 <loadbalancer_IP>:80
    ```
    {: pre}
