...
使用存储类动态供应持久性存储器时，供应的是具有特定配置（也称为“类型模板”）的持久性存储器。供应存储器后，无法更改此类型模板。 

如果要更改存储设备的大小、IOPS、类型或保留策略，必须供应新的存储设备，并将旧存储器中的数据复制到新存储设备。 
...
3. 将现有存储设备中的数据复制到新存储设备。每种类型的存储器都有不同的数据备份和复原选项。 
...
4. 更新应用程序部署以引用新的存储设备。 
   ...
   3. 获取用于使用旧持久性存储设备的部署的部署 YAML。
      ```
      kubectl get deployment <deployment_name> -o yaml
      ```
      {: pre}
      
   4. 在部署的 `spec.volumes` 部分中更改 PVC 的名称。
   5. 将更改应用于部署。应用更改时，将创建新的 pod，并且绑定 PV 的 PVC 将安装到该 pod。系统会除去安装旧持久性存储器实例的 pod。
      ```
...
**注：**仅当您具有在多专区功能可用之前创建的现有 PV 时，才需要执行这些步骤。多专区功能可用后创建的 PV 已包含 Kubernetes 区域和专区标签。 
