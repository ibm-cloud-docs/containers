...
<dd>在 Kubernetes 中，基于角色的访问控制 (RBAC) 是保护集群中资源的一种方法。RBAC 角色确定用户可对这些资源执行的 Kubernetes 操作。分配有服务角色的每个用户都将自动分配有相应的 RBAC 集群角色。此 RBAC 集群角色将应用于特定名称空间或所有名称空间，具体取决于您是否将策略的作用域限定为某个名称空间。</br></br>
...
<dt>Kubernetes 名称空间</dt>
  ...
  <p class="important">将服务访问策略的作用域限定为名称空间时，必须[使用 {{site.data.keyword.Bluemix_notm}} 控制台](/docs/containers?topic=containers-users#add_users)。不能使用 CLI 将服务访问策略的作用域限定为名称空间。此外，无法在资源组级别分配作用域限定为名称空间的服务访问角色，也不能同时将其作为平台角色进行分配。</p></dd>
<dt>资源组</dt>
...
2. 要确保可以成功执行与帐户相关的所有操作，请验证用户是否具有正确的 {{site.data.keyword.Bluemix_notm}} IAM 平台角色。
    ...
    2. 单击要设置 API 密钥的用户的名称，或者单击要为 API 密钥设置其凭证的用户的名称，然后单击**访问策略**选项卡。
...
4. 验证集群是否使用的是您设置的基础架构帐户凭证。
  1. 打开 [{{site.data.keyword.containerlong_notm}} 控制台 ![外部链接图标](../icons/launch-glyph.svg "外部链接图标")](https://cloud.ibm.com/containers-kubernetes/clusters)，然后选择集群。
...
3. 为资源组或资源实例分配策略。如果要将策略的作用域限定为名称空间，那么必须将策略分配给资源实例（而不是分配给资源组）。
  * **对于资源组**：
    ...
    4. 在**服务**列表中，输入 **{{site.data.keyword.containershort_notm}}**。
    ...
    6. 选择策略的角色。
       ...
       * **服务访问角色**：授予对 Kubernetes 的访问权，使得可从集群内进行访问，以便用户可以管理 Kubernetes 资源，例如 pod、部署、服务和名称空间。要查找每个角色的受支持操作列表，请参阅[服务角色参考页面](/docs/containers?topic=containers-access_reference#service)。<p class="note">如果在资源组级别分配了角色，那么无法将服务访问角色的作用域限定为名称空间。请改为分配对资源实例的访问权。</p>
    7. 单击**分配**。
  ...
  * **对于资源组内或跨资源组的资源实例**：
    1. 单击**分配对资源的访问权**。
    2. 在**服务**列表中，输入 **{{site.data.keyword.containershort_notm}}**。
...
不能使用 CLI 将服务角色的作用域限定为 Kubernetes 名称空间。[请改为使用 {{site.data.keyword.Bluemix_notm}} 控制台](#add_users)。
{: important}
...
**要通过 CLI 分配 {{site.data.keyword.Bluemix_notm}} IAM 策略，请执行以下操作**：
...
1.  创建 {{site.data.keyword.Bluemix_notm}} IAM 访问策略以设置 {{site.data.keyword.containerlong_notm}} 的许可权 (**`--service-name containers-kubernetes`**)。根据要分配对其的访问权的内容来限定访问策略的作用域。
    ...
    <caption>用于限定访问策略作用域的选项</caption>
      ...
      <tbody>
        ...
        <td>`--role`</td>
        <td>选择要分配的角色。
        <ul><li>[平台角色](/docs/containers?topic=containers-access_reference#iam_platform)：授予对 {{site.data.keyword.containerlong_notm}} 的访问权，以便用户可以管理基础架构资源，例如集群、工作程序节点、工作程序池、Ingress 应用程序负载均衡器和存储器。可能的值为：`Administrator`（管理员）、`Operator`（操作员）、`Editor`（编辑者）或 `Viewer`（查看者）。</li>
        <li>[服务角色](/docs/containers?topic=containers-access_reference#service)：授予对 Kubernetes 的访问权，使得可从集群内进行访问，以便用户可以管理 Kubernetes 资源，例如 pod、部署、服务和名称空间。可能的值为：`Manager`（管理者）、`Writer`（写入者）或 `Reader`（读取者）。</li></td>
        </tr>
      </tbody>
      </table>
    ...
    *  为单个用户分配对缺省资源组和美国东部区域中一个集群的**查看者**平台访问角色和**管理者**服务访问角色：
       ```
    ...
    *  为 `auditors` 用户组分配对所有资源组中所有集群的**查看者**平台访问角色和**读取者**服务访问角色：
       ```
...
3.  如果仅为用户分配了服务角色，那么必须为用户提供集群名称和标识，以便他们可以执行 `ibmcloud ks cluster-config` [命令](/docs/containers?topic=containers-cs_cli_reference#cs_cluster_config)，然后[通过 CLI 启动 Kubernetes 仪表板](/docs/containers?topic=containers-app#db_cli)或者以其他方式与 Kubernetes API 进行交互。如果希望这些用户仍能够通过 CLI 访问 {{site.data.keyword.containerlong_notm}} 集群控制台，并列出集群和其他基础架构资源，请重复上述步骤以授予用户**查看者**平台角色。

4.  要使更改生效，被授予访问权的用户必须刷新集群配置。
    ```
    ibmcloud ks cluster-config --cluster <cluster_name_or_id>
    ```
    {: pre}

5.  **可选**：验证用户是否已添加到相应的 [RBAC 角色绑定或集群角色绑定](#role-binding)。请注意，您必须是集群管理员（**管理者**服务角色），才能检查角色绑定和集群角色绑定。
    检查角色的角色绑定或集群角色绑定。
    *   读取者：
        ```
    ...
    *   管理者，所有名称空间：
    ...
    例如，如果为用户 `user@email.com` 和访问组 `team1` 分配了**读取者**服务角色并运行了 `kubectl get rolebinding ibm-view -o yaml -n default`，那么输出如下所示：
...
要了解有关每个 RBAC 角色所允许的操作的更多信息，请查看 [{{site.data.keyword.Bluemix_notm}} IAM 服务角色](/docs/containers?topic=containers-access_reference#service)参考主题。要查看每个 RBAC 角色授予的对单个 Kubernetes 资源的许可权，请查看[每个 RBAC 角色的 Kubernetes 资源许可权](/docs/containers?topic=containers-access_reference#rbac)。
{: tip}
...
您可能还希望将附加组件集成到集群。例如，[在集群中设置 Helm](/docs/containers?topic=containers-integrations#helm) 时，必须在 `kube-system` 名称空间中为 Tiller 创建服务帐户，并为 `tiller-deploy` pod 创建 Kubernetes RBAC 集群角色绑定。
...
1. 确定用户创建的集群。
    1.  登录到 [{{site.data.keyword.containerlong_notm}} 控制台 ![外部链接图标](../icons/launch-glyph.svg "外部链接图标")](https://cloud.ibm.com/containers-kubernetes/clusters)。
    2.  从表中选择您的集群。
