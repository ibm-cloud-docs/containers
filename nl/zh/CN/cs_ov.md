...
要了解有关 Kubbernetes 术语的更多信息，请尝试查阅 <a href="https://kubernetes.io/docs/tutorials/kubernetes-basics/" target="_blank">Kubernetes 基础知识教程</a>。</dd>
</dl>
...
|优点|描述|
|-------|-----------|
|隔离了计算、网络和存储基础架构的单租户 Kubernetes 集群|<ul><li>创建自己的定制基础架构，以满足企业和开发环境的需求。</li><li>使用 {{site.data.keyword.BluSoftlayer_full}} 提供的资源来供应专用而安全的 Kubernetes 主节点、工作程序节点、虚拟网络和存储器。</li><li>存储持久数据，在 Kubernetes pod 之间共享数据，以及在需要时使用集成和安全卷服务复原数据。</li><li>全面管理的高可用性 Kubernetes 主节点，由 {{site.data.keyword.IBM_notm}} 持续监视。</li><li>受益于对所有本机 Kubernetes API 的完全支持。</li></ul>|
...
<img usemap="#public_options" border="0" class="image" id="cs_ov_environments__image_ztx_crb_f1b" src="images/cs_public_options.png" width="320" alt="使用 Bluemix Public，可以创建 Kubernetes 集群或将单个和可扩展容器组迁移到集群。" style="width:320px;" />
    <map name="public_options" id="public_options">
    <area href="container_index.html#clusters" alt="在 Bluemix 中开始使用 Kubernetes 集群" title="在 Bluemix 中开始使用 Kubernetes 集群" shape="rect" coords="-5, -6, 123, 154" />
    <area href="cs_classic.html#cs_classic" alt="在 IBM Bluemix Container Service (Kraken) 中运行单个和可扩展容器" title="在 IBM Bluemix Container Service (Kraken) 中运行单个和可扩展容器" shape="rect" coords="181, -5, 320, 161" />
    </map>
...
*   {{site.data.keyword.IBM_notm}} 拥有并管理已部署工作程序节点、VLAN 和子网的 {{site.data.keyword.BluSoftlayer_notm}} 帐户，而不是您拥有的帐户。
...
单击以下某个选项以开始：
 ...
 <img usemap="#dedicated_options" border="0" class="image" id="cs_ov_environments__image_hjb_4ln_j1b" src="images/cs_dedicated_options.png" width="600" alt="使用 Bluemix Dedicated，可以创建 Kubernetes 集群或将单个和可扩展容器组迁移到集群。" style="width:600px;" />
    <map name="dedicated_options" id="dedicated_options">
    <area href="#setup_dedicated" alt="在 Bluemix Dedicated 上设置 IBM Bluemix Container Service（封闭 Beta 版）" title="在 Bluemix Dedicated 上设置 IBM Bluemix Container Service（封闭 Beta 版）" shape="rect" coords="-5, -15, 100, 153" />
    <area href="container_index.html#dedicated" alt="在 Bluemix Dedicated 中开始使用 Kubernetes 集群（封闭 Beta 版）" title="在 Bluemix Dedicated 中开始使用 Kubernetes 集群（封闭 Beta 版）" shape="rect" coords="153, -10, 276, 182" />
    <area href="cs_classic.html#cs_classic" alt="在 IBM Bluemix Container Service (Kraken) 中运行单个和可扩展容器" title="在 IBM Bluemix Container Service (Kraken) 中运行单个和可扩展容器" shape="rect" coords="317, -11, 436, 188" />
    <area href="container_ha.html#container_group_ui" alt="通过 Bluemix GUI 作为容器组运行长期服务" title="通过 Bluemix GUI 作为容器组运行长期服务" shape="rect" coords="485, -1, 600, 173" />
    </map>
...
|--|--------------|--------------------------------|
|集群创建|创建 Lite 集群或为标准集群指定以下详细信息：<ul><li>集群类型</li><li>名称</li><li>Kubernetes 版本</li><li>位置</li><li>机器类型</li><li>工作程序节点数</li><li>公共 VLAN</li><li>专用 VLAN</li><li>硬件</li></ul>|为标准集群指定以下详细信息：<ul><li>名称</li><li>Kubernetes 版本</li><li>机器类型</li><li>工作程序节点数</li></ul><p>**注**：在创建 {{site.data.keyword.Bluemix_notm}} 环境期间，会预先定义 VLAN 和硬件设置。</p>|
|集群硬件和所有权|在标准集群中，硬件可由其他 {{site.data.keyword.IBM_notm}} 客户共享或仅供您专用。公用和专用 VLAN 由您在 {{site.data.keyword.BluSoftlayer_notm}} 帐户中所拥有和管理。|在 {{site.data.keyword.Bluemix_notm}} Dedicated 上的集群中，硬件始终是专用的。
...
|负载均衡器和 Ingress 联网|在标准集群供应期间，会自动执行以下操作。<ul><li>将公共可移植子网绑定到您的集群，并分配给您的 {{site.data.keyword.BluSoftlayer_notm}} 帐户。</li><li>一个可移植公共 IP 地址用于一个高可用性 Ingress 控制器，并分配格式为 &lt;cluster_name&gt;.containers.mybluemix.net 的唯一公共路径。您可以使用此路径向公众公开多个应用程序。</li><li>向集群分配 4 个可移植公共 IP 地址，这些 IP 地址可用于通过 LoadBalancer 服务向公众公开应用程序。通过 {{site.data.keyword.BluSoftlayer_notm}} 帐户可以请求更多子网。</li></ul>|创建 Dedicated 帐户时，您将作出以下决策：<ul><li>需要多少子网</li><li>需要的子网类型，可以是负载均衡器或 Ingress。{{site.data.keyword.IBM_notm}} 将为您创建子网并执行网络管理任务。根据您的选择，可能会创建 Ingress 控制器，并且可能会分配公共路径。
...
4.  通过 Public 控制台访问 {{site.data.keyword.Bluemix_notm}} Dedicated 帐户并开始创建集群。
    ...
    3.  从 {{site.data.keyword.Bluemix_notm}} Dedicated 实例的目录中，选择**容器**，然后单击 **Kubernetes 集群**。

接下来，有关创建集群的更多信息，请参阅[在 {{site.data.keyword.Bluemix_notm}} Dedicated 中通过 GUI 创建 Kubernetes 集群（封闭 Beta 版）](cs_cluster.html#creating_ui_dedicated)。


## Kubernetes 集群的工作方式 
{: #cs_ov_architecture}

Kubernetes 集群由一个或多个物理机器或虚拟机（也称为工作程序节点）组成；工作程序节点松散耦合且可扩展，并通过 Kubernetes 主节点进行集中监视和管理。对于每个客户帐户，Kubernetes 主节点均由 IBM 管理，并且具有高弹性和高可用性。
{:shortdesc}
...
*图 1. IBM Bluemix Containers Service 中的 Kubernetes 体系结构和联网情况*
...
<dl>
<dt>容器</dt>
<dd>容器是一种标准的打包方法，用于打包应用程序及其所有依赖项，以便应用程序可以在环境之间移动，并且无需更改即可运行。与虚拟机不同，容器不会对设备、其操作系统和底层硬件进行虚拟化。容器中仅打包有应用程序代码、运行时、系统工具、库和设置。容器在其部署到的计算主机上作为隔离进程运行，并共享主机操作系统及其硬件资源。这种方法使得容器比虚拟机更轻便、可移植性更高且更高效。</dd>
<dt>映像</dt>
