...
<td>
<ul>
<li>Ajout de l'alias `ibmcloud oc` au plug-in {{site.data.keyword.containershort_notm}} pour la gestion des clusters Red Hat OpenShift on IBM Cloud. </li>
<li>Ajout de la commande [`ibmcloud ks cluster-subnet-detach`](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_cluster_subnet_detach) pour déconnecter un sous-réseau portable public ou privé d'un compte d'infrastructure IBM Cloud à partir d'un cluster. </li>
<li>Changement de nom de la commande `ibmcloud ks machine-types` en `ibmcloud ks flavors`. Vous toujours utiliser l'alias `machine-types`. </li>
<li>Dans la sortie de la commande `ibmcloud ks flavors (machine-types)`, sont indiquées les versions qui sont prises en charge uniquement pour {{site.data.keyword.containerlong_notm}} ou uniquement pour Red Hat OpenShift on IBM Cloud. </li>
<li>Dans la sortie de la commande `ibmcloud ks cluster-get`, le terme `Owner` est remplacé par `Creator` pour indiquer que la zone renvoie des informations sur l'utilisateur qui a créé le cluster. </li>
<li>Amélioration du traitement d'erreurs pour `ibmcloud ks zone-add`. </li>
...
<td><ul>
<li>Correction d'un bogue de sorte qu'aucun message de rééquilibrage de pool de noeuds worker ne soit renvoyé lorsque le programme de mise à l'échelle automatique de cluster est activé. </li>
<li>Correction d'un bogue pour permettre la prise en charge de la version de cluster OpenShift par défaut. </li>
<li>Mise à jour du texte d'aide pour les commandes `cluster-feature-enable private-service-endpoint` et `nlb-dns-monitor-configure`. </li>
<li>Mise à jour de la traduction du texte d'aide</li>
</ul>
</tr>
<tr>
<td>0.3.49</td>
<td>18 juin 2019</td>
<td>Mise à jour de la version Go vers 1.12.6.</td>
</tr>
<tr>
<td>0.3.47</td>
<td>15 juin 2019</td>
<td><ul>
<li>Correction d'un bogue de sorte que des tables vides ne soient pas renvoyées dans la sortie de `ibmcloud ks kube-versions`. </li>
<li>Mise à jour du modèle DNS NLB de sorte qu'un tableau d'adresses IP NLB soit renvoyé par `ibmcloud ks nlb-dnss`. </li>
<li>Modification du texte de description pour le plug-in d'interface CLI {{site.data.keyword.containerlong_notm}}. </li>
</ul></td>
</tr>
<tr>
<td>0.3.34</td>
...
<td>23 mai 2019</td>
<td><ul>
<li>Ajout de la commande [<code>ibmcloud ks infra-permissions-get</code>](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#infra_permissions_get) permettant de vérifier si les droits d'infrastructure suggérés ou manquants pour la région et le groupe de ressources ciblés ne contiennent pas les données d'identification qui autorisent l'[accès au portefeuille d'infrastructure IBM Cloud](/docs/containers?topic=containers-users#api_key). </li>
<li>Ajout de l'option <code>--private-only</code> à la commande `zone-network-set` afin d'annuler la définition du VLAN public pour les métadonnées de pool de noeuds worker. Les noeuds worker suivants dans cette zone de pool de noeuds worker sont connectés à un VLAN privé uniquement. </li>
...
<li>Ajout de la colonne **Métropole à zones multiples** à la sortie de la commande `supported-locations` pour désigner les zones compatibles avec plusieurs zones.</li>
...
<td>0.2.102</td>
<td>15 avril 2019</td>
<td>Ajout du [groupe de commandes `ibmcloud ks nlb-dns`](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#nlb-dns) afin d'enregistrer et de gérer un nom d'hôte pour les adresses IP NLB, et du [groupe de commandes `ibmcloud ks nlb-dns-monitor` ](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_nlb-dns-monitor-configure) afin de créer et modifier des moniteurs de diagnostic d'intégrité pour les noms d'hôte NLB. Pour plus d'informations, voir [Enregistrement d'adresses IP NLB avec un nom d'hôte DNS](/docs/containers?topic=containers-loadbalancer_hostname#loadbalancer_hostname_dns).
</td>
</tr>
<tr>
<td>0.2.99</td>
...
<li>Mise à jour de la version Go vers 1.11.6.</li>
...
<td>26 fév. 2019</td>
<td><ul>
<li>Ajout de la commande `cluster-pull-secret-apply`, qui crée un ID de service IAM pour le cluster, les règles, la clé d'API et les secrets d'extraction d'image pour que les conteneurs qui s'exécutent dans l'espace de nom Kubernetes `default` puissent extraire des images depuis IBM Cloud Container Registry. Pour les nouveaux clusters, les secrets d'extraction d'image qui utilisent des données d'identification IAM sont créés par défaut. Utilisez cette commande pour mettre à jour des clusters existants ou si votre cluster comporte une erreur de secretd'extraction d'image lors de sa création. Pour plus d'informations, voir [la documentation](/docs/containers?topic=containers-images#cluster_registry_auth).</li>
...
<td>8 fév. 2019</td>
<td><ul>
<li>Ajout de l'option `--skip-rbac` dans la commande `ibmcloud ks cluster-config` pour ignorer l'ajout de rôles RBAC Kubernetes aux utilisateurs en fonction des rôles d'accès au service {{site.data.keyword.cloud_notm}} IAM dans la configuration du cluster. Incluez cette option uniquement si vous [gérez vos propres rôles Kubernetes RBAC](/docs/containers?topic=containers-users#rbac). Si vous utilisez des [rôles d'accès au service {{site.data.keyword.cloud_notm}} IAM](/docs/containers?topic=containers-access_reference#service) pour gérer tous vos utilisateurs RBAC, n'incluez pas cette option. </li>
