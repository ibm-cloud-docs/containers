...
Faites-vous une idée des décisions que vous pouvez prendre pour configurer et personnaliser les clusters Kubernetes dans {{site.data.keyword.containershort_notm}}. Certaines de ces configurations ne peuvent pas être modifiées une fois que le cluster a été créé. En vos familiarisant à l'avance avec ces configurations, vous pouvez vous assurer que toutes les ressources comme la mémoire, l'espace disque et les adresses IP, soient disponibles pour vos équipes de développement quand elles ont besoin.
{:shortdesc}
...
|---------------|-------------|-----------------|
|[Disponible dans {{site.data.keyword.Bluemix_notm}}](cs_ov.html#cs_ov)|<img src="images/confirm.svg" width="32" alt="Fonction disponible" style="width:32px;" />|<img src="images/confirm.svg" width="32" alt="Fonction disponible" style="width:32px;" />|
...
<dd>Lorsque vous disséminez les application entre clusters dans différentes régions, vous pouvez permettre un équilibrage de charge basé surla région où est situé l'utilisateur. Si le cluster, un matériel, voire un emplacement complet dans une région tombe
...
Lorsque vous créez un cluster léger, votre noeud worker est automatiquement provisionné en tant que noeud dans le compte d'infrastructure IBM Cloud (SoftLayer).
...
L'interface de réseau public des noeuds d'agent dans les clusters légers et standard est protégée par les règles réseau Calico. Ces règles bloquent par défaut la plupart du trafic entrant. Le trafic entrant nécessaire au fonctionnement de Kubernetes est toutefois autorisé, comme les connexions aux services NodePort, Loadbalancer et Ingress. Pour plus d'informations sur ces règles, notamment pour savoir comment les modifier, voir [Règles réseau](cs_security.html#cs_security_network_policies).
...
Lorsque vous créez un cluster standard, {{site.data.keyword.containershort_notm}} demande automatiquement cinq adresses IP publiques portables et cinq adresses IP privées portables et les provisionne dans votre compte d'infrastructure IBM Cloud (SoftLayer) lors de la création du cluster. Deux des adresses IP portables, une privée et l'autre publique, sont utilisées pour les [équilibreurs de charge d'application Ingress](#cs_ingress). Quatre adresses IP portables publiques et quatre adresses IP portables privées sont utilisées pour exposer des applications en créant un service LoadBalancer.
...
**Remarque **: si vous utilisez un pare-feu, [autorisez l'accès sortant](cs_security.html#opening_ports) pour les plages IP de l'infrastructure IBM Cloud (SoftLayer) des emplacements (centres de données) où sont situés vos clusters, de manière à pouvoir créer des réservations de volumes persistants.
{:shortdesc}
...
<tr>
<td>Istio</td>
<td><a href="https://www.ibm.com/cloud/info/istio" target="_blank">Istio<img src="../icons/launch-glyph.svg" alt="External link icon"></a> est un service open source qui permet aux développeurs de connecter, de sécuriser et de gérer de manière uniforme un réseau de microservices, également dénomé maillage de services, sur des plateformes d'orchestration cloud telles que Kubernetes. Pour en savoir plus sur le projet open source, consultez l'article de blogue comme quoi <a href="https://developer.ibm.com/dwblog/2017/istio/" target="_blank">IBM a con-fondé et a lancé Istio<img src="../icons/launch-glyph.svg" alt="External link icon"></a>. Pour installer Istio sur votre cluster Kubernetes dans {{site.data.keyword.containershort_notm}} et commencer à utiliser l'exemple d'application, voir [Tutoriel : Gestion de microservices avec Istio](cs_tutorials_istio.html#istio_tutorial).</td>
</tr>
<tr>
<td>Prometheus</td>
...
|------------|-----------|----------------------------------------------|
|Comptes Lite|Les comptes légers ne peuvent pas provisionner des clusters.|<ul><li>[Mettez à niveau votre compte léger vers un compte {{site.data.keyword.Bluemix_notm}} Paiement à la carte](/docs/pricing/billable.html#upgradetopayg) configuré avec accès au portefeuille d'infrastructure IBM Cloud (SoftLayer).</li></ul>|
...
|Compte d'infrastructure IBM Cloud (SoftLayer), aucun compte {{site.data.keyword.Bluemix_notm}}|Pour créer un cluster standard, vous devez disposer d'un compte {{site.data.keyword.Bluemix_notm}}.|<ul><li>[Créez un nouveau compte de type Paiement à la carte](/docs/pricing/billable.html#billable) configuré pour accès au portefeuille d'infrastructure IBM Cloud (SoftLayer). Si vous sélectionnez cette option, une infrastructure IBM Cloud (SoftLayer) est créée pour vous. Vous disposez de deux comptes d'infrastructure IBM Cloud (SoftLayer) différents avec facturation distincte.</li></ul>|
