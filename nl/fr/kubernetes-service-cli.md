...
Reportez-vous aux commandes suivantes pour créer et gérer des **clusters Kubernetes de communauté ou OpenShift** dans {{site.data.keyword.containerlong}}.
{:shortdesc}

* **Kubernetes de communauté** : [Installez le plug-in d'interface CLI](/docs/containers?topic=containers-cs_cli_install#cs_cli_install_steps), qui utilise l'alias `ibmcloud ks`. 
* **OpenShift** : [Installez le plug-in d'interface CLI](/docs/openshift?topic=openshift-openshift-cli), qui utilise l'alias `ibmcloud oc`. 
...
* Pour utiliser `0.4` ou `1.0`, affectez à la variable d'environnement `IKS_BETA_VERSION` la version bêta que vous souhaitez utiliser :
    ...
    ```
    {: pre}
* Pour utiliser la fonctionnalité de noeud final régional obsolète, ciblez le noeud final régional à l'aide de la commande [`init`](#cs_init).
    ```
...
* Francfort (Europe centrale, eu-de) : `https://eu-de.containers.cloud.ibm.com`
...
<dt><code>--endpoint <em>ENDPOINT</em></code></dt>
  <dd>Noeud final d'API {{site.data.keyword.containerlong_notm}}. <strong>Remarque</strong> : ce noeud final est différent des noeuds finaux {{site.data.keyword.cloud_notm}}. Cette valeur est obligatoire pour définir le noeud final d'API.
   </dd>
...
<dt><code>-f</code></dt>
<dd>Facultatif : ce module complémentaire Istio est une dépendance des modules complémentaires gérés <code>istio-extras</code>, <code>istio-sample-bookinfo</code> et <code>knative</code>. Incluez cet indicateur pour désactiver également ces modules.</dd>
</dl>
...
<dt><code>-f</code></dt>
<dd>Facultatif : ce module complémentaire Istio est une dépendance du module complémentaire géré <code>istio-sample-bookinfo</code>. Incluez cet indicateur pour désactiver également ce module.</dd>
</dl>
...
Après la connexion, téléchargez les données de configuration et les certificats Kubernetes pour vous connecter à votre cluster et exécuter des commandes `kubectl`. Les fichiers sont téléchargés dans `user_home_directory/.bluemix/plugins/kubernetes-service/clusters/<cluster_name>`.
{: shortdesc}
...
**Droits minimum requis** : rôle de service {{site.data.keyword.cloud_notm}} IAM **Afficheur** ou **Lecteur** pour le cluster dans {{site.data.keyword.containerlong_notm}}. De plus, si vous ne disposez que d'un seul rôle de plateforme ou d'un rôle de service, des contraintes supplémentaires sont applicables.
...
<dd>Ignorer l'ajout de rôles RBAC Kubernetes de l'utilisateur en fonction des rôles d'accès au service {{site.data.keyword.cloud_notm}} IAM dans la configuration du cluster. Incluez cette option uniquement si vous [gérez vos propres rôles RBAC Kubernetes](/docs/containers?topic=containers-users#rbac). Si vous utilisez des [rôles d'accès au service {{site.data.keyword.cloud_notm}} IAM](/docs/containers?topic=containers-access_reference#service) pour gérer tous vos utilisateurs RBAC, n'incluez pas cette option. </dd>
...
* Rôle **Superutilisateur** pour l'infrastructure IBM Cloud
...
<dd>Niveau d'isolation du matériel pour votre noeud worker. Utilisez `dedicated` pour que les ressources physiques disponibles vous soient dédiées exclusivement ou `shared` pour permettre leur partage avec d'autres clients IBM. La valeur par défaut est `shared`. Cette valeur est facultative pour les clusters standard de machine virtuelle et n'est pas disponible pour les clusters gratuits. Pour les versions bare metal, indiquez `dedicated`. </dd>
...
<dd>Choisissez une version, ou un type de machine, pour vos noeuds worker. Vous pouvez déployer vos noeuds worker en tant que machines virtuelles sur du matériel partagé ou dédié ou en tant que machines physiques sur un serveur bare metal. Les versions physiques et virtuelles disponibles varient en fonction de la zone de déploiement du cluster. Pour plus d'informations, voir la documentation relative à la [commande](#cs_machine_types) `ibmcloud ks flavors (machine-types)`. Cette valeur est obligatoire pour les clusters standard et n'est pas disponible pour les clusters gratuits.</dd>
...
<dd>Version Kubernetes du noeud du maître cluster. Cette valeur est facultative. Lorsque la version n'est pas spécifiée, le cluster est créé avec la valeur par défaut des versions Kubernetes prises en charge. Pour voir les versions disponibles, exécutez la commande <code>ibmcloud ks versions</code>.</dd>
...
<li>Si vous avez déjà créé un cluster standard dans cette zone ou créé un VLAN privé dans l'infrastructure IBM Cloud, vous devez spécifier ce VLAN privé. Les routeurs de VLAN privé commencent toujours par <code>bcr</code> (routeur de back-end) et les routeurs de VLAN public par <code>fcr</code> (routeur de front-end). Lorsque vous créez un cluster et que vous spécifiez les VLAN publics et privés, le nombre et la combinaison de lettres après ces préfixes doivent correspondre. </li>
</ul>
...
<li>Si vous avez déjà créé un cluster standard dans cette zone ou créé un VLAN public dans l'infrastructure IBM Cloud, vous devez spécifier ce VLAN public. Si vous désirez connecter vos noeuds worker uniquement à un VLAN privé, n'indiquez pas cette option. Les routeurs de VLAN privé commencent toujours par <code>bcr</code> (routeur de back-end) et les routeurs de VLAN public par <code>fcr</code> (routeur de front-end). Lorsque vous créez un cluster et que vous spécifiez les VLAN publics et privés, le nombre et la combinaison de lettres après ces préfixes doivent correspondre. </li>
</ul>
...
Désactivez le noeud final de service public pour un cluster.
{: shortdesc}
...
<p class="important">Lorsque vous exécutez cette commande, la création de données d'identification IAM et de secrets d'extraction d'image est initiée et peut durer un certain temps. Vous ne pouvez pas déployer des conteneurs pour extraire une image des domaines `icr.io` d'{{site.data.keyword.registrylong_notm}} tant que les secrets d'extraction d'image ne sont pas créés. Pour vérifier les secrets d'extraction d'image, exécutez la commande `kubectl get secrets | grep icr`.</br></br>Si vous avez ajouté des règles IAM à un ID de service existant, notamment pour limiter l'accès à un registre régional, l'ID de service, les règles IAM et la clé d'API pour le paramètre ImagePullSecret sont réinitialisés par cette commande. </p>
...
Cette commande est dépréciée. Utilisez la [commande `ibmcloud ks versions`](#cs_versions_command) à la place.
{: deprecated}
...
Permet de créer des données d'identification de service pour un service {{site.data.keyword.cloud_notm}} et de stocker ces données d'identification dans un secret Kubernetes dans votre cluster. Pour afficher les services {{site.data.keyword.cloud_notm}} disponibles dans le catalogue {{site.data.keyword.cloud_notm}}, exécutez la commande `ibmcloud service offerings`. **Remarque **: vous ne pouvez ajouter que des services {{site.data.keyword.cloud_notm}} qui prennent en charge les clés de service.
{: shortdesc}
...
Retirez un service {{site.data.keyword.cloud_notm}} d'un cluster en supprimant sa liaison avec un espace de nom Kubernetes.
{: shortdesc}



Lorsque vous supprimez un service {{site.data.keyword.cloud_notm}}, ses données d'identification du service sont supprimées du cluster. Si un pod utilise encore ce service, son opération échoue vu que les données d'identification du service sont introuvables.
{: tip}
...
<dt><code>--service <em>SERVICE_INSTANCE</em></code></dt>
<dd>Nom de l'instance de service {{site.data.keyword.cloud_notm}} que vous souhaitez retirer. Pour trouver le nom de l'instance de service, exécutez la commande `ibmcloud ks cluster-services --cluster <cluster_name_or_ID>`. Cette valeur est obligatoire.</dd>
...
<dd>Type de webhook. Seul Slack est pris en charge actuellement. Cette valeur est obligatoire.</dd>
...
Permet de rendre un sous-réseau public ou privé portable existant dans votre compte d'infrastructure IBM Cloud disponible pour un cluster ou de réutiliser des sous-réseaux à partir d'un cluster supprimé au lieu d'utiliser les sous-réseaux mis à disposition automatiquement.
{: shortdesc}
...
</br>Pour activer la communication entre des noeuds worker qui se trouvent sur différents sous-réseaux d'un même VLAN dans des comptes non VRF, vous devez [activer le routage entre les sous-réseaux sur le même VLAN](/docs/containers?topic=containers-subnets#subnet-routing). </p>
...
Créez un sous-réseau portable dans un compte d'infrastructure IBM Cloud sur votre VLAN public ou privé et rendez-le disponible pour un cluster.
{: shortdesc}
...
Déconnectez un sous-réseau portable public ou privé d'un compte d'infrastructure IBM Cloud à partir d'un cluster. Le sous-réseau reste disponible dans votre compte d'infrastructure IBM Cloud.**Remarque :** tout service déployé sur une adresse IP depuis le sous-réseau reste actif après le retrait de celui-ci.
{: shortdesc}

<p class="note"><img src="images/icon-classic.png" alt="Icône de fournisseur d'infrastructure classique" width="15" style="width:15px; border-style: none"/> Commande classique uniquement. </p>
...
<dt><code>--vlan <em>VLAN_ID</em></code></dt>
<dd>ID du sous-réseau public ou privé que vous souhaitez déconnecter. Pour trouver l'ID de sous-réseau, exécutez d'abord la commande <code>ibmcloud ks cluster-get --cluster &lt;cluster&gt; --showResources</code> et recherchez le routage CIDR de sous-réseau dans la section <strong>Subnet VLANs</strong> dans la sortie. Ensuite, à l'aide du routage CIDR de sous-réseau, exécutez la commande <code>ibmcloud ks subnets</code> et recherchez l'<strong>ID</strong> du sous-réseau. </dd>
</dl>
...
Ce sous-réseau privé n'est pas celui qui est fourni par l'infrastructure IBM Cloud. De ce fait, vous devez configurer tout routage de trafic entrant et sortant pour le sous-réseau. Pour ajouter un sous-réseau d'infrastructure IBM Cloud, utilisez la [commande](#cs_cluster_subnet_add) `ibmcloud ks cluster-subnet-add`. 
...
<dd>CIDR (Classless InterDomain Routing) du sous-réseau. Cette valeur est obligatoire et ne doit pas entrer en conflit avec un sous-réseau utilisé par l'infrastructure IBM Cloud. Les préfixes pris en charge sont compris entre `/30` (1 adresse IP) et `/24` (253 adresses IP). Si vous avez défini un CIDR avec une longueur de préfixe et que vous devez modifier sa valeur, ajoutez d'abord le nouveau CIDR, puis [supprimez l'ancien CIDR](#cs_cluster_user_subnet_rm).</dd>

<dt><code>--private-vlan <em>PRIVATE_VLAN</em></code></dt>
<dd>ID du VLAN privé. Cette valeur est obligatoire. L'ID doit être celui d'un VLAN privé dans le cluster sur lequel résident un ou plusieurs des noeuds worker. Pour voir les VLAN privés dans votre cluster, exécutez la commande `ibmcloud ks cluster-get --cluster <cluster_name_or_ID> --showResources`. Dans la section **Subnet VLANs** de la sortie, recherchez les VLAN ayant la valeur `false` pour **Public**. </dd>
</dl>
...
Permet de répertorier les sous-réseaux portables disponibles dans votre compte d'infrastructure IBM Cloud.
{: shortdesc}
...
Vérifie si les mises à jour automatiques du module complémentaire ALB Ingress sont activées et si les ALB sont mis à jour vers la dernière version.
{: shortdesc}
...
Vous pouvez également utiliser le paramètre `--update` pour mettre à jour des certificats, par exemple, pour mettre à jour le certificat dans votre cluster après l'avoir renouvelé dans {{site.data.keyword.cloudcerts_short}}. Vous ne pouvez mettre à jour que des certificats importés depuis la même instance {{site.data.keyword.cloudcerts_long_notm}}.
...
<dd>Met à jour le certificat d'un secret d'équilibreur de charge d'application (ALB) dans un cluster. Vous pouvez utiliser ce paramètre pour mettre à jour le certificat après l'avoir renouvelé dans {{site.data.keyword.cloudcerts_short}}. </dd>

<dt><code>--secret-name <em>SECRET_NAME</em></code></dt>
<dd>Spécifiez un nom pour le secret ALB lorsqu'il est créé dans le cluster. Cette valeur est obligatoire. Vérifiez que vous n'avez pas créé un secret ayant le même nom que le secret Ingress fourni par IBM. Vous pouvez obtenir le nom du secret Ingress fourni par IBM en exécutant la commande <code>ibmcloud ks cluster-get --cluster <cluster_name_or_ID> | grep Ingress</code>.</dd>
...
Exemple de déploiement d'un secret ALB :
```
...
Exemple de mise à jour d'un secret ALB existant :
```
...
<dt><code>--secret-name <em>SECRET_NAME</em></code></dt>
<dd>Nom du secret ALB. Cette valeur est requise pour obtenir des informations sur un secret ALB spécifique dans le cluster.</dd>

<dt><code>--cert-crn <em>CERTIFICATE_CRN</em></code></dt>
<dd>CRN du certificat. Cette valeur est requise pour obtenir des informations sur tous les secrets ALB correspondant à un CRN de certificat spécifique dans le cluster. </dd>
...
Exemple d'obtention d'informations relatives à un secret ALB :
```
ibmcloud ks alb-cert-get --cluster my_cluster --secret-name my_alb_secret
```
{: pre}

Exemple d'obtention d'informations relatives à tous les secrets ALB correspondant à un CRN de certificat spécifié :
```
...
<dt><code>--secret-name <em>SECRET_NAME</em></code></dt>
<dd>Nom du secret ALB. Cette valeur est requise pour retirer un secret ALB spécifique dans le cluster.</dd>

<dt><code>--cert-crn <em>CERTIFICATE_CRN</em></code></dt>
<dd>CRN du certificat. Cette valeur est requise pour retirer tous les secrets ALB qui correspondent à un certificat CRN spécifique dans le cluster. </dd>
...
Exemple de retrait d'un secret ALB :
```
ibmcloud ks alb-cert-rm --cluster my_cluster --secret-name my_alb_secret
```
{: pre}

Exemple de retrait de tous les secrets ALB correspondant à un CRN de certificat spécifié :
```
...
* Activez un ALB privé par défaut. Lorsque vous créez un cluster, un ALB privé par défaut est créé pour vous dans chaque zone où se trouvent des noeuds worker et un sous-réseau privé disponible, mais les ALB privés par défaut ne sont pas activés. Cependant, tous les ALB publics par défaut sont automatiquement activés. 
...
<dd>ID d'un équilibreur de charge ALB. Pour afficher les ID des ALB dans un cluster, exécutez la commande <code>ibmcloud ks albs --cluster <em>CLUSTER</em></code>.
  Cette valeur est obligatoire.</dd>
...
Si vos pods d'ALB ont été récemment mis à jour, mais qu'une configuration personnalisée de vos ALB est affectée par la dernière version, vous pouvez rétromigrer la mise à jour vers la version que vos pods d'ALB utilisaient auparavant. Tous les pods d'équilibreur de charge d'application de votre cluster reviennent à leur état précédent.
{: shortdesc}

Après avoir annulé une mise à jour, les mises à jour automatiques des pods d'ALB sont désactivées. Pour réactiver les mises à jour automatiques, utilisez la [commande `alb-autoupdate-enable`](#cs_alb_autoupdate_enable).
...
Répertoriez les types d'équilibreur de charge d'application Ingress qui sont pris en charge.
{: shortdesc}
...
Si les mises à jour automatiques du module complémentaire ALB Ingress sont désactivées et que vous souhaitez le mettre à jour, vous pouvez forcer une mise à jour à usage unique pour vos pods d'ALB. Lorsque vous optez pour une mise à jour manuelle du module complémentaire, les pods d'ALB du cluster sont mis à jour vers la dernière version. Vous ne pouvez pas mettre à jour un équilibreur de charge ALB individuel ou choisir la version vers laquelle mettre à jour le module complémentaire. Les mises à jour automatiques restent désactivées.
...
Permet d'afficher le nom et l'adresse e-mail du propriétaire de la clé d'API {{site.data.keyword.cloud_notm}} IAM (Identity and Access Management) dans un groupe de ressources {{site.data.keyword.containerlong_notm}}.
{: shortdesc}



La clé d'API {{site.data.keyword.cloud_notm}} IAM est définie automatiquement pour un groupe de ressources et une région lorsque la première action qui nécessite la règle d'accès admin {{site.data.keyword.containerlong_notm}} est effectuée. Par exemple, supposons que l'un de vos administrateurs crée le premier cluster dans le groupe de ressources `default` dans la région `us-south`. Pour cette opération, la clé d'API {{site.data.keyword.cloud_notm}} IAM de cet utilisateur est stockée dans le compte correspondant à ce groupe de ressources et à cette région. La clé d'API est utilisée pour commander des ressources dans l'infrastructure IBM Cloud, par exemple de nouveaux noeuds worker ou VLAN. Une autre clé d'API peut être définie pour chaque région au sein d'un groupe de ressources.

Lorsqu'un autre utilisateur effectue une action qui nécessite une interaction avec le portefeuille d'infrastructure IBM Cloud dans ce groupe de ressources et dans cette région, par exemple la création d'un nouveau cluster ou le rechargement d'un noeud worker, la clé d'API stockée est utilisée pour déterminer s'il existe des droits suffisants pour effectuer cette action. Pour vous assurer que les actions liées à l'infrastructure dans votre cluster peuvent être effectuées sans problème, affectez à vos administrateurs {{site.data.keyword.containerlong_notm}} la règle d'accès **Superutilisateur** de l'infrastructure. Pour plus d'informations, voir [Gestion de l'accès utilisateur](/docs/containers?topic=containers-users#infra_access).
...
**Astuce :** la clé d'API renvoyée par cette commande ne peut pas être utilisée si les données d'identification d'infrastructure IBM Cloud ont été définies manuellement à l'aide de la commande [ibmcloud ks credential-set](#cs_credentials_set). 
...
Permet de remplacer la clé d'API {{site.data.keyword.cloud_notm}} IAM en cours dans un groupe de ressources {{site.data.keyword.cloud_notm}} et une région {{site.data.keyword.containershort_notm}}.
{: shortdesc}



Cette commande nécessite la règle d'accès admin {{site.data.keyword.containerlong_notm}} et stocke la clé d'API de l'utilisateur qui exécute cette commande dans le compte. La clé d'API {{site.data.keyword.cloud_notm}} IAM est nécessaire pour commander l'infrastructure depuis le portefeuille d'infrastructure IBM Cloud. Une fois stockée, la clé d'API est utilisée pour toutes les actions dans une région qui nécessite des droits d'accès à l'infrastructure indépendamment de l'utilisateur qui exécute cette commande. Pour plus d'informations sur le mode de fonctionnement des clés d'API {{site.data.keyword.cloud_notm}} IAM, voir la [commande `ibmcloud ks api-key-info`](#cs_api_key_info). 

Avant d'utiliser cette commande, assurez-vous que l'utilisateur qui l'exécute dispose des [droits {{site.data.keyword.containerlong_notm}} et des droits d'infrastructure IBM Cloud](/docs/containers?topic=containers-users#users) requis. Ciblez le groupe de ressources et la région pour lesquels vous voulez définir la clé d'API.
{: important}
...
Si vous configurez votre compte {{site.data.keyword.cloud_notm}} pour utiliser différentes données d'identification afin d'accéder au portefeuille de l'infrastructure IBM Cloud, obtenez le nom d'utilisateur de l'infrastructure pour la région et le groupe de ressources que vous ciblez actuellement.
{: shortdesc}
...
Avant d'utiliser cette commande, assurez-vous que l'utilisateur dont les données d'identification sont utilisées dispose des[droits {{site.data.keyword.containerlong_notm}} et des droits de l'infrastructure IBM Cloud](/docs/containers?topic=containers-users#users) requis.
{: important}
...
<dd>Nom d'utilisateur d'API du compte d'infrastructure IBM Cloud. Cette valeur est obligatoire. Le nom d'utilisateur de l'API d'infrastructure est différent de l'IBMid. Pour connaître le nom d'utilisateur de l'API d'infrastructure, voir [Gestion des clés d'API d'infrastructure classique](/docs/iam?topic=iam-classic_keys).</dd>

<dt><code>--infrastructure-api-key <em>API_KEY</em></code></dt>
<dd>Clé d'API du compte d'infrastructure IBM Cloud. Cette valeur est obligatoire. Pour visualiser ou générer une clé d'API d'infrastructure, voir [Gestion des clés d'API d'infrastructure classique](/docs/iam?topic=iam-classic_keys).</dd>
...
Permet de retirer les données d'identification pour un groupe de ressources et une région qui vous permettent d'accéder au portefeuille de l'infrastructure IBM Cloud via votre compte {{site.data.keyword.cloud_notm}}.
{: shortdesc}

Après le retrait des données d'identification, la [clé d'API {{site.data.keyword.cloud_notm}} IAM](#cs_api_key_info) est utilisée pour commander des ressources dans l'infrastructure IBM Cloud. 
...
Afficher la liste des versions de noeud worker disponibles. Les versions varient en fonction des zones.
{:shortdesc}



Chaque version inclut la quantité d'UC virtuelles, de mémoire et d'espace disque pour chaque noeud worker dans le cluster. Par défaut, le répertoire du disque de stockage secondaire dans lequel sont stockées toutes les données des conteneurs, est chiffré avec le chiffrement LUKS. Si l'option `disable-disk-encrypt` est incluse lors de la création du cluster, les données de l'environnement d'exécution de conteneur de l'hôte ne sont pas chiffrées. [En savoir plus sur le chiffrement](/docs/containers?topic=containers-security#encrypted_disk).

Vous pouvez mettre à disposition votre noeud worker en tant que machine virtuelle sur un matériel dédié ou partagé, ou pour les clusters classiques uniquement, en tant que machine physique sur un serveur bare metal. [En savoir plus sur les options de version](/docs/containers?topic=containers-planning_worker_nodes#planning_worker_nodes). 
...
<dl>
<dt><code>--zone <em>ZONE</em></code></dt>
<dd>Entrez la zone dans laquelle vous voulez répertorier les versions disponibles. Cette valeur est obligatoire. Pour voir les zones disponibles pour les clusters classiques, exécutez la commande `ibmcloud ks zones`. </dd>
...
Permet de vérifier si les droits d'infrastructure suggérés ou requis sont manquants dans les données d'identification qui permettent [d'accéder au portefeuille de l'infrastructure IBM Cloud](/docs/containers?topic=containers-users#api_key) pour le groupe de ressources et la région ciblés.
{: shortdesc}
...
Une fois les données d'identification de l'infrastructure configurées, vous pouvez contrôler les actions que vos utilisateurs peuvent effectuer en affectant à ces derniers des [rôles de plateforme {{site.data.keyword.cloud_notm}}](/docs/containers?topic=containers-access_reference#iam_platform) IAM. 
...
Affichez le statut de la fonction Spanning VLAN d'un compte d'infrastructure IBM Cloud. La fonction Spanning VLAN permet à toutes les unités d'un même compte de communiquer les unes avec les autres par le biais du réseau privé, indépendamment du VLAN qui leur est affecté.
{: shortdesc}

<p class="note">L'option Spanning VLAN est désactivée pour les clusters qui sont créés dans un compte VRF. Lorsque VRF est activé, tous les VLAN du compte peuvent communiquer automatiquement entre eux via le réseau privé. Pour vérifier si la fonction VRF est activée, utilisez la commande `ibmcloud account show`. Pour plus d'informations, voir [Planification de votre configuration de réseau de cluster : Communication entre les noeuds worker](/docs/containers?topic=containers-plan_clusters#worker-worker). </p>
...
Répertoriez les VLAN publics et privés disponibles pour une zone dans votre compte d'infrastructure IBM Cloud. Pour répertorier ces VLAN, vous devez disposer d'un compte payant.
{: shortdesc}
...
<dl>
<dt><code>--zone <em>ZONE</em></code></dt>
<dd>Indiquez la zone où répertorier vos VLAN privés et publics. Cette valeur est obligatoire. Pour afficher les zones disponibles, exécutez la commande `ibmcloud ks zones`. </dd>
...
<dd>Si le type de consignation correspond à <code>syslog</code>, cette valeur est le nom d'hôte ou l'adresse IP du serveur collecteur de journal. Cette valeur est obligatoire pour <code>syslog</code>. Si le type de consignation correspond à <code>ibm</code>, cette valeur est l'URL d'ingestion {{site.data.keyword.loganalysislong_notm}}. Vous trouverez [ici](/docs/services/CloudLogAnalysis?topic=cloudloganalysis-log_ingestion#log_ingestion_urls) la liste des URL d'ingestion disponibles. Si vous ne spécifiez pas d'URL d'ingestion, le noeud final de la région où votre cluster a été créé est utilisé.</dd>
...
<dt><code>--force-update</code></dt>
<dd>Force la mise à jour de vos pods Fluentd à la version la plus récente. Fluentd doit être à la version la plus récente pour que des modifications puissent être apportées à vos configurations de consignation.</dd>
...
Actualise la configuration de consignation pour le cluster. Cette action actualise le jeton de consignation de toute configuration de consignation qui achemine des données au niveau de l'espace dans votre cluster.
{: shortdesc}
...
Supprimez une configuration d'acheminement des journaux ou toutes les configurations de consignation d'un cluster. La suppression de la configuration de consignation cesse l'acheminement des journaux à un serveur syslog distant ou à {{site.data.keyword.loganalysisshort_notm}}.
{: shortdesc}
...
L'exemple suivant permet de filtrer tous les journaux transférés, de niveau <code>info</code> ou inférieur, à partir d'un cluster spécifique. La sortie est renvoyée au format JSON.
```
...
Retirer une adresse IP d'équilibreur de charge de réseau d'un nom d'hôte. Si vous retirez toutes les adresses IP d'un nom d'hôte, le nom d'hôte existe toujours mais aucune adresse IP ne lui est associée. <strong>Remarque</strong> : vous devez exécuter cette commande pour chaque adresse IP que vous souhaitez retirer.
{: shortdesc}
...
<dd>Délai d'attente, exprimé en secondes, avant que l'adresse IP soit considérée comme inaccessible. Le diagnostic d'intégrité attend pendant le nombre de secondes spécifié au paramètre `interval` avant de tenter d'atteindre à nouveau l'adresse IP. La valeur doit être un entier compris entre 1 et 15. Valeur par défaut : <code>5</code></dd>

<dt><code>--retries <em>RETRIES</em></code></dt>
<dd>Lorsqu'un dépassement de délai d'attente se produit, nombre de tentatives avant que l'adresse IP inaccessible soit considérée comme défectueuse. Les nouvelles tentatives sont effectuées immédiatement. La valeur doit être un entier compris entre 1 et 5. Valeur par défaut : <code>2</code></dd>

<dt><code>--interval <em>INTERVAL</em></code></dt>
<dd>Intervalle, en secondes, entre chaque diagnostic d'intégrité. Des intervalles courts peuvent améliorer les temps de reprise, mais augmenter la charge sur les adresses IP. La valeur doit être un entier compris entre 10 et 3600 et doit être supérieure à `(RETRIES + 1) * TIMEOUT`. Valeur par défaut : <code>60</code></dd>
...
Permet de répertorier les régions disponibles. Le nom indiqué dans `Region Name` correspond au nom {{site.data.keyword.containerlong_notm}} et `Region Alias` correspond au nom {{site.data.keyword.cloud_notm}} général pour la région.
{: shortdesc}

Les noeuds finaux propres à une région sont dépréciés. Utilisez à la place le [noeud final global](/docs/containers?topic=containers-regions-and-zones#endpoint).
{: deprecated}
...
Permet d'ajouter des noeuds worker autonomes à un cluster.
{: shortdesc}

Cette commande est dépréciée. Créez un pool de noeuds worker en exécutant la commande [`ibmcloud ks worker-pool-create`](#cs_worker_pool_create) ou ajoutez des noeuds worker à un pool de noeuds worker existant en exécutant la commande [`ibmcloud ks worker-pool-resize`](#cs_worker_pool_resize).
{: deprecated}
...
<p class="note">Si vous indiquez la même option dans la commande comme paramètre dans le fichier YAML, la valeur de l'option de la commande est prioritaire sur la valeur définie dans le fichier YAML. Par exemple, si vous indiquez une version dans votre fichier YAML et utilisez l'option <code>--machine-type</code> dans la commande, la valeur que vous avez entrée dans l'option de commande se substitue à la valeur définie dans le fichier YAML. </p>
...
<td>Remplacez <code><em>&lt;machine_type&gt;</em></code> par la version, ou type de machine, où vous envisagez de déployer vos noeuds worker. Vous pouvez déployer vos noeuds worker en tant que machines virtuelles sur du matériel partagé ou dédié ou en tant que machines physiques sur un serveur bare metal. Les versions physiques et virtuelles disponibles varient en fonction de la zone de déploiement du cluster. Pour plus d'informations, voir la [commande](#cs_machine_types) `ibmcloud ks flavors (machine-types)`. </td>
</tr>
<tr>
...
<td>Pour les versions virtuelles : niveau d'isolement matériel de votre noeud worker. Utilisez `dedicated` si vous désirez que les ressources physiques disponibles vous soient dédiées exclusivement ou `shared` pour permettre leur partage avec d'autres clients IBM. La valeur par défaut est `shared`.</td>
</tr>
<tr>
...
<dt><code>--hardware <em>HARDWARE</em></code></dt>
<dd>Niveau d'isolation du matériel pour votre noeud worker. Utilisez `dedicated` pour que les ressources physiques disponibles vous soient dédiées exclusivement ou `shared` pour permettre leur partage avec d'autres clients IBM. La valeur par défaut est `shared`. Cette valeur est facultative. Pour les versions bare metal, indiquez `dedicated`. </dd>

<dt><code>--machine-type <em>MACHINE_TYPE</em></code></dt>
<dd>Choisissez un type de machine, ou version, pour vos noeuds worker. Vous pouvez déployer vos noeuds worker en tant que machines virtuelles sur du matériel partagé ou dédié ou en tant que machines physiques sur un serveur bare metal. Les types de machines virtuelles et physiques disponibles varient en fonction de la zone de déploiement du cluster. Pour plus d'informations, voir la documentation relative à la [commande](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_machine_types) `ibmcloud ks flavors (machine-types)`. Cette valeur est obligatoire pour les clusters standard et n'est pas disponible pour les clusters gratuits.</dd>
...
<dt><code>--private-vlan <em>PRIVATE_VLAN</em></code></dt>
<dd>VLAN privé spécifié lors de la création du cluster. Cette valeur est obligatoire. Les routeurs de VLAN privé commencent toujours par <code>bcr</code> (routeur de back-end) et les routeurs de VLAN public par <code>fcr</code> (routeur de front-end). Lorsque vous créez un cluster et que vous spécifiez les VLAN publics et privés, le nombre et la combinaison de lettres après ces préfixes doivent correspondre. </dd>

<dt><code>--public-vlan <em>PUBLIC_VLAN</em></code></dt>
<dd>VLAN public spécifié lors de la création du cluster. Cette valeur est facultative. Si vous désirez que vos noeuds worker ne soient présents que sur un réseau virtuel local privé, n'indiquez pas d'ID de réseau virtuel local public. Les routeurs de VLAN privé commencent toujours par <code>bcr</code> (routeur de back-end) et les routeurs de VLAN public par <code>fcr</code> (routeur de front-end). Lorsque vous créez un cluster et que vous spécifiez les VLAN publics et privés, le nombre et la combinaison de lettres après ces préfixes doivent correspondre. <p class="note">Si les noeuds worker sont configurés uniquement avec un VLAN privé, vous devez permettre aux noeuds worker et au maître cluster de communiquer en [activant le noeud final de service privé](/docs/containers?topic=containers-cs_network_cluster#set-up-private-se) ou en [configurant un périphérique de passerelle](/docs/containers?topic=containers-plan_clusters#workeruser-master).</p></dd>
...
Réamorcez un noeud worker dans un cluster.
{: shortdesc}



Lors du réamorçage, l'état de votre noeud worker reste inchangé. Par exemple, vous pouvez recourir à un réamorçage si le statut du noeud worker dans l'infrastructure IBM Cloud est `Powered Off` et qu'il vous faut mettre le noeud worker sous tension. Un réamorçage efface les répertoires temporaires mais pas le système de fichiers complet et n'entraîne ps le reformatage des disques. L'adresse IP du noeud worker reste la même après l'opération de réamorçage.
...
Recharge les configurations pour un noeud worker.
{: shortdesc}



Un rechargement peut s'avérer utile en cas de problème sur votre noeud worker, par exemple une dégradation des performances ou une immobilisation dans un mauvais état de santé. Durant le rechargement, la machine de votre noeud worker est mise à jour avec l'image la plus récente et les données sont supprimées si elles ne sont pas [stockées hors du noeud worker](/docs/containers?topic=containers-storage_planning#persistent_storage_overview). L'adresse IP privée et l'adresse IP publique du noeud worker restent inchangées après l'opération de rechargement. 
...
Vous devrez peut-être modifier vos fichiers YAML en vue des déploiements avant la mise à jour. Consultez cette [note sur l'édition](/docs/containers?topic=containers-cs_versions) pour plus de détails.
...
<dt><code>--machine-type <em>MACHINE_TYPE</em></code></dt>
<dd>Choisissez un type de machine, ou version. Vous pouvez déployer vos noeuds worker en tant que machines virtuelles sur du matériel partagé ou dédié ou en tant que machines physiques sur un serveur bare metal. Les types de machines virtuelles et physiques disponibles varient en fonction de la zone de déploiement du cluster. Pour plus d'informations, voir la documentation relative à la [commande](#cs_machine_types) `ibmcloud ks flavors (machine-types)`. Cette valeur est obligatoire pour les clusters standard et n'est pas disponible pour les clusters gratuits.</dd>
...
<dd>Niveau d'isolation du matériel pour votre noeud worker. Utilisez `dedicated` si vous désirez que les ressources physiques disponibles vous soient dédiées exclusivement ou `shared` pour permettre leur partage avec d'autres clients IBM. La valeur par défaut est `shared`. Pour les versions bare metal, indiquez `dedicated`. Cette valeur est obligatoire.</dd>
...
<dd>Liste séparée par une virgule de pools de noeuds worker auxquels est ajoutée la zone. Au moins un pool de noeuds worker est requis. </dd>
...
<dd><p>ID du VLAN privé. Cette valeur est conditionnelle.</p>
    ...
    <p>Si vous ne disposez pas de VLAN public ou privé dans cette zone, n'indiquez pas cette option. Un VLAN privé et un VLAN public sont automatiquement créés pour vous la première fois que vous ajoutez une zone dans votre pool de noeuds worker.</p>
...
<li>Vérifiez si les ID de VLAN public et privé que vous souhaitez utiliser sont compatibles. Pour qu'ils soient compatibles, le routeur (<strong>Router</strong>) doit avoir le même ID de pod. Les routeurs de VLAN privé commencent toujours par <code>bcr</code> (routeur de back-end) et les routeurs de VLAN public par <code>fcr</code> (routeur de front-end). Lorsque vous créez un cluster et que vous spécifiez les VLAN publics et privés, le nombre et la combinaison de lettres après ces préfixes doivent correspondre. <pre class="pre"><code>ibmcloud ks vlans --zone &lt;zone&gt;</code></pre><p>Exemple de sortie :</p>
<pre class="screen"><code>ID        Name   Number   Type      Router         Supports Virtual Workers
229xxxx          1234     private   bcr01a.dal12   true
229xxxx          5678     public    fcr01a.dal12   true</code></pre><p>Notez que les ID de pod de la section <strong>Router</strong> correspondent à : `01a` et `01a`. Si un ID de pod était `01a` et que l'autre était `02a`, vous ne pourriez pas définir ces ID de VLAN public et privé pour votre pool de noeuds worker.
     </p></li>
...
Avant de retirer une zone, assurez-vous de disposer d'un nombre de noeuds worker suffisants dans d'autres zones du cluster de sorte que vos pods puissent être replanifiés.
Cela permettra d'éviter toute indisponibilité de votre application ou l'altération des données sur votre noeud worker.
{: tip}
...
<dl>
<dt><code>--zone <em>ZONE</em></code></dt>
<dd>Zone que vous désirez retirer. Cette valeur est obligatoire.</dd>
