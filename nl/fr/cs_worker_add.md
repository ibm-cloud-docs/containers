...
3.  Pour chaque zone, consultez les [versions disponibles pour les noeuds worker](/docs/containers?topic=containers-planning_worker_nodes#planning_worker_nodes). 
...
3. Répertoriez les versions disponibles dans cette zone. 
   ```
   ibmcloud ks flavors --zone <zone>
   ```
   {: pre}

4. Ajoutez des noeuds worker autonomes dans le cluster. Pour les versions bare metal, indiquez `dedicated`. 
   ```
