...
cluster et exécutez les commandes `kubectl`. Les fichiers sont téléchargés sous `user_home_directory/.bluemix/plugins/container-service/clusters/<cluster_name>`.
...
il se peut que vous ayez besoin d'une autorisation des autorités pour stocker physiquement les données dans un autre pays.</p>
    ...
    <dd>(Non disponible pour les clusters légers). 
...
Permet d'afficher des informations sur un cluster dans votre organisation.
...
Permet de retirer un cluster de votre organisation.
...
Permet d'ajouter un service {{site.data.keyword.Bluemix_notm}} à un cluster.
...
Permet de retirer un service {{site.data.keyword.Bluemix_notm}} d'un cluster.
...
Permet de répertorier les services liés à un ou à tous les espaces de nom Kubernetes dans un cluster. Si aucune option n'est spécifiée, les services pour l'espace de nom par défaut sont affichés.
...
Permet de rendre un sous-réseau d'un compte {{site.data.keyword.BluSoftlayer_notm}} disponible pour un cluster spécifié.
...
Permet d'afficher la liste des clusters dans votre organisation.
...
Permet de définir les informations d'authentification de compte {{site.data.keyword.BluSoftlayer_notm}} pour votre compte {{site.data.keyword.Bluemix_notm}}. Ces informations vous permettent d'accéder au portefeuille {{site.data.keyword.BluSoftlayer_notm}} via votre compte {{site.data.keyword.Bluemix_notm}}.
...
Permet de retirer mes informations d'authentification de compte {{site.data.keyword.BluSoftlayer_notm}} de votre compte {{site.data.keyword.Bluemix_notm}}. Après cette suppression, vous ne pouvez plus accéder au portefeuille {{site.data.keyword.BluSoftlayer_notm}} via votre compte {{site.data.keyword.Bluemix_notm}}.
...
Permet d'afficher la liste des emplacements disponibles dans lesquels vous pouvez créer un cluster.
...
Permet d'afficher la liste des commandes et paramètres pris en charge.
...
Permet d'initialiser le plug-in {{site.data.keyword.containershort_notm}} ou de spécifier la région dans laquelle vous souhaitez créer ou accéder à des clusters Kubernetes.
...
<strong>Options de commande</strong> :
   ...
   <dd>(Facultatif) Noeud final d'API {{site.data.keyword.containershort_notm}} que vous désirez utiliser. Exemples :
    ...
    <li>Sud du Royaume-Uni :<pre class="codeblock">
...
Permet d'afficher la liste des types de machine disponibles pour vos noeuds d'agent. Chaque type de machine inclut la quantité d'UC virtuelles, de mémoire et d'espace disque pour chaque noeud d'agent dans le cluster.
...
Permet d'afficher la liste des sous-réseaux qui sont disponibles dans un compte {{site.data.keyword.BluSoftlayer_notm}}.
...
Permet d'afficher la liste des réseaux locaux virtuels publics et privés disponibles pour un emplacement dans votre compte {{site.data.keyword.BluSoftlayer_notm}}. Pour répertorier ces réseaux, vous devez disposer d'un compte payant.
...
<strong>Options de commande</strong> :
   ...
   <dd>(Facultatif) Niveau de notification, tel que <code>Normal</code> ou <code>Warning</code>. <code>Warning</code> est la valeur par défaut.</dd>
...
Permet d'ajouter des noeuds d'agent à votre cluster standard.
...
Permet d'afficher les informations détaillées d'un noeud d'agent.
...
Permet de redémarrer les noeuds d'agent dans un cluster. Si un problème affecte un noeud d'agent, essayez d'abord de réamorcer celui-ci, ce qui a pour effet de redémarrer le noeud d'agent. Si le réamorçage ne résout pas le problème, exécutez la commande 'worker-reload'. L'état des agents ne change pas pendant le réamorçage. L'état demeure 'deployed', mais le statut de l'agent est mis à jour.<strong>Options de commande</strong> :
...
Permet de recharger les noeuds d'agent dans un cluster. Si un problème affecte un noeud d'agent, essayez d'abord de le réamorcer. Si le réamorçage ne résout pas le problème, exécutez la commande 'worker-reload', laquelle recharge toutes les configurations requises pour le noeud d'agent.<strong>Options de commande</strong> :
...
Permet de retirer un ou plusieurs noeuds d'agent d'un cluster.
...
Permet d'afficher la liste des noeuds d'agent dans un cluster et la statut de chacun d'eux.
...
Vous pouvez afficher l'état actuel du cluster en exécutant la commande bx cs clusters et en localisant la zone **State**. L'état du cluster vous donne des informations sur la disponibilité et de la capacité du cluster, et sur les problèmes qui se sont éventuellement produits. {:shortdesc}

|Etat du cluster|Cause|
|-------------|------|
|Deploying|Le maître Kubernetes n'est pas encore entièrement déployé. Vous ne pouvez pas accéder à votre cluster.|
...
|Normal|Tous les noeuds d'agent d'un cluster sont opérationnels. Vous pouvez accéder au cluster et déployer des applications sur le cluster.
