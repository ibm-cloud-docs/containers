...
La méthode précédente qui consistait à autoriser l'accès d'un cluster à {{site.data.keyword.registrylong_notm}} par la création automatique d'un [jeton](/docs/services/Registry?topic=registry-registry_access#registry_tokens) et en stockant ce jeton dans une valeur imagePullSecret est prise en charge mais dépréciée. [Mettez à jour votre cluster pour utiliser la méthode ayant recours à une clé d'API](#imagePullSecret_migrate_api_key) pour la valeur imagePullSecret et mettez à jour les déploiements pour extraire des images de noms de domaine `icr.io`.
{: deprecated}

### Mise à jour de clusters existants pour utiliser la valeur imagePullSecret de la clé d'API
{: #imagePullSecret_migrate_api_key}

Les nouveaux clusters {{site.data.keyword.containerlong_notm}} stockent une clé d'API dans une valeur imagePullSecret pour autoriser l'accès à {{site.data.keyword.registrylong_notm}}. Avec ces valeurs confidentielles ImagePullSecrets, vous pouvez déployer des conteneurs à partir d'images stockées dans les domaines de registre `icr.io`. Pour les clusters créés avant le **25 février 2019**, vous devez mettre à jour votre cluster pour stocker une clé d'API au lieu d'un jeton de registre dans la valeur confidentielle d'extraction d'image (imagePullSecret).
{: shortdesc}
...
*   Vérifiez que vous disposez des droits suivants :
    *   Rôle de plateforme {{site.data.keyword.Bluemix_notm}} IAM **Opérateur ou Administrateur** pour {{site.data.keyword.containerlong_notm}}
    *   Rôle de plateforme {{site.data.keyword.Bluemix_notm}} IAM **Administrateur** pour {{site.data.keyword.registrylong_notm}}

Pour mettre à jour la valeur imagePullSecret de votre cluster :
1.  Obtenez l'ID de votre cluster.
    ```
    ibmcloud ks clusters
    ```
    {: pre}
2.  Exécutez la commande suivante pour créer un ID de service pour le cluster, affecter l'ID de service à un rôle de service IAM **Lecteur** pour {{site.data.keyword.registrylong_notm}}, créer une clé d'API pour représenter les données d'identification de l'ID de service et stocker la clé d'API dans une valeur confidentielle Kubernetes imagePullSecret dans le cluster. Cette valeur confidentielle se trouve dans l'espace de nom Kubernetes `default`.
    ```
    ibmcloud ks cluster-pull-secret-apply --cluster <cluster_name_or_ID>
    ```
    {: pre}

    Lorsque vous exécutez cette commande, la création de données d'identification IAM et de valeurs confidentielles imagePullSecret est initiée et peut prendre un certain temps. Vous ne pouvez pas déployer de conteneurs qui extraient une image des domaines {{site.data.keyword.registrylong_notm}} `icr.io` tant que les valeurs confidentielles d'extraction d'image (imagePullSecret) ne sont pas créées.
    {: important}
...
**Etape suivante ?**
*   Pour extraire des images dans d'autres espaces de nom Kubernetes que `default` ou à partir d'autres comptes {{site.data.keyword.Bluemix_notm}}, [copiez ou créez une autre valeur confidentielle imagePullSecret](/docs/containers?topic=containers-images#other).
...
Vous pouvez déployer dans votre cluster des conteneurs depuis une image fournie par IBM ou depuis une image privée stockée dans votre espace de nom {{site.data.keyword.registryshort_notm}}. Pour plus d'informations sur les modes d'accès, voir [Comment votre cluster est-il autorisé à extraire des images d'{{site.data.keyword.registrylong_notm}}](#cluster_registry_auth).
{:shortdesc}

Avant de commencer :
1. [Configurez un espace de nom dans {{site.data.keyword.registryshort_notm}} et envoyez des images dans cet espace de nom](/docs/services/Registry?topic=registry-index#registry_namespace_add).
2. [Créez un cluster](/docs/containers?topic=containers-clusters#clusters_cli).
3. Si vous utilisez un cluster créé avant le **<DATE>**, [mettez à jour votre cluster pour utiliser la valeur `imagePullSecret` de la clé d'API](#imagePullSecret_migrate_api_key).
...
2.  Définissez le déploiement et l'image de votre espace de nom que vous désirez utiliser dans {{site.data.keyword.registryshort_notm}}.
    ...
    *  **`<region>`** : pour répertorier le domaine correspondant à la région dans laquelle vous vous trouvez, exécutez la commande `ibmcloud cr api`.
    *  **`<namespace>`** : pour obtenir des informations sur votre espace de nom, exécutez la commande `ibmcloud cr namespace-list`.
    *  **`<my_image>:<tag>`** : pour obtenir les images disponibles dans votre registre, exécutez la commande `ibmcloud cr images`.
...
## Utilisation d'une valeur confidentielle d'extraction d'image pour accéder à d'autres espaces de nom Kubernetes, d'autres comptes {{site.data.keyword.Bluemix_notm}} ou à des registres privés externes
{: #other}

Utilisez votre propre valeur confidentielle d'extraction d'image pour déployer des conteneurs dans d'autres espaces de nom Kubernetes que `default`, utiliser des images stockées dans d'autres comptes {{site.data.keyword.Bluemix_notm}} ou utiliser des images stockées dans des registres privés externes. Par ailleurs, vous pouvez créer une nouvelle valeur confidentielle d'extraction d'image pour appliquer des règles d'accès IAM limitant les droits à des référentiels d'images de registre, des espaces de nom ou des actions (telles que `push` ou `pull`) spécifiques.
{:shortdesc}
...
1.  [Configurez un espace de nom dans {{site.data.keyword.registryshort_notm}} et envoyez des images dans cet espace de nom](/docs/services/Registry?topic=registry-index#registry_namespace_add).
2.  [Créez un cluster](/docs/containers?topic=containers-clusters#clusters_cli).
3.  Si vous utilisez un cluster créé avant le **<DATE>**, [mettez à jour votre cluster pour utiliser la valeur confidentielle d'extraction d'image de la clé d'API](#imagePullSecret_migrate_api_key).
4.  [Connectez-vous à votre compte. Ciblez la région appropriée et, le cas échéant, le groupe de ressources. Définissez le contexte de votre cluster](/docs/containers?topic=containers-cs_cli_install#cs_cli_configure).

<br/>
Pour utiliser votre propre valeur confidentielle d'extraction d'image, choisissez parmi les options suivantes :
- [Copier la valeur confidentielle d'extraction d'image de l'espace de nom Kubernetes default dans d'autres espaces de nom de votre cluster](#copy_imagePullSecret).
- [Créer une valeur confidentielle d'extraction d'image pour accéder à des images dans d'autres comptes {{site.data.keyword.Bluemix_notm}} ou pour appliquer des règles IAM limitant l'accès au registre](#other_registry_accounts).
...
### Copier la valeur confidentielle d'extraction d'image de l'espace de nom default dans d'autres espaces de nom de votre cluster
{: #copy_imagePullSecret}

Vous pouvez copier la valeur confidentielle d'extraction d'image qui est créée automatiquement pour l'espace de nom Kubernetes `default` dans d'autres espaces de nom de votre cluster. Si vous souhaitez utiliser un autre ID de service {{site.data.keyword.Bluemix_notm}} IAM et les données d'identification de la clé d'API pour cet espace de nom, vous pouvez [créer une valeur confidentielle d'extraction d'image](#other_registry_accounts) à la place.
{: shortdesc}
...
### Créer une valeur confidentielle d'extraction d'image pour accéder à des images dans d'autres comptes {{site.data.keyword.Bluemix_notm}} ou utiliser des règles IAM limitant l'accès au registre
{: #other_registry_accounts}

Pour accéder à des images dans d'autres comptes {{site.data.keyword.Bluemix_notm}}, vous devez créer une clé d'API pour un ID de service avec une règle d'accès au service {{site.data.keyword.Bluemix_notm}} IAM pour {{site.data.keyword.registryshort_notm}}. Ensuite, sauvegardez les données d'identification de la clé d'API dans une valeur confidentielle d'extraction d'image (imagePullSecret). Par ailleurs, vous pouvez créer une nouvelle valeur confidentielle d'extraction d'image pour appliquer des règles d'accès IAM limitant les droits à des référentiels d'images de registre, des espaces de nom ou des actions (telles que `push` ou `pull`) spécifiques. Vous pouvez choisir de stocker ces mêmes données d'identification IAM de l'ID de service dans une clé d'API que vous utilisez pour créer plusieurs valeurs confidentielles d'extraction d'image dans plusieurs clusters.
{: shortdesc}

Au lieu d'utiliser un ID de service, vous envisagerez peut-être de créer une clé d'API pour un ID utilisateur disposant d'une règle d'accès au service {{site.data.keyword.Bluemix_notm}} IAM pour {{site.data.keyword.registryshort_notm}}. Cependant, assurez-vous que l'utilisateur correspond à un ID fonctionnel ou dispose d'un plan au cas où il partirait, afin que le cluster puisse toujours accéder au registre.
{: note}
...
### Accéder à des images stockées dans d'autres registres privés
{: #private_images}

Si vous disposez déjà d'un registre privé, vous devez stocker les données d'identification du registre dans une valeur confidentielle d'extraction d'image (imagePullSecret) Kubernetes et référencez cette valeur dans votre fichier de configuration.
{:shortdesc}
...
Vous pouvez déployer dans votre cluster des conteneurs depuis une image fournie par IBM ou depuis une image privée stockée dans votre espace de nom dans {{site.data.keyword.registryshort_notm}}. Les clusters existants utilisent un [jeton](/docs/services/Registry?topic=registry-registry_access#registry_tokens) de registre stocké dans la valeur confidentielle `imagePullSecret` d'un cluster pour autoriser l'accès permettant d'extraire des images dans les noms de domaine `registry.bluemix.net`.
{:shortdesc}
...
Chaque jeton doit être stocké dans un élément Kubernetes `imagePullSecret` de sorte à être accessible à un cluster Kubernetes lorsque vous déployez une application conteneurisée. Lorsque votre cluster est créé, {{site.data.keyword.containerlong_notm}} stocke automatiquement les jetons pour le registre global (images publiques fournies par IBM) et pour les registres régionaux dans des valeurs confidentielles Kubernetes pour extraction d'images. Les valeurs confidentielles d'extraction d'image (imagePullSecret) sont ajoutées à l'espace de nom Kubernetes `default`, à l'espace de nom `kube-system` et la liste des valeurs confidentielles (secrets) dans le compte de service `default` correspondant à ces espaces de nom.
...
Pour accéder à des images figurant dans d'autres régions ou comptes {{site.data.keyword.Bluemix_notm}}, vous devez créer un jeton de registre et sauvegarder vos données d'identification dans une valeur confidentielle d'extraction d'image (imagePullSecret).
{: shortdesc}
...
## Déploiement de conteneurs en utilisant la valeur confidentielle d'extraction d'image créée
{: #use_imagePullSecret}
...
* [Stocker la valeur confidentielle d'extraction d'image dans le compte de service Kubernetes](#store_imagePullSecret) : utilisez cette option pour octroyer l'accès aux images de votre registre pour les déploiements dans les espaces de nom Kubernetes sélectionnés.
...
2.  Définissez le pod et la valeur confidentielle d'extraction d'image pour accéder au registre privé {{site.data.keyword.registrylong_notm}}. 
