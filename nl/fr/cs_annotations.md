...
<dl>
<dt>Description</dt>
<dd>Ajouter l'élément `:server_port` à l'en-tête d'hôte d'une demande client avant de transférer la demande à votre application de back end.
...
<p class="tip">L'annotation <code>response-add-headers</code> ne prend pas en charge les en-têtes globaux pour tous les services. Pour ajouter un en-tête pour les réponses de tous les services au niveau du serveur, vous pouvez utiliser l'[annotation <code>server-snippets</code>](#server-snippets) :</br> <pre class="codeblock">annotations:
  ingress.bluemix.net/server-snippets: |
    add_header &lt;header1&gt; &lt;value1&gt;;</pre></p>
</dd>
...
<dl>
<dt>Description</dt>
<dd>Pour des services spécifiques, cette annotation permet de limiter le débit de traitement des demandes et le nombre de connexions par une clé définie émanant d'une même adresse IP pour tous les chemins des back end sélectionnés.
</dd>
