...
## IAM 플랫폼 및 Kubernetes RBAC
{: #platform}

{{site.data.keyword.containerlong_notm}}는 {{site.data.keyword.Bluemix_notm}} Identity and Access Management(IAM) 역할을 사용하도록 구성됩니다. IAM 플랫폼 역할은 사용자가 클러스터에 대해 수행할 수 있는 조치를 결정합니다. IAM 플랫폼 역할이 지정된 모든 사용자에게는 기본 네임스페이스의 해당 Kubernetes 역할 기반 액세스 제어(RBAC) 역할이 자동으로 지정됩니다. 추가로, IAM 플랫폼 역할은 사용자의 기본 인프라 권한을 자동으로 설정합니다. IAM 정책을 설정하려면 [IAM 플랫폼 권한 지정](cs_users.html#platform)을 참조하십시오. RBAC 역할에 대해 자세히 알아보려면 [RBAC 권한 지정](cs_users.html#role-binding)을 참조하십시오. 

다음 표는 각 IAM 플랫폼 역할에 의해 부여되는 클러스터 관리 권한과 해당 RBAC 역할에 대한 Kubernetes 리소스 권한을 보여줍니다. 
...
<table>
  ...
  <tr>
    ...
    <td>
      클러스터:<ul>
        <li>리소스 그룹 및 지역에 대해 IAM API 키 소유자의 이름 및 이메일 주소 보기</li>
  ...
  <tr>
    ...
    <td>이 역할에는 이 계정의 모든 클러스터에 대한 편집자, 운영자 및 뷰어 역할의 모든 권한이 있으며, 그 외에도 다음 권한이 있습니다. </br></br>
      클러스터:<ul>
        ...
        <li>계정의 기타 기존 사용자에 대한 IAM 플랫폼 역할 지정 및 변경</li>
...
Ingress:<ul>
      ...
      <strong>참고</strong>: 머신, VLAN 및 서브넷과 같은 리소스를 작성하려면 관리자에게 **수퍼유저** 인프라 역할이 필요합니다.
    </td>
...
Cloud Foundry 역할은 계정 내 조직 및 영역에 대한 액세스 권한을 부여합니다. {{site.data.keyword.Bluemix_notm}}에 있는 Cloud Foundry 기반 서비스의 목록을 보려면 `ibmcloud service list`를 실행하십시오. 더 자세히 알아보려면 IAM 문서에서 모든 사용 가능한 [조직 및 영역 역할](/docs/iam/cfaccess.html)을 참조하거나 [Cloud Foundry 액세스 권한 관리](/docs/iam/mngcf.html) 단계를 참조하십시오. 
...
**참고**: **수퍼유저** 인프라 액세스 역할이 있는 사용자가 [지역 또는 리소스 그룹의 API를 설정](cs_users.html#api_key)할 때, 계정 내 다른 사용자의 인프라 권한은 IAM 플랫폼 역할에 의해 설정됩니다. 사용자가 다른 사용자의 IBM Cloud 인프라(SoftLayer) 권한을 편집할 필요는 없습니다. API 키를 설정하는 사용자에게 **수퍼유저**를 지정할 수 없는 경우에만 다음 표를 사용하여 사용자의 IBM Cloud 인프라(SoftLayer) 권한을 사용자 정의하십시오. 자세한 정보는 [인프라 권한 사용자 정의](cs_users.html#infra_access)를 참조하십시오. 
