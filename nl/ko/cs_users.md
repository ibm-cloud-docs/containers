...
Cloud Foundry 역할에 의해 허용되는 조치의 예를 들면 새 Cloud Foundry 서비스 인스턴스 작성, 클러스터에 Cloud Foundry 서비스 인스턴스 바인딩 등이 있습니다. 더 자세히 알아보려면 {{site.data.keyword.Bluemix_notm}} IAM 문서에서 사용 가능한 [조직 및 영역 역할](/docs/iam/cfaccess.html)을 참조하거나 [Cloud Foundry 액세스 권한 관리](/docs/iam/mngcf.html) 단계를 참조하십시오. </dd>
</dl>
...
4. 클러스터 내 리소스를 격리하기 위해 Kubernetes 네임스페이스를 사용하는 경우에는 [다른 네임스페이스에 **Viewer** 및 **Editor** {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할에 대한 Kubernetes RBAC 역할 바인딩을 복사](#role-binding)하십시오.
...
계정 및 리소스를 설정하는 데 대한 자세한 정보를 얻으려면 [사용자, 팀 및 애플리케이션 구성에 대한 우수 사례](/docs/tutorials/users-teams-applications.html)와 관련된 튜토리얼을 수행해 보십시오.
{: tip}
...
<caption>계정 유형별 표준 클러스터 작성 옵션</caption>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><p><strong>옵션 1:</strong> IBM Cloud 인프라(SoftLayer) 포트폴리오에 대한 액세스 권한으로 설정된 [새 종량과금제 계정을 작성](/docs/account/index.html#paygo)합니다. 이 옵션을 선택하는 경우에는 두 개의 별도 {{site.data.keyword.Bluemix_notm}} 계정과 비용 청구가 있습니다.</p><p>계속해서 구독 계정을 사용하려는 경우 새 종량과금제 계정을 사용하여 IBM Cloud 인프라(SoftLayer)에 API 키를 생성할 수 있습니다. 그리고 구독 계정에 대한 IBM Cloud 인프라(SoftLayer) API 키를 수동으로 설정해야 합니다. IBM Cloud 인프라(SoftLayer) 리소스가 새 종량과금제 계정을 통해 비용 청구된다는 점을 유념하십시오.</p><p><strong>옵션 2:</strong> 사용하고자 하는 기존 IBM Cloud 인프라(SoftLayer) 계정을 이미 보유 중인 경우에는 {{site.data.keyword.Bluemix_notm}} 계정에 대한 IBM Cloud 인프라(SoftLayer) 인증 정보를 수동으로 설정할 수 있습니다.</p><p class="note">IBM Cloud 인프라(SoftLayer) 계정에 수동으로 연결하는 경우 인증 정보가 {{site.data.keyword.Bluemix_notm}} 계정의 모든 IBM Cloud 인프라(SoftLayer) 특정 조치에 사용됩니다. 사용자가 클러스터를 작성하고 작업할 수 있도록 설정한 API 키에 [충분한 인프라 권한](cs_users.html#infra_access)이 있는지 확인해야 합니다.</p></td>
    </tr>
    <tr>
...
CLI 세션용 {{site.data.keyword.Bluemix_notm}} IAM 토큰을 보려면 `ibmcloud iam oauth-tokens`를 실행하십시오. {{site.data.keyword.Bluemix_notm}} IAM 토큰은 [{{site.data.keyword.containerlong_notm}} API를 직접 호출](cs_cli_install.html#cs_api)하는 데도 사용할 수 있습니다.
{: tip}
...
계정 내의 사용자가 실행할 수 있는 조치를 감사하려는 경우 [{{site.data.keyword.cloudaccesstrailshort}}](cs_at_events.html)를 사용하여 클러스터 관련 이벤트를 모두 볼 수 있습니다.
{: tip}
...
클러스터에서 지역 및 리소스 그룹에 대해 설정된 API 키가 손상된 경우 해당 키를 인증으로 사용하여 호출이 이루어질 수 없도록 [ API 키를 삭제](../iam/userid_keys.html#deleting-an-api-key)하십시오. Kubernetes API 서버에 대한 액세스 보안 설정에 대한 자세한 정보는 [Kubernetes API 서버 및 etcd](cs_secure.html#apiserver) 보안 주제를 참조하십시오. 
...
3. 클러스터 내의 모든 인프라 관련 조치가 성공적으로 수행될 수 있도록 하려면 사용자에게 올바른 인프라 액세스 정책이 있는지 확인하십시오.
    1. 메뉴 ![메뉴 아이콘](../icons/icon_hamburger.svg "메뉴 아이콘")에서 **인프라**를 선택하십시오. 
...
[`ibmcloud ks api-key-info`](cs_cli_reference.html#cs_api_key_info)를 실행하여 현재 API 키 소유자를 찾을 수 있습니다. 지역에 대해 저장된 API 키를 업데이트해야 한다고 판단되면 [`ibmcloud ks api-key-reset`](cs_cli_reference.html#cs_api_key_reset) 명령을 실행하여 이를 수행할 수 있습니다. 이 명령은 {{site.data.keyword.containerlong_notm}} 관리자 액세스 정책이 필요하고 계정에서 이 명령을 실행하는 사용자의 API 키를 저장합니다.
...
지역 내 클러스터를 위한 인프라를 주문하는 데 IBM Cloud 인프라(SoftLayer) 계정을 사용하는 대신, 이미 보유하고 있는 다른 IBM Cloud 인프라(SoftLayer) 계정을 사용하려 하는 경우가 있습니다. [`ibmcloud ks credential-set`](cs_cli_reference.html#cs_credentials_set) 명령을 사용하여 이 인프라 계정을 {{site.data.keyword.Bluemix_notm}} 계정에 연결할 수 있습니다. 지역에 대해 저장된 기본 종량과금제 계정의 인증 정보 대신 이 IBM Cloud 인프라(SoftLayer) 인증 정보가 사용됩니다.
{: shortdesc}

`ibmcloud ks credential-set` 명령으로 설정된 IBM Cloud 인프라(SoftLayer) 인증 정보는 세션이 종료된 후에도 지속됩니다. [`ibmcloud ks credential-unset`](cs_cli_reference.html#cs_credentials_unset) 명령을 사용하여 수동으로 설정한 IBM Cloud 인프라(SoftLayer) 인증 정보를 제거하면 기본 종량과금제 계정 인증 정보가 사용됩니다. 그러나 이 인프라 계정 인증 정보 변경은 [고아 클러스터](cs_troubleshoot_clusters.html#orphaned)를 발생시킬 수 있습니다.
{: important}
...
4. 자신이 설정한 인프라 계정 신임 정보를 클러스터가 사용하는지 확인하십시오.
  1. [{{site.data.keyword.containerlong_notm}} 콘솔 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://console.bluemix.net/containers-kubernetes/clusters)을 열고 클러스터를 선택하십시오. 
...
사용자가 {{site.data.keyword.containerlong_notm}} 내의 클러스터에 대해 작업할 수 있도록 [{{site.data.keyword.Bluemix_notm}} 콘솔](#add_users) 또는 [CLI](#add_users_cli)에서 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 관리 정책을 설정하십시오. 시작하기 전에, [액세스 정책 및 역할 이해](#access_policies)를 참조하여 어떤 정책이 있는지, 정책을 누구에게 지정할 수 있는지, 어떤 리소스에 정책을 부여할 수 있는지 검토하십시오.
{: shortdesc}
...
{{site.data.keyword.Bluemix_notm}} 콘솔에서 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 관리 역할을 지정하여 클러스터에 대한 액세스 권한을 사용자에게 부여하십시오.
{: shortdesc}

시작하기 전에, 작업 중인 {{site.data.keyword.Bluemix_notm}} 계정에 대한 **관리자** 플랫폼 역할이 사용자에게 지정되었는지 확인하십시오. 

1. [{{site.data.keyword.Bluemix_notm}} 콘솔](https://console.bluemix.net/)에 로그인하여 **관리 > 계정 > 사용자**로 이동하십시오. 
...
3. 정책을 지정하십시오.
  ...
  * 한 리소스 그룹 내의 한 클러스터, 또는 모든 리소스 그룹 내의 모든 클러스터에 대한 액세스의 경우에는 다음 작업을 수행하십시오.
    ...
    5. **역할 선택** 섹션에서 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할을 선택하십시오. 각 역할에 대해 지원되는 조치의 목록을 찾으려면 [사용자 액세스 권한](/cs_access_reference.html#platform)을 참조하십시오. 참고: 하나의 클러스터에 대한 **관리자** 플랫폼 역할만 사용자에게 지정한 경우에는 리소스 그룹의 해당 지역에 속한 모든 클러스터에 대한 **뷰어** 플랫폼 역할도 이 사용자에게 지정해야 합니다. 
    6. **지정**을 클릭하십시오.

4. 사용자가 기본 외의 리소스 그룹에 속한 클러스터에 대해 작업할 수 있도록 하려는 경우 이러한 사용자에게는 클러스터가 속한 리소스 그룹에 대한 추가 액세스 권한이 필요합니다. 이러한 사용자에게는 최소한 리소스 그룹에 대한 **뷰어** 플랫폼 역할을 지정할 수 있습니다. 
  1. **한 리소스 그룹 내의 액세스 권한 지정**을 클릭하십시오.
  2. 리소스 그룹 이름을 선택하십시오.
...
CLI에서 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 관리 역할을 지정하여 클러스터에 대한 액세스 권한을 사용자에게 부여하십시오.
{: shortdesc}
...
**CLI를 사용하여 개별 사용자에게 {{site.data.keyword.Bluemix_notm}} IAM 역할을 지정하려면 다음 작업을 수행하십시오. **

1.  {{site.data.keyword.containerlong_notm}}(**`--service-name containers-kubernetes`**)에 대한 권한을 설정할 {{site.data.keyword.Bluemix_notm}} IAM 액세스 정책을 작성하십시오. 뷰어, 편집자, 운영자 및 관리자를 플랫폼 역할로 선택할 수 있습니다. 각 역할에 대해 지원되는 조치의 목록을 찾으려면 [사용자 액세스 권한](cs_access_reference.html#platform)을 참조하십시오.
    * 한 리소스 그룹 내의 한 클러스터에 대한 액세스 권한을 지정하려면 다음 명령을 실행하십시오.
      ...
      **참고**: 하나의 클러스터에 대한 **관리자** 플랫폼 역할만 사용자에게 지정한 경우에는 리소스 그룹의 해당 지역에 속한 모든 클러스터에 대한 **뷰어** 플랫폼 역할도 이 사용자에게 지정해야 합니다. 
...
4. {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할이 해당 [RBAC 역할 바인딩 또는 클러스터 역할 바인딩](#role-binding)으로 자동으로 적용됩니다. 지정한 플랫폼 역할에 대해 다음 명령 중 하나를 실행하여 사용자가 해당 RBAC 역할에 추가되었는지 확인하십시오. 
    * Viewer:
        ```
  ...
  예를 들어, 사용자 `john@email.com`에게 **뷰어** 플랫폼 역할을 지정하고 `kubectl get rolebinding ibm-view -o yaml -n default`를 실행하면 다음과 같은 출력이 표시됩니다. 
...
**CLI를 사용하여 액세스 그룹 내의 여러 사용자에게 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할을 지정하려면 다음 작업을 수행하십시오. **
...
3. {{site.data.keyword.containerlong_notm}}에 대한 권한을 설정할 {{site.data.keyword.Bluemix_notm}} IAM 액세스 정책을 작성하십시오. 뷰어, 편집자, 운영자 및 관리자를 플랫폼 역할로 선택할 수 있습니다. 각 역할에 대해 지원되는 조치의 목록을 찾으려면 [사용자 액세스 권한](/cs_access_reference.html#platform)을 참조하십시오.
  * 한 리소스 그룹 내의 한 클러스터에 대한 액세스 권한을 지정하려면 다음 명령을 실행하십시오.
      ```
      ...
      하나의 클러스터에 대한 **관리자** 플랫폼 역할만 사용자에게 지정한 경우에는 리소스 그룹의 해당 지역에 속한 모든 클러스터에 대한 **뷰어** 플랫폼 역할도 이 사용자에게 지정해야 합니다. {: note}
...
6. {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할이 해당 [RBAC 역할 바인딩 또는 클러스터 역할 바인딩](#role-binding)으로 자동으로 적용됩니다. 지정한 플랫폼 역할에 대해 다음 명령 중 하나를 실행하여 사용자가 해당 RBAC 역할에 추가되었는지 확인하십시오. 
    * Viewer:
        ```
        kubectl get rolebinding ibm-view -o yaml -n default
        ```
  ...
  예를 들어, 액세스 그룹 `team1`에 **뷰어** 플랫폼 역할을 지정하고 `kubectl get rolebinding ibm-view -o yaml -n default`를 실행하면 다음과 같은 출력이 표시됩니다. 
  ```
  apiVersion: rbac.authorization.k8s.io/v1
  kind: RoleBinding
  metadata:
        ...
        <caption>YAML 컴포넌트 이해</caption>
          ...
          <tbody>
            ...
            <tr>
              ...
              <li>`Group`: Kubernetes 1.11 이상을 실행하는 클러스터의 경우, RBAC 역할 또는 클러스터 역할을 계정 내의 [{{site.data.keyword.Bluemix_notm}}IAM 액세스 그룹](/docs/iam/groups.html#groups)에 바인드하십시오.</li>
              <li>`ServiceAccount`: RBAC 역할 또는 클러스터 역할을 클러스터 내의 네임스페이스에 있는 서비스 계정에 바인드하십시오.</li></ul></td>
            </tr>
            <tr>
              <td><code>subjects.name</code></td>
              <td><ul><li>`User`의 경우: 개별 사용자의 이메일 주소를 다음 URL에 추가하십시오.<ul><li>Kubernetes 1.11 이상을 실행하는 클러스터의 경우: <code>IAM#user@email.com</code></li><li>Kubernetes 1.10 또는 이전 버전을 실행하는 클러스터의 경우: <code>https://iam.ng.bluemix.net/kubernetes#user@email.com</code></li></ul></li>
              <li>`Group`: Kubernetes 1.11 이상을 실행하는 클러스터의 경우, 계정에 있는 [{{site.data.keyword.Bluemix_notm}}IAM 액세스 그룹](/docs/iam/groups.html#groups)의 이름을 지정하십시오. </li>
...
[{{site.data.keyword.Bluemix_notm}}IAM 플랫폼 관리 역할](#platform)이 지정된 모든 사용자에게는 해당 RBAC 클러스터 역할이 자동으로 지정됩니다. 이러한 RBAC 클러스터 역할은 사전 정의되며 사용자가 클러스터 내의 Kubernetes 리소스와 상호작용할 수 있도록 해 줍니다. 또한, 역할 바인딩은 특정 네임스페이스에 클러스터 역할을 적용하기 위해 작성되고, 클러스터 역할 바인딩은 모든 네임스페이스에 클러스터 역할을 적용하기 위해 작성됩니다.

다음 표에서는 {{site.data.keyword.Bluemix_notm}} 플랫폼 역할 및 플랫폼 역할에 대해 자동으로 작성되는 해당 클러스터 역할, 역할 바인딩 또는 클러스터 역할 바인딩 간의 관계에 대해 설명합니다. 

<table>
  <tr>
    <th>{{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할</th>
    <th>RBAC 클러스터 역할</th>
    <th>RBAC 역할 바인딩</th>
...
[Kubernetes 네임스페이스를 사용하여 클러스터를 파티셔닝하고 워크로드의 격리를 제공](cs_secure.html#container)하는 경우에는 특정 네임스페이스에 대한 사용자 액세스 권한을 지정해야 합니다. **운영자** 또는 **관리자** 플랫폼 역할을 사용자에게 지정하면 사전 정의된 해당 `admin` 및 `cluster-admin` 클러스터 역할이 전체 클러스터에 자동으로 적용됩니다. 그러나 **뷰어** 또는 **편집자** 플랫폼 역할을 사용자에게 지정하면 사전 정의된 해당 `view` 및 `edit` 클러스터 역할이 기본 네임스페이스에만 자동으로 적용됩니다. 다른 네임스페이스에서 동일한 사용자 액세스 권한 레벨을 적용하려는 경우에는 이러한 클러스터 역할의 [역할 바인딩을 복사](#rbac_copy)하여 다른 네임스페이스에 복사할 수 있습니다(`ibm-view` 및 `ibm-edit`).

**사용자 정의 역할 또는 클러스터 역할을 작성할 수 있습니까?**

`view`, `edit`, `admin` 및 `cluster-admin` 클러스터 역할은 해당 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할을 사용자에게 지정하면 자동으로 작성되는 사전 정의된 역할입니다. 그 외의 Kubernetes 권한을 부여하려는 경우에는 [사용자 정의 RBAC 권한을 작성](#rbac)할 수 있습니다.
...
예를 들어 사용자 "john@email.com"에게 **편집자** 플랫폼 관리 역할을 지정했다고 가정합시다. 사전 정의된 RBAC 클러스터 역할 `edit`이 클러스터에 자동으로 작성되며, `ibm-edit` 역할 바인딩이 `default` 네임스페이스에서 해당 권한을 적용합니다. 사용자는 "john@email.com"이 개발 네임스페이스에서도 Editor 액세스 권한을 갖도록 하기 위해 `ibm-edit` 역할 바인딩을 `default`에서 `development`로 복사합니다. **참고**: 사용자가 `view` 또는 `edit` 역할에 추가될 때마다 역할 바인딩을 복사해야 합니다.
...
`view`, `edit`, `admin` 및 `cluster-admin` 클러스터 역할은 해당 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 관리 역할을 지정하면 자동으로 작성됩니다. 클러스터 액세스 정책이 이러한 사전 정의된 권한보다 더 세부적으로 권한을 부여하도록 하려 하십니까? 걱정하지 마십시오! 이러한 경우에는 사용자 정의 RBAC 역할 및 클러스터 역할을 작성할 수 있습니다.
{: shortdesc}
        ...
        <caption>YAML 컴포넌트 이해</caption>
          ...
          <tbody>
            ...
            <tr>
              <td><code>subjects.name</code></td>
              <td><ul><li>`User`의 경우: 개별 사용자의 이메일 주소를 다음 URL에 추가하십시오.<ul><li>Kubernetes 1.11 이상을 실행하는 클러스터의 경우: <code>IAM#user@email.com</code></li><li>Kubernetes 1.10 또는 이전 버전을 실행하는 클러스터의 경우: <code>https://iam.ng.bluemix.net/kubernetes#user@email.com</code></li></ul></li>
              <li>`Group`의 경우: Kubernetes 1.11 이상을 실행하는 클러스터의 경우, 계정에 있는 [{{site.data.keyword.Bluemix_notm}}IAM 그룹](/docs/iam/groups.html#groups)의 이름을 지정하십시오.</li>
              <li>`ServiceAccount`의 경우: 서비스 계정 이름을 지정하십시오.</li></ul></td>
            </tr>
            <tr>
              <td><code>subjects.apiGroup</code></td>
...
시작하기 전에:
- [사용자의 인프라 인증 정보가 API 키를 설정하는 데 사용되거나 `ibmcloud ks credential-set` 명령에 사용되지 않았는지 확인하십시오](#removing). 
...
1. 사용자가 작성한 클러스터를 판별하십시오. 
    1.  [{{site.data.keyword.containerlong_notm}} 콘솔![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://console.bluemix.net/containers-kubernetes/clusters)에 로그인하십시오.
...
2. 사용자가 작성한 클러스터마다 다음 단계를 수행하십시오. 
    1. 사용자가 클러스터를 프로비저닝하는 데 사용한 인프라 계정을 확인하십시오. 
        ...
        2.  메뉴 ![메뉴 아이콘](../icons/icon_hamburger.svg "메뉴 아이콘")를 열고 **인프라**를 클릭하십시오. 
    ...
    2. 사용자가 퇴사한 후 사용자가 클러스터를 프로비저닝하는 데 사용한 인프라 계정이 어떻게 처리되는지 판별하십시오. 
        ...
        * 사용자가 인프라 계정을 소유하고 있는 사용자가 퇴사면 인프라 계정이 삭제됩니다. 이러한 클러스터에 대해 계속 작업할 수 없습니다. 고아 클러스터가 발생하지 않도록 사용자가 퇴사하기 전에 클러스터를 삭제해야 합니다. 사용자가 퇴사했지만 클러스터가 삭제되지 않은 경우 `ibmcloud ks credential-set` 명령을 사용하여 인프라 인증 정보를 클러스터 작업자 노드가 프로비저닝된 계정으로 변경한 후 클러스터를 삭제해야 합니다. 자세한 정보는 [고아 상태의 클러스터에 있는 인프라를 수정하거나 삭제할 수 없음](cs_troubleshoot_clusters.html#orphaned)을 참조하십시오. 

3. {{site.data.keyword.Bluemix_notm}} 계정에서 사용자를 제거하십시오. 
    1. **관리 > 계정 > 사용자**로 이동하십시오.
    2. 사용자의 사용자 이름을 클릭하십시오. 
    3. 사용자에 대한 테이블 항목에서 조치 메뉴를 클릭하고 **사용자 제거**를 선택하십시오. 사용자를 제거하면 사용자에게 지정된 {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할, Cloud Foundry 역할 및 IBM Cloud 인프라(SoftLayer) 역할도 자동으로 제거됩니다. 

4. {{site.data.keyword.Bluemix_notm}} IAM 플랫폼 권한이 제거되면 사용자의 권한도 연관된 사전 정의된 RBAC 역할에서 자동으로 제거됩니다. 그러나 사용자 정의 RBAC 역할 또는 클러스터 역할을 작성한 경우 [해당 RBAC 역할 바인딩 또는 클러스터 역할 바인딩에서 사용자를 제거](#remove_custom_rbac)하십시오. 

5. {{site.data.keyword.Bluemix_notm}} 계정에 자동으로 연결되는 종량과금제 계정이 있는 경우 사용자의 IBM Cloud 인프라(SoftLayer) 역할이 자동으로 제거됩니다. 그러나 [다른 유형의 계정](#understand_infra)이 있는 경우 IBM Cloud 인프라(SoftLayer)에서 사용자를 수동으로 제거해야 합니다. 
    1. [{{site.data.keyword.Bluemix_notm}} 콘솔](https://console.bluemix.net/) 메뉴 ![메뉴 아이콘](../icons/icon_hamburger.svg "메뉴 아이콘")에서 **인프라**를 클릭하십시오. 
    2. **계정 > 사용자 > 사용자 목록**으로 이동하십시오. 
    2. 사용자에 대한 테이블 항목을 찾으십시오. 
        * 사용자 항목이 보이지 않는다면 사용자가 이미 제거된 것입니다. 추가 조치가 필요하지 않습니다. 
        * 사용자 항목이 보일 경우 다음 단계로 진행하십시오. 
    3. 사용자에 대한 테이블 항목에서 조치 메뉴를 클릭하십시오. 
    4. **사용자 상태 변경**을 선택하십시오. 
    5. 상태 목록에서 **사용 안함**을 선택하십시오. **저장**을 클릭하십시오. 
...
시작하기 전에 [사용자의 인프라 인증 정보가 API 키를 설정하는 데 사용되거나 `ibmcloud ks credential-set` 명령에 사용되지 않았는지 확인하십시오](#removing). 그런 후 다음을 제거할 수 있습니다. 
...
1. [{{site.data.keyword.Bluemix_notm}} 콘솔](https://console.bluemix.net/)에 로그인하여 **관리 > 계정 > 사용자**로 이동하십시오. 
2. 권한을 제거할 사용자의 이름을 클릭하십시오. 
3. **액세스 그룹** 탭을 클릭하십시오. 
4. 액세스 그룹에 대한 테이블 항목에서 조치 메뉴를 클릭하고 **사용자 제거**를 선택하십시오. 사용자가 제거되면 액세스 그룹에 지정된 역할도 사용자로부터 제거됩니다. 
...
1. [{{site.data.keyword.Bluemix_notm}} 콘솔](https://console.bluemix.net/)에 로그인하여 **관리 > 계정 > 사용자**로 이동하십시오. 
2. 권한을 제거할 사용자의 이름을 클릭하십시오. 
3. 제거할 권한에 대한 테이블 항목에서 조치 메뉴를 클릭하십시오. 
4. **제거**를 선택하십시오. 
...
#### IBM Cloud 기반 구조(SoftLayer) 권한 제거
{: #remove_infra}

{{site.data.keyword.Bluemix_notm}} 콘솔을 사용하여 사용자의 IBM Cloud 기반 구조(SoftLayer) 권한을 제거할 수 있습니다.
{: shortdesc}

1. [{{site.data.keyword.Bluemix_notm}} 콘솔](https://console.bluemix.net/)에 로그인하십시오. 
2. 메뉴 ![메뉴 아이콘](../icons/icon_hamburger.svg "메뉴 아이콘")에서 **인프라**를 클릭하십시오. 
3. 사용자의 이메일 주소를 클릭하십시오. 
4. **포털 권한** 탭을 클릭하십시오. 
5. 각 탭에서 특정 권한을 선택 취소하십시오. 
6. 변경사항을 저장하려면 **포털 권한 편집**을 클릭하십시오.
7. **디바이스 액세스** 탭에서 특정 디바이스를 선택 취소하십시오. 
8. 변경사항을 저장하려면 **디바이스 액세스 업데이트**를 클릭하십시오. 잠시 후 권한이 다운그레이드됩니다. 
