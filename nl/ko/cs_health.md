...
UI를 통해 로깅을 구성하려면 조직 및 영역을 지정해야 합니다. 계정 레벨에서 로깅을 사용으로 설정하려면 CLI를 사용하십시오.
{: tip}
...
클러스터 로깅에 대한 구성을 작성할 수 있습니다. 관련 플래그를 사용하여 서로 다른 로그 소스를 구별할 수 있습니다. 구성 옵션의 전체 목록은 [CLI 참조](cs_cli_reference.html#logging_commands)에서 검토할 수 있습니다. 문제가 발생하는 경우에는 이 [문제점 해결 안내서](cs_troubleshoot_health.html)를 읽어 보십시오. 
...
1. 로그 전달 구성을 작성하십시오.
  ...
  {: pre}
    ...
    * 애플리케이션 로깅 구성 및 출력 예:
      ...
      컨테이너에서 실행되는 애플리케이션 중 STDOUT 또는 STDERR에 로그를 기록하도록 구성할 수 없는 애플리케이션이 있는 경우에는 애플리케이션 로그 파일로부터 로그를 전달하도록 하는 로깅 구성을 작성할 수 있습니다.
      {: tip}
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>선택사항: 지정된 메시지를 포함하는 로그를 필터링합니다. </td>
      </tr>
    </tbody>
  </table>
...
1. 검사를 JSON 형식으로 정의하는 구성 맵 파일을 작성하십시오. 예를 들어, 다음 YAML 파일은 세 가지 검사(하나의 HTTP 검사와 두 개의 Kubernetes API 서버 검사)를 정의합니다.</br>
   **팁**: 구성 맵의 `data` 섹션에서 각 검사를 고유 키로 정의하십시오. 
   ...
   예제의 검사에서는 3분 마다 YAML이 실행됩니다. 3회 연속 실패 시, 작업자 노드가 다시 부팅됩니다. 이 조치는 <code>bx cs worker-reboot</code>를 실행하는 것과 같습니다. HTTP 검사는 <b>Enabled</b> 필드를 <code>true</code>로 설정할 때까지 비활성화됩니다.</td>
   </tr>
   </tbody>
   </table>
