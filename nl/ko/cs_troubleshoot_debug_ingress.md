...
## 1단계: Ingress 리소스 또는 ALB 팟(Pod) 로그에서 오류 메시지 확인
{: #errors}
...
1. Ingress 리소스 배치를 확인하고 경고 또는 오류 메시지를 찾으십시오.
  ...
  kubectl describe ingress <myingress>
    ...
    출력의 **Events** 섹션에서 Ingress 리소스 또는 사용된 특정 어노테이션의 올바르지 않은 값에 대한 경고 메시지가 표시될 수 있습니다. [Ingress 리소스 구성 문서](cs_ingress.html#public_inside_3) 또는 [어노테이션 문서](cs_annotations.html)를 확인하십시오.
...
## 2단계: ALB 하위 도메인 및 공인 IP 주소에 대한 Ping 실행
{: #ping}
...
1. 공용 ALB가 청취 중인 IP 주소를 가져오십시오.
    ...
    * 공용 ALB에 IP 주소가 없는 경우에는 [Ingress ALB가 구역에 배치되지 않음](cs_troubleshoot_network.html#cs_multizone_subnet_limit)을 참조하십시오. 
...
## 3단계: 도메인 맵핑 및 Ingress 리소스 구성 확인
{: #config}
...
2. 클러스터의 Ingress 리소스 구성 파일을 확인하십시오.
    ...
    1. 하위 도메인 및 TLS 인증서가 올바른지 확인하십시오. IBM 제공 Ingress 하위 도메인 및 TLS 인증서를 찾으려면 다음을 실행하십시오. `ibmcloud ks cluster-get <cluster_name_or_ID>`

    2.  Ingress의 **path** 섹션에 구성된 동일한 경로에서 앱이 청취하는지 확인하십시오. 루트 경로에서 청취하도록 앱이 설정된 경우에는 `/`를 경로로 사용하십시오. 이 경로에 대한 수신 트래픽을 앱이 청취하는 다른 경로로 라우팅해야 하는 경우에는 [rewrite paths](cs_annotations.html#rewrite-path) 어노테이션을 사용하십시오. 

    3. 필요하면 리소스 구성 YAML을 편집하십시오. 편집기를 닫으면 변경사항이 저장되고 자동으로 적용됩니다.
        ```
...
6. 이제 ALB IP가 프로덕션에서 제거되었으므로 이를 통해 앱에 대해 디버깅 테스트를 실행할 수 있습니다. 이 IP를 통해 앱에 대한 통신을 테스트하기 위해, 예제 값을 사용자 자신의 값으로 대체하여 다음 cURL 명령을 실행할 수 있습니다.
    ...
    * 응답에서 오류를 받는 경우, 이 특정 ALB에만 적용되는 구성이나 앱에 오류가 있을 수 있습니다. 앱 코드, [Ingress 리소스 구성 파일](cs_ingress.html#public_inside_3) 또는 이 ALB에만 적용된 기타 구성을 확인하십시오. 
