...
`bx cr` 명령을 찾고 계십니까? [{{site.data.keyword.registryshort_notm}} CLI 참조](/docs/cli/plugins/registry/index.html)를 확인하십시오. `kubectl` 명령을 찾고 계십니까? [Kubernetes 문서 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://kubernetes.io/docs/reference/kubectl/overview/)를 참조하십시오.
{:tip}
...
지역을 전환하려면 대신 `bx cs region-set` [명령](#cs_region-set)을 사용하십시오.
{: tip}
...
<li>이 위치에서 이전에 표준 클러스터를 작성했거나 IBM Cloud 인프라(SoftLayer)에서 이전에 공용 VLAN을 작성한 경우 해당 공용 VLAN을 지정하십시오. 작업자 노드가 사설 VLAN에만 연결하려는 경우 이 옵션을 지정하지 마십시오.
...
**예제**:
  ...
  **무료 클러스터 작성**: 클러스터 이름만 지정하십시오. 그 외에는 모두 기본값으로 설정됩니다. 무료 클러스터는 21일 후 자동으로 삭제됩니다. 한 번에 하나의 무료 클러스터가 제공됩니다. Kubernetes의 전체 기능을 활용하려면 표준 클러스터를 작성하십시오.
  ...
  **첫 번째 표준 클러스터 작성**: 위치에 작성되는 첫 번째 표준 클러스터도 사설 VLAN을 작성합니다. 따라서 `--public-vlan` 플래그를 포함하지 마십시오.
  ...
  **후속 표준 클러스터 작성**: 이 위치에서 이미 표준 클러스터를 작성했거나 IBM Cloud 인프라(SoftLayer)에서 이전에 공용 VLAN을 작성한 경우 `--public-vlan` 플래그를 사용하여 해당 공용 VLAN을 지정하십시오. 특정 위치에 대한 공용 VLAN이 이미 있는지 찾거나 기존 공용 VLAN의 이름을 찾으려면 `bx cs vlans <location>`을 실행하십시오.
...
작업자 노드에 대해 사용 가능한 머신 유형의 목록을 봅니다. 머신 유형은 위치에 따라 다릅니다. 각각의 머신 유형에는
...
<strong>명령 옵션</strong>:
   ...
   <dd>사용 가능한 머신 유형을 나열하려는 위치를 입력하십시오. 이 값은 필수입니다. [사용 가능한 위치](cs_regions.html#locations)를 검토하십시오.</dd>
...
<dl>
  ...
  <dt><code>--logsource <em>LOG_SOURCE</em></code></dt>    
    <dd>로그 전달을 사용할 로그 소스입니다. 이 인수는 구성을 적용할 로그 소스의 쉼표로 구분된 목록을 지원합니다. 허용되는 값은 <code>container</code>, <code>application</code>, <code>worker</code>, <code>kubernetes</code>, <code>ingress</code> 및 <code>kube-audit</code>입니다. 로그 소스를 제공하지 않는 경우 로깅 구성은 <code>container</code> 및 <code>ingress</code> 로그 소스에 작성됩니다.</dd>
  ...
  <dt><code>--syslog-protocol</code></dt>
    <dd>로깅 유형이 <code>syslog</code>인 경우 사용되는 전송 계층 프로토콜입니다. 지원되는 값은 <code>TCP</code> 및 기본값인 <code>UDP</code>입니다. <code>udp</code> 프로토콜을 사용하여 rsyslog 서버에 전달하는 경우 1KB를 초과하는 로그는 잘립니다.</dd>
...
사용자가 클러스터를 작성하기 위해 사용할 수 있는 위치의 목록을 봅니다. 사용 가능한 위치는 사용자가 로그인한 지역에 따라 다릅니다. 지역을 전환하려면 `bx cs region-set`를 실행하십시오.
...
**예제**:
 ...
 사용 가능한 실제 및 가상 머신 유형은 클러스터를 배치하는 위치에 따라 다릅니다. 자세한 정보는 `bx cs machine-types` [명령](cs_cli_reference.html#cs_machine_types)에 대한 문서를 참조하십시오. 이 값은 표준 클러스터의 경우 필수이며 무료 클러스터에는 사용할 수 없습니다.</dd>
  ...
  <dt><code>--kube-version <em>VERSION</em></code></dt>
    <dd>작업자 노드가 작성될 Kubernetes의 버전입니다. 이 값이 지정되지 않으면 기본 버전이 사용됩니다.</dd>
  ...
  <dt><code>--private-only </code></dt>
    <dd>작업자 풀에 공용 VLAN이 없음을 지정합니다. 기본값은 <code>false</code>입니다.</dd>
...
<dl>
  <dt><code>--worker-pool <em>WORKER_POOL</em></code></dt>
    <dd>세부사항을 보려는 작업자 노드 풀의 이름입니다. 이 값은 필수입니다.</dd>
  <dt><code>--cluster <em>CLUSTER</em></code></dt>
...
풀에 있는 모든 작업자 노드를 지정된 마스터와 일치하는 최신 Kubernetes 버전으로 업데이트합니다.
...
<dl>
  ...
  <dt><code>--cluster <em>CLUSTER</em></code></dt>
    <dd>작업자 풀을 업데이트하려는 클러스터의 이름 또는 ID입니다. 이 값은 필수입니다.</dd>
</dl>
...
작업자 풀의 크기를 조정하여 클러스터의 각 구역에 있는 작업자 노드의 수를 늘리거나 줄입니다.
