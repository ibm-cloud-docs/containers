...
1. [바인드할 서비스 인스턴스가 포함된 지역에 로그인합니다.](cs_regions.html#bluemix_regions)

2. 서비스 인스턴스의 GUID를 가져옵니다. 
  ```
...
{: tsSymptoms}
클러스터에 앱의 Ingress 리소스를 작성하여 공용으로 앱을 노출했습니다. Ingress 제어기의 하위 도메인 또는 공인 IP 주소를 통해 앱에 연결하려고 할 때 연결에 실패했거나 제한시간이 초과되었습니다.
...
{: tsCauses}
클러스터가 [Kubernetes version 1.7](cs_versions.html) 이상이 아닙니다. 

{: tsResolve}
[클러스터를 업데이트](cs_cluster_update.html#master)하거나 이전 버전의 Kubernetes와 호환되는 명령으로 `<ETCD_URL>`을 검색하십시오.
