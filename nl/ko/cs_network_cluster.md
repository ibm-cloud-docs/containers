...
* 사용자가 다중 구역 클러스터의 앱을 보유합니다. 오직 클러스터 내의 팟(Pod)에 또는 클러스터와 동일한 사설 VLAN에 연결된 기타 클러스터의 팟(Pod)에 앱을 노출하고자 합니다. 
