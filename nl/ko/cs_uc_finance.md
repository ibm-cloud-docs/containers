...
# {{site.data.keyword.Bluemix_notm}}의 금융 서비스 유스 케이스
{: #cs_uc_finance}
...
* [고가용성을 위한 여러 지역](cs_regions.html#regions-and-zones)
...
* 고성능 컴퓨팅 그리드는 금융 모델링에 있어서 중요한 인프라 구성요소입니다.

현재 회사의 문제는 스케일과 제공 소요 시간입니다. 

회사의 현재 온프레미스 환경은 도입한지 7년 이상 지났으며 제한된 컴퓨팅, 스토리지 및 입출력 능력을 갖고 있습니다.
...
**솔루션 모델: 필요한 경우 온프레미스 엔터프라이즈 자산에 안전하게 액세스할 수 있는, 퍼블릭 클라우드에서 실행되는 요청 시 컴퓨팅, 스토리지 및 입출력 서비스**
...
이 솔루션은 확장성에 대해 즉각적인 이점을 제공합니다. 모기지 회사는 Kubernetes의 풍부한 배치 및 런타임 오브젝트 세트를 사용하여 앱에 대한 업그레이드를 안정적으로 모니터하고 관리합니다. 또한 정의된 규칙 및 자동화된 Kubernetes 오케스트레이터를 사용하여 앱을 복제하고 스케일링할 수도 있습니다.
