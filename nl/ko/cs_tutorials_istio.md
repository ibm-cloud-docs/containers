...
-  [IBM Cloud CLI, {{site.data.keyword.containershort_notm}} 플러그인 및 Kubernetes CLI를 설치](cs_cli_install.html#cs_cli_install_steps)하십시오. Istio에는 Kubernetes 버전 1.9 이상이 필요합니다. 클러스터의 Kubernetes 버전과 일치하는 `kubectl` CLI 버전을 설치해야 합니다.
...
1. [IBM Istio Helm 차트 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://console.bluemix.net/containers-kubernetes/solutions/helm-charts/ibm/ibm-istio)를 사용하여 Istio를 설치하십시오. 
    1. [클러스터에 대한 Helm을 설치하고 IBM 저장소를 Helm 인스턴스에 추가](cs_integrations.html#helm)하십시오.
    2. Istio의 사용자 정의 리소스 정의를 설치하십시오.
        ```
    ...
    3. 클러스터에 Helm 차트를 설치하십시오.
        ...
        ```
        {: pre}
    4. `istioctl` 클라이언트를 사용자의 PATH에 추가하십시오. 예를 들어, MacOS 또는 Linux 시스템에서 다음 명령을 실행하십시오.
       ```
       export PATH=$PWD/istio-1.0/bin:$PATH
       ```
       {: pre}

2. 계속하기 전에 Prometheus 및 9개의 Istio 서비스에 대한 팟(Pod)이 모두 완전히 배치되었는지 확인하십시오.
    ```
...
2. BookInfo 앱을 배치하십시오. 앱 마이크로서비스가 배치될 때 Envoy 사이드카도 각 마이크로서비스 팟(Pod)에 배치됩니다.
...
3. 마이크로서비스와 해당 팟(Pod)이 배치되었는지 확인하십시오.
    ```
    kubectl get svc
    ```
    {: pre}
...
4. 앱 배치를 확인하려면 클러스터의 공인 주소를 가져오십시오. 
    * 표준 클러스터: 
...
6. 브라우저에서 `http://$GATEWAY_URL/productpage`로 이동하여 BookInfo 웹 페이지를 보십시오.
