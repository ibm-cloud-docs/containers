...
*그림 1. {{site.data.keyword.containershort_notm}}의 Kubernetes 데이터 평면*

![{{site.data.keyword.containerlong_notm}} Kubernetes 아키텍처](images/networking.png)

이 다이어그램은 Kubernetes가 {{site.data.keyword.containershort_notm}}에서 사용자 네트워크 트래픽을 운반하는 방법을 보여줍니다. 라이트 또는 표준 클러스터를 작성했는지 여부에 따라 인터넷에서 앱에 다양한 방법으로 액세스할 수 있습니다. 
...
<dd>
 <ul>
  ...
  <li>Iptables는 로드 밸런스가 앱의 포드 전체에서 요청하는 기능이며, 고성능 네트워크 라우팅과 네트워크 액세스 제어를 제공합니다. </li>
  <li>로드 밸런서에 지정된 포터블 공인 IP 주소는 영구적이며 클러스터에서 작업자 노드가 다시 작성될 때 변경되지 않습니다.
...
<dd>
 <ul>
  ...
  <li>Ingress는 세 개의 기본 컴포넌트(Ingress 리소스, Ingress 제어기 및 애플리케이션 로드 밸런서)로 구성됩니다. <ul>
    <li>Ingress 리소스는 앱에 대한 수신 요청을 라우팅하고 로드 밸런싱하는 방법에 대한 규칙을 정의합니다. </li>
    <li>Ingress 제어기를 사용하여 애플리케이션 로드 밸런서를 사용으로 설정할 수 있으며, 애플리케이션 로드 밸런서는 수신 HTTP 또는 HTTPS 서비스 요청을 청취하고 각 Ingress 리소스에 대해 정의된 규칙을 기반으로 요청을 전달합니다. </li>
    <li>애플리케이션 로드 밸런서는 전체 앱의 포드에서 요청을 로드 밸런싱합니다. </ul>
  <li>사용자 정의 라우팅 규칙으로 사용자 고유의 로드 밸런서를 구현하려는 경우와 앱에 대한 SSL 종료가 필요한 경우 Ingress를 사용하십시오.
...
<area href="/docs/containers/cs_apps.html#cs_apps_public_nodeport" alt="Nodeport 서비스" shape="circle" coords="52, 283, 45"/>
<area href="/docs/containers/cs_apps.html#cs_apps_public_load_balancer" alt="Loadbalancer 서비스" shape="circle" coords="247, 419, 44"/>
<area href="/docs/containers/cs_apps.html#cs_apps_public_ingress" alt="Ingress 서비스" shape="circle" coords="445, 420, 45"/>
</map>
...
2.  노출시키려는 앱에 대해 로드 밸런서 서비스를 정의하십시오. 공용 인터넷 또는 사설 네트워크에서 앱을 사용할 수 있으려면 앱에 대한 Kubernetes 서비스를 작성하십시오. 앱을 구성하는 모든 포드를 로드 밸런싱에 포함하도록 서비스를 구성하십시오.
    ...
    2.  노출하려는 앱에 대한 로드 밸런서 서비스를 정의하십시오.
        ...
        <tr>
          ...
          <td>사설 LoadBalancer를 작성하는 경우, <em>&lt;loadBalancerIp&gt;</em>를 LoadBalancer에 사용할 IP 주소로 대체하십시오. </td>
        </tr>
        </tbody></table>
...
### Ingress를 사용하여 앱에 대한 액세스 구성
{: #cs_apps_public_ingress}

IBM 제공 Ingress 제어기가 관리하는 Ingress 리소스를 작성하여 클러스터에서 다중 앱을 노출합니다. Ingress 제어기는 애플리케이션 로드 밸런서를 사용하는 데 필요한 리소스를 작성합니다. 애플리케이션 로드 밸런서는 안전하고 고유한 공용 또는 사설 시작점을 사용하여 수신 요청을 클러스터 내부 또는 외부의 앱으로 라우팅하는 외부 HTTP 또는 HTTPS 로드 밸런서입니다. 
...
표준 클러스터를 작성하는 경우, Ingress 제어기는 포터블 공인 IP 주소 및 공용 라우트가 지정된 애플리케이션 로드 밸런서를 자동으로 작성하고 사용합니다. 포터블 사설 IP 주소 및 사설 라우트에 지정된 애플리케이션 로드 밸런서도 자동으로 작성되지만 자동으로 사용되지는 않습니다. 이러한 애플리케이션 로드 밸런서를 구성할 수 있으며 공용 또는 사설 네트워크에 노출할 모든 앱에 대해 개별 라우팅 규칙을 정의할 수 있습니다. Ingress를 통해 공용으로 노출된 모든 앱에는 고유 URL을 사용하여 클러스터에서 앱에 공용으로 액세스할 수 있도록 공용 라우트에 추가된 고유 경로가 지정됩니다.

앱을 공용으로 노출하려면 다음 시나리오에 맞게 공용 애플리케이션 로드 밸런서를 구성할 수 있습니다. 
...
사설 네트워크에 앱을 노출하려면 먼저 [사설 애플리케이션 로드 밸런서를 사용으로 설정](#private_ingress)하십시오. 그리고 다음 시나리오에 맞게 사설 애플리케이션 로드 밸런서를 구성할 수 있습니다. 
...
<img usemap="#ingress_map" border="0" class="image" src="images/networkingdt-ingress.png" width="750px" alt="이 이미지는 Ingress 제어기를 위한 최고의 구성을 선택하는 과정을 안내합니다. 이 이미지가 표시되지 않는 경우에도 문서에서 해당 정보를 찾을 수 있습니다. " style="width:750px;" />
<map name="ingress_map" id="ingress_map">
<area href="/docs/containers/cs_apps.html#private_ingress_no_tls" alt="사용자 정의 도메인과 함께 사설 Ingress 제어기 사용" shape="rect" coords="25, 246, 187, 294"/>
<area href="/docs/containers/cs_apps.html#private_ingress_tls" alt="사용자 정의 도메인 및 TLS 인증서와 함께 사설 Ingress 제어기 사용" shape="rect" coords="161, 337, 309, 385"/>
<area href="/docs/containers/cs_apps.html#external_endpoint" alt="네트워크 트래픽을 클러스터 외부의 앱으로 라우팅하도록 공용 Ingress 제어기 구성" shape="rect" coords="313, 229, 466, 282"/>
<area href="/docs/containers/cs_apps.html#custom_domain_cert" alt="사용자 정의 도메인 및 TLS 인증서와 함께 공용 Ingress 제어기 사용" shape="rect" coords="365, 415, 518, 468"/>
<area href="/docs/containers/cs_apps.html#ibm_domain" alt="IBM 제공 도메인과 함께 공용 Ingress 제어기 사용" shape="rect" coords="414, 609, 569, 659"/>
<area href="/docs/containers/cs_apps.html#ibm_domain_cert" alt="IBM 제공 도메인 및 TLS 인증서와 함께 공용 Ingress 제어기 사용" shape="rect" coords="563, 681, 716, 734"/>
</map>

#### TLS 종료 없이 IBM 제공 도메인 사용
{: #ibm_domain}

클러스터의 앱에 대한 HTTP 로드 밸런서로서 애플리케이션 로드 밸런서를 구성하고 인터넷에서 앱에 액세스하기 위해 IBM 제공 도메인을 사용할 수 있습니다. 
...
애플리케이션 로드 밸런서를 구성하려면 다음을 수행하십시오. 
...
4.  Ingress 리소스를 작성하십시오. Ingress 리소스는 앱용으로 작성된 Kubernetes 서비스에 대한 라우팅 규칙을 정의하며, 수신 네트워크 트래픽을 서비스로 라우팅하기 위해 애플리케이션 로드 밸런서에 의해 사용됩니다. 클러스터 내에서 Kubernetes 서비스를 통해 모든 앱이 노출되는 한, 하나의 Ingress 리소스를 사용하여 다중 앱에 대한 라우팅 규칙을 정의할 수 있습니다. 
...
앱에 대한 수신 TLS 연결을 관리하도록 애플리케이션 로드 밸런서를 구성하며, IBM 제공 TLS 인증서를 사용하여 네트워크 트래픽을 복호화하고, 클러스터에 노출된 앱으로 암호화되지 않은 요청을 전달할 수 있습니다. 
...
2.  노출할 앱에 대한 Kubernetes 서비스를 작성하십시오. 애플리케이션 로드 밸런서는 클러스터 내에서 Kubernetes 서비스를 통해 앱이 노출되는 경우에만 앱을 Ingress 로드 밸런싱에 포함시킬 수 있습니다. 
    1.  선호하는 편집기를 열고 예를 들어, `myservice.yaml`이라는 이름의 서비스 구성 파일을 작성하십시오. 
    2.  공용으로 노출시키려는 앱에 대해 서비스를 정의하십시오. 
...
#### 사용자 정의 도메인 및 TLS 인증서와 함께 애플리케이션 로드 밸런서 사용
{: #custom_domain_cert}

IBM 제공 도메인이 아니라 사용자 정의 도메인을 사용하는 동안 수신 네트워크 트래픽을 클러스터의 앱으로 라우팅하도록 애플리케이션 로드 밸런서를 구성하고 고유 TLS 인증서를 사용하여 TLS 종료를 관리할 수 있습니다.
{:shortdesc}
...
2.  수신 네트워크 트래픽을 IBM 제공 애플리케이션 로드 밸런서로 라우팅하도록 사용자의 도메인을 구성하십시오. 다음 옵션 중에서 선택하십시오.
    ...
    -   IP 주소를 레코드로 추가하여 IBM 제공 애플리케이션 로드 밸런서의 포터블 공인 IP 주소로 사용자 정의 도메인을 맵핑하십시오. 애플리케이션 로드 밸런서의 포터블 공인 IP 주소를 찾으려면 `bx cs alb-get <public_alb_ID>`를 실행하십시오. 
...
4.  TLS 인증서 및 키를 Kubernetes 시크릿에 저장하십시오. 
    ...
    2.  TLS 인증서 및 키를 사용하는 시크릿을 정의하십시오. <em>&lt;mytlssecret&gt;</em>을 Kubernetes 시크릿의 이름으로, <tls_key_filepath>를 사용자 정의 TLS 키 파일의 경로로, <tls_cert_filepath>를 사용자 정의 TLS 인증서 파일의 경로로 대체하십시오. 
...
6.  노출할 앱에 대한 Kubernetes 서비스를 작성하십시오. 애플리케이션 로드 밸런서는 클러스터 내에서 Kubernetes 서비스를 통해 앱이 노출되는 경우에만 앱을 Ingress 로드 밸런싱에 포함시킬 수 있습니다. 
    ...
    5.  공용으로 노출시키려는 모든 앱에 대해 이러한 단계를 반복하십시오. 
7.  Ingress 리소스를 작성하십시오. Ingress 리소스는 앱용으로 작성된 Kubernetes 서비스에 대한 라우팅 규칙을 정의하며, 수신 네트워크 트래픽을 서비스로 라우팅하기 위해 애플리케이션 로드 밸런서에 의해 사용됩니다. 클러스터 내에서 Kubernetes 서비스를 통해 모든 앱이 노출되는 한, 하나의 Ingress 리소스를 사용하여 다중 앱에 대한 라우팅 규칙을 정의할 수 있습니다. 
    1.  선호하는 편집기를 열고 예를 들어, `myingress.yaml`이라는 이름의 Ingress 구성 파일을 작성하십시오. 
    2.  사용자 정의 도메인을 사용하여 수신 네트워크 트래픽을 서비스로 라우팅하고 사용자 정의 인증서를 사용하여 TLS 종료를 관리하는 Ingress 리소스를 구성 파일에 정의하십시오. 모든 서비스에 대해 사용자 정의 도메인에 추가된 개별 경로를 정의하여 사용자의 앱에 고유 경로(예: `https://mydomain/myapp`)를 작성할 수 있습니다. 이 라우트를 웹 브라우저에 입력하면 네트워크 트래픽이 애플리케이션 로드 밸런서로 라우팅됩니다. 애플리케이션 로드 밸런서는 연관된 서비스를 찾은 후에 네트워크 트래픽을 서비스로 전송하고 앱을 실행 중인 포드로 전송합니다. 
...
#### 클러스터 외부의 앱으로 네트워크 트래픽을 라우팅하도록 애플리케이션 로드 밸런서 구성
{: #external_endpoint}

클러스터 로드 밸런싱에 포함될 클러스터의 외부에 위치하는 앱에 대해 애플리케이션 로드 밸런서를 구성할 수 있습니다. IBM 제공 또는 사용자 정의 도메인의 수신 요청이 외부 앱으로 자동으로 전달됩니다. 
...
IBM 제공 도메인에서 수신 네트워크 트래픽을 클러스터의 외부에 위치한 앱으로 라우팅하도록 애플리케이션 로드 밸런서를 구성할 수 있습니다. 사용자 정의 도메인 및 TLS 인증서를 대신 사용하려는 경우, IBM 제공 도메인 및 TLS 인증서를 [사용자 정의 도메인 및 TLS 인증서](#custom_domain_cert)로 대체하십시오. 
...
4.  Ingress 리소스를 작성하십시오. Ingress 리소스는 앱용으로 작성된 Kubernetes 서비스에 대한 라우팅 규칙을 정의하며, 수신 네트워크 트래픽을 서비스로 라우팅하기 위해 애플리케이션 로드 밸런서에 의해 사용됩니다. 클러스터 내에서 Kubernetes 서비스를 통해 모든 앱이 자체 외부 엔드포인트로 노출되는 한, 하나의 Ingress 리소스를 사용하여 다중 외부 앱에 대한 라우팅 규칙을 정의할 수 있습니다. 
    1.  선호하는 편집기를 열고 예를 들어, `myexternalingress.yaml`이라는 이름의 Ingress 구성 파일을 작성하십시오. 
    2.  이전에 정의한 외부 엔드포인트를 사용하여 수신 네트워크 트래픽을 사용자의 외부 앱으로 라우팅하기 위한 IBM 제공 도메인 및 TLS 인증서를 사용하는 Ingress 리소스를 구성 파일에 정의하십시오. 모든 서비스에 대해 IBM 제공 또는 사용자 정의 도메인에 추가된 개별 경로를 정의하여 사용자의 앱에 고유 경로(예: `https://ingress_domain/myapp`)를 작성할 수 있습니다. 이 라우트를 웹 브라우저에 입력하면 네트워크 트래픽이 애플리케이션 로드 밸런서로 라우팅됩니다. 애플리케이션 로드 밸런서는 연관된 서비스를 찾은 후에 네트워크 트래픽을 서비스로 전송하고 외부 앱으로 전송합니다. 
...
특정 경로에서 청취하지 않지만 루트 경로와 특정 포트를 사용합니다. 이 경우에는 루트 경로를 /로 정의하고 앱에 대한 개별 경로를 지정하지 마십시오. 
        ...
        모든 Kubernetes 서비스에 대해 도메인에 추가된 개별 경로를 정의하여 사용자의 앱에 고유 경로(예: <code>https://ibmdomain/myservicepath1</code>)를 작성할 수 있습니다. 이 라우트를 웹 브라우저에 입력하면 네트워크 트래픽이 애플리케이션 로드 밸런서로 라우팅됩니다. 애플리케이션 로드 밸런서는 연관된 서비스를 찾고 동일한 경로를 사용하여 네트워크 트래픽을 외부 앱으로 전송합니다. 수신 네트워크 트래픽을 수신하려면 이 경로에서 청취하도록 앱을 설정해야 합니다. 
...
표준 클러스터를 작성하는 경우, Ingress 제어기는 사설 애플리케이션 로드 밸런서를 자동으로 작성하지만 자동으로 사용하지는 않습니다. 사설 애플리케이션 로드 밸런서를 사용하려면 먼저 사전 지정된 IBM 제공 포터블 사설 IP 주소 또는 고유 포터블 사설 IP 주소를 사용하여 사용으로 설정해야 합니다. **참고**: 클러스터를 작성했을 때 `--no-subnet` 플래그를 사용한 경우, 사설 애플리케이션 로드 밸런서를 사용하려면 먼저 포터블 사설 서브넷 또는 사용자 관리 서브넷을 추가해야 합니다. 자세한 정보는 [클러스터의 추가 서브넷 요청](cs_cluster.html#add_subnet)을 참조하십시오.
...
사전 지정된 IBM 제공 포터블 사설 IP 주소를 사용하여 사설 애플리케이션 로드 밸런서를 사용으로 설정하려면 다음을 수행하십시오. 

1. 클러스터에서 사용 가능한 애플리케이션 로드 밸런서를 나열하여 사설 애플리케이션 로드 밸런서의 ID를 가져오십시오. <em>&lt;cluser_name&gt;</em>을 노출할 앱이 배치된 클러스터의 이름으로 대체하십시오.
    ...
    사설 애플리케이션 로드 밸런서의 **상태** 필드는 _disabled_입니다. 
    ```
...
2. 사설 애플리케이션 로드 밸런서를 사용으로 설정하십시오. 이전 단계의 출력에서 <em>&lt;private_ALB_ID&gt;</em>를 사설 애플리케이션 로드 밸런서의 ID로 대체하십시오. 
...
고유 포터블 사설 IP 주소를 사용하여 사설 애플리케이션 로드 밸런서를 사용으로 설정하려면 다음을 수행하십시오. 
...
2. 클러스터에서 사용 가능한 애플리케이션 로드 밸런서를 나열하여 사설 애플리케이션 로드 밸런서의 ID를 가져오십시오. 
...
3. 사설 애플리케이션 로드 밸런서를 사용으로 설정하십시오. 이전 단계의 출력에서 <em>&lt;private_ALB_ID&gt;</em>를 사설 애플리케이션 로드 밸런서의 ID로 대체하고 <em>&lt;user_ip&gt;</em>를 사용할 사용자 관리 서브넷의 IP 주소로 대체하십시오. 
...
#### 사용자 정의 도메인에서 사설 애플리케이션 로드 밸런서 사용
{: #private_ingress_no_tls}

사용자 정의 도메인을 사용하여 수신 네트워크 트래픽을 클러스터의 앱으로 라우팅하도록 사설 애플리케이션 로드 밸런서를 구성할 수 있습니다.
{:shortdesc}

시작하기 전에 [사설 애플리케이션 로드 밸런서를 사용으로 설정](#private_ingress)하십시오. 

사설 애플리케이션 로드 밸런서를 구성하려면 다음을 수행하십시오. 

1.  사용자 정의 도메인을 작성하십시오. 사용자 정의 도메인을 작성하려면 도메인 이름 서비스(DNS) 제공업체를 찾아서 사용자 정의 도메인을 등록하십시오. 

2.  IP 주소를 레코드로 추가하여 IBM 제공 사설 애플리케이션 로드 밸런서의 포터블 사설 IP 주소로 사용자 정의 도메인을 맵핑하십시오. 사설 애플리케이션 로드 밸런서의 포터블 사설 IP 주소를 찾으려면 `bx cs albs --cluster<cluster_name>`를 실행하십시오. 
...
4.  노출할 앱에 대한 Kubernetes 서비스를 작성하십시오. 사설 애플리케이션 로드 밸런서는 클러스터 내에서 Kubernetes 서비스를 통해 앱이 노출되는 경우에만 앱을 Ingress 로드 밸런싱에 포함시킬 수 있습니다. 
...
7.  Ingress 리소스를 작성하십시오. Ingress 리소스는 앱용으로 작성된 Kubernetes 서비스에 대한 라우팅 규칙을 정의하며, 수신 네트워크 트래픽을 서비스로 라우팅하기 위해 애플리케이션 로드 밸런서에 의해 사용됩니다. 클러스터 내에서 Kubernetes 서비스를 통해 모든 앱이 노출되는 한, 하나의 Ingress 리소스를 사용하여 다중 앱에 대한 라우팅 규칙을 정의할 수 있습니다. 
    1.  선호하는 편집기를 열고 예를 들어, `myingress.yaml`이라는 이름의 Ingress 구성 파일을 작성하십시오. 
    2.  사용자 정의 도메인을 사용하여 수신 네트워크 트래픽을 사용자 서비스로 라우팅하는 Ingress 리소스를 구성 파일에 정의하십시오. 모든 서비스에 대해 사용자 정의 도메인에 추가된 개별 경로를 정의하여 사용자의 앱에 고유 경로(예: `https://mydomain/myapp`)를 작성할 수 있습니다. 이 라우트를 웹 브라우저에 입력하면 네트워크 트래픽이 애플리케이션 로드 밸런서로 라우팅됩니다. 애플리케이션 로드 밸런서는 연관된 서비스를 찾은 후에 네트워크 트래픽을 서비스로 전송하고 앱을 실행 중인 포드로 전송합니다. 
...
특정 경로에서 청취하지 않지만 루트 경로와 특정 포트를 사용합니다. 이 경우에는 루트 경로를 `/`로 정의하고 앱에 대한 개별 경로를 지정하지 마십시오. 
        ...
        <td><em>&lt;private_ALB_ID&gt;</em>를 사설 Ingress 제어기의 ALB ID로 대체하십시오. <code>bx cs albs --cluster <my_cluster></code>를 실행하여 ALB ID를 찾으십시오. 이 Ingress 어노테이션에 대한 자세한 정보는 [애플리케이션 로드 밸런서 ID(ALB_ID)](cs_annotations.html#alb-id)를 참조하십시오. </td>
        </tr>
        <td><code>host</code></td>
        ...
        모든 Kubernetes 서비스의 경우, 사용자 정의 도메인에 추가된 개별 경로를 정의하여 사용자의 앱에 대한 고유 경로(예: <code>custom_domain/myservicepath1</code>)를 작성할 수 있습니다. 이 라우트를 웹 브라우저에 입력하면 네트워크 트래픽이 애플리케이션 로드 밸런서로 라우팅됩니다. 애플리케이션 로드 밸런서는 연관된 서비스를 찾은 후에 네트워크 트래픽을 서비스로 전송하고 동일한 경로를 사용하여 앱을 실행 중인 포드로 전송합니다. 수신 네트워크 트래픽을 수신하려면 이 경로에서 청취하도록 앱을 설정해야 합니다. 
...
#### 사용자 정의 도메인 및 TLS 인증서와 함께 사설 애플리케이션 로드 밸런서 사용
{: #private_ingress_tls}

사용자 정의 도메인을 사용하는 동안 수신 네트워크 트래픽을 클러스터의 앱으로 라우팅하도록 사설 애플리케이션 로드 밸런서를 구성하고 고유 TLS 인증서를 사용하여 TLS 종료를 관리할 수 있습니다.
{:shortdesc}
...
6.  노출할 앱에 대한 Kubernetes 서비스를 작성하십시오. 사설 애플리케이션 로드 밸런서는 클러스터 내에서 Kubernetes 서비스를 통해 앱이 노출되는 경우에만 앱을 Ingress 로드 밸런싱에 포함시킬 수 있습니다. 
...
특정 경로에서 청취하지 않지만 루트 경로와 특정 포트를 사용합니다. 이 경우에는 루트 경로를 `/`로 정의하고 앱에 대한 개별 경로를 지정하지 마십시오. 
        ...
        <tr>
        <td><code>ingress.bluemix.net/ALB-ID</code></td>
        <td><em>&lt;private_ALB_ID&gt;</em>를 사설 애플리케이션 로드 밸런서의 ID로 대체하십시오. 애플리케이션 로드 밸런서 ID를 찾으려면 <code>bx cs albs --cluster <my_cluster></code>를 실행하십시오. 이 Ingress 어노테이션에 대한 자세한 정보는 [사설 애플리케이션 로드 밸런서 라우팅(ALB-ID)](cs_annotations.html#alb-id)을 참조하십시오. </td>
        </tr>
        <tr>
        <td><code>tls/hosts</code></td>
        <td><em>&lt;mycustomdomain&gt;</em>을 TLS 종료를 위해 구성하려는 사용자 정의 도메인으로 대체합니다.
...
포터블 공인 및 사설 서브넷과 IP 주소를 사용하여 앱을 클러스터에 노출시켜 인터넷 또는 사설 네트워크에서 액세스 가능하도록 설정할 수 있습니다.
{:shortdesc}
...
{{site.data.keyword.containershort_notm}}에서 사용자는 클러스터에 네트워크 서브넷을 추가하여 Kubernetes 서비스에 대한 안정적인 포터블 IP를 추가할 수 있습니다. 표준 클러스터를 작성하면 {{site.data.keyword.containershort_notm}}에서 포터블 공인 서브넷에 5개의 포터블 공인 IP 주소를, 포터블 사설 서브넷에 5개의 포터블 사설 IP 주소를 자동으로 프로비저닝합니다. 포터블 IP 주소는 정적이며 작업자 노드 또는 클러스터가 제거되더라도 변경되지 않습니다. 
 ...
 포터블 IP 주소 중 2개(하나는 공인, 하나는 사설)는 클러스터에서 다중 앱을 노출하는 데 사용할 수 있는 [Ingress 애플리케이션 로드 밸런서](#cs_apps_public_ingress)에 사용됩니다. [로드 밸런서 서비스를 작성](#cs_apps_public_load_balancer)하여 앱을 노출시키는 데 4개의 포터블 공인 IP 주소와 4개의 포터블 사설 IP 주소를 사용할 수 있습니다.
...
6.  {: #cs_apps_volume_mount}지속적 볼륨 클레임을 배치에 마운트하려면 구성 파일을 작성하십시오. 구성을 `.yaml` 파일로 저장하십시오. 
