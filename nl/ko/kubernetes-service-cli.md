...
다시 디자인된 {{site.data.keyword.containerlong_notm}} 플러그인의 다음 베타 버전을 사용할 수 있습니다.
...
<dd>특정 위치 또는 쉼표로 구분된 위치 목록으로 구역을 필터링합니다. 지원되는 위치를 보려면 <code>ibmcloud ks supported-locations</code>를 실행하십시오.</dd>
...
### 더 이상 사용되지 않음: ibmcloud ks kube-versions
{: #cs_kube_versions}

{{site.data.keyword.containerlong_notm}}에서 지원되는 Kubernetes 버전 목록을 봅니다. [클러스터 마스터](#cs_cluster_update) 및 [작업자 노드](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_worker_update)를 안정적인 최신 기능을 위한 기본 버전으로 업데이트합니다.
{: shortdesc}

이 명령은 더 이상 사용되지 않습니다. 대신 [ibmcloud ks versions command](#cs_versions_command)를 사용하십시오.
{: deprecated}
...
{{site.data.keyword.containerlong_notm}} 클러스터에 사용 가능한 모든 컨테이너 플랫폼 버전을 나열합니다. [클러스터 마스터](#cs_cluster_update) 및 [작업자 노드](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_worker_update)를 안정적인 최신 기능을 위한 기본 버전으로 업데이트합니다.
{: shortdesc}
...
<dd>지정된 컨테이너 플랫폼에 대한 버전만 표시합니다. 지원되는 값은 <code>kubernetes</code> 또는 <code>openshift</code>입니다.</dd>
...
{{site.data.keyword.Bluemix_notm}} 서비스의 서비스 인증 정보를 작성하고 클러스터의 Kubernetes 시크릿에 이 인증 정보를 저장합니다. {{site.data.keyword.Bluemix_notm}} 카탈로그에서 사용 가능한 {{site.data.keyword.Bluemix_notm}} 서비스를 보려면 `ibmcloud service offerings`를 실행하십시오. **참고**: 서비스 키를 지원하는 {{site.data.keyword.Bluemix_notm}} 서비스만 추가할 수 있습니다.
{: shortdesc}

서비스 바인딩에 대한 자세한 정보와 클러스터에 추가할 수 있는 서비스에 대해 알아보려면 [IBM Cloud 서비스 바인딩을 사용하여 서비스 추가](/docs/containers?topic=containers-service-binding)를 참조하십시오.
...
**최소 필요 권한**: {{site.data.keyword.containerlong_notm}}의 클러스터에 대한 **편집자** 플랫폼 역할 및 **개발자** Cloud Foundry 역할
...
<dd>기존 서비스 키의 이름 또는 GUID입니다. 이 값은 선택사항입니다. `service-binding` 명령 사용 시 새 서비스 인증 정보가 서비스 인스턴스에 맞게 자동으로 작성되고 IAM 사용 서비스에 대한 IAM **작성자** 서비스 액세스 역할이 지정됩니다. 이전에 작성한 기존 서비스 키를 사용하려면 이 옵션을 사용하십시오. 서비스 키가 이미 특정 IAM 서비스 액세스 역할로 작성되었으므로, 서비스 키를 정의하는 경우 동시에 `--role` 옵션을 설정할 수 없습니다. </dd>

<dt><code>--namespace <em>KUBERNETES_NAMESPACE</em></code></dt>
<dd>서비스 인증 정보에 맞게 Kubernetes 시크릿을 작성할 Kubernetes 네임스페이스의 이름입니다. 이 값은 필수입니다.</dd>

<dt><code>--service <em>SERVICE_INSTANCE</em></code></dt>
<dd>바인딩할 {{site.data.keyword.Bluemix_notm}} 서비스 인스턴스의 이름입니다. 이름을 찾으려면 <code>ibmcloud service list</code>(Cloud Foundry 서비스의 경우) 및 <code>ibmcloud resource service-instances</code>(IAM 사용 서비스의 경우)를 실행하십시오. 이 값은 필수입니다. </dd>

<dt><code>--role <em>IAM_SERVICE_ROLE</em></code></dt>
<dd>서비스 키에 필요한 {{site.data.keyword.Bluemix_notm}} IAM 역할입니다. 이 값은 선택사항이며 IAM 사용 서비스에만 사용될 수 있습니다. 이 옵션을 설정하지 않으면 서비스 인증 정보가 자동으로 작성되며 **작성자** 서비스 액세스 역할이 지정됩니다. `--key` 옵션을 사용하여 기존 서비스 키를 사용할 경우 이 옵션을 포함하지 마십시오. <br><br>
...
Kubernetes 네임스페이스에서 {{site.data.keyword.Bluemix_notm}} 서비스를 바인딩 해제하여 이 서비스를 제거합니다.
{: shortdesc}
...
<dt><code>--service <em>SERVICE_INSTANCE</em></code></dt>
<dd>제거할 {{site.data.keyword.Bluemix_notm}} 서비스 인스턴스의 이름입니다. 서비스 인스턴스의 이름을 찾으려면 `ibmcloud ks cluster-services --cluster <cluster_name_or_ID>`를 나열하십시오. 이 값은 필수입니다.</dd>
...
**최소 필요 권한**: {{site.data.keyword.registrylong_notm}}에 대한 **독자** 서비스 액세스 역할. **참고**: 리소스 그룹 레벨에서 {{site.data.keyword.registryshort_notm}}에 대한 정책을 지정하지 마십시오.
...
### 베타: ibmcloud ks key-protect-enable
{: #cs_key_protect}
...
IBM Cloud 인프라(SoftLayer) 계정의 기존 포터블 공인 또는 사설 서브넷을 클러스터에서 사용할 수 있도록 하거나, 자동으로 프로비저닝된 서브넷을 사용하는 대신 삭제된 클러스터의 서브넷을 재사용합니다.
{: shortdesc}
...
공용 또는 사설 VLAN의 IBM Cloud 인프라(SoftLayer) 계정에서 포터블 서브넷을 작성하고 클러스터에서 사용할 수 있도록 합니다.
{: shortdesc}
...
<dd>포터블 서브넷에서 작성할 IP 주소의 수입니다. 허용되는 값은 8, 16, 32 또는 64입니다. <p class="note"> 서브넷에 대한 포터블 IP 주소를 추가하는 경우에는 세 개의 IP 주소를 사용하여 클러스터 내부 네트워킹을 설정합니다. Ingress 애플리케이션 로드 밸런서(ALB)에 대해 또는 네트워크 로드 밸런서(NLB) 서비스를 작성하는 데 세 개의 IP 주소를 사용할 수 없습니다. 예를 들어, 8개의 포터블 공인 IP 주소를 요청하는 경우 이 중에서 5개를 사용하여 앱을 공용으로 노출할 수 있습니다.</p> </dd>

<dt><code>--vlan <em>VLAN_ID</em></code></dt>
<dd>서브넷을 작성할 공용 또는 사설 VLAN의 ID입니다. 기존 작업자 노드가 연결되어 있는 공용 또는 사설 VLAN을 선택해야 합니다. 작업자 노드가 연결되는 공용 또는 사설 VLAN을 검토하려면 <code>ibmcloud ks cluster-get --cluster &lt;cluster&gt; --showResources</code>를 실행하고 출력에서 <strong>서브넷 VLAN</strong> 섹션을 찾으십시오. 서브넷은 VLAN이 있는 동일한 구역에서 프로비저닝됩니다.</dd>
...
<dd>서브넷 CIDR(Classless InterDomain Routing)입니다. 이 값은 필수이며 IBM Cloud 인프라(SoftLayer)에서 사용되는 서브넷과 충돌하지 않아야 합니다. 지원되는 접두부의 범위는 `/30`(1개의 IP 주소) - `/24`(253개의 IP 주소)입니다. 하나의 접두부 길이에 CIDR을 설정하고 나중에 이를 변경해야 하는 경우 먼저 새 CIDR을 추가한 후 [이전 CIDR을 제거](#cs_cluster_user_subnet_rm)하십시오.</dd>
...
지정된 클러스터에서 고유한 사설 서브넷을 제거합니다. 고유한 사설 서브넷에서 IP 주소로 배치된 서비스는 서브넷이 제거된 후에도 활성 상태를 유지합니다.
{: shortdesc}
...
```
{: pre}
**최소 필요 권한**: {{site.data.keyword.containerlong_notm}}

의 클러스터에 대한 **운영자** 플랫폼 역할**명령 옵션**:
<dl>
<dt><code>--cluster <em>CLUSTER</em></code></dt>
<dd>클러스터의 이름 또는 ID입니다. 이 값은 필수입니다.</dd>
...
IBM Cloud 인프라(SoftLayer) 계정에서 사용 가능한 포터블 서브넷을 나열합니다.
{: shortdesc}

모든 위치에 있는 서브넷이 리턴됩니다. 특정 위치로 서브넷을 필터링하려면 `--locations` 플래그를 포함하십시오. 
...
### 베타: ibmcloud ks alb-cert-deploy
{: #cs_alb_cert_deploy}
...
표준 클러스터에서 ALB를 사용 또는 사용 안함으로 설정합니다.
{: shortdesc}

이 명령을 사용하여 다음을 수행할 수 있습니다. 
* 기본 사설 ALB를 사용으로 설정하십시오. 클러스터를 작성하는 경우 작업자가 있고 사설 서브넷이 사용 가능한 각 구역에 기본 사설 ALB가 작성되지만 기본 사설 ALB는 사용으로 설정되지 않습니다. 그러나 모든 공용 ALB가 자동으로 사용으로 설정되며 `ibmcloud ks alb-create` 명령으로 작성하는 공용 또는 사설 ALB도 기본적으로 사용으로 설정되어야 합니다. 
* 이전에 사용 안함으로 설정한 ALB를 사용으로 설정하십시오. 
* 새 VLAN에서 ALB를 작성한 후 이전 VLAN에서 ALB를 사용 안함으로 설정하십시오. 자세한 정보는 [VLAN 간에 ALB 이동](/docs/containers?topic=containers-ingress#migrate-alb-vlan)을 참조하십시오.
* 자체 Ingress 제어기를 배치하고, Ingress 제어기를 노출하는 데 사용되는 로드 밸런서 서비스 또는 IBM 제공 Ingress 하위 도메인에 대한 DNS 등록을 활용할 수 있도록 IBM 제공 ALB 배치를 사용 안함으로 설정하십시오. 
...
<dd>클러스터에서 ALB를 사용 안함으로 설정하려면 이 플래그를 포함하십시오. <p class="note">ALB를 사용 안함으로 설정하면 다른 서비스에서 IP를 사용할 수 있도록 ALB에서 사용한 IP 주소가 사용 가능한 포터블 IP의 풀로 다시 돌아갑니다. 나중에 ALB를 다시 사용으로 설정하려는 경우 이전에 사용한 IP 주소가 현재 다른 서비스에서 사용되면 ALB는 오류를 보고할 수 있습니다. ALB를 다시 사용으로 설정할 때 다른 서비스의 실행을 중지하거나 다른 IP 주소를 지정하여 사용할 수 있습니다. </p></dd>

<dt><code>--disable-deployment</code></dt>
<dd>IBM 제공 ALB 배치를 사용 안함으로 설정하려면 이 플래그를 포함하십시오. 이 플래그는 Ingress 제어기를 노출하는 데 사용되는 IBM 제공 Ingress 하위 도메인 또는 로드 밸런서 서비스에 대한 DNS 등록을 제거하지 않습니다.</dd>

<dt><code>--user-ip <em>USER_IP</em></code></dt>
<dd>선택사항: <code>--enable</code> 플래그를 사용하여 ALB를 사용으로 설정하는 경우 ALB가 작성된 구역의 VALN에 있는 IP 주소를 지정할 수 있습니다. ALB는 사용으로 설정되며 이 공인 또는 사설 IP 주소를 사용합니다. 이 IP 주소는 클러스터의 다른 로드 밸런서 또는 ALB에서 사용되면 안 됩니다. IP 주소가 제공되지 않으면 ALB는 클러스터를 작성할 때 자동으로 프로비저닝된 포터블 공용 또는 사설 서브넷의 공인 또는 사설 IP 주소로 배치되거나 이전에 ALB에 지정한 공인 또는 사설 IP 주소로 배치됩니다. </dd>
...
구역에 공용 또는 사설 ALB를 작성합니다. 기본적으로 작성하는 ALB는 사용으로 설정됩니다.
{: shortdesc}
...
<dd>ALB의 유형(<code>public</code> 또는 <code>private</code>)입니다. </dd>

<dt><code>--zone <em>ZONE</em></code></dt>
<dd>ALB를 작성하는 구역입니다. </dd>

<dt><code>--vlan <em>VLAN_ID</em></code></dt>
<dd>ALB를 작성할 VLAN의 ID입니다. 이 VLAN은 ALB <code>type</code>과 일치하고 작성할 ALB와 동일한 <code>zone</code>에 있어야 합니다. </dd>

<dt><code>--user-ip <em>IP</em></code></dt>
<dd>선택사항: ALB에 지정할 IP 주소입니다. 이 IP는 지정한 <code>vlan</code>에 있어야 하며 작성할 ALB와 동일한 <code>zone</code>에 있어야 합니다. 이 IP 주소는 클러스터의 다른 로드 밸런서 또는 ALB에서 사용되면 안 됩니다. </dd>
...
ALB 팟(Pod)이 최근에 업데이트되었지만 ALB의 사용자 정의 구성이 최신 빌드의 영향을 받는 경우, ALB 팟(Pod)이 실행되던 이전 빌드로 업데이트를 롤백할 수 있습니다. 클러스터의 모든 ALB 팟(Pod)은 이전 실행 상태로 되돌아갑니다.
{: sortdesc}

업데이트를 롤백하면 ALB 팟(Pod)의 자동 업데이트가 사용 안함으로 설정됩니다. 자동 업데이트를 다시 사용으로 설정하려면 [`alb-autoupdate-enable` 명령](#cs_alb_autoupdate_enable)을 사용하십시오.
...
지역에서 지원되는 Ingress ALB 유형을 나열합니다.
{: shortdesc}
...
클러스터의 Ingress ALB 팟(Pod)을 최신 버전으로 강제 업데이트합니다.
{: shortdesc}
...
클러스터에 모든 Ingress ALB ID를 나열하고 ALB 팟(Pod)에 대한 업데이트가 가능한지 여부를 봅니다.
{: shortdesc}

ALB ID가 리턴되지 않으면 클러스터에 포터블 서브넷이 없습니다. 서브넷을 [작성](#cs_cluster_subnet_create)하거나 클러스터에 [추가](#cs_cluster_subnet_add)할 수 있습니다.
...
서로 다른 인증 정보를 사용하여 IBM Cloud 인프라(SoftLayer) 포트폴리오에 액세스하도록 {{site.data.keyword.Bluemix_notm}} 계정을 설정한 경우에는 현재 대상으로 지정된 지역 및 리소스 그룹에 대한 인프라 사용자 이름을 가져옵니다.
{: shortdesc}
...
{{site.data.keyword.Bluemix_notm}} 계정을 통해 사용자가 IBM Cloud 인프라(SoftLayer) 포트폴리오에 액세스할 수 있도록 리소스 그룹 및 지역에 대한 인증 정보를 설정합니다.
{: shortdesc}
...
<dd>IBM Cloud 인프라(SoftLayer) 계정 API 사용자 이름입니다. 이 값은 필수입니다. 참고로, 인프라 API 사용자 이름은 IBM ID와 동일하지 않습니다. 인프라 API 사용자 이름을 보려면 [클래식 인프라 API 키 관리](/docs/iam?topic=iam-classic_keys)를 참조하십시오.</dd>

<dt><code>--infrastructure-api-key <em>API_KEY</em></code></dt>
<dd>IBM Cloud 인프라(SoftLayer) 계정 API 키입니다. 이 값은 필수입니다. 인프라 API 사용자 이름을 보거나 생성하려면 [클래식 인프라 API 키 관리](/docs/iam?topic=iam-classic_keys)를 참조하십시오.</dd>
...
{{site.data.keyword.Bluemix_notm}} 계정을 통해 사용자가 IBM Cloud 인프라(SoftLayer) 포트폴리오에 액세스할 수 있도록 리소스 그룹 및 지역에 대한 인증 정보를 제거합니다.
{: shortdesc}
...
대상으로 지정된 리소스 그룹 및 지역에 맞게 [IBM Cloud 인프라(SoftLayer) 포트폴리오에 대한 액세스](/docs/containers?topic=containers-users#api_key)를 허용하는 인증 정보에 제안되거나 필요한 인프라 권한이 없는지 확인합니다.
{: shortdesc}

**`required` 및 `suggested` 인프라 권한은 무엇을 의미합니까?**<br>
지역 및 리소스 그룹에 대한 인프라 인증 정보에 권한이 없는 경우 이 명령의 출력은 `required` 및 `suggested` 권한의 목록을 리턴합니다. 
*   **필수**: 이 권한은 작업자 노드와 같은 인프라 리소스를 주문하고 관리하는 데 필요합니다. 인프라 인증 정보에 이러한 권한 중 하나가 없는 경우 `worker-reload`와 같은 공통 조치는 지역 및 리소스 그룹의 모든 클러스터에 대해 실패할 수 있습니다. 
*   **제한됨**: 이 권한은 인프라 권한을 포함하는 데 유용하며 특정 유스 케이스에 필요할 수 있습니다. 예를 들어, `Add Compute with Public Network Port` 인프라 권한이 제안됩니다. 공용 네트워킹을 사용하려는 경우 이 권한이 필요하기 때문입니다. 그러나 유스 케이스가 사설 VLAN 전용의 클러스터인 경우 권한이 `required`로 간주되지 않기 때문에 필요하지 않습니다. 

권한별 공통 유스 케이스의 목록을 보려면 [인프라 역할](/docs/containers?topic=containers-access_reference#infra)을 참조하십시오.

**콘솔 또는 [인프라 역할](/docs/containers?topic=containers-access_reference#infra) 테이블에서 찾을 수 없는 인프라 권한이 표시되면 어떻게 해야 합니까?**<br>
`Support Case` 권한은 인프라 권한이 아닌 콘솔의 다른 부분에서 관리됩니다. [인프라 권한 사용자 정의](/docs/containers?topic=containers-users#infra_access)의 8단계를 참조하십시오.

**지정하는 인프라 권한은 무엇입니까?**<br>
권한에 대한 회사의 정책이 엄격한 경우 클러스터의 유스 케이스에 대한 `suggested` 권한을 제한해야 할 수 있습니다. 그렇지 않으면, 지역 및 리소스 그룹에 대한 인프라 인증 정보에 모든 `required` 및 `suggested` 권한이 포함되어야 합니다. 

대부분의 유스 케이스의 경우 적절한 인프라 권한을 사용하여 지역 및 리소스 그룹에 대한 [API 키를 설정](/docs/containers?topic=containers-users#api_key)하십시오. 현재 계정과 다른 인프라 계정을 사용해야 하는 경우 [수동 인증 정보 설정](/docs/containers?topic=containers-users#credentials)을 참조하십시오.

**사용자가 수행할 수 있는 조치를 제어하는 방법은 무엇입니까?**<br>
인프라 인증 정보가 설정된 후 사용자가 [{{site.data.keyword.Bluemix_notm}} IAM 플랫폼 역할](/docs/containers?topic=containers-access_reference#iam_platform)을 지정하여 수행할 수 있는 조치를 제어할 수 있습니다. 
...
<dl>
<dt><code>--region <em>REGION</em></code></dt>
<dd>지역을 지정합니다. 사용 가능한 지역을 나열하려면 <code>ibmcloud ks regions</code>를 실행하십시오. 이 값은 필수입니다.</dd>
...
작업자 노드에 대해 사용 가능한 작업자 머신 유형 또는 특성의 목록을 봅니다. 머신 유형은 구역에 따라 다릅니다.
{:shortdesc}

각각의 머신 유형에는 클러스터의 각 작업자 노드에 대한 가상 CPU, 메모리 및 디스크 공간의 양이 포함됩니다. 기본적으로, 모든 컨테이너 데이터가 저장된 보조 스토리지 디스크 디렉토리는 LUKS 암호화를 통해 암호화됩니다. `disable-disk-encrypt` 옵션이 클러스터 작성 중에 포함된 경우에는 호스트의 컨테이너 런타임 데이터가 암호화되지 않습니다. [암호화에 대해 자세히 알아보십시오](/docs/containers?topic=containers-security#encrypted_disk).

공유 또는 전용 하드웨어에서 가상 머신으로서 또는 베어메탈에서 실제 머신으로서 작업자 노드를 프로비저닝할 수 있습니다. [머신 유형 옵션에 대해 자세히 알아보십시오](/docs/containers?topic=containers-planning_worker_nodes#planning_worker_nodes).
...
<dl>
<dt><code>--cluster <em>CLUSTER</em></code></dt>
<dd>스냅샷을 작성할 클러스터의 이름 또는 ID입니다.</dd>

<dt><code>--cos-bucket <em>BUCKET_NAME</em></code></dt>
<dd>로그를 저장할 {{site.data.keyword.cos_short}} 버킷의 이름입니다.</dd>

<dt><code>--cos-endpoint <em>ENDPOINT</em></code></dt>
<dd>로그를 저장하는 버킷의 지역, 교차 지역 또는 단일 데이터 센터 {{site.data.keyword.cos_short}} 엔드포인트입니다. 사용 가능한 엔드포인트는 {{site.data.keyword.cos_short}} 문서에서 [엔드포인트 및 스토리지 위치](/docs/services/cloud-object-storage/basics?topic=cloud-object-storage-endpoints)를 참조하십시오. </dd>

<dt><code>--hmac-key-id <em>HMAC_KEY_ID</em></code></dt>
<dd>{{site.data.keyword.cos_short}} 인스턴스에 대한 HMAC 인증 정보의 고유 ID입니다. </dd>

<dt><code>--hmac-key <em>HMAC_KEY</em></code></dt>
<dd>{{site.data.keyword.cos_short}} 인스턴스에 대한 HMAC 키입니다.</dd>

<dt><code>--type <em>LOG_TYPE</em></code></dt>
<dd>선택사항: 스냅샷을 작성할 로그의 유형입니다. 현재는 `master`가 유일한 선택사항이며 기본값입니다.</dd>
...
**최소 필요 권한**: `kube-audit`을 제외한 모든 로그 소스의 경우 클러스터에 대한 **편집자** 플랫폼 역할 또는 `kube-audit` 로그 소스의 경우 클러스터에 대한 **관리자** 플랫폼 역할
...
<dt><code>--type <em>LOG_TYPE</em></code></dt>
<dd>로그를 전달할 위치입니다. 옵션은 <code>ibm</code>(로그를 {{site.data.keyword.loganalysisshort_notm}}로 전달) 및 <code>syslog</code>(로그를 외부 서버로 전달)입니다.<p class="deprecated">{{site.data.keyword.loganalysisshort_notm}}은 더 이상 사용되지 않습니다. 이 명령 옵션은 2019년 9월 30일까지 지원됩니다. </p></dd>
...
<dd>상태 검사 모니터를 구성할 호스트 이름입니다. 호스트 이름을 나열하려면 <code>ibmcloud ks nlb-dnss --cluster CLUSTER</code>를 실행하십시오.</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>상태 검사를 모니터하는 호스트 이름입니다. 호스트 이름을 나열하려면 <code>ibmcloud ks nlb-dnss --cluster CLUSTER</code>를 실행하십시오.</dd>
...
<dt><code>--nlb-host <em>HOST_NAME</em></code></dt>
<dd>하나의 호스트 이름에 대해서만 상태를 보려면 이 플래그를 포함하십시오. 호스트 이름을 나열하려면 <code>ibmcloud ks nlb-dnss --cluster CLUSTER</code>를 실행하십시오.</dd>
...
### 더 이상 사용되지 않음: ibmcloud ks region-get
{: #cs_region}

현재 대상으로 지정된 {{site.data.keyword.containerlong_notm}} 지역을 찾으십시오.
{: shortdesc}

`ibmcloud ks region-set`를 실행하여 지역을 설정하고 작업을 수행할 리소스가 다른 지역에 있는 경우에도 모든 위치에서 액세스할 수 있는 리소스 관련 작업을 수행할 수 있습니다. 다른 지역에서 이름이 동일한 클러스터가 있는 경우, 명령 실행 시 클러스터 ID를 사용하거나 `ibmcloud ks region-set` 명령을 사용하여 지역을 설정한 다음 명령 실행 시 클러스터 이름을 사용할 수 있습니다. 

<p class="deprecated">레거시 작동:<ul><li>{{site.data.keyword.containerlong_notm}} 플러그인 버전 <code>0.3</code> 이상을 사용하고 하나의 위치에서만 리소스를 나열하고 리소스 관련 작업을 수행해야 하는 경우 <code>ibmcloud ks init</code> [명령](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_init)을 사용하여 글로벌 엔드포인트 대신 지역 엔드포인트를 대상으로 지정할 수 있습니다.</li><li>[{{site.data.keyword.containerlong_notm}} 플러그인의 <code>IKS_BETA_VERSION</code> 환경 변수를 <code>0.2</code>](/docs/containers-cli-plugin?topic=containers-cli-plugin-kubernetes-service-cli#cs_beta)로 설정하는 경우 지역에 특정한 클러스터를 작성하고 관리합니다.<code>ibmcloud ks region-set</code> 명령을 사용하여 지역을 변경할 수 있습니다.</li></ul></p>
...
### 더 이상 사용되지 않음: ibmcloud ks region-set
{: #cs_region-set}

{{site.data.keyword.containerlong_notm}}의 지역을 설정합니다.
{: shortdesc}
...
{{site.data.keyword.containerlong_notm}} 플러그인의 `0.2` 베타 버전(레거시)을 사용하는 경우 지역에 특정한 클러스터를 작성하고 관리합니다. 예를 들어, 미국 남부 지역에서 {{site.data.keyword.Bluemix_notm}}에 로그인하고 클러스터를 작성할 수 있습니다. 그 다음에는 `ibmcloud ks region-set eu-central`을 사용하여 중앙 유럽 지역을 대상으로 지정하고 다른 클러스터를 작성할 수 있습니다. 최종적으로는 `ibmcloud ks region-set us-south`를 사용하여 미국 남부로 돌아가서 해당 지역의 클러스터를 관리할 수 있습니다.
{: deprecated}
...
사용 가능한 지역의 목록을 보려면 [위치](/docs/containers?topic=containers-regions-and-zones)를 검토하거나 `ibmcloud ks regions` [명령](#cs_regions)을 사용하십시오.</dd></dl>
...
### 더 이상 사용되지 않음: ibmcloud ks regions
{: #cs_regions}
...
{{site.data.keyword.containerlong_notm}}에서 지원하는 위치를 나열합니다. 리턴되는 위치에 대한 자세한 정보는 [{{site.data.keyword.containerlong_notm}} 위치](/docs/containers?topic=containers-regions-and-zones#locations)를 참조하십시오.
{: shortdesc}
...
사용자가 클러스터를 작성할 수 있는 사용 가능한 구역 목록을 봅니다.
{: shortdesc}

모든 위치에 있는 구역이 리턴됩니다. 특정 위치로 구역을 필터링하기 위해 `--locations` 플래그를 포함합니다. 

{{site.data.keyword.containerlong_notm}} 플러그인의 `0.2` 베타 버전(레거시)을 사용하는 경우 사용 가능한 구역은 로그인하는 위치에 따라 달라집니다. 지역을 전환하려면 `ibmcloud ks region-set`를 실행하십시오.
{: deprecated}
...
클러스터에 독립형 작업자 노드를 추가합니다. 이 명령은 더 이상 사용되지 않습니다. [`ibmcloud ks worker-pool-create`](#cs_worker_pool_create)를 실행하여 작업자 풀을 작성하거나 [`ibmcloud ks worker-pool-resize`](#cs_worker_pool_resize)를 실행하여 작업자를 기존 작업자 풀에 추가합니다.
{: deprecated}
...
<td><code><em>&lt;machine_type&gt;</em></code>을 작업자 노드를 배치하려는 머신 유형으로 대체합니다. 공유 또는 전용 하드웨어에서 가상 머신으로서 또는 베어메탈에서 실제 머신으로서 작업자 노드를 배치할 수 있습니다. 사용 가능한 실제 및 가상 머신 유형은 클러스터가 배치되는 구역에 따라 다양합니다. 자세한 정보는 `ibmcloud ks machine-types` [명령](#cs_machine_types)을 참조하십시오.</td>
</tr>
<tr>
<td><code><em>private-vlan</em></code></td>
<td><code><em>&lt;private_VLAN&gt;</em></code>을 작업자 노드에 사용할 사설 VLAN의 ID로 대체합니다. 사용 가능한 VLAN을 나열하려면 <code>ibmcloud ks vlans --zone <em>&lt;zone&gt;</em></code>을 실행하고 <code>bcr</code>(백엔드 라우터)로 시작하는 VLAN 라우터를 찾으십시오.</td>
</tr>
<tr>
<td><code>public-vlan</code></td>
<td><code>&lt;public_VLAN&gt;</code>을 작업자 노드에 사용할 공용 VLAN의 ID로 대체합니다. 사용 가능한 VLAN을 나열하려면 <code>ibmcloud ks vlans --zone &lt;zone&gt;</code>을 실행하고 <code>fcr</code>(백엔드 라우터)로 시작하는 VLAN 라우터를 찾으십시오.<br><strong>참고</strong>: 작업자 노드가 사설 VLAN 전용으로 설정된 경우에는 [개인 서비스 엔드포인트를 사용으로 설정](/docs/containers?topic=containers-cs_network_cluster#set-up-private-se)하거나 [게이트웨이 디바이스를 구성](/docs/containers?topic=containers-plan_clusters#workeruser-master)하여 작업자 노드와 클러스터 마스터가 통신할 수 있도록 해야 합니다.</td>
</tr>
<tr>
...
<dt><code>--public-vlan <em>PUBLIC_VLAN</em></code></dt>
<dd>클러스터가 작성될 때 지정된 공용 VLAN입니다. 이 값은 선택사항입니다. 작업자 노드가 사설 VLAN에만 존재하도록 하려는 경우 공용 VLAN ID를 제공하지 마십시오. 사설 VLAN 라우터는 항상 <code>bcr</code>(벡엔드 라우터)로 시작하고 공용 VLAN 라우터는 항상 <code>fcr</code>(프론트 엔드 라우터)로 시작합니다. 클러스터를 작성하고 공인 및 사설 VLAN을 지정할 때는 이러한 접두부 뒤의 숫자 및 문자 조합이 일치해야 합니다.<p class="note">작업자 노드가 사설 VLAN 전용으로 설정된 경우에는 [개인 서비스 엔드포인트를 사용으로 설정](/docs/containers?topic=containers-cs_network_cluster#set-up-private-se)하거나 [게이트웨이 디바이스를 구성](/docs/containers?topic=containers-plan_clusters#workeruser-master)하여 작업자 노드와 클러스터 마스터가 통신할 수 있도록 해야 합니다.</p></dd>
...
<dd>작업자 노드의 이름입니다. <code>ibmcloud ks workers --cluster <em>CLUSTER</em></code>를 실행하여 클러스터의 작업자 노드에 대한 ID를 볼 수 있습니다. 이 값은 필수입니다.</dd>
...
클러스터에서 작업자 노드를 다시 부팅합니다. 다시 부팅 중에 작업자 노드의 상태가 변경되지 않습니다. 예를 들어, IBM Cloud 인프라(SoftLayer)의 작업자 노드 상태가 `Powered Off`이며 사용자가 작업자 노드를 켜야 하는 경우에는 재부팅을 사용할 수 있습니다. 재부팅은 임시 디렉토리를 지우지만 전체 파일 시스템을 지우거나 디스크를 다시 포맷하지는 않습니다. 작업자 노드 IP 주소는 다시 부팅 오퍼레이션 후에도 동일하게 유지됩니다.
{: shortdesc}

작업자 노드를 다시 부팅하면 작업자 노드에서 데이터 손상이 발생할 수 있습니다. 이 명령은 다시 부팅하면 작업자 노드를 복구하는 데 도움이 된다고 알고 있는 경우에 주의하여 사용하십시오. 다른 모든 경우에는 대신 [작업자 노드를 다시 로드](#cs_worker_reload)하십시오.
{: important}
...
kubectl get nodes
   ```
   {: pre}
   이 명령에서 리턴되는 **이름**은 작업자 노드에 지정된 사설 IP 주소입니다. `ibmcloud ks workers --cluster <cluster_name_or_ID>` 명령을 실행하고 동일한 **사설 IP** 주소로 작업자 노드를 검색하면 작업자 노드에 대한 추가 정보를 찾을 수 있습니다.
...
5. 작업자 노드를 다시 부팅하십시오. `ibmcloud ks workers --cluster <cluster_name_or_ID>` 명령에서 리턴되는 작업자 ID를 사용하십시오.
  ```
...
작업자 노드의 구성을 다시 로드합니다. 다시 로드는 작업자 노드에 성능 저하와 같은 문제점이 발생하거나 작업자 노드가 비정상적인 상태인 경우 유용할 수 있습니다. 다시 로드하는 중에 작업자 노드 머신은 최신 이미지로 업데이트되며 [작업자 노드의 외부에 저장](/docs/containers?topic=containers-storage_planning#persistent_storage_overview)되지 않은 경우 데이터가 삭제됨을 유념하십시오. 작업자 노드 공용 및 사설 IP 주소는 다시 로드 오퍼레이션 후에도 동일하게 유지됩니다.
{: shortdesc}
...
상태가 **SchedulingDisabled**로 표시되는 경우 작업자 노드가 팟(Pod) 스케줄링에 사용 불가능합니다.
 4. 작업자 노드에서 팟(Pod)을 제거하고 클러스터에 남아 있는 작업자 노드로 다시 스케줄하도록 강제 실행하십시오.
    ...
    {: pre}
    이 프로세스에는 몇 분 정도 소요될 수 있습니다.
 5. 작업자 노드를 다시 로드하십시오. `ibmcloud ks workers --cluster <cluster_name_or_ID>` 명령에서 리턴되는 작업자 ID를 사용하십시오.
    ```
...
4. 작업자 노드에서 팟(Pod)을 제거하고 클러스터에 남아 있는 작업자 노드로 다시 스케줄하도록 강제 실행하십시오.
   ...
   {: pre}
   이 프로세스에는 몇 분 정도 소요될 수 있습니다.
5. 작업자 노드를 제거하십시오. `ibmcloud ks workers --cluster <cluster_name_or_ID>` 명령에서 리턴되는 작업자 ID를 사용하십시오.
   ```
...
작업자 노드를 업데이트하여 운영 체제에 최신 보안 업데이트 및 패치를 적용하고, Kubernetes 마스터의 버전과 일치하도록 Kubernetes 버전을 업데이트합니다. `ibmcloud ks cluster-update` [명령](/docs/containers?topic=containers-cli-plugin-kubernetes-service-cli#cs_cluster_update)으로 마스터 Kubernetes 버전을 업데이트할 수 있습니다. 작업자 노드 IP 주소는 업데이트 오퍼레이션 후에도 동일하게 유지됩니다.
{: shortdesc}
...
이 명령의 그룹을 사용하여 사용 가능한 위치를 보고, 클러스터에 대한 작업자 풀을 수정합니다.
{: shortdesc}
...
작업자 노드를 삭제한 후에 작업자 풀을 리밸런싱합니다. 이 명령을 실행하는 경우 작업자 풀에 사용자가 지정한 동일한 수의 노드가 있도록 새 작업자가 작업자 풀에 추가됩니다.
{: shortdesc}
...
작업자 풀의 크기를 조정하여 클러스터의 각 구역에 있는 작업자 노드의 수를 늘리거나 줄입니다. 작업자 풀에는 최소한 한 개의 작업자 노드가 있어야 합니다.
{: shortdesc}
...
클러스터 또는 작업자 풀을 작성한 후에 구역을 추가할 수 있습니다. 구역을 추가하면 작업자 풀에 대해 지정한 구역당 작업자 수와 일치하도록 작업자 노드가 새 구역에 추가됩니다. 클러스터가 다중 구역 메트로에 있는 경우 두 개 이상의 구역만 추가할 수 있습니다.
{: shortdesc}
...
<dt><code>--private-only </code></dt>
<dd>공용 VLAN이 작성되지 않도록 하려면 이 옵션을 사용하십시오. `--private-vlan` 플래그를 지정하고 `--public-vlan` 플래그를 포함하지 않은 경우에만 필요합니다.<p class="note">작업자 노드가 사설 VLAN 전용으로 설정된 경우에는 개인 서비스 엔드포인트를 사용으로 설정하거나 게이트웨이 디바이스를 구성해야 합니다. 자세한 정보는 [사설 클러스터 및 작업자 노드 설정 계획](/docs/containers?topic=containers-plan_clusters#private_clusters)을 참조하십시오.</p></dd>
...
<dt><code>--private-only </code></dt>
<dd>선택사항: 이 구역의 작업자가 사설 VLAN에만 연결되도록 공용 VLAN을 설정 해제합니다. </dd>
...
<li>사용할 공용 및 사설 VLAN ID가 호환 가능한지 확인하십시오. 호환 가능하려면 <strong>라우터</strong>의 팟(Pod) ID가 동일해야 합니다. 사설 VLAN 라우터는 항상 <code>bcr</code>(벡엔드 라우터)로 시작하고 공용 VLAN 라우터는 항상 <code>fcr</code>(프론트 엔드 라우터)로 시작합니다. 클러스터를 작성하고 공인 및 사설 VLAN을 지정할 때는 이러한 접두부 뒤의 숫자 및 문자 조합이 일치해야 합니다.<pre class="pre"><code>ibmcloud ks vlans --zone &lt;zone&gt;</code></pre><p>출력 예:</p>
