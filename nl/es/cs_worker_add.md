...
Antes de comenzar, asegúrese de tener el [rol de plataforma **Operador** o **Administrador** de {{site.data.keyword.cloud_notm}} IAM](/docs/containers?topic=containers-users#platform). A continuación, elija una de las secciones siguientes:
...
3.  Para cada zona, revise los [Versiones disponibles para los nodos trabajadores](/docs/containers?topic=containers-planning_worker_nodes#planning_worker_nodes).
...
3. Obtenga una lista de las versiones disponibles en dicha zona.
   ```
   ibmcloud ks flavors --zone <zone>
   ```
   {: pre}

4. Añada nodos trabajadores autónomos al clúster. Para las versiones nativas, especifique `dedicated`.
   ```
