...
![Registro de códigos fuentes](images/log_sources.png)

<ol>
<li><p><code>application</code>: información sobre los sucesos que se producen a nivel de aplicación. Puede ser la notificación de que se ha producido un suceso, como un inicio de sesión correcto, un aviso sobre almacenamiento u otras operaciones que se puedan realizar a nivel de app.</p> <p>Vías de acceso: puede definir las vías de acceso a las que se reenvían los registros. Sin embargo, para que se puedan enviar los registros, se debe utilizar una vía de acceso absoluta en la configuración de registro o, de lo contrario, no se podrán leer los registros. Si la vía de acceso está montada en su nodo trabajador, podría haber creado un enlace simbólico. Ejemplo: Si la vía de acceso especificada es <code>/usr/local/<b>spark</b>/work/app-0546/0/stderr</code> pero los registros en realidad van a <code>/usr/local/<b>spark-1.0-hadoop-1.2</b>/work/app-0546/0/stderr</code>, entonces no se podrán leer los registros.</p></li>

<li><p><code>container</code>: información que registra un contenedor en ejecución.</p> <p>Vías de acceso: todo lo que se escribe en <code>STDOUT</code> o <code>STDERR</code>.</p></li>

<li><p><code>ingress</code>: información sobre el tráfico de red que entra en un clúster a través del equilibrador de carga de aplicación de Ingress. Para ver información específica de configuración, consulte la [documentación de Ingress](cs_ingress.html#ingress_log_format).</p> <p>Vías de acceso: <code>/var/log/alb/ids/&ast;.log</code> <code>/var/log/alb/ids/&ast;.err</code>, <code>/var/log/alb/customerlogs/&ast;.log</code>, <code>/var/log/alb/customerlogs/&ast;.err</code></p></li>

<li><p><code>kube-audit</code>: información sobre las acciones relacionadas con el clúster que se envía al servidor de API de Kubernetes; incluye la hora, el usuario y el recurso afectado.</p></li>

<li><p><code>kubernetes</code>: información de kubelet, kube-proxy y otros sucesos de Kubernetes que se producen en el nodo trabajador y que se ejecutan en el espacio de nombres kube-system.</p><p>Vías de acceso: <code>/var/log/kubelet.log</code>, <code>/var/log/kube-proxy.log</code>, <code>/var/log/event-exporter/*.log</code></p></li>

<li><p><code>worker</code>: información específica de la configuración de la infraestructura que tiene para el nodo trabajador. Los registros de nodo trabajador se capturan en syslog y contienen sucesos del sistema operativo. En auth.log encontrará información sobre las solicitudes de autenticación que se realizan en el sistema operativo. </p><p>Vías de acceso: <code>/var/log/syslog</code> y <code>/var/log/auth.log</code></p></li>
</ol>

</br>

**¿De qué opciones de configuración dispongo?**
...
2. Elija como [destino de la CLI](cs_cli_install.html#cs_cli_configure) el clúster en el que se encuentra el origen de registro. Si utiliza una cuenta dedicada, debe iniciar sesión en el punto final de {{site.data.keyword.cloud_notm}} público y definir como objetivo el espacio y la organización públicos para permitir el reenvío de registros.
...
4. Elija como [destino de la CLI](cs_cli_install.html#cs_cli_configure) el clúster en el que se encuentra el origen de registro. Si utiliza una cuenta dedicada, debe iniciar sesión en el punto final de {{site.data.keyword.cloud_notm}} público y definir como objetivo el espacio y la organización públicos para permitir el reenvío de registros.
