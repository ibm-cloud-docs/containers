...
**¿Qué se suministra con Ingress?**</br>
...
**¿Cómo llega a mi app una solicitud con Ingress en un clúster de una sola zona?**</br>
...
**¿Cómo llega a mi app una solicitud con Ingress en un clúster multizona?**</br>
...
2. Un servicio del sistema DNS, que actúa como el equilibrador de carga global, resuelve el nombre de host en el URL como una dirección IP que MZLB ha detectado que está en buen estado. MZLB comprueba continuamente las direcciones IP públicas portátiles de los servicios del equilibrador de carga que exponen ALB públicos en el clúster. Las direcciones IP se resuelven en un ciclo en rueda, lo que garantiza que las solicitudes se equilibran equitativamente entre los ALB en buen estado de varias zonas.
...
2. Obtenga el dominio proporcionado por IBM en el campo **Subdominio de Ingress**. Si desea utilizar TLS, obtenga también el secreto de TLS proporcionado por IBM en el campo **Secreto Ingress**.
    **Nota**: para obtener información sobre la certificación de TLS comodín, consulte [esta nota](#wildcard_tls).
...
2.  Configure el dominio de modo que direccione el tráfico de red de entrada al ALB proporcionado por IBM. Puede elegir entre las siguientes opciones:
    -   Defina un alias para el dominio personalizado especificando el dominio proporcionado por IBM como CNAME (Registro de nombre canónico). Para encontrar el dominio de Ingress proporcionado por IBM, ejecute `ibmcloud ks cluster-get <cluster_name>` y busque el campo **Subdominio de Ingress**.
...
3.   Opcional: para utilizar TLS, importe o cree un certificado TLS y un secreto de clave. Si utiliza un dominio comodín, asegúrese de importar o de crear un certificado comodín en el espacio de nombres <code>default</code> de forma que el ALB pueda acceder y utilizar el certificado en cada espacio de nombres.
...
1. Obtenga los detalles del clúster. Sustituya _&lt;cluster_name_or_ID&gt;_ con el nombre del clúster en el que se despliegan las apps que desea exponer.
    ...
    ```
    {: screen}
2. Obtenga el dominio proporcionado por IBM en el campo **Subdominio de Ingress**. Si desea utilizar TLS, obtenga también el secreto de TLS proporcionado por IBM en el campo **Secreto Ingress**. **Nota**: para obtener información sobre la certificación de TLS comodín, consulte [esta nota](#wildcard_tls).
...
## Habilitación de un ALB privado predeterminado
{: #private_ingress}
...
### Aumento del tiempo de conexión de estado activo (keepalive)
{: #keepalive_time}

Las conexiones de estado activo pueden tener un gran impacto en el rendimiento al reducir la sobrecarga de CPU y de red necesaria para abrir y cerrar conexiones. Para optimizar el rendimiento de los ALB, puede cambiar el valor predeterminado del tiempo de estado activo para las conexiones entre el ALB y el cliente.
{: shortdesc}

En el mapa de configuración de Ingress `ibm-cloud-provider-ingress-cm`, el campo `keep-alive` establece el tiempo de espera, en segundos, durante el cual la conexión del cliente en estado activo permanece abierta para el ALB de Ingress. De forma predeterminada, el campo `keep-alive` tiene el valor `8s`. Puede modificar el valor predeterminado editando el mapa de configuración de Ingress.
...
2. Cambie el valor de `keep-alive` de `8s` por un valor mayor.
...
## Configuración de controlador de Ingress gestionado por el usuario
{: #user_managed}

Puede ejecutar su propio controlador de Ingress en {{site.data.keyword.Bluemix_notm}} a la vez que aprovecha el subdominio de Ingress proporcionado por IBM y el certificado TLS asignado a su clúster.
{: shortdesc}

La configuración de su propio controlador de Ingress personalizado puede resultar útil cuando tiene requisitos de Ingress específicos. Cuando ejecute su propio controlador de Ingress en lugar del ALB de Ingress proporcionado por IBM, es responsable de suministrar la imagen del controlador, de mantener el controlador y de actualizar el controlador.
...
3. Prepare el archivo de configuración para el controlador de Ingress. Por ejemplo, puede utilizar el archivo de configuración YAML para el [controlador de Ingress nginx community![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://github.com/kubernetes/ingress-nginx/blob/master/deploy/mandatory.yaml).
