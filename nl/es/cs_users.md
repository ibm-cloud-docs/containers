...
1.  [Elija la política de acceso y el rol correctos para los usuarios](#access_roles)
2.  [Asigne roles de acceso a individuos o a grupos de usuarios en {{site.data.keyword.Bluemix_notm}} IAM](#iam_individuals_groups)
3.  [Limite el acceso de los usuarios a instancias del clúster o a grupos de recursos](#resource_groups)
...
### Elija la política de acceso y el rol correctos para los usuarios
{: #access_roles}
...
Obtenga información sobre los distintos tipos de permisos y roles, qué rol puede realizar cada acción y cómo se relacionan los roles entre sí.

Para ver los permisos de {{site.data.keyword.containerlong_notm}} específicos de cada rol, consulte el tema de referencia [Permisos de acceso de usuario](cs_access_reference.html).
{: tip}

<dl>

<dt><a href="#platform">Plataforma IAM</a></dt>
<dd>{{site.data.keyword.containerlong_notm}} utiliza roles de {{site.data.keyword.Bluemix_notm}} Identity and Access Management (IAM) para otorgar a los usuarios acceso al clúster. Los roles de la plataforma IAM determinan las acciones que pueden realizar los usuarios sobre un clúster. Puede establecer las políticas para estos roles por región. A cada usuario que tiene asignado un rol de la plataforma IAM también se le asigna automáticamente un rol RBAC correspondiente en el espacio de nombres de Kubernetes `default`. Además, los roles de la plataforma IAM le autorizan a realizar acciones de la infraestructura en el clúster, pero no a otorgar acceso a recursos de la infraestructura IBM Cloud (SoftLayer). El acceso a los recursos de la infraestructura de IBM Cloud (SoftLayer) viene determinado por la [clave de API que se ha establecido para la región](#api_key).</br></br>
Algunas acciones de ejemplo que están permitidas por los roles de la plataforma IAM son crear o eliminar clústeres, enlazar servicios a un clúster o añadir nodos trabajadores adicionales.</dd>
<dt><a href="#role-binding">RBAC</a></dt>
<dd>En Kubernetes, el control de acceso basado en rol (RBAC) es una forma de proteger los recursos dentro de su clúster. Los roles RBAC determinan las acciones de Kubernetes que los usuarios pueden llevar a cabo en dichos recursos. A cada usuario que tiene asignado un rol de la plataforma IAM también se le asigna automáticamente un rol de clúster RBAC correspondiente en el espacio de nombres de Kubernetes `default`. Este rol de clúster RBAC se aplica en el espacio de nombres predeterminado o en todos los espacios de nombres, en función del rol de plataforma IAM que elija.</br></br>
...
<dd>Los roles de infraestructura permiten el acceso a los recursos de la infraestructura de IBM Cloud (SoftLayer). Configure un usuario con el rol de infraestructura **Superusuario** y almacene las credenciales de infraestructura de este usuario en una clave de API. A continuación, establezca la clave de API en cada región en la que desee crear clústeres. Después de configurar la clave de API, otros usuarios a los que otorgue acceso a {{site.data.keyword.containerlong_notm}} no necesitarán roles de infraestructura, ya que la clave de API se comparte entre todos los usuarios de la región. En su lugar, los roles de la plataforma {{site.data.keyword.Bluemix_notm}} IAM determinan las acciones de infraestructura que los usuarios pueden llevar a cabo. Si no configura la clave de API con la infraestructura completa de <strong>Superusuario</strong> o tiene que otorgar acceso a un dispositivo específico a los usuarios, puede [personalizar los permisos de infraestructura](#infra_access). </br></br>
...
Algunas acciones de ejemplo que están permitidas por los roles de Cloud Foundry son crear una nueva instancia de servicio Cloud Foundry o enlazar una instancia de servicio de Cloud Foundry a un clúster. Para obtener más información, consulte los [roles de espacio y organización](/docs/iam/cfaccess.html) disponibles o los pasos para [gestionar el acceso a Cloud Foundry](/docs/iam/mngcf.html) en la documentación de {{site.data.keyword.Bluemix_notm}} IAM.</dd>
</dl>

### Asigne roles de acceso a individuos o a grupos de usuarios en {{site.data.keyword.Bluemix_notm}} IAM
...
### Limite el acceso de los usuarios a instancias del clúster o a grupos de recursos
{: #resource_groups}

En {{site.data.keyword.Bluemix_notm}} IAM, puede asignar roles de acceso de usuario a instancias de recursos o a grupos de recursos.
{: shortdesc}

Cuando crea su cuenta de {{site.data.keyword.Bluemix_notm}}, se crea automáticamente un grupo de recursos predeterminado. Si no especifica un grupo de recursos al crear el recurso, las instancias de recursos (clústeres) pertenecen al grupo de recursos predeterminado. Si desea añadir un grupo de recursos en su cuenta, consulte [Prácticas recomendadas para configurar su cuenta ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](/docs/tutorials/users-teams-applications.html) y [Configuración de grupos de recursos](/docs/resources/bestpractice_rgs.html#setting-up-your-resource-groups).
...
<dt>Grupo de recursos</dt>
  ...
  <p>Si tiene previsto utilizar [{{site.data.keyword.monitoringlong_notm}} para métricas](cs_health.html#view_metrics), considere la posibilidad de asignar nombres exclusivos a los clústeres de los grupos de recursos y a las regiones de la cuenta para evitar conflictos de nombres de métricas. No se puede cambiar el nombre de un clúster.</p>
...
3. Para permitir a los usuarios enlazar servicios con el clúster o ver los registros que se reenvían desde las configuraciones de registro del clúster, [otorgue a los usuarios roles de Cloud Foundry](/docs/iam/mngcf.html) para la organización y el espacio en los que se despliegan los servicios o donde se recopilan los registros.
4. Si utiliza espacios de nombres de Kubernetes para aislar recursos dentro del clúster, [copie los enlaces de rol RBAC de Kubernetes para los roles de **Visor** y de **Editor** {{site.data.keyword.Bluemix_notm}} de la plataforma IAM en otros espacios de nombres](#role-binding).
...
6. Para otras configuraciones avanzadas para controlar el acceso a los recursos de clúster en el nivel de pod, consulte [Configuración de la seguridad de pod](/docs/containers/cs_psp.html).

</br>

Para obtener más información sobre cómo configurar la cuenta y los recursos, consulte esta guía de aprendizaje acerca de las [prácticas recomendadas para organizar usuarios, equipos y aplicaciones](/docs/tutorials/users-teams-applications.html).
{: tip}
...
Para suministrar correctamente clústeres y trabajar con ellos, debe asegurarse de que la cuenta de {{site.data.keyword.Bluemix_notm}} esté correctamente configurada para acceder al portafolio de infraestructura de IBM Cloud (SoftLayer).
{: shortdesc}
...
6. Repita el proceso para cada región y grupo de recursos en los que desea crear clústeres.
...
<caption>Opciones de creación de clúster estándar por tipo de cuenta</caption>
  ...
  <tbody>
    <tr>
      ...
      <td>[Actualice su cuenta Lite a una cuenta de pago según uso de {{site.data.keyword.Bluemix_notm}}](/docs/account/index.html#paygo).</td>
    </tr>
    <tr>
...
 
Para permitir que todos los usuarios puedan acceder al potfolio de infraestructura de IBM Cloud (SoftLayer), el usuario cuyas credenciales están almacenadas en la clave de API debe tener [el rol de infraestructura **Superusuario** y el rol de plataforma **Administrador** para {{site.data.keyword.containerlong_notm}} y para {{site.data.keyword.registryshort_notm}}](#owner_permissions) en su cuenta de {{site.data.keyword.Bluemix_notm}}. A continuación, deje que el usuario realice la primera acción de administración en una región y grupo de recursos. Las credenciales de infraestructura del usuario se almacenan en una clave de API para dicha región y grupo de recursos.

Otros usuarios de la cuenta comparten la clave de API para acceder a la infraestructura. Cuando los usuarios inician sesión en la cuenta de
{{site.data.keyword.Bluemix_notm}}, se genera una señal {{site.data.keyword.Bluemix_notm}} IAM que se basa en la clave de API para la sesión de CLI y permite que se ejecuten mandatos relacionados con la infraestructura en un clúster.

Para ver la señal {{site.data.keyword.Bluemix_notm}} IAM para una sesión de CLI, puede ejecutar
`ibmcloud iam oauth-tokens`. Las señales {{site.data.keyword.Bluemix_notm}} IAM también se pueden utilizar para [realizar llamadas directamente a la API de {{site.data.keyword.containerlong_notm}}](cs_cli_install.html#cs_api).
{: tip}

**Si los usuarios tienen acceso al portfolio a través de una señal
{{site.data.keyword.Bluemix_notm}}, ¿cómo limito qué mandatos puede ejecutar un usuario?**

Tras configurar el acceso al portfolio para los usuarios de su cuenta, puede controlar las acciones de la infraestructura que pueden llevar a cabo los usuarios asignando el [rol de la plataforma](#platform) adecuado. Al asignar roles de {{site.data.keyword.Bluemix_notm}} IAM a los usuarios, se establecen límites en cuanto a los mandatos que pueden ejecutar en un clúster. Por ejemplo, debido a que el propietario de la clave de API tiene el rol de infraestructura **Superusuario**, todos los mandatos relacionados con la infraestructura se pueden ejecutar en un clúster. Sin embargo, en función del rol de {{site.data.keyword.Bluemix_notm}} IAM que se haya asignado a un usuario, el usuario podrá ejecutar solo algunos de esos mandatos relacionados con la infraestructura.
...
Para auditar las acciones que ejecutan los usuarios de la cuenta, puede utilizar [{{site.data.keyword.cloudaccesstrailshort}}](cs_at_events.html) para ver todos los sucesos relacionados con el clúster.
{: tip}
...
Si una clave de API establecida para una región y grupo de recursos del clúster se ve comprometida, [suprímala](../iam/userid_keys.html#deleting-an-api-key) para que no se realicen más llamadas utilizando la clave de API como autenticación. Para obtener más información sobre cómo proteger el acceso al servidor de API de Kubernetes, consulte el tema de seguridad [Servidor de API Kubernetes y etcd](cs_secure.html#apiserver).
...
1. Inicie una sesión en la [consola de {{site.data.keyword.Bluemix_notm}}![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://console.bluemix.net/).
...
2. Para asegurarse de que todas las acciones relacionadas con la cuenta se pueden realizar satisfactoriamente, verifique que el usuario tenga los roles correctos de la plataforma {{site.data.keyword.Bluemix_notm}} IAM.
    ...
    2. Pulse el nombre del usuario que desea que establezca la clave de API o cuyas credenciales desea establecer para la clave de API.
...
3. Para asegurarse de que todas las acciones relacionadas con la infraestructura del clúster se pueden realizar satisfactoriamente, verifique que el usuario tenga las políticas correctas de acceso de la infraestructura.
    1. En el menú ![Icono de menú](../icons/icon_hamburger.svg "Icono de menú"), seleccione **Infraestructura**.
    2. En la barra de menús, seleccione **Cuenta** > **Usuarios** > **Lista de usuarios**.
    3. En la columna **Clave de API**, verifique que el usuario tiene una clave de API o pulse **Generar**.
    4. Seleccione el nombre del perfil de usuario y compruebe los permisos del usuario.
    5. Si el usuario no tiene el rol de **Superusuario**, pulse el separador **Permisos del portal**.
        1. Utilice la lista desplegable **Permisos rápidos** para asignar el rol de **Superusuario**.
        2. Pulse **Establecer permisos**.
...
Puede encontrar el propietario de clave de API actual ejecutando [`ibmcloud ks api-key-info`](cs_cli_reference.html#cs_api_key_info). Si necesita actualizar la clave de API que hay almacenada para una región, puede hacerlo mediante la ejecución del mandato [`ibmcloud ks api-key-reset`](cs_cli_reference.html#cs_api_key_reset). Este mandato requiere la política de acceso de administrador de {{site.data.keyword.containerlong_notm}} y almacena la clave de API del usuario que ejecuta este mandato en la cuenta.
...
2. [Cree un clúster](cs_clusters.html). Para crear el clúster, se utilizan las credenciales de clave de API que ha establecido para la región y el grupo de recursos.

### Acceso a otra cuenta de infraestructura de IBM Cloud (SoftLayer)
{: #credentials}

En lugar de utilizar la cuenta enlazada predeterminada de infraestructura de IBM Cloud (SoftLayer) para solicitar la infraestructura para clústeres dentro de una región, es posible que desee otra cuenta de infraestructura de IBM Cloud (SoftLayer) que ya tenga. Puede enlazar esta cuenta de infraestructura a la cuenta de {{site.data.keyword.Bluemix_notm}} mediante el mandato [`ibmcloud ks credential-set`](cs_cli_reference.html#cs_credentials_set). Se utilizan las credenciales de la infraestructura de IBM Cloud (SoftLayer) en lugar de las credenciales de la cuenta de pago según uso predeterminada almacenadas para la región.
{: shortdesc}

Las credenciales de la infraestructura de IBM Cloud (SoftLayer) establecidas mediante el mandato `ibmcloud ks credential-set` permanecen después de que finalice la sesión. Si elimina las credenciales de la infraestructura de IBM Cloud (SoftLayer) establecidas manualmente con el mandato [`ibmcloud ks credential-unset`](cs_cli_reference.html#cs_credentials_unset), se utilizan las credenciales de la cuenta de pago según uso predeterminada. Sin embargo, este cambio en las credenciales de la cuenta de infraestructura puede provocar [clústeres huérfanos](cs_troubleshoot_clusters.html#orphaned).
{: important}
...
3. [Cree un clúster](cs_clusters.html). Para crear el clúster, se utilizan las credenciales de la infraestructura que ha establecido para la región y el grupo de recursos.

4. Verifique que el clúster utiliza las credenciales de la cuenta de infraestructura que ha establecido.
  1. Abra la [consola de {{site.data.keyword.containerlong_notm}} ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://console.bluemix.net/containers-kubernetes/clusters) y seleccione el clúster.
...
Otorgue a los usuarios acceso a sus clústeres asignando roles de gestión de la plataforma {{site.data.keyword.Bluemix_notm}} IAM con la consola de {{site.data.keyword.Bluemix_notm}}.
{: shortdesc}
...
1. Inicie una sesión en la [consola de {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/) y vaya a ** Gestionar > Cuenta > Usuarios**.
...
2. Seleccione los usuarios individualmente o cree un grupo de acceso de usuarios.
    * Para asignar roles a un usuario individual:
      1. Pulse el nombre del usuario cuyos permisos desea establecer. Si el usuario no se muestra, pulse **Invitar a usuarios** para añadirlo a la cuenta.
      2. Pulse **Asignar acceso**.
    * Para asignar roles a varios usuarios de un grupo de acceso:
      1. En el panel de navegación de la izquierda, pulse **Grupos de acceso**.
      ...
      6. Pulse **Políticas de acceso**.
      7. Pulse **Asignar acceso**.

3. Asigne una política.
  * Para el acceso a todos los clústeres de un grupo de recursos:
    1. Pulse **Asignar acceso dentro de un grupo de recursos**.
    2. Seleccione el nombre del grupo de recursos.
    3. En la lista **Servicios**, seleccione **{{site.data.keyword.containershort_notm}}**.
    4. En la lista **Región**, seleccione una región o todas ellas.
    5. Seleccione un **Rol de acceso de plataforma**. Para ver una lista de las acciones admitidas por rol, consulte [Permisos de acceso de usuario](/cs_access_reference.html#platform).
    6. Pulse **Asignar**.
  * Para acceder a un clúster de un grupo de recursos o a todos los clústeres de todos los grupos de recursos:
    1. Pulse **Asignar acceso a recursos**.
    2. En la lista **Servicios**, seleccione **{{site.data.keyword.containershort_notm}}**.
    3. En la lista **Región**, seleccione una región o todas ellas.
    4. En la lista **Instancia de servicio**, seleccione un nombre de clúster o **Todas las instancias de servicio**.
    5. En la sección **Seleccionar roles**, elija un rol de acceso de la plataforma {{site.data.keyword.Bluemix_notm}} IAM. Para ver una lista de las acciones admitidas por rol, consulte [Permisos de acceso de usuario](/cs_access_reference.html#platform). Nota: si asigna a un usuario el rol de plataforma **Administrador** solo para un clúster, también debe asignar al usuario el rol de plataforma **Visor** para todos los clústeres de dicha región en el grupo de recursos.
    6. Pulse **Asignar**.

4. Si desea que los usuarios puedan trabajar con clústeres en un grupo de recursos que no sea el predeterminado, estos usuarios necesitan acceso adicional a los grupos de recursos en los que se encuentran los clústeres. Puede asignar a estos usuarios el rol de plataforma **Visor**, como mínimo, para los grupos de recursos.
  1. Pulse **Asignar acceso dentro de un grupo de recursos**.
  2. Seleccione el nombre del grupo de recursos.
...
Otorgue a los usuarios acceso a sus clústeres asignando roles de gestión de la plataforma {{site.data.keyword.Bluemix_notm}} IAM con la CLI.
{: shortdesc}

**Antes de empezar**:

- Verifique que tiene asignado el rol `cluster-admin` {{site.data.keyword.Bluemix_notm}} de la plataforma IAM para la cuenta de {{site.data.keyword.Bluemix_notm}} en la que está trabajando.
...
**Para asignar roles de {{site.data.keyword.Bluemix_notm}} IAM a un usuario individual con la CLI:**

1.  Cree una política de acceso de {{site.data.keyword.Bluemix_notm}} IAM para establecer permisos para {{site.data.keyword.containerlong_notm}} (**`--service-name containers-kubernetes`**). Puede elegir entre Visor, Editor, Operador y Administrador para el rol de plataforma. Para ver una lista de las acciones admitidas por rol, consulte [Permisos de acceso de usuario](cs_access_reference.html#platform).
    * Para asignar acceso a un clúster de un grupo de recursos:
      ```
      ...
      **Nota**: si asigna a un usuario el rol de plataforma **Administrador** solo para un clúster, también debe asignar al usuario el rol de plataforma **Visor** para todos los clústeres de la región en el grupo de recursos.

    * Para asignar acceso a todos los clústeres de un grupo de recursos:
      ```
    ...
    * Para asignar acceso a todos los clústeres de todos los grupos de recursos:
      ```
...
3. Para que los cambios entren en vigor, renueve la configuración del clúster.
    ```
    ibmcloud ks cluster-config --cluster <cluster_name_or_id>
    ```
    {: pre}

4. El rol de la plataforma {{site.data.keyword.Bluemix_notm}} IAM se aplica automáticamente como un [enlace de rol RBAC o enlace de rol de clúster](#role-binding) correspondiente. Verifique que el usuario se ha añadido al rol RBAC ejecutando uno de los mandatos siguientes para el rol de plataforma que ha asignado:
    * Visor:
        ```
  ...
  Por ejemplo, si asigna al usuario `john@email.com` el rol de plataforma **Visor** y ejecuta `kubectl get rolebinding ibm-view -o yaml -n default`, la salida es similar a la siguiente:
...
**Para asignar roles de la plataforma {{site.data.keyword.Bluemix_notm}} IAM a varios usuarios de un grupo de acceso con la CLI:**
...
2. Añada usuarios al grupo de acceso.
    ```
    ibmcloud iam access-group-user-add <access_group_name> <user_email>
    ```
    {: pre}

3. Cree una política de acceso de {{site.data.keyword.Bluemix_notm}} IAM para establecer permisos para {{site.data.keyword.containerlong_notm}}. Puede elegir entre Visor, Editor, Operador y Administrador para el rol de plataforma. Para ver una lista de las acciones admitidas por rol, consulte [Permisos de acceso de usuario](/cs_access_reference.html#platform).
  * Para asignar acceso a un clúster de un grupo de recursos:
      ```
      ...
      Si asigna a un usuario el rol de plataforma **Administrador** solo para un clúster, también debe asignar al usuario el rol de plataforma **Visor** para todos los clústeres de la región en el grupo de recursos.
      {: note}
...
4. Si desea que los usuarios puedan trabajar con clústeres en un grupo de recursos que no sea el predeterminado, estos usuarios necesitan acceso adicional a los grupos de recursos en los que se encuentran los clústeres. Puede asignar a estos usuarios el rol de **Visor**, como mínimo, para los grupos de recursos. Para encontrar el ID de grupo de recursos, ejecute `ibmcloud resource group <resource_group_name> --id`.
    ...
    1. Si ha asignado acceso a todos los clústeres de todos los grupos de recursos, repita este mandato para cada grupo de recursos de la cuenta.

5. Para que los cambios entren en vigor, renueve la configuración del clúster.
    ```
    ibmcloud ks cluster-config --cluster <cluster_name_or_id>
    ```
    {: pre}

6. El rol de la plataforma {{site.data.keyword.Bluemix_notm}} IAM se aplica automáticamente como un [enlace de rol RBAC o enlace de rol de clúster](#role-binding) correspondiente. Verifique que el usuario se ha añadido al rol RBAC ejecutando uno de los mandatos siguientes para el rol de plataforma que ha asignado:
    * Visor:
        ```
        kubectl get rolebinding ibm-view -o yaml -n default
        ```
  ...
  Por ejemplo, si asigna al grupo de acceso `team1` el rol de plataforma **Visor** y ejecuta `kubectl get rolebinding ibm-view -o yaml -n default`, la salida es similar a la siguiente:
  ```
  apiVersion: rbac.authorization.k8s.io/v1
  kind: RoleBinding
  metadata:
        ...
        <caption>Visión general de los componentes del archivo YAML</caption>
          ...
          <tbody>
            ...
            <tr>
              ...
              <li>`Group`: para los clústeres que ejecutan Kubernetes 1.11 o posterior, enlace el rol de RBAC o el rol de clúster a un [grupo de acceso de {{site.data.keyword.Bluemix_notm}} IAM](/docs/iam/groups.html#groups) de la cuenta.</li>
            ...
            <tr>
              ...
              <td><ul><li>Para `User`: añada la dirección de correo electrónico del usuario individual a uno de los siguientes URL.<ul><li>Para los clústeres que ejecutan Kubernetes 1.11 o posteriores: <code>IAM#user@email.com</code></li><li>Para los clústeres que ejecutan Kubernetes 1.10 o anteriores: <code>https://iam.ng.bluemix.net/kubernetes#user@email.com</code></li></ul></li>
              <li>Para `Group`: para clústeres que ejecutan Kubernetes 1.11 o posterior, especifique el nombre del [grupo de acceso de {{site.data.keyword.Bluemix_notm}} IAM](/docs/iam/groups.html#groups) en la cuenta.</li>
...
A cada usuario que tiene asignado un [rol de gestión de plataforma {{site.data.keyword.Bluemix_notm}} IAM](#platform) se le asigna automáticamente un rol de clúster RBAC correspondiente. Estos roles de clúster RBAC están predefinidos y permiten a los usuarios interactuar con los recursos de Kubernetes del clúster. Además, se crea un enlace de rol para aplicar el rol de clúster a un espacio de nombres específico, o se crea un enlace de rol de clúster para aplicar el rol de clúster a todos los espacios de nombres.

En la tabla siguiente se describen las relaciones entre los roles de la plataforma {{site.data.keyword.Bluemix_notm}} y los roles de clúster correspondientes y los enlaces de rol o los enlaces de rol de clúster que se crean automáticamente para los roles de la plataforma.

<table>
  <tr>
    <th>Rol de plataforma {{site.data.keyword.Bluemix_notm}} IAM</th>
    <th>Rol de clúster RBAC</th>
    <th>Enlace de rol RBAC</th>
    <th>Enlace de rol de clúster RBAC</th>
  </tr>
  <tr>
    <td>Visor</td>
    <td><code>view</code></td>
    <td><code>ibm-view</code> en el espacio de nombres predeterminado</td>
    <td>-</td>
  </tr>
  <tr>
    <td>Editor</td>
    <td><code>edit</code></td>
    <td><code>ibm-edit</code> en el espacio de nombres predeterminado</td>
    <td>-</td>
  </tr>
  <tr>
    <td>Operador</td>
...
Para obtener más información acerca de las acciones permitidas por cada rol RBAC, consulte el tema de referencia [Permisos de acceso de usuario](cs_access_reference.html#platform).
{: tip}

**¿Cómo puedo gestionar permisos RBAC para espacios de nombres específicos en mi clúster?**

Si utiliza [espacios de nombres de Kubernetes para particionar el clúster y proporcionar aislamiento para las cargas de trabajo](cs_secure.html#container), debe asignar al usuario acceso a espacios de nombres específicos. Cuando asigna a un usuario los roles de plataforma **Operador** o **Administrador**, los roles de clúster predefinidos `admin` y `cluster-admin` correspondientes se aplican automáticamente a todo el clúster. Sin embargo, cuando asigna a un usuario los roles de plataforma **Visor** o **Editor**, los roles de clúster predefinidos correspondientes `view` y `edit` se aplican automáticamente solo en el espacio de nombres predeterminado. Para imponer el mismo nivel de acceso de usuario en otros espacios de nombres, puede [copiar los enlaces de rol](#rbac_copy) para dichos roles de clúster, `ibm-view` e `ibm-edit`, en otros espacios de nombres.

**¿Puedo crear roles de clúster o roles personalizados?**

Los roles de clúster `view`, `edit`, `admin` y `cluster-admin` son roles predefinidos que se crean automáticamente cuando asigna a un usuario el rol correspondiente de la plataforma {{site.data.keyword.Bluemix_notm}} IAM. Para otorgar otros permisos de Kubernetes, puede [crear permisos RBAC personalizados](#rbac).

**¿Cuándo debo utilizar los enlaces de rol de clúster y enlaces de rol que no están enlazados a los permisos de {{site.data.keyword.Bluemix_notm}} IAM que he establecido?**

Es posible que desee autorizar quién puede crear y actualizar los pods en el clúster. Mediante las [políticas de seguridad de pod](https://console.bluemix.net/docs/containers/cs_psp.html#psp), puede utilizar enlaces de rol de clúster existentes que vienen con el clúster o puede crear los suyos propios.

Es posible que también desee integrar complementos en el clúster. Por ejemplo, cuando [configure Helm en el clúster](cs_integrations.html#helm), debe crear una cuenta de servicio para Tiller en el espacio de nombres `kube-system` y un enlace de rol de clúster RBAC de Kubernetes para el pod `tiller-deploy`.

### Copia de un enlace de rol RBAC en otro espacio de nombres
{: #rbac_copy}

Algunos roles y roles de clúster solo se aplican a un espacio de nombres. Por ejemplo, los roles de clúster predefinidos `view` y `edit` solo se aplican automáticamente en el espacio de nombres `default`. Para imponer el mismo nivel de acceso de usuario en otros espacios de nombres, puede copiar los enlaces de rol para dichos roles o roles de clúster en otros espacios de nombres.
{: shortdesc}

Por ejemplo, supongamos que asigna al usuario "john@email.com" el rol de gestión de plataforma **Editor**. El rol de clúster RBAC predefinido `edit` se crea automáticamente en el clúster, y el enlace de rol `ibm-edit` aplica los permisos en el espacio de nombres `default`. Supongamos que desea que "john@email.com" también tenga acceso de Editor en el espacio de nombres de desarrollo; entonces copia el enlace de rol `ibm-edit` de `default` en `development`. **Nota**: debe copiar el enlace de rol cada vez que se añada un usuario a los roles `view` o `edit`.

1. Copie el enlace de rol de `default` en otro espacio de nombres.
    ```
    kubectl get rolebinding <role_binding_name> -o yaml | sed 's/default/<namespace>/g' | kubectl -n <namespace> create -f -
    ```
    {: pre}

    Por ejemplo, para copiar el enlace de rol `ibm-edit` en el espacio de nombres `testns`:
    ```
    kubectl get rolebinding ibm-edit -o yaml | sed 's/default/testns/g' | kubectl -n testns create -f -
    ```
    {: pre}

2. Verifique que el enlace de rol `ibm-edit` se ha copiado.
    ```
    kubectl get rolebinding -n <namespace>
    ```
    {: pre}
...
Los roles de clúster `view`, `edit`, `admin` y `cluster-admin` se crean automáticamente cuando asigna el rol de gestión de la plataforma {{site.data.keyword.Bluemix_notm}} IAM correspondiente. ¿Necesita que las políticas de acceso de clúster sean más granulares de lo que permiten estos permisos predefinidos? No hay problema. Puede crear roles de clúster y roles RBAC personalizados.
{: shortdesc}

Puede asignar roles RBAC y roles de clúster personalizados a usuarios individuales, grupos de usuarios (en clústeres que ejecutan Kubernetes v1.11 o posteriores) o cuentas de servicio. Cuando se crea un enlace para un grupo, este afecta a cualquier usuario que se añada o se elimine de dicho grupo. Cuando añade usuarios a un grupo, reciben los derechos de acceso del grupo además de los derechos de acceso individuales que les otorgue. Si se elimina, su acceso se revoca. **Nota**: no se pueden añadir cuentas de servicio a grupos de acceso.

Si desea asignar acceso a un proceso que se ejecuta en pods, como por ejemplo una cadena de herramientas de entrega continua, puede utilizar [cuentas del servicio Kubernetes ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://kubernetes.io/docs/reference/access-authn-authz/service-accounts-admin/). Si desea seguir una guía de aprendizaje en la que se muestra cómo configurar cuentas de servicio para Travis y Jenkins y cómo asignar roles RBAC personalizados a cuentas de servicio, consulte la publicación del blog sobre [Cuentas del servicio Kubernetes para utilizarlas en sistemas automatizados ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://medium.com/@jakekitchener/kubernetes-serviceaccounts-for-use-in-automated-systems-515297974982).

**Nota**: para evitar cambios de última hora, no modifique los roles de clúster predefinidos `view`, `edit`, `admin` ni `cluster-admin`.

**¿Debo crear un rol o un rol de clúster? ¿Lo tengo que aplicar con un enlace de rol o con un enlace de rol de clúster?**

* Para permitir que un usuario, un grupo de acceso o una cuenta de servicio acceda a un recurso dentro de un espacio de nombres específico, elija una de las combinaciones siguientes:
        ...
        <caption>Visión general de los componentes del archivo YAML</caption>
          ...
          <tbody>
            ...
            <tr>
              <td><code>subjects.name</code></td>
              <td><ul><li>Para `User`: añada la dirección de correo electrónico del usuario individual a uno de los siguientes URL.<ul><li>Para los clústeres que ejecutan Kubernetes 1.11 o posteriores: <code>IAM#user@email.com</code></li><li>Para los clústeres que ejecutan Kubernetes 1.10 o anteriores: <code>https://iam.ng.bluemix.net/kubernetes#user@email.com</code></li></ul></li>
              <li>Para `Group`: para clústeres que ejecutan Kubernetes 1.11 o posterior, especifique el nombre del [grupo de {{site.data.keyword.Bluemix_notm}} IAM](/docs/iam/groups.html#groups) en la cuenta.</li>
              <li>Para `ServiceAccount`: especifique el nombre de la cuenta de servicio.</li></ul></td>
            </tr>
            <tr>
              <td><code>subjects.apiGroup</code></td>
...
2. Pulse el nombre del usuario cuyos permisos desea establecer.

3. Pulse **Asignar acceso** y luego pulse **Asignar acceso a la cuenta de SoftLayer**.

4. Pulse el separador **Permisos de portal** para personalizar el acceso de usuario. Los permisos que los usuarios necesitan dependen de los recursos de infraestructura que necesitan utilizar. Tiene dos opciones para asignar acceso:
    * Utilice la lista desplegable **Permisos rápidos** para asignar uno de los siguientes roles predefinidos. Después de seleccionar un rol, pulse **Establecer permisos**.
        * **Usuario Solo ver** solo proporciona a los usuarios permisos para ver detalles de la infraestructura.
    ...
    * Seleccione permisos individuales en cada separador. Para revisar los permisos que se necesitan para realizar tareas comunes en {{site.data.keyword.containerlong_notm}}, consulte [Permisos de acceso de usuario](cs_access_reference.html#infra).
...
6.  En el separador **Acceso de dispositivos**, seleccione los dispositivos a los que desea otorgar acceso.
    ...
    * En la lista desplegable **Tipo de dispositivo**, puede otorgar acceso a **Todos los dispositivos** para que estos usuarios puedan trabajar con los tipos de máquina virtual y física (hardware nativo) para todos los nodos trabajadores.
...
7. Para guardar los cambios, pulse **Actualizar acceso de dispositivos**.
...
Antes de eliminar los permisos de acceso específicos de un usuario o eliminar el usuario de la cuenta completamente, asegúrese de que las credenciales de infraestructura del usuario no se utilizan para establecer la clave de API ni para el mandato
`ibmcloud ks credential-set`. De lo contrario, los demás usuarios de la cuenta podrían perder el acceso al portal de infraestructura de IBM Cloud (SoftLayer) y los mandatos relacionados con la infraestructura podrían fallar.
{: important}
...
2. Compruebe el propietario de la clave de API o las credenciales de infraestructura establecidas para dicha región y grupo de recursos.
    * Si utiliza la [clave de API para acceder al portfolio de infraestructura de IBM Cloud (SoftLayer)](#default_account):
        ```
        ibmcloud ks api-key-info --cluster <cluster_name_or_id>
        ```
        {: pre}
    * Si ha establecido las [credenciales de infraestructura para acceder al portfolio de infraestructura de IBM Cloud (SoftLayer)](#credentials):
        ```
        ibmcloud ks credential-get
        ```
        {: pre}
...
Si un usuario de la cuenta va a dejar la organización, debe eliminar los permisos de dicho usuario cuidadosamente para asegurarse de que no deja huérfanos clústeres ni otros recursos. Después, puede eliminar el usuario de la cuenta de
...
1. Determine qué clústeres ha creado el usuario.
    1.  Inicie una sesión en la [consola de {{site.data.keyword.containerlong_notm}} ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://console.bluemix.net/containers-kubernetes/clusters).
...
2. Para cada clúster que haya creado el usuario, siga estos pasos:
    ...
    2. Determine qué ocurre con la cuenta de infraestructura que ha utilizado el usuario para suministrar los clústeres después de que el usuario se vaya.
        ...
        * Si el usuario es propietario de la cuenta de infraestructura, se suprimirá la cuenta de infraestructura cuando el usuario se vaya. No puede seguir trabajando con estos clústeres. Para evitar que el clúster quede huérfano, el usuario debe suprimir los clústeres antes de que el usuario se vaya. Si el usuario se ha ido pero no se han suprimido los clústeres, deberá utilizar el mandato `ibmcloud ks credential-set` para cambiar las credenciales de infraestructura a la cuenta en la que se han suministrado los nodos trabajadores del clúster, y suprimir el clúster. Para obtener más información, consulte [No se ha podido modificar o suprimir la infraestructura en un clúster huérfano](cs_troubleshoot_clusters.html#orphaned).

3. Elimine el usuario de la cuenta de {{site.data.keyword.Bluemix_notm}}.
    1. Vaya a **Gestionar > Cuenta > Usuarios**.
    2. Pulse el nombre de usuario del usuario.
    3. En la entrada de tabla del usuario, pulse el menú de acciones y seleccione **Eliminar usuario**. Al eliminar un usuario, los roles de plataforma {{site.data.keyword.Bluemix_notm}} asignados al usuario, así como los roles de Cloud Foundry y los roles de infraestructura de IBM Cloud (SoftLayer) se eliminarán automáticamente.

4. Cuando se eliminan permisos de plataforma {{site.data.keyword.Bluemix_notm}} IAM, los permisos del usuario también se eliminan automáticamente de los roles RBAC predefinidos asociados. No obstante, si ha creado roles RBAC personalizados o roles de clúster, [elimine el usuario de dichos enlaces de rol RBAC o enlaces de rol de clúster](#remove_custom_rbac).

5. Si tiene una cuenta de Pago según uso enlazada automáticamente a su cuenta de {{site.data.keyword.Bluemix_notm}}, los roles de infraestructura de IBM Cloud (SoftLayer) del usuario se eliminarán automáticamente. Sin embargo, si tiene un [tipo distinto de cuenta](#understand_infra), es posible que necesite eliminar manualmente el usuario de la infraestructura de IBM Cloud (SoftLayer).
    1. En el menú ![Icono de menú](../icons/icon_hamburger.svg "Icono de menú") de la [consola de {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/), pulse **Infraestructura**.
    2. Vaya a **Cuenta > Usuarios > Lista de usuarios**.
    2. Busque una entrada de tabla del usuario.
        * Si no puede ver ninguna entrada para el usuario, el usuario ya se ha eliminado. No se necesita ninguna otra acción.
        * Si ve una entrada para el usuario, continúe con el paso siguiente.
    3. En la entrada de tabla del usuario, pulse sobre el menú Acciones.
    4. Seleccione **Cambiar estado de usuario**.
    5. En la lista de estados, seleccione **Inhabilitado**. Pulse **Guardar**.
...
1. Inicie una sesión en la [consola de {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/) y vaya a ** Gestionar > Cuenta > Usuarios**.
2. Pulse el nombre del usuario para el que desee eliminar permisos.
3. Pulse el separador **Grupo de acceso**.
4. En la entrada de tabla del grupo de acceso, pulse el menú de acciones y seleccione
**Eliminar usuario**. Cuando se elimine el usuario, los roles que se asignaron al grupo de acceso se eliminarán del usuario.

#### Elimine los permisos de la plataforma {{site.data.keyword.Bluemix_notm}} IAM y los permisos de RBAC predefinidos asociados
{: #remove_iam_rbac}

1. Inicie una sesión en la [consola de {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/) y vaya a ** Gestionar > Cuenta > Usuarios**.
2. Pulse el nombre del usuario para el que desee eliminar permisos.
3. En la entrada de tabla del permiso que desee eliminar, pulse sobre el menú de acciones.
...
#### Elimine permisos de RBAC personalizados
{: #remove_custom_rbac}
...
#### Elimine permisos de Cloud Foundry
...
1. Inicie una sesión en la [consola de {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/) y vaya a ** Gestionar > Cuenta > Usuarios**.
2. Pulse el nombre del usuario para el que desee eliminar permisos.
3. Pulse sobre el separador **Acceso de Cloud Foundry**.
...
#### Elimine permisos de infraestructura de IBM Cloud (SoftLayer)
{: #remove_infra}

Puede eliminar permisos de infraestructura de IBM Cloud (SoftLayer) para un usuario utilizando la consola de
{{site.data.keyword.Bluemix_notm}}.
{: shortdesc}

1. Inicie una sesión en la [consola de {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/).
2. En el menú ![Icono de menú](../icons/icon_hamburger.svg "Icono de menú"), pulse **Infraestructura**.
3. Pulse sobre la dirección de correo electrónico del usuario.
4. Pulse el separador **Permisos de portal**.
5. En cada separador, deseleccione permisos específicos.
6. Para guardar los cambios, pulse **Editar permisos del portal**.
7. En el separador **Acceso de dispositivos**, deseleccione dispositivos específicos.
8. Para guardar los cambios, pulse **Actualizar acceso de dispositivos**. Los permisos se degradarán después de unos minutos.
