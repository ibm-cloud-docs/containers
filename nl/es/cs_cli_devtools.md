...
Para ver una lista de las ubicaciones disponibles, ejecute <code>cs bx ubicaciones</code>.</td> 
    ...
    <dd>(No disponible para clústeres de tipo lite.) 
...
empiezan por <code>fcr</code> (direccionador frontal). La combinación de números y letras que hay tras estos prefijos debe coincidir para poder utilizar dichas VLAN al crear un clúster. No utilice VLAN públicas y privadas que no coincidan para crear un clúster.</p></li>
    ...
    <dd>(No disponible para clústeres de tipo lite.) 
        ...
        <li>Si ha creado un clúster antes en esta ubicación o ha creado una VLAN pública en {{site.data.keyword.BluSoftlayer_notm}} antes, debe especificar la VLAN pública.
...
**Ejemplos**:
  ...
  Ejemplo para un entorno de {{site.data.keyword.Bluemix_notm}} dedicado:
...
<strong>Opciones del mandato</strong>:<dl>
   <dt><code><em>CLUSTER</em></code></dt>
   <dd>(Obligatorio) El nombre o ID del clúster.</dd>
   </dl>
...
Ver una lista de los servicios vinculados a uno o a todos los espacios de nombres de Kubernetes de un clúster. Si no se especifica ninguna opción, se muestran los servicios correspondientes al espacio de nombres predeterminado.
...
Hacer que una subred de una cuenta de {{site.data.keyword.BluSoftlayer_notm}} esté disponible para un determinado clúster.
...
Definir credenciales de cuenta de {{site.data.keyword.BluSoftlayer_notm}} para la cuenta de {{site.data.keyword.Bluemix_notm}}. Estas credenciales le permiten acceder al portafolio de {{site.data.keyword.BluSoftlayer_notm}} a través de su cuenta de {{site.data.keyword.Bluemix_notm}}.
...
Eliminar las credenciales de cuenta de {{site.data.keyword.BluSoftlayer_notm}} de la cuenta de {{site.data.keyword.Bluemix_notm}}. Después de eliminar las credenciales, ya no podrá acceder al portafolio de {{site.data.keyword.BluSoftlayer_notm}} a través de su cuenta de {{site.data.keyword.Bluemix_notm}}.
...
Ver una lista de los mandatos y parámetros admitidos.
...
Inicialice el plug-in de {{site.data.keyword.containershort_notm}} o especifique la región en la que desea crear o acceder a clústeres de Kubernetes.
<strong>Opciones del mandato</strong>:<dl>
...
Ver una lista de los tipos de máquinas disponibles para los nodos trabajadores. Cada tipo de máquina incluye cantidad de CPU virtual, memoria y espacio de disco para cada nodo trabajador del clúster.
...
Ver una lista de las subredes disponibles en una cuenta de {{site.data.keyword.BluSoftlayer_notm}}.
...
Ver una lista de las VLAN públicas y privadas que están disponibles para una ubicación en su cuenta de {{site.data.keyword.BluSoftlayer_notm}}. Para ver una lista de las VLAN disponibles,
...
Ver una lista de los nodos trabajadores y el estado de cada uno en un clúster.
...
|-------------|------|
|Desplegando|El nodo Kubernetes maestro aún no se ha desplegado por completo. No puede acceder al clúster.|
|Pendiente|El nodo Kubernetes maestro se ha desplegado. Los nodos trabajadores se están suministrando y aún no están disponibles en el clúster. Puede acceder al clúster, pero no puede desplegar apps en el clúster.|
