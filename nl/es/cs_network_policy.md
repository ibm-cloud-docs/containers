...
Cada clúster de {{site.data.keyword.containerlong}} se configura con un plugin de red llamado Calico. Las políticas de red predeterminadas se configuran para proteger la interfaz de red pública de cada nodo trabajador en el clúster.
{: shortdesc}
...
Kubernetes y se aplican mediante mandatos `calicoctl`. Las políticas de Calico añaden las características siguientes.
  * Permitir o bloquear el tráfico de red en las interfaces de red especificadas, independientemente del CIDR o dirección IP de origen o destino del pod Kubernetes.
  * Permitir o bloquear el tráfico de red para pods entre espacios de nombres.
  * [Bloquear el tráfico de entrada a los servicios LoadBalancer o NodePort de Kubernetes](#block_ingress).
 ...
 <caption>Políticas predeterminadas de host de Calico para cada clúster</caption>
  ...
  <tbody>
   ...
   <tr>
      ...
      <td>Permite conexiones entrantes para sistemas de infraestructura de IBM Cloud específicos que se utilizan para gestionar los nodos trabajadores.</td>
   </tr>
   <tr>
...
También se crea una política predeterminada de Kubernetes que limita el acceso al Panel de control de Kubernetes. Las políticas de Kubernetes no se aplican al punto final de host sino que, por el contrario, al pod `kube-dashboard`. Esta política se aplica a clústeres que están conectados únicamente a VLAN privadas y a clústeres que están conectados a una VLAN privada y pública.
...
4. [Descargue la CLI de Calico ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://github.com/projectcalico/calicoctl/releases).
    ...
    Si está utilizando OS X, descargue la versión `-darwin-amd64`. Si utiliza Windows, instale la CLI de Calico en el mismo directorio que la CLI de {{site.data.keyword.cloud_notm}}. Esta configuración le ahorra algunos cambios en la vía de acceso a archivo cuando ejecute mandatos posteriormente. Asegúrese de guardar el archivo como `calicoctl.exe`.
    {: tip}
...
1. Defina una política de red pre-DNAT de Calico para el acceso de ingress (tráfico entrante) a los servicios de Kubernetes.
    ...
    * Si gestiona el tráfico destinado a un [NLB 2.0](/docs/containers?topic=containers-loadbalancer-about#planning_ipvs), debe incluir los campos `applyOnForward: true` y `doNotTrack: true` en la sección `spec` de la política.
...
## Aislamiento del clúster en la red pública
{: #isolate_workers_public}

Puede aislar el clúster de la red pública aplicando [políticas de red pública de Calico ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://github.com/IBM-Cloud/kube-samples/tree/master/calico-policies/public-network-isolation).
{: shortdesc}

Este conjunto de políticas de Calico funcionan conjuntamente con las [políticas predeterminadas de Calico](#default_policy) para bloquear la mayoría del tráfico de red pública de un clúster al tiempo que permite la comunicación que es necesaria para que el clúster funcione con subredes específicas. Para ver una lista de los puertos abiertos por estas políticas y una lista de las políticas que se incluyen, consulte el [README para las políticas de red pública de Calico ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://github.com/IBM-Cloud/kube-samples/blob/master/calico-policies/public-network-isolation/README.md).

Cuando aplica las políticas de pod de egress que se incluyen en este conjunto de políticas, sólo se permite el tráfico de red a las subredes y los puertos especificados en las políticas de pod. El tráfico de todas las subredes o puertos que no se especifican en las políticas se bloquea para todos los pods de todos los espacios de nombres. Puesto que en estas políticas solo se especifican los puertos y las subredes que son necesarios para que los pods funcionen en {{site.data.keyword.containerlong_notm}}, sus pods no pueden enviar tráfico de red a través de Internet hasta que se añada o cambie la política de Calico para darles permiso.
{: important}
...
Para proteger el clúster en la red pública mediante políticas de Calico:
...
2. Acceda al directorio de política pública para la región en la que está su clúster. Mandato de ejemplo para un clúster en EE.UU. sur:
  ```
  cd <filepath>/IBM-Cloud/kube-samples/calico-policies/public-network-isolation/us-south
  ```
  {: pre}

3. Aplique las políticas.
  ```
...
4. Opcional: para permitir a los nodos trabajadores acceder a otros servicios de {{site.data.keyword.Bluemix_notm}} a través de la red pública, aplique las políticas `allow-public-services.yaml` y `allow-public-services-pods.yaml`. La política permite el acceso a las direcciones IP de {{site.data.keyword.registryshort_notm}}, y si los servicios están disponibles en la región, {{site.data.keyword.la_full_notm}} y {{site.data.keyword.mon_full_notm}}. Para acceder a otros servicios de IBM Cloud, debe agregar manualmente a esta política las subredes para dichos servicios.
  ```
...
5. Compruebe que las políticas se aplican.
  ```
...
Puede aislar el clúster de otros sistemas de la red privada aplicando [políticas de red privada de Calico ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://github.com/IBM-Cloud/kube-samples/tree/master/calico-policies/private-network-isolation/calico-v3).
{: shortdesc}

Este conjunto de políticas de Calico y puntos finales de host puede aislar el tráfico de red privada de un clúster de otros recursos de la red privada de la cuenta, al tiempo que permite la comunicación en la red privada que es necesaria para que el clúster funcione. Por ejemplo, cuando permite la [expansión de VLAN o VRF](/docs/containers?topic=containers-plan_clusters#worker-worker) para permitir a los nodos trabajadores comunicarse entre sí en la red privada, cualquier instancia conectada con cualquier VLAN privada en la misma cuenta de {{site.data.keyword.Bluemix_notm}} se puede comunicar con sus nodos trabajadores.

Para ver una lista de los puertos abiertos por estas políticas y una lista de las políticas que se incluyen, consulte el [README para las políticas de red pública de Calico ![Icono de enlace externo](../icons/launch-glyph.svg "Icono de enlace externo")](https://github.com/IBM-Cloud/kube-samples/blob/master/calico-policies/private-network-isolation/README.md).

Cuando aplica las políticas de pod de egress que se incluyen en este conjunto de políticas, sólo se permite el tráfico de red a las subredes y los puertos especificados en las políticas de pod. El tráfico de todas las subredes o puertos que no se especifican en las políticas se bloquea para todos los pods de todos los espacios de nombres. Puesto que en estas políticas solo se especifican los puertos y las subredes que son necesarios para que los pods funcionen en {{site.data.keyword.containerlong_notm}}, sus pods no pueden enviar tráfico de red a través de la red privada hasta que se añada o cambie la política de Calico para darles permiso.
{: important}
...
2. Acceda al directorio de política privada `calico-v3` para la región en la que está su clúster. Mandato de ejemplo para un clúster en EE.UU. sur:
  ```
  cd <filepath>/IBM-Cloud/kube-samples/calico-policies/private-network-isolation/calico-v3/us-south
  ```
  {: pre}

3. Configure puntos finales de host privados para los nodos trabajadores. Cuando los nodos trabajadores tienen puntos finales de host privados, las políticas que se aplican en los siguientes pasos pueden dirigirse a la interfaz privada de nodo trabajador (eth0) y a la red de pod de un clúster.
  1. Abra la política `generic-privatehostendpoint.yaml`.
  2. Sustituya `<worker_name>` por el nombre de un nodo trabajador.
    <p class="note">Algunos nodos trabajadores deben seguir una estructura de denominación distinta para las políticas de Calico. Debe utilizar el nombre que se devuelve cuando ejecuta `calicoctl get nodes --config=<filepath>/calicoctl.cfg`.</p>
  ...
  4. Para cada nodo trabajador del clúster, repita estos pasos en una entrada distinta en el archivo.
    <p class="important">Cada vez que añade un nodo trabajador a un clúster, debe actualizar el archivo de puntos finales de host con las nuevas entradas.</p>
...
5. Opcional: para permitir a los trabajadores y a los pods acceder a {{site.data.keyword.registryshort_notm}} a través de la red privada, aplique las políticas `allow-private-services.yaml` y `allow-private-services-pods.yaml`. Para acceder a otros servicios de IBM Cloud que dan soporte a puntos finales de servicio privados, debe añadir manualmente las subredes para dichos servicios a esta política.
  ```
...
6. Opcional: para exponer las apps con los equilibradores de carga de red (NLB) privada o los equilibradores de carga de aplicación (ALB) de Ingress, debe abrir el protocolo VRRP aplicando la política `allow-vrrp-private`.
  ...
  ```
  {: pre}
  Puede seguir controlando el acceso a los servicios de red creando [políticas pre-DNAT de Calico](/docs/containers?topic=containers-network_policies#block_ingress). En la política pre-DNAT, asegúrese de que utiliza `selector: ibm.role=='worker_private'` para aplicar la política a los puntos finales de host privados de los trabajadores.
  {: tip}

7. Compruebe que las políticas se aplican.
  ```
  calicoctl get GlobalNetworkPolicies -o yaml --config=<filepath>/calicoctl.cfg
  ```
  {: pre}
...
5. Busque entradas de registro escritas en la vía de acceso `/var/log/syslog`. Las direcciones IP de DST (destino) o SRC (origen) en la entrada de registro pueden ser distintas de lo esperado debido a los proxies, a la conversión de direcciones de red (NAT) y a otros procesos de red. La entrada de registro se parecerá a lo siguiente.
  ```
