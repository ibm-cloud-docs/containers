...
{: important}
 En clústeres que ejecuten Kubernetes versión 1.10 o posterior, los maestros tienen alta disponibilidad e incluyen réplicas para que el servidor de API de Kubernetes, etcd, el planificador y el gestor de controladores en hosts independientes puedan protegerse frente a interrupciones como, por ejemplo, una actualización del maestro.</p><p>Para proteger el maestro de clúster de un fallo zonal, puede: <ul><li>Crear un clúster multizona, que distribuye el maestro en zonas.</li><li>Configurar un segundo clúster en otra zona.</li></ul></p>
