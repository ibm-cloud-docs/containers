...
*   **Altre opzioni**: i cluster gratuiti vengono distribuiti nell'area geografica da te selezionata, ma non puoi scegliere la zona. Per avere il controllo sulla zona, sulla rete e sull'archiviazione persistente, crea un cluster standard. [Scopri di più sui vantaggi dei cluster gratuiti e standard](/docs/containers?topic=containers-cs_ov#cluster_types).
...
* Esegui la [prima esercitazione di {{site.data.keyword.containerlong_notm}}](/docs/containers?topic=containers-cs_cluster_tutorial#cs_cluster_tutorial) per creare un terminale Kubernetes, installare la CLI, creare un registro privato, configurare il tuo ambiente cluster e aggiungere un servizio al tuo cluster.
