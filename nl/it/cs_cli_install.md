...
-   [Comandi `ibmcloud cr`](/docs/cli/plugins/registry/index.html)
...
Tutti i comandi `kubectl` disponibili in Kubernetes 1.10.8 sono supportati per l'utilizzo con i cluster in {{site.data.keyword.Bluemix_notm}}. Dopo che hai creato un cluster, imposta il
...
3. Per creare e gestire i cluster in un gruppo di risorse diverso da quello predefinito, specifica tale gruppo di risorse. **Nota**: devi disporre dell'[accesso come **Visualizzatore**](cs_users.html#platform) al gruppo di risorse.
    ```
    ibmcloud target -g <resource_group_name>
    ```
    {: pre}

3.  Per creare o accedere ai cluster Kubernetes in un'altra regione diversa dalla regione {{site.data.keyword.Bluemix_notm}} che hai selezionato precedentemente, esegui `ibmcloud ks region-set`.
...
-   Versione CLI {{site.data.keyword.Bluemix_notm}} 0.8.0 o successive
-   Plugin {{site.data.keyword.containerlong_notm}}
-   CLI Kubernetes versione 1.10.8 o successiva
