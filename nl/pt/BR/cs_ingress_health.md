...
Por padrão, os logs do Ingress são formatados em JSON e exibem campos de log comuns. No entanto, também é possível criar um formato de log customizado.
...
Antes de iniciar, assegure-se de que você tenha a [função de serviço **Gravador** ou **Gerente** do {{site.data.keyword.Bluemix_notm}}](cs_users.html#platform) IAM para o namespace `kube-system`.
