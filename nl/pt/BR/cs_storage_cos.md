...
2. {: #service_credentials}Recupere as credenciais de serviço do  {{site.data.keyword.cos_full_notm}} .
   ...
   5.  Opcional: em **Incluir parâmetros de configuração sequenciais (opcional)**, insira `{"HMAC":true}` para criar credenciais adicionais do HMAC para o serviço {{site.data.keyword.cos_full_notm}}. A autenticação do HMAC inclui uma camada extra de segurança na autenticação OAuth2 padrão, evitando o uso indevido de tokens OAuth2 expirados ou criados aleatoriamente.
...
5. Crie o segredo em seu cluster.
   ```
   kubectl aplicar -f filepath/secret.yaml
   ```
   {: pre}
...
Antes de iniciar, [direcione sua CLI](cs_cli_install.html#cs_cli_configure) para o cluster no qual você deseja instalar o plug-in do {{site.data.keyword.cos_full_notm}}.

1. Siga as [instruções](cs_integrations.html#helm) para instalar o cliente do Helm em sua máquina local, instalar o servidor Helm (tiller) em seu cluster e incluir o repositório do gráfico Helm do {{site.data.keyword.Bluemix_notm}} no cluster no qual você deseja usar o plug-in do {{site.data.keyword.cos_full_notm}}.
...
2. Inclua o repositório do Helm do {{site.data.keyword.Bluemix_notm}} em seu cluster.
   ```
...
4. Instale o  {{site.data.keyword.cos_full_notm}}  plug-in Helm  ` ibmc `. O plug-in é usado para recuperar automaticamente o local do cluster e para configurar o terminal de API para os buckets do {{site.data.keyword.cos_full_notm}} em suas classes de armazenamento.
   1. Faça download do gráfico Helm e descompacte o gráfico em seu diretório atual.    
      ```
      helm fetch -- untar ibm/ibmcloud-object-storage-plugin
      ```
      {: pre}
   2. Instale o plug-in Helm.
      ```
...
5. Verifique se o plug-in `ibmc` está instalado com êxito.
   ```
   helm ibmc -- help
   ```
   {: pre}
...
6. Opcional: limite o plug-in do {{site.data.keyword.cos_full_notm}} para acessar somente os segredos do Kubernetes que contêm as credenciais de serviço do {{site.data.keyword.cos_full_notm}}. Por padrão, o plug-in está autorizado a acessar todos os segredos do Kubernetes em seu cluster.
...
8. Instale o plug-in do {{site.data.keyword.cos_full_notm}}. Quando você instala o plug-in, as classes de armazenamento predefinidas são incluídas em seu cluster.
   ...
   **Instale sem uma limitação a segredos específicos do Kubernetes:**
   ```
   helm ibmc install ibm/ibmcloud-object-storage-plugin -f ibmcloud-object-storage-plugin/ibm/values.yaml
   ```
   {: pre}

   **Instale com uma limitação a segredos específicos do Kubernetes, conforme descrito na etapa anterior:**
   ```
...
9. Verifique se o plug-in está instalado corretamente.
   ```
...
10. Verifique se as classes de armazenamento foram criadas com êxito.
   ```
...
11. Repita as etapas para todos os clusters nos quais você deseja acessar os buckets do {{site.data.keyword.cos_full_notm}}.
...
5. Remova o plug-in Helm  ` ibmc ` .
   ```
   rm -rf ~/.helm/plugins/helm-ibmc
   ```
   {: pre}

6. Verifique se o plug-in `ibmc` foi removido.
   ```
   helm plugin list
   ```
   {: pre}
