...
Alternando regiões? Use o [comando](#cs_region-set) `bx cs region-set` no lugar.
{: tip}
...
<li>Se você criou um cluster padrão antes neste local ou criou uma VLAN pública na infraestrutura do IBM Cloud (SoftLayer) antes, especifique essa VLAN pública. Se você deseja conectar seus nós do trabalhador somente a uma VLAN privada, não especifique esta opção.
...
**Exemplos**:
  ...
  **Criar um cluster grátis**: especifique somente o nome do cluster, todo o resto é configurado para um valor padrão. Um cluster grátis é excluído automaticamente após 21 dias. É possível ter um cluster grátis de cada vez. Para aproveitar os recursos integrais do Kubernetes, crie um cluster padrão.
  ...
  **Criar seu primeiro cluster padrão**: o primeiro cluster padrão que é criado em um local também cria uma VLAN privada. Portanto, não inclua a sinalização `--public-vlan`.
  ...
  **Criar clusters padrão subsequentes**: se você já criou um cluster padrão neste local ou criou uma VLAN pública na infraestrutura do IBM Cloud (SoftLayer) antes, especifique essa VLAN pública com a sinalização `--public-vlan`. Para descobrir se você já tem uma VLAN pública para um local específico ou para localizar o nome de uma VLAN pública existente, execute `bx cs vlans <location>`.
...
Visualizar uma lista de tipos de máquina disponíveis para seus nós do trabalhador. Tipos de máquina variam por local. Cada tipo de máquina inclui a
...
<strong>Opções de comando</strong>:
   ...
   <dd>Insira o local no qual você deseja listar tipos de máquina disponíveis. Este valor é necessário. Revise [os locais disponíveis](cs_regions.html#locations).</dd>
...
### Bx cs logging CLUSTER refresh-config-[ -s ]
{: #cs_logging_refresh}
...
Visualizar uma lista de locais disponíveis para você criar um cluster. Os locais disponíveis variam pela região em que você efetuou login. Para alternar regiões, execute `bx cs region-set`.
...
<dt><code>--disable-disk-encrypt</code></dt>
<dd>Os nós do trabalhador apresentam criptografia de disco por padrão; [saiba mais](cs_secure.html#worker). Para desativar a criptografia, inclua essa opção.</dd>
...
**Exemplos**:
 ...
 No qual você implementa o cluster. Para obter mais informações, veja a documentação para o comando `bx cs machine-types` [](cs_cli_reference.html#cs_machine_types). Esse valor é necessário para clusters padrão e não está disponível para clusters livres.</dd>
  ...
  <dt><code>-- kube-version <em>VERSION</em></code></dt>
    <dd>A versão do Kubernetes com a qual você deseja que os nós do trabalhador sejam criados. A versão padrão será usada se este valor não for especificado.</dd>
  ...
  <dt><code>-- somente privado </code></dt>
    <dd>Especifica que não há VLANs públicas no conjunto do trabalhador. O valor padrão é <code>false</code>.</dd>

  <dt><code>-- Compiz de disco encrpyt</code></dt>
...
### Conjuntos-bx cs worker -- cluster CLUSTER
{: #cs_worker_pools}
...
**Comando de exemplo**:
  ...
  ```
  Conjuntos-bx cs worker -- cluster my_cluster
  ```
  {: pre}
...
<dl>
  <dt><code>-- worker-conjunto <em>WORKER_POOL</em></code></dt>
    <dd>O nome do conjunto de nós do trabalhador do qual você deseja visualizar os detalhes. Este valor é obrigatório.</dd>
  <dt><code>--cluster <em>CLUSTER</em></code></dt>
...
Atualize todos os nós do trabalhador em seu conjunto para a versão mais recente do Kubernetes que corresponde ao principal especificado.
...
<dl>
  ...
  <dt><code>--cluster <em>CLUSTER</em></code></dt>
    <dd>O nome ou ID do cluster para o qual você deseja atualizar conjuntos de trabalhadores. Este valor é obrigatório.</dd>
</dl>
...
<dl>
  ...
  <dt><code>--size-per-zona <em>WORKERS_PER_ZONE</em></code></dt>
    <dd>O número de trabalhadores que você deseja criar em cada zona. Este valor é obrigatório.</dd>
</dl>
...
### Bx cs workers CLUSTER [-- show-excluído][--json] [ -s ]
{: #cs_workers}
