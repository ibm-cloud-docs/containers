...
Faça uma sub-rede em uma conta do IBM Bluemix Infrastructure (SoftLayer) disponível para um cluster especificado.
...
Crie uma sub-rede em uma conta do IBM Bluemix Infrastructure (SoftLayer) e torne-a disponível para um cluster especificado em {{site.data.keyword.containershort_notm}}.
...
<strong>Opções de comando</strong>:
   ...
   <dd>Nome do usuário da conta do IBM Bluemix Infrastructure (SoftLayer). Este valor é obrigatório.</dd>
   </dl>

   <dt><code>--infrastructure-api-key <em>API_KEY</em></code></dt>
   <dd>Chave API da conta do IBM Bluemix Infrastructure (SoftLayer). Esse valor é necessário.
...
Remova as credenciais de conta do IBM Bluemix Infrastructure (SoftLayer) de sua conta do {{site.data.keyword.Bluemix_notm}}. Após a remoção das credenciais, não é possível acessar o portfólio do IBM Bluemix Infrastructure (SoftLayer) por meio de sua conta do {{site.data.keyword.Bluemix_notm}} mais.
...
<dd>A origem de log para a qual você deseja ativar o encaminhamento de log. Os valores aceitos são <code>application</code>, <code>worker</code>, <code>kubernetes</code> e <code>ingress</code>. Esse valor é obrigatório para origens de log diferentes de namespaces do contêiner do Docker.</dd>
...
**Exemplo de `namespace`** de origem de log:
...
**Exemplo de `ingresso`** de origem de log:
...
<strong>Opções de comando</strong>:
   ...
   <dd>O nome ou ID do cluster. Este valor é obrigatório.</dd>
   <dt><code>--logsource <em>LOG_SOURCE</em></code></dt>
   <dd>O tipo de origem de log para a qual você deseja filtrar. Apenas as configurações de criação de log dessa origem de log no cluster são retornadas. Os valores aceitos são <code>namespace</code>, <code>application</code>, <code>worker</code>, <code>kubernetes</code> e <code>ingress</code>. Esse valor é opcional.</dd>
   </dl>
...
<strong>Opções de comando</strong>:
   ...
   <dd>O nome ou ID do cluster. Este valor é obrigatório.</dd>
   <dt><code>--namespace <em>KUBERNETES_NAMESPACE</em></code></dt>
   <dd>O namespace do contêiner do Docker do qual você deseja parar o encaminhamento de logs para syslog. Esse valor é necessário para namespaces de contêiner do Docker.</dd>
   <dt><code>--id <em>LOG_SOURCE_LOGGING_ID</em></code></dt>
   <dd>O ID de configuração de criação de log que você deseja remover da origem de log. Esse valor é obrigatório para origens de log diferentes de namespaces do contêiner do Docker.</dd>
   </dl>
...
<strong>Opções de comando</strong>:
   ...
   <dt><code>--logsource <em>LOG_SOURCE</em></code></dt>
   <dd>A origem de log para a qual você deseja atualizar o encaminhamento de log. Os valores aceitos são <code>application</code>, <code>worker</code>, <code>kubernetes</code> e <code>ingress</code>. Esse valor é obrigatório para origens de log diferentes de namespaces do contêiner do Docker.</dd>
   <dt><code>--id <em>LOG_SOURCE_LOGGING_ID</em></code></dt>
...
|Crítico|O mestre do Kubernetes não pode ser atingido ou todos os nós do trabalhador no cluster estão inativos. <ol><li>Liste os nós do trabalhador em seu cluster.<pre class="pre"><code>bx cs workers &lt;cluser_name_or_id&gt;</code></pre><li>Obtenha os detalhes para cada nó do trabalhador.<pre class="pre"><code>bx cs worker-get &lt;worker_id&gt;</code></pre></li><li>Revise os campos <strong>Estado</strong>, <strong>Status</strong> e <strong>Detalhes</strong> para localizar o problema raiz do motivo de o nó do trabalhador estar inativo.</li><li>Se o estado do nó do trabalhador mostrar <strong>Provision_failed</strong>, talvez você não tenha as permissões necessárias para provisionar um nó do trabalhador do portfólio do IBM Bluemix Infrastructure (SoftLayer). Para localizar as permissões necessárias, consulte [Configurar o acesso ao portfólio do IBM Bluemix Infrastructure (SoftLayer) para criar clusters do Kubernetes padrão](cs_planning.html#cs_planning_unify_accounts).</li><li>Se o estado do nó do trabalhador mostrar <strong>Crítico</strong> e o status do nó do trabalhador mostrar <strong>Sem disco</strong>, seu nó do trabalhador terá ficado sem capacidade. Será possível reduzir a carga de trabalho em seu nó do trabalhador ou incluir um nó do trabalhador em seu cluster para ajudar no balanceamento de carga da carga de trabalho.</li><li>Se o estado do nó do trabalhador mostrar <strong>Crítico</strong> e o status do nó do trabalhador mostrar <strong>Desconhecido</strong>, o mestre do Kubernetes não estará disponível. Entre em contato com o suporte do {{site.data.keyword.Bluemix_notm}} abrindo um chamado de suporte do [{{site.data.keyword.Bluemix_notm}}](/docs/support/index.html#contacting-support).</li></ol>|
