...
**Opções de comando**:
   ...
   <dd>(Opcional) Faça download dos certificados e dos arquivos de permissão para a função rbac do Administrador. Os usuários com esses arquivos podem executar ações de administrador no cluster, como remover o cluster.</dd>
   </dl>
...
<dl>
   ...
   <dd>(Opcional para clusters padrão. Não disponível para clusters lite.) O caminho para o arquivo YAML para criar seu cluster padrão. Em vez de definir as características de seu cluster usando as opções fornecidas nesse comando, será possível usar um arquivo YAML.
    ...
    <dd>(Necessário para clusters padrão. Não disponível para clusters lite.) O tipo de máquina que você escolhe afeta a quantia de memória e espaço em disco que está disponível para os contêineres que são implementados em seu nó do trabalhador. Para listar os tipos de máquina disponíveis, veja [bx cs machine-types <em>LOCATION</em>](cs_cli_reference.html#cs_machine_types).</dd>

    <dt><code>--name <em>NAME</em></code></dt>
    <dd>(Necessário) O nome para o cluster.</dd>

    <dt><code>--no-subnet</code></dt>
    <dd>Inclua a sinalização para criar um cluster sem uma sub-rede móvel. O padrão é não usar a sinalização e criar uma sub-rede em seu portfólio do {{site.data.keyword.BluSoftlayer_full}}.</dd>
    ...
    <dd>(Não disponível para clusters lite.)
        ...
        <li>Se você criou um cluster antes nesse local ou criou uma VLAN privada no {{site.data.keyword.BluSoftlayer_notm}} antes, deve-se especificar essa VLAN privada.
    ...
    <dd>(Não disponível para clusters lite.)
        ...
        <ul>
        <li>Se esse cluster é o primeiro cluster que você cria nesse local, não use essa sinalização. Uma VLAN pública é criada para você quando o cluster é criado.</li>
        <li>Se você criou um cluster antes nesse local ou criou uma VLAN pública no {{site.data.keyword.BluSoftlayer_notm}} antes, deve-se especificar essa VLAN pública.
    ...
    <dd>(Opcional para clusters padrão. Não disponível para clusters lite.) O número de nós do trabalhador que você deseja implementar em seu cluster. Se não especificar essa opção, um cluster com 1 nó do trabalhador será criado.
...
**Exemplos**:
  ...
  Exemplo para um ambiente {{site.data.keyword.Bluemix_notm}} Dedicated:
...
Incluir uma serviço do {{site.data.keyword.Bluemix_notm}} a um cluster.
...
<strong>Opções de comando</strong>:
   ...
   <dd>(Necessário) O ID da instância de serviço do {{site.data.keyword.Bluemix_notm}} que você deseja ligar.</dd>
   </dl>
...
Remover um serviço do {{site.data.keyword.Bluemix_notm}} de um cluster.

**Nota:** ao remover um serviço do {{site.data.keyword.Bluemix_notm}}, as credenciais de serviço são removidas do cluster. Se um pod ainda está usando o serviço, ele falha porque as credenciais de serviço não podem ser localizadas.
...
<strong>Opções de comando</strong>:
   ...
   <dt><code><em>SERVICE_INSTANCE_GUID</em></code></dt>
   <dd>(Necessário) O ID da instância de serviço do {{site.data.keyword.Bluemix_notm}} que você deseja remover.</dd>
   </dl>
...
Listar os serviços ligados a um ou a todos os namespaces do Kubernetes em um cluster. Se nenhuma opção é especificada, os serviços para o namespace padrão são exibidos.
...
Disponibilizar uma sub-rede em uma conta do {{site.data.keyword.BluSoftlayer_notm}} para um cluster especificado.

**Nota:** quando você disponibiliza uma sub-rede para um cluster, os endereços IP dessa sub-rede são usados para propósitos de rede do cluster. Para evitar conflitos de endereço IP, certifique-se de usar uma sub-rede com somente um cluster. Não use uma sub-rede para múltiplos clusters ou para outros propósitos fora do {{site.data.keyword.containershort_notm}} ao mesmo tempo.
...
Configurar credenciais de conta do {{site.data.keyword.BluSoftlayer_notm}} para sua conta do {{site.data.keyword.Bluemix_notm}}. Essas credenciais permitem que você acesse o portfólio do {{site.data.keyword.BluSoftlayer_notm}} por meio de sua conta do {{site.data.keyword.Bluemix_notm}}.

<strong>Opções de comando</strong>:

   <dl>
   <dt><code>--infrastructure-username <em>USERNAME</em></code></dt>
   <dd>(Necessário) Um nome de usuário da conta do {{site.data.keyword.BluSoftlayer_notm}}.</dd>
   </dl>

   <dt><code>--infrastructure-api-key <em>API_KEY</em></code></dt>
   <dd>(Necessário) Uma chave API da conta do {{site.data.keyword.BluSoftlayer_notm}}.
...
Remover as credenciais de conta do {{site.data.keyword.BluSoftlayer_notm}} de sua conta do {{site.data.keyword.Bluemix_notm}}. Após a remoção das credenciais, não será mais possível acessar o portfólio do {{site.data.keyword.BluSoftlayer_notm}} por meio da conta do {{site.data.keyword.Bluemix_notm}}.
...
<strong>Opções de comando</strong>:
   ...
   <dd>(Opcional) O terminal de API do {{site.data.keyword.containershort_notm}} que você deseja usar. Exemplos:
...
Visualizar uma lista de tipos de máquina disponíveis para seus nós do trabalhador. Cada tipo de máquina inclui a quantia de CPU, memória e espaço em disco virtual para cada nó do trabalhador no cluster.

<strong>Opções de comando</strong>:

   <dl>
   <dt><em>LOCATION</em></dt>
   <dd>(Necessário) Insira o local no qual você deseja listar os tipos de máquina disponíveis. Os locais disponíveis são: <ul><li>Sul dos EUA<ul><li>dal10 [Dallas]</li><li>dal12 [Dallas]</li></ul></li><li>Sul do Reino Unido<ul><li>lon02 [Londres]</li><li>lon04 [London]</li></ul></li><li>UE Central<ul><li>ams03 [Amsterdã]</li><li>ra02 [Frankfurt]</li></ul></li><li>Sul da Ásia-Pacífico<ul><li>syd01 [Sydney]</li><li>syd04 [Sydney]</li></ul></li></ul></dd></dl>
...
Listar as VLANs públicas e privadas disponíveis para um local em sua conta do {{site.data.keyword.BluSoftlayer_notm}}. Para listar as VLANs disponíveis, deve-se ter uma conta paga.

<strong>Opções de comando</strong>:

   <dl>
   <dt>LOCALIDADE</dt>
   <dd>(Necessário) Insira o local no qual você deseja listar suas VLANs privadas e públicas. Os locais disponíveis são: <ul><li>Sul dos EUA<ul><li>dal10 [Dallas]</li><li>dal12 [Dallas]</li></ul></li><li>Sul do Reino Unido<ul><li>lon02 [Londres]</li><li>lon04 [London]</li></ul></li><li>UE Central<ul><li>ams03 [Amsterdã]</li><li>ra02 [Frankfurt]</li></ul></li><li>Sul da Ásia-Pacífico<ul><li>syd01 [Sydney]</li><li>syd04 [Sydney]</li></ul></li></ul></dd>
   </dl>
...
<strong>Opções de comando</strong>:
   ...
   <dd>(Opcional) O nível de notificação, como <code>Normal</code> ou <code>Warning</code>. <code>Warning</code> é o valor padrão.</dd>
...
Incluir nós do trabalhador em seu cluster padrão.
...
<strong>Opções de comando</strong>:
   ...
   <dd>(Necessário) O nome ou ID do cluster.</dd>
    ...
    <dd>(Necessário) O tipo de máquina que você escolhe afeta a quantia de memória e espaço em disco que está disponível para os contêineres que são implementados em seu nó do trabalhador. Para listar os tipos de máquina disponíveis, veja [bx cs machine-types LOCATION](cs_cli_reference.html#cs_machine_types).</dd>
    ...
    <dd>(Necessário) Se você tem uma VLAN privada disponível para usar no local, deve-se especificar a VLAN. Se este é o primeiro cluster que você cria nesse local, não use essa sinalização. Uma VLAN privada é criada para você.
    ...
    <dd>(Necessário) Se você tem uma VLAN pública disponível para usar no local, deve-se especificar a VLAN. Se este é o primeiro cluster que você cria nesse local, não use essa sinalização. Uma VLAN pública é criada para você.
...
Reinicializar os nós do trabalhador em um cluster. Se existir um problema com um nó do trabalhador, primeiro tente reinicializar o nó do trabalhador, que o reinicia. Se a reinicialização não resolver o problema, tente o comando `worker-reload`. O estado dos trabalhadores não muda durante a reinicialização. O estado permanece `deployed`, mas o status é atualizado.
...
<strong>Opções de comando</strong>:
   ...
   <dd>(Opcional) Use essa opção para fazer uma reinicialização forçada de um nó do trabalhador cortando a energia para o nó do trabalhador. Use essa opção se o nó do trabalhador não responder ou se ele tiver uma interrupção do Docker.</dd>

   <dt><code><em>WORKER</em></code></dt>
   <dd>(Necessário) O nome ou ID de um ou mais nós do trabalhador. Use um espaço para listar múltiplos nós do trabalhador.</dd>
   </dl>
...
Recarregar os nós do trabalhador em um cluster. Se existir um problema com um nó do trabalhador, primeiro tente reinicializar o nó do trabalhador. Se a reinicialização não resolver o problema, tente o comando `worker-reload`, que recarregará todas as configurações necessárias para o nó do trabalhador.
