...
estático](cs_cluster.html#cs_cluster_volume_create) de volumes.|Use [fornecimento dinâmico](cs_apps.html#cs_apps_volume_claim) de volumes. [Abra um chamado de suporte](/docs/support/index.html#contacting-support) para solicitar um backup para seus volumes, solicitar uma restauração de seus volumes e executar outras funções de armazenamento.</li></ul>|
...
{: caption="Tabela 1. Diferenças de recurso entre o {{site.data.keyword.Bluemix_notm}} público e o {{site.data.keyword.Bluemix_dedicated_notm}}" caption-side="top"}
...
*Figura 1. Arquitetura e rede do Kubernetes no {{site.data.keyword.Bluemix_dedicated_notm}}*
...
Antes de iniciar:
  ...
  * Caso o seu sistema local ou sua rede corporativa controle os terminais de Internet pública usando proxies ou firewalls, deve-se [abrir portas e endereços IP requeridos em seu firewall](cs_security.html#opening_ports).
...
2.  O proprietário de sua conta do {{site.data.keyword.Bluemix_dedicated_notm}} pode convidar um único usuário ou vários para sua conta pública.
    ...
    2.  Convide os usuários para a conta pública.
        * Para convidar um único usuário:
            ...
            Substitua <em>&lt;user_IBMid&gt;</em> pelo e-mail do usuário que você deseja convidar, <em>&lt;public_api_key&gt;</em> pela chave de API gerada na etapa anterior e <em>&lt;public_org_id&gt;</em> pelo GUID da organização da conta pública.
        ...
        * Para convidar todos os usuários atualmente em uma organização de conta dedicada:
            ...
            Substitua <em>&lt;dedicated_org_id&gt;</em> pelo ID da organização da conta dedicada, <em>&lt;public_api_key&gt;</em> pela chave API gerada na etapa anterior e <em>&lt;public_org_id&gt;</em> pela GUID da organização de conta pública.
    ...
    4.  Verifique se os usuários foram incluídos na conta.
        ...
        Os usuários convidados que têm um IBMid existente terão um status de `ACTIVE`. Os usuários convidados que não tem um IBMid existente terão um status de `PENDING` ou `ACTIVE`, dependendo se eles já aceitaram o convite para a conta ou não.

3.  Quando os usuários são incluídos em sua conta pública, deve-se conceder a função de Administrador para qualquer usuário que precisa criar um cluster. Consulte [Gerenciando acesso de usuário](cs_cluster.html#cs_cluster_user) para obter informações sobre como conceder a função de Administrador.
...
4.  Os usuários podem agora efetuar login no terminal de conta dedicada para iniciar a criação de clusters.
    ...
    2.  Se você estiver efetuando login pela primeira vez, forneça seu ID do usuário e senha da conta dedicada quando solicitado. Isso autentica a conta dedicada e vincula as contas dedicadas e públicas. Toda vez que você efetuar o login após essa primeira vez, você usará somente o seu IBMid para fazer login.
...
5.  Se desejar desvincular suas contas, será possível desconectar seu IBMid do seu ID do usuário dedicado.
...
Para novos namespaces, veja as opções em [Usando registros de imagem privada e pública com o {{site.data.keyword.containershort_notm}}](cs_cluster.html#cs_apps_images). Para namespaces que tenham sido configurados para grupos únicos e escaláveis, [use um token e crie um segredo do
Kubernetes](cs_dedicated_tokens.html#cs_dedicated_tokens) para autenticação.

### Incluindo sub-redes nos clusters
{: #dedicated_cluster_subnet}

Mude o conjunto de endereços IP públicos móveis disponíveis, incluindo sub-redes em seu cluster. Para obter mais informações, consulte [Incluindo sub-redes nos clusters](cs_cluster.html#cs_cluster_subnet). Revise as diferenças a seguir para incluir sub-redes nos clusters dedicados.
...
1. Para usar sua própria sub-rede, [abra um chamado de suporte](/docs/support/index.html#contacting-support) e forneça a lista de sub-CIDRs que você deseja usar.
...
Revise as opções a seguir para outras configurações de cluster:
  ...
  * [Visualizando recursos de cluster do Kubernetes](cs_cluster.html#cs_weavescope)
  * [Removendo Clusters](cs_cluster.html#cs_cluster_remove)
...
Para implementar apps em clusters, é possível seguir as instruções para [implementar apps em clusters públicos do {{site.data.keyword.Bluemix_notm}}](cs_apps.html#cs_apps). Revise as diferenças a seguir para clusters do {{site.data.keyword.Bluemix_dedicated_notm}}.
...
Se você quiser usar endereços IP públicos para o balanceador de carga, assegure-se de que uma lista de desbloqueio firewall corporativo tenha sido fornecido para a IBM ou [abra um chamado de suporte](/docs/support/index.html#contacting-support) para configurar o desbloqueio de firewall. Em seguida, siga as etapas em [Configurando acesso a um app usando o tipo de serviço de balanceador de carga](cs_apps.html#cs_apps_public_load_balancer).

#### Configurando o acesso público a um app usando o Ingress
{: #dedicated_apps_public_ingress}

Se você quiser usar endereços IP públicos para o balanceador de carga de aplicativo, assegure-se de que uma lista de desbloqueio firewall corporativo foi fornecido para a IBM ou [abra um chamado de suporte](/docs/support/index.html#contacting-support) para configurar o desbloqueio de firewall. Em seguida, siga as etapas em [Configurando acesso a um app usando o Ingress](cs_apps.html#cs_apps_public_ingress).

### Criando armazenamento persistente
{: #dedicated_apps_volume_claim}

Para revisar as opções para criar armazenamento persistente, consulte [armazenamento de dados persistentes](cs_planning.html#cs_planning_apps_storage). Para solicitar um backup para seus volumes, uma restauração de seus volumes e outras funções de armazenamento, deve-se [abrir um chamado de suporte](/docs/support/index.html#contacting-support).
