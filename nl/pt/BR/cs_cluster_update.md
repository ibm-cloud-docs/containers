...
Por exemplo, se a sua versão atual do servidor da API do Kubernetes é 1.7 e você deseja atualizar para a 1.10, deve-se primeiro atualizar para a 1.8 ou 1.9. É possível forçar para que ocorra a atualização, mas a atualização de três ou mais versões secundárias pode causar resultados inesperados ou falha.
...
Não. Primeiro, [atualize seu mestre](#update_master) para a versão mais recente do Kubernetes. Em seguida, [atualize os nós do trabalhador](#worker_node) em seu cluster. Diferentemente do principal, deve-se também atualizar seus trabalhadores para cada versão de correção.
