...
Se você tiver uma conta {{site.data.keyword.Bluemix}} Private, será possível usá-la com os serviços do {{site.data.keyword.cloud_notm}} selecionados, incluindo o {{site.data.keyword.containerlong}}. Para obter mais informações, consulte o blog em [Experiência híbrida no {{site.data.keyword.cloud_notm}} Private e IBM Public Cloud![Ícone de link externo](../icons/launch-glyph.svg "Ícone de link externo")](http://ibm.biz/hybridJune2018).
{: shortdesc}

Você entende as [oferta do {{site.data.keyword.cloud_notm}}](/docs/containers?topic=containers-cs_ov#differentiation) e desenvolveu a sua estratégia de Kubernetes para as [cargas de trabalho que executam na nuvem](/docs/containers?topic=containers-strategy#cloud_workloads). Agora, é possível conectar sua nuvem pública e privada usando o serviço VPN do strongSwan ou o {{site.data.keyword.BluDirectLink}}.
...
* Com o [{{site.data.keyword.cloud_notm}} Direct Link](#hybrid_dl), é possível criar uma conexão direta e privada entre seus ambientes de rede remota e o {{site.data.keyword.containerlong_notm}} sem rotear pela Internet pública.
...
1.  Crie um cluster padrão com o {{site.data.keyword.containerlong}} no {{site.data.keyword.cloud_notm}} Public ou use um existente. Para criar um cluster, escolha entre as opções a seguir:
...
4.  [Crie um cluster no {{site.data.keyword.cloud_notm}} Private![Ícone de link externo](../icons/launch-glyph.svg "Ícone de link externo")](https://www.ibm.com/support/knowledgecenter/SSBS6K_2.1.0.3/installing/installing.html).

5.  Em seu cluster do {{site.data.keyword.cloud_notm}} Private, implemente o serviço de VPN IPSec strongSwan.
    ...
    2.  [Configure o gráfico Helm de VPN strongSwan ![Ícone de link externo](../icons/launch-glyph.svg "Ícone de link externo")](https://www.ibm.com/support/knowledgecenter/SSBS6K_2.1.0.3/app_center/create_release.html) em seu cluster privado.
        ...
        *  Lembre-se de que o `local.id` privado deve corresponder ao `remote.id` público, o `remote.id` privado deve corresponder ao `local.id` público e os valores `preshared.secret` para privado e público devem corresponder.
        ...
        Agora, é possível iniciar uma conexão do cluster do {{site.data.keyword.cloud_notm}} Private para o cluster do {{site.data.keyword.containerlong_notm}}.

6.  [Teste a conexão VPN](/docs/containers?topic=containers-vpn#vpn_test) entre seus clusters.

7.  Repita essas etapas para cada cluster que você deseja conectar.
...
## Conectando sua nuvem pública e privada com o {{site.data.keyword.cloud_notm}} Direct Link
{: #hybrid_dl}
...
Para escolher uma oferta do {{site.data.keyword.cloud_notm}} Direct Link e configurar uma conexão do {{site.data.keyword.cloud_notm}} Direct Link, consulte [Introdução ao {{site.data.keyword.cloud_notm}} Direct Link](/docs/infrastructure/direct-link?topic=direct-link-get-started-with-ibm-cloud-direct-link#how-do-i-know-which-type-of-ibm-cloud-direct-link-i-need-) na documentação do {{site.data.keyword.cloud_notm}} Direct Link.
...
## Executando imagens do {{site.data.keyword.cloud_notm}} Private em contêineres públicos do Kubernetes
{: #hybrid_ppa_importer}

É possível executar a seleção de produtos IBM licenciados que foram empacotados para o {{site.data.keyword.cloud_notm}} Private em um cluster no {{site.data.keyword.cloud_notm}} Public usando a ferramenta de importador do PPA.  
{: shortdesc}


O software licenciado está disponível no [IBM Passport Advantage ![Ícone de link externo](../icons/launch-glyph.svg "Ícone de link externo")](https://www-01.ibm.com/software/passportadvantage/index.html). Para usar esse software em um cluster no {{site.data.keyword.cloud_notm}} Public, deve-se fazer download do software, extrair a imagem e fazer upload da imagem para seu namespace no {{site.data.keyword.registryshort}} usando a ferramenta de importador do PPA, o comando `ibmcloud cr ppa-archive-load`. Independentemente do ambiente no qual você planeja usar o software, deve-se obter a licença necessária para o produto primeiro.

A tabela a seguir é uma visão geral de produtos disponíveis do {{site.data.keyword.cloud_notm}} Private que podem ser usados em seu cluster no {{site.data.keyword.cloud_notm}} Public.
...
{: caption="Tabela. Produtos suportados do {{site.data.keyword.cloud_notm}} Private para serem usados no {{site.data.keyword.cloud_notm}} Public." caption-side="top"}
...
Para implementar uma imagem do {{site.data.keyword.cloud_notm}} Private em um cluster no {{site.data.keyword.cloud_notm}} Public:

1.  Siga as etapas na [documentação do {{site.data.keyword.registryshort}}](/docs/services/Registry?topic=registry-ts_index#ts_ppa) para fazer download do software licenciado por meio do IBM Passport Advantage, enviar por push a imagem para o seu namespace e instale o gráfico do Helm em seu cluster.
...
3.  Para obter mais informações sobre como configurar e usar o produto com seu cluster, consulte a documentação específica do produto.
