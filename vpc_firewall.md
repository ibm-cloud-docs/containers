...
<img src="images/icon-vpc.png" alt="VPC infrastructure provider icon" width="15" style="width:15px; border-style: none"/> This firewall information is specific to VPC on Classic clusters. For firewall information for classic clusters, see [Opening required ports and IP addresses in your firewall for classic clusters](/docs/containers?topic=containers-firewall).
{: note}
...
## Allowing the cluster to access resources through access control lists
{: #firewall_acls}

If you use [access control lists](/docs/containers?topic=containers-vpc-network-policy) on your VPC subnets to act as a firewall to restrict all worker node egress, you must allow your worker nodes to access the resources that are required for the cluster to function.
{: shortdesc}

To use ACLs to act as your cluster firewall for public ingress and egress, you must create the rules listed in [link to public acl rules](/docs/containers?topic=containers-vpc-network-policy#).

To use ACLs to act as your cluster firewall for private ingress and egress, you must create the rules listed in [link to priate acl rules](/docs/containers?topic=containers-vpc-network-policy#).
