...
|-------------|------|
|Aborted|The deletion of the cluster is requested by the user before the Kubernetes master is deployed.|
...
|Delete failed|The Kubernetes master or at least one worker node cannot be deleted. <ol><li>List the worker nodes in your cluster.<pre class="pre"><code>bx cs workers &lt;cluser_name_or_id&gt;</code></pre></li><li>If no worker nodes are listed in your cluster, then the deletion of the Kubernetes master failed. Contact {{site.data.keyword.Bluemix_notm}} support by opening an [{{site.data.keyword.Bluemix_notm}} support ticket](/docs/get-support/howtogetsupport.html#using-avatar).</li><li>If worker nodes are listed, get the details for each worker node.<pre class="pre"><code>bx cs worker-get &lt;worker_id&gt;</code></pre></li><li>Review the <strong>State</strong> and <strong>Status</strong> fields to find the root problem for why the worker node could not be deleted.</li><li> Resolve the error on the worker node. For example, you might not have the required [permission to delete the worker node](/docs/containers/cs_infrastructure.html#infrastructure) or you have removed the [credentials to connect to your IBM Cloud infrastructure (SoftLayer) account](/docs/containers/cs_cli_reference.html#cs_credentials_set). You can also open an [{{site.data.keyword.Bluemix_notm}} support ticket](/docs/get-support/howtogetsupport.html#using-avatar). </li><li>Repeat the deletion of your cluster. </li></ol> |
|Deleted|The cluster is deleted but not yet removed from your dashboard. If your cluster is stuck in this state for a long time, open an [{{site.data.keyword.Bluemix_notm}} support ticket](/docs/get-support/howtogetsupport.html#using-avatar).|
|Deleting|The cluster is being deleted and cluster infrastructure is being dismantled. You cannot access the cluster. |
|Deploy failed|The deployment of the Kubernetes master could not be completed. Contact {{site.data.keyword.Bluemix_notm}} support by opening an [{{site.data.keyword.Bluemix_notm}} support ticket](/docs/get-support/howtogetsupport.html#using-avatar).|
...
|Requested|A request to create the cluster and order the infrastructure for the Kubernetes master and worker nodes is sent. When the deployment of the cluster starts, the cluster state changes to `Deploying`. If your cluster is stuck in the `Requested` state for a long time, open an [{{site.data.keyword.Bluemix_notm}} support ticket](/docs/get-support/howtogetsupport.html#using-avatar).|
|Updating|The Kubernetes master is being updated to a new Kubernetes API version. During the update you cannot access or change the cluster. Worker nodes, apps, and resources that have been deployed by the user are not modified and continue to run.|
