...
|`scaled_down`| The master resources have been scaled down to zero replicas. This is a temporary state that occurs while etcd is being restored after a backup. Wait for the etcd restoration to complete and the master state to return to `deployed`.|
