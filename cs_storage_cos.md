...
Before you begin, [target your CLI](cs_cli_install.html#cs_cli_configure) to the cluster where you want to install the {{site.data.keyword.cos_full_notm}} plug-in. 
...
2. Add the staging repo to your cluster. 
   ```
...
9. Verify that the plug-in is installed correctly. 
   ```
   ...
   The installation is successful when you see one `ibmcloud-object-storage-plugin` pod and one or more `ibmcloud-object-storage-driver` pods. The number of `ibmcloud-object-storage-driver` pods equals the number of worker nodes in your cluster. All pods must be in a `Running` state for the plug-in to function properly. If the pods fail, run `kubectl describe pod <pod_name>` to find the root cause for the failure. 
...
11. Repeat the steps for all clusters where you want to access {{site.data.keyword.cos_full_notm}} buckets.
...
4. Optional: If you plan to access your data with a non-root user, make sure that the [files have the correct permission](cs_troubleshoot_storage.html#cos_nonroot_access) assigned so that your app can successfully read and update the files as needed. 
