...
Version 1.0 NLBs can be created in classic clusters only, and can't be created in VPC clusters. To load balance in VPC clusters, see [Exposing apps with load balancers for VPC](/docs/containers?topic=containers-vpclb-about).
{: note}
...
When you [label worker nodes as edge nodes](/docs/containers?topic=containers-edge) and also [taint the edge nodes](/docs/containers?topic=containers-edge-workload-prevent), load balancer service pods deploy only to those edge nodes, and app pods can't deploy to edge nodes. When source IP is enabled for the NLB service, the load balancer pods on the edge nodes can't forward incoming requests to your app pods on other worker nodes.
{: shortdesc}
