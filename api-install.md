...
API reference docs
:    [v1 and v2 API](https://cloud.ibm.com/apidocs/kubernetes/containers-v1-v2){: external}
:    [v3 API](https://cloud.ibm.com/apidocs/kubernetes/containers-v3){: external}.
...
Federated ID
:    - **Generate an {{site.data.keyword.cloud_notm}} API key:** [{{site.data.keyword.cloud_notm}} API keys](/docs/iam?topic=iam-userapikey&interface=ui#create_user_key) are dependent on the {{site.data.keyword.cloud_notm}} account they are generated for. You can't combine your {{site.data.keyword.cloud_notm}} API key with a different account ID in the same {{site.data.keyword.cloud_notm}} IAM token. To access clusters that were created with an account other than the one your {{site.data.keyword.cloud_notm}} API key is based on, you must log in to the account to generate a new API key.
...
5. Review the [{{site.data.keyword.containerlong_notm}} API documentation](https://cloud.ibm.com/apidocs/kubernetes/containers-v1-v2){: external} to find a list of supported APIs.
...
1. Follow the steps in [Automating cluster deployments with the API](#cs_api) to retrieve your {{site.data.keyword.cloud_notm}} IAM access token, your {{site.data.keyword.cloud_notm}} API key, the ID of the cluster where you want to run Kubernetes API requests, and the {{site.data.keyword.containerlong_notm}} region where your cluster is located.

1. Retrieve an {{site.data.keyword.cloud_notm}} IAM ID, IAM access, and IAM refresh token using the {{site.data.keyword.cloud_notm}} API key. In your API output, you can find the IAM ID token in the `id_token` field, the IAM access token in the `access_token` field, and the IAM refresh token in the `refresh_token` field.
    ```sh
    POST https://iam.cloud.ibm.com/identity/token
    ```
    {: codeblock}
    ...
    Alternatively, using the`ibmcloud ks cluster config --cluster <cluster_name> --output json` CLI command will display the `id_token` and the `refresh_token`.

1. Before you can access your cluster with your current identity, you must run the following request.
    ```curl
...
1. Note that RBAC sync is asynchronous, so run the following request until it has synced.
    ```curl
    GET https://containers.cloud.ibm.com/global/v2/getRBACStatus?cluster=<cluster_name_or_ID>
  -H "Authorization: ${BEARER2}"
    ```
    {: pre}
...
1. Retrieve the URL of the default service endpoint for your Kubernetes master by using the IAM access token and the name or ID of your cluster. You can find the URL in the **`masterURL`** of your API output.
...
5. To use a private cloud service endpoint, you must first [expose the private cloud service endpoint by using a load balancer IP that is routable from your VPN connection into the private network](/docs/containers?topic=containers-access-private-classic).
...
Before you begin, make sure that you have an [{{site.data.keyword.cloud_notm}} API key](/docs/iam?topic=iam-userapikey&interface=ui#create_user_key) that you can use to request a new access token.
...
2. Continue working with the [{{site.data.keyword.containerlong_notm}} API documentation](https://cloud.ibm.com/apidocs/kubernetes/containers-v1-v2){: external} by using the token from the previous step.
