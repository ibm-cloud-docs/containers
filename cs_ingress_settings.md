...
### Adding {{site.data.keyword.appid_short_notm}} for ALBs that run the {{site.data.keyword.containerlong_notm}} Ingress image
{: #app-id-iks}

If your ALB runs the [{{site.data.keyword.containerlong_notm}} custom Ingress image](/docs/containers?topic=containers-ingress-types), you can use the custom {{site.data.keyword.containerlong_notm}} annotation, `ingress.bluemix.net/appid-auth`, in your Ingress resources. For steps on how to set up Ingress with {{site.data.keyword.appid_short_notm}} and how to configure the annotation, see [{{site.data.keyword.appid_short_notm}} Authentication (`appid-auth`)](/docs/containers?topic=containers-ingress_annotation#appid-auth).
{: shortdesc}

### Adding {{site.data.keyword.appid_short_notm}} for ALBs that run the community Kubernetes Ingress image
{: #app-id-kube}

If your ALB runs the [community Kubernetes Ingress image](/docs/containers?topic=containers-ingress-types), you can use the following steps to configure Ingress with {{site.data.keyword.appid_short_notm}}.
{: shortdesc}
...
2. Add redirect URLs for your app. A redirect URL is the callback endpoint of your app. To prevent phishing attacks, {{site.data.keyword.appid_full_notm}} validates the request URL against the whitelist of redirect URLs.
  ...
  3. In the **Authentication settings** tab, add redirect URLs for your app in the format `http://<hostname>/<App_ID_service_instance_name>/callback` or `https://<hostname>/<App_ID_service_instance_name>/callback`.

    {{site.data.keyword.appid_full_notm}} offers a logout function: If `/logout` exists in your {{site.data.keyword.appid_full_notm}} path, cookies are removed and the user is sent back to the login page. To use this function, you must append `/sign_out` to your domain in the format `http://<hostname>/<App_ID_service_instance_name>/sign_out` or `https://<hostname>/<App_ID_service_instance_name>/sign_out` and include this URL in the redirect URLs list.
    {: note}
...
4. Get the contents of the secret that was created in your cluster namespace. In the output, note the values of the `oauthServerURL`, `clientId`, and `secret` fields.
  ```
  kubectl get secret binding-<App_ID_service_instance_name> -n <namespace> -o=jsonpath='{$.data.binding}' | base64 -D
  ```
  {: pre}

5. Generate a cookie secret for the service instance. The cookie secret is used to encrypt the browser cookies that {{site.data.keyword.appid_short_notm}} uses for authentication. For example, to generate a new secret, you can run the following `docker` command.
  ```
  docker run -ti --rm python:3-alpine python -c 'import secrets,base64; print(base64.b64encode(base64.b64encode(secrets.token_bytes(16))));'
  ```
  {: pre}

6. Using the redirect URL from step 2, the secret values from step 4, and the cookie secret in step 5, create the following configuration file. Save the file as `<App_ID_service_instance_name>.conf`. This configuration file contains all of the information that is necessary to create an OAuth2-Proxy deployment, which you specify in step 8.
   ```
   provider = "oidc"
   http_address = "0.0.0.0:4180"
   proxy_prefix = "/<App_ID_service_instance_name>"
...
7. Use the configuration file to create a Kubernetes secret.
  ```
  kubectl create secret generic <App_ID_service_instance_name> --from-file=./<App_ID_service_instance_name>.conf -n <namespace>
  ```
  {: pre}

8. Create a configuration file for the OAuth2-Proxy deployment and a service to expose the deployment. Save the file as `oauth2_proxy.yaml`.
   ```yaml
   apiVersion: apps/v1
   kind: Deployment
   metadata:
     ...
     namespace: <namespace>
     labels:
       app: oauth2-<App_ID_service_instance_name>
   spec:
     replicas: 1
     selector:
       matchLabels:
     ...
     template:
       ...
       spec:
         ...
         - name: oauth2-proxy
           args:
           - --config=/etc/oauth-proxy-conf/<App_ID_service_instance_name>.conf
         ...
         volumes:
           - name: oauth-config-volume
             secret:
               secretName: <App_ID_service_instance_name>
   ---
   apiVersion: v1
   kind: Service
   metadata:
...
9. Deploy the OAuth2-Proxy and service to your cluster.
  ```
  kubectl create -f oauth2_proxy.yaml
  ```
  {: pre}

10. Create an Ingress resource that defines the routing of OIDC messages from the ALB to the OAuth2-Proxy deployment. Replace `<ingress_subdomain>` and `<ingress_tls_secret_name>` with the same Ingress host and TLS secret that you use to expose your apps, such as the default `Ingress Subdomain` and `Ingress Secret` for your cluster. Save the resource as `oauth2_proxy_ingress.yaml`.<p class="note">This Ingress resource file configures routing for the OAuth2-Proxy deployment only. Do not specify service paths for your apps in this Ingress resource.</p>
   ```yaml
   ...
   metadata:
     ...
     namespace: default
     annotations:
       kubernetes.io/ingress.class: iks-k8-nginx
   ...
   spec:
     ...
     - host: <ingress_subdomain>
       http:
         paths:
         - path: /<App_ID_service_instance_name>
           backend:
             serviceName: oauth2-<App_ID_service_instance_name>
             servicePort: 4180
     tls:
     - hosts:
       - <ingress_subdomain>
...
11. Apply the OAuth2-Proxy Ingress resource to your cluster.
  ```
  kubectl create -f oauth2_proxy_ingress.yaml
  ```
  {: pre}

12. In the Ingress resources for apps that you want to add {{site.data.keyword.appid_short_notm}} authentication to, add the following annotations to the `metadata.annotations` section. If you want to send web requests to {{site.data.keyword.appid_short_notm}} instead of API requests, include the last annotation, `auth-signin`.
   ```yaml
   ...
   annotations:
     nginx.ingress.kubernetes.io/auth-response-headers: Authorization
     nginx.ingress.kubernetes.io/auth-url: https://$host/<App_ID_service_instance_name>/auth
   #  nginx.ingress.kubernetes.io/auth-signin: https://$host/<App_ID_service_instance_name>/start?rd=$escaped_request_uri
   ...
   ```
   {: codeblock}

13. Re-apply your Ingress resources to enforce {{site.data.keyword.appid_short_notm}} authentication.
  ```
  kubectl create -f <app_ingress_resource>.yaml
  ```
  {: pre}

14. Verify that {{site.data.keyword.appid_short_notm}} authentication is enforced for your apps by accessing your apps in a web browser. If {{site.data.keyword.appid_short_notm}} is correctly applied, you are redirected to an {{site.data.keyword.appid_short_notm}} authentication log-in page.
