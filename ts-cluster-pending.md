...
- Check to see whether your VLAN is valid. To be valid, a VLAN must be associated with infrastructure that can host a worker with local disk storage. You can [list your VLANs](/docs/containers?topic=containers-kubernetes-service-cli#vlan-ls-cli) by running `ibmcloud ks vlan ls --zone ZONE` if the VLAN does not show in the list, then it is not valid. Choose a different VLAN.
