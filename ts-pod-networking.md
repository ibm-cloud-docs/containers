...
1. Apply the daemonset to deploy a test pod on each worker node.
...
1. Run the `curl` command on the pod and note the output. Specify the IP of a test pod that you did **not** log in to. This tests the network connection between pods on different nodes.
...
1. Run the `ping` command on the pod and note the output. Specify the IP of a test pod that you did **not** log in to with the `exec` command. This tests the network connection between pods on different nodes.
...
1. Run the `nc` command on the pod and note the output. Specify the IP of a test pod that you did **not** log in to with the `exec` command. This tests the network connection between pods on different nodes.
...
- If the commands functioned normally on the test pods, but you still have networking issues with the application pods in your default namespace, there might be issues related specifically to your application.
