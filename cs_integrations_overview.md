...
|Headlamp dashboard add-on|Cluster management|[Headlamp](https://headlamp.dev/){: external} is a Kubernetes dashboard that provides a graphical user interface for managing and monitoring your cluster resources. The Headlamp add-on provides seamless installation with automatic lifecycle management and integration with {{site.data.keyword.cloud_notm}} IAM for authentication. To get started, see [Headlamp Kubernetes dashboard add-on](/docs/containers?topic=containers-headlamp-addon).|Yes|Yes|
...
|Service|Description|Classic|VPC|
|----|------------|----|----|
|Heptio Velero|You can use [Heptio Velero](https://github.com/velero-io/velero){: external} to back up and restore cluster resources and persistent volumes. For more information, see the Heptio Velero [Use cases for disaster recovery and cluster migration](https://github.com/velero-io/velero/blob/release-0.9/docs/use-cases.md){: external}.|Yes|Yes|
