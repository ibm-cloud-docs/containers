...
{: tsSymptoms}
When you run `ibmcloud cluster-service-bind <cluster_name> <namespace> <service_instance_name>`, you see the following message.
...
{: tsResolve}
Use the service GUID instead of the service instance name in the `ibmcloud cluster-service-bind` command.
...
3. Bind the service to the cluster again.
  ```
  ibmcloud cluster-service-bind <cluster_name> <namespace> <service_instance_GUID>
  ```
  {: pre}
...
5.  Reboot the worker node that was not deleted.
  ...
  ```
  ibmcloud worker-reboot CLUSTER_ID NODE_ID
  ```
  {: pre}
...
You can try one of the following solutions:
  - Check the status of your cluster by running `ibmcloud clusters`. Then, check to be sure that your worker nodes are deployed by running `ibmcloud workers <cluster_name>`.
  - Check to see whether your VLAN is valid. To be valid, a VLAN must be associated with infrastructure that can host a worker with local disk storage. You can [list your VLANs](/docs/containers/cs_cli_reference.html#cs_vlans) by running `ibmcloud vlans LOCATION` if the VLAN does not show in the list, then it is not valid. Choose a different VLAN.
...
4.  If you don't have enough capacity in your cluster, add another worker node to your cluster.
    ...
    ```
    ibmcloud worker-add <cluster_name_or_ID> 1
    ```
    {: pre}
...
{: tip}
When you report an issue, include your cluster ID. To get your cluster ID, run `ibmcloud clusters`.
