...
{: tsSymptoms}
You enabled source IP preservation for a [version 1.0 load balancer](/docs/containers?topic=containers-loadbalancer#node_affinity_tolerations) or [Ingress ALB](/docs/containers?topic=containers-ingress#preserve_source_ip) service by changing `externalTrafficPolicy` to `Local` in the service's configuration file. However, no traffic reaches the back-end service for your app.
...
You can [delete your existing worker pool](/docs/containers?topic=containers-cs_cli_reference#cs_worker_pool_rm), then [create a new worker pool](/docs/containers?topic=containers-cs_cli_reference#cs_worker_pool_create).
...
2.  Get a new private and public VLAN for each zone that your cluster is in by [contacting {{site.data.keyword.Bluemix_notm}} support](/docs/infrastructure/vlans?topic=vlans-ordering-premium-vlans#ordering-premium-vlans).
...
5.  Use the `zone-network-set` [command](/docs/containers?topic=containers-cs_cli_reference#cs_zone_network_set) to change the worker pool network metadata.
