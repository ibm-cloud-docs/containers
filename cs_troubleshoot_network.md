...
3.  Check your load balancer service and review the **Events** section to find potential errors.
    ...
    <li>If at least two available worker nodes are found, list the worker node details.</br><pre class="codeblock"><code>ic cs worker-get [&lt;cluster_name_or_ID&gt;] &lt;worker_ID&gt;</code></pre></li>
    <li>Make sure that the public and private VLAN IDs for the worker nodes that were returned by the <code>kubectl get nodes</code> and the <code>ic cs [&lt;cluster_name_or_ID&gt;] worker-get</code> commands match.</li></ol></li></ul>
...
2.  Retrieve the ALB subdomain and public IP address, and then ping each one.
    ...
    1.  Retrieve the ALB subdomain.
      ...
      ```
      ic cs cluster-get <cluster_name_or_ID> | grep "Ingress subdomain"
      ```
      {: pre}
...
4.  Check your Ingress resource configuration file.
    ...
    1.  Check that the ALB subdomain and TLS certificate are correct. To find the IBM provided subdomain and TLS certificate, run `ic cs cluster-get <cluster_name_or_ID>`.
...
<caption>Troubleshooting Ingress application load balancer secrets</caption>
 ...
 <td><ol><li>Check the accuracy of the certificate CRN string you provide.</li><li>If the certificate CRN is found to be accurate, then try to update the secret: <code>ic cs alb-cert-deploy --update --cluster &lt;cluster_name_or_ID&gt; --secret-name &lt;secret_name&gt; --cert-crn &lt;certificate_CRN&gt;</code></li><li>If this command results in the <code>update_failed</code> status, then remove the secret: <code>ic cs alb-cert-rm --cluster &lt;cluster_name_or_ID&gt; --secret-name &lt;secret_name&gt;</code></li><li>Deploy the secret again: <code>ic cs alb-cert-deploy --cluster &lt;cluster_name_or_ID&gt; --secret-name &lt;secret_name&gt; --cert-crn &lt;certificate_CRN&gt;</code></li></ol></td>
 </tr>
 <tr>
 <td>The certificate CRN provided at time of update is incorrect.</td>
 <td><ol><li>Check the accuracy of the certificate CRN string you provide.</li><li>If the certificate CRN is found to be accurate, then remove the secret: <code>ic cs alb-cert-rm --cluster &lt;cluster_name_or_ID&gt; --secret-name &lt;secret_name&gt;</code></li><li>Deploy the secret again: <code>ic cs alb-cert-deploy --cluster &lt;cluster_name_or_ID&gt; --secret-name &lt;secret_name&gt; --cert-crn &lt;certificate_CRN&gt;</code></li><li>Try to update the secret: <code>ic cs alb-cert-deploy --update --cluster &lt;cluster_name_or_ID&gt; --secret-name &lt;secret_name&gt; --cert-crn &lt;certificate_CRN&gt;</code></li></ol></td>
 </tr>
 <tr>
...
{: tsSymptoms}
When you run `ic cs cluster-get <cluster>`, your cluster is in a `normal` state but no **Ingress Subdomain** is available.
...
1.  Check that the subnets that you want to use are available. **Note**: The infrastructure account that you are using might be shared across multiple {{site.data.keyword.Bluemix_notm}} accounts. If so, even if you run the `ic cs subnets` command to see subnets with **Bound Clusters**, you can see information only for your clusters. Check with the infrastructure account owner to make sure that the subnets are available and not in use by any other account or team.
...
3.  Use the `ic cs cluster-subnet-add` [command](cs_cli_reference.html#cs_cluster_subnet_add) to add existing subnets to your cluster. For more information, see [Adding or reusing custom and existing subnets in Kubernetes clusters](cs_subnets.html#custom).
...
1. View your cluster Kubernetes version.
    ```
    ic cs cluster-get <cluster_name>
    ```
    {: pre}
